...
struct OperatorMask {
    ...
    void Init(GridManipulator *grid, int level, GridMask *mask = NULL) {
        ...
        if(!ar) {
            fcout << " OperatorMask::Init Grid is not in area = BAD.\n";
            fcout.flush();
            abort();
        }
        Manip = ar->GetMaskStr();
    ...
    void CreateShift() {
        ...
        for(int d = 0; d < Dim; d++) {
            ...
            for(int or = 1; or <= MaxShift; or ++) {
                Shift[d][MaxShift + or ] =
                    Shift[d][MaxShift + or -1] + GridMaskShift(d, 1, Manip);
                Shift[d][MaxShift - or ] =
                    Shift[d][MaxShift - or +1] + GridMaskShift(d, -1, Manip);
            }
        }
    }
    inline GridMask *GetShift(const int &axis, const int &lev) {
        return &(Shift[axis][lev + MaxShift]);
    }
};
...
struct OperatorOneD : SavableClass {
    ...
    //  GridManipulator *Grid;
    int Axis, Level;
    virtual void Set(OperatorMask *shift, int ax, int lev) {
        Shift = shift;
        Axis = ax;
        Level = lev;
    }
...
struct OperatorSpace : OperatorOneD {
    ...
    void Set(OperatorOneD *oper, OperatorMask *shift, int level) {
        ...
        if(cd == NULL) {
            fcout
                << "Something very bad in OperatorSpace. Could not create, ClassDesc ; may have to register \n"
                << oper->class_name() << "\n";
            fcout.flush();
            abort();
        }
        for(k = 0; k < Dim; k++) {
            ...
            if(Oper[k] == NULL) {
                fcout
                    << "Something very bad in OperatorSpace. Could not create, may have to register \n"
                    << oper->class_name() << "\n";
                fcout.flush();
                abort();
            }
    ...
    virtual void Init(GridField<double> *vec0, GridField<double> *vec1 = NULL) {
        for(int k = 0; k < Oper.SetDim(); k++)
            Oper[k]->Init(vec0, vec1);
    }
    virtual void Init(GridVectorField<double> *arr, GridField<double> *vec = NULL) {
        for(int k = 0; k < Oper.SetDim(); k++)
            Oper[k]->Init(arr, vec);
    ...
    virtual void Operate(
        ...
        for(int k = 1; k < Dim; k++) {
            Operate(ClcMask, dat, tmp);
            res(ClcMask) += tmp(ClcMask);
        }
    };
    virtual void Operate(
        GridMask &ClcMask,
        ...
        if(resType == ResSum) {
            ...
            for(int k = 1; k < Dim; k++) {
                Operate(ClcMask, dat, tmp);
                for(int k1 = 0; k1 < ResD; k1++)
                    (*res.GetX(k1))(ClcMask) += (*tmp.GetX(k1))(ClcMask);
            }
        } else {
            int AxD = ResD / Dim;
            if(AxD * Dim != ResD) {
                fcout << " Bad Dim in Operate SpaceDim " << Dim << " ArrayDim " << ResD
                      << "\n";
                abort();
            }
            ...
            for(int k = 0; k < Dim; k++) {
                Operate(ClcMask, dat, tmp);
                for(int k1 = 0; k1 < ResD; k1++)
                    (*res.GetX(k1 + k * Dim))(ClcMask) = (*tmp.GetX(k1))(ClcMask);
            }
        }
    };
