...
DescribedClass *create_GridField_double_cd() {
    return new GridField<double>;
}
static ClassDesc GridField_double_cd(
    typeid(GridField<double>),
    "GridField_double",
    1,
    "Grid",
    &create_GridField_double_cd);

DescribedClass *create_GridField_int_cd() {
    return new GridField<int>;
}
static ClassDesc GridField_int_cd(
    typeid(GridField<int>),
    "GridField_int",
    1,
    "Grid",
    &create_GridField_int_cd);

static ClassDesc ResizeVector_FieldInterface_CopyStructRef_cd(
    typeid(ResizeVector<Ref<FieldInterface>, CopyStructRef<Ref<FieldInterface>>>),
    "ResizeVector_FieldInterface_CopyStructRef");
...
void BoundaryInterfaceStd::GetPntBound(GridIndex k, int kpos, GridBoundPnt &bnd) {
    ...
    ExEnv::err() << " CouldNot Find Linked Reference to point in  Grid<T>::GetPntBound."
                 << endl;
    abort();
Good:;
    ...
    delta_pnt = k_pnt - k1_pnt;
    double norm = delta_pnt * delta_pnt;
    if(norm > MathZer)
        bnd.NormalDir = delta_pnt / sqrt(norm);
    else
        bnd.NormalDir = delta_pnt * 0;
}
...
void BoundaryInterfaceStd::GetBoundary(GridBoundPnt &bnd) {
    if(bnd.Second != -1) {
        ExEnv::err() << " Point is not boundary in  Grid<T>::GetBoundary." << endl;
        abort();
    }
