...
struct MaskedVector {
    ...
    int ToDeleteVec;
    ~MaskedVector() {
        if((ToDeleteVec) && (vec->GetGridManip()))   //vec->SetStore(NULL);}
            vec->GetGridManip()->DelVar(vec.pointer());
        vec = NULL;
        mask = NULL;
    }   //
    ...
    MaskedVector(const MaskedVector<T, CpM> &dat) {
        vec = dat.vec;
        mask = dat.mask;
        ToDeleteVec = 0;
    }
...
MaskedVector<T, CpM>::MaskedVector(GridVectorField<T, CpM> *dat, GridMaskVector *m) {
    if(dat->SetDim() != m->SetDim()) {
        fcout
            << " Error in MaskedVector<T,CpM>(GridVectorField<T,CpM> *dat,GridMaskVector *m) . Abort\n";
        fcout.flush();
        abort();
    }
    ToDeleteVec = 1;
...
template <class T, class CpM>
MaskedVector<T, CpM>::MaskedVector(
    GridVectorField<T, CpM> *dat,
    const GridMaskVector &m) {
    if(dat->SetDim() != m.SetDim()) {
        fcout
            << " Error in MaskedVector<T,CpM>(GridVectorField<T,CpM> *dat,const GridMaskVector &m) . Abort\n";
        fcout.flush();
        abort();
    }
    ToDeleteVec = 0;
...
MaskedVector<T, CpM> &MaskedVector<T, CpM>::operator=(const MaskedVector<T, CpM> &dat) {
    ...
    if(N1 != N) {
        fcout << " Error in MaskedVector<T,CpM>& operator = . Abort\n";
        fcout.flush();
        abort();
    }
    ...
    for(k = 0; k < N; k++)
        (*(vec->GetX(k)))((*mask)[k]) = (*(dat.vec->GetX(k)))((*(dat.mask))[k]);
    return *this;
};
...
MaskedVector<T, CpM>::MaskedVector(GridField<T, CpM> *dat, const GridMaskVector &m) {
    ToDeleteVec = 1;
    int D = m.SetDim();
    vec = new GridVectorField<T, CpM>(D, dat->SetStore());
    mask = new GridMaskVector;
    for(int k = 0; k < D; k++)
        *(vec->GetX(k)) = *dat;
    mask = new GridMaskVector;
