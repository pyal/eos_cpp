...
GridMask Dat2Mask(FieldInterface *dat) {
    ...
    if(mask_arr == NULL) {
        ExEnv::err()
            << " Dat2Mask dat - cannot be converted to GridField<GridIndex>. Error. \n"
            << endl;
        abort();
    }
...
void Mask2Dat(GridMask &mask, FieldInterface *dat) {
    ...
    if(mask_arr == NULL) {
        ExEnv::err()
            << " Mask2Dat dat - cannot be converted to GridField<GridIndex>. Error. \n"
            << endl;
        abort();
    }
    GridManipulator *grid = mask_arr->GetGridManip();
    ...
    for(GridIndex i = grid->StartPnt(); i >= 0; i = grid->NextPnt(i))
        mask_arr->GetPnt(i) = 0;
    for(k = 0; k < mask.SetDim(); k++)
        mask_arr->GetPnt(mask[k]) = 1;
};
...
//void SetCircleBoundaryMask(GridManipulator *grid,GridMask &MaskBndGet,GridMask &MaskBndPut,int AccuracySpace)
{
    ...
    if(!tmp) {
        fcout << " MaskBoundaryClc::SetNewGrid NULL area\n";
        abort();
    }
    ...
    MaskBndGet.SetDim(N0 * N1);
    MaskBndPut.SetDim(N0 * N1);
    SL[0] = MaskEnd0;
    SR[0] = MaskEnd0;
    for(k = 1; k < 2 * N1; k++) {
        SL[k] = mask_manip->ShiftMask(SL[k - 1], 1, 1);
        SR[k] = mask_manip->ShiftMask(SR[k - 1], 0, 1);
    }
    int ng = 0, np = 0;
    for(k = 0; k < N0; k++) {
        if(SL[1][k] < 0) {
            for(k1 = 0; k1 < N1; k1++) {
                MaskBndPut[ng] = SR[k1][k];
                ng++;
            }
        } else {
            for(k1 = 0; k1 < N1; k1++) {
                MaskBndPut[ng] = SL[k1][k];
                ng++;
            }
        }
    }
    ...
    for(k = N0 - 1; k >= 0; k--) {
        if(SL[1][k] < 0) {
            for(k1 = 2 * N1 - 1; k1 >= N1; k1--) {
                MaskBndGet[ng] = SR[k1][k];
                ng++;
            }
        } else {
            for(k1 = 2 * N1 - 1; k1 >= N1; k1--) {
                MaskBndGet[ng] = SL[k1][k];
                ng++;
            }
        }
    }
...
void SetData2Coord(FieldInterface *data, GridMask &mask, int axis) {
    ...
    if(dat == NULL) {
        fcout << " SetData2Coord : data have to be vector of double.\n";
        fcout.flush();
        abort();
    }
    DataVector<SpacePoint, CopyStructSlow<SpacePoint>> coord =
        grid->GetGridMaskedPnt(mask);
