...
struct MaskManipulatorStruct : NamedObjectInited {
    ...
    MaskManipulatorStruct(GridManipulator *grid, GridMask &mask) : Grid(NULL) {
...
struct GridMaskShift {
    ...
    GridMaskShift(int axis, int level, MaskManipulatorStruct *sh)
        : Axis(axis), Level(level), Shift(sh){};
};
