...
struct MaskedData {
    ...
    int ToDeleteVec;
    ~MaskedData() {
        if((ToDeleteVec) && (vec->GetGridManip()))
            vec->GetGridManip()->DelVar(vec.pointer());
    }   //vec=NULL;}//vec->GetGridManip(NULL);}
    ...
    MaskedData(const MaskedData<T, CpM> &dat) {
        vec = dat.vec;
        mask = dat.mask;
        ToDeleteVec = 0;
    }
...
MaskedData<T, CpM>::MaskedData(GridField<T, CpM> *dat, GridMask *m) {
    ToDeleteVec = 1;
    vec = new GridField<T, CpM>(dat->GetGridManip());
    mask = new GridMask;
    *vec = *dat;
    *mask = *m;
}
...
MaskedData<T, CpM> &MaskedData<T, CpM>::operator=(const MaskedData<T, CpM> &dat) {
    ...
    if(N1 != N) {
        fcout << " Error in MaskedData<T,CpM>& operator = . Abort\n";
        fcout.flush();
        abort();
    }
