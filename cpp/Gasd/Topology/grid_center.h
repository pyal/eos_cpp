...
public:
    ...
    GridCenterPnt(const SpacePoint &Spnt, int Npnt = 0)
...
struct GridCenter : public Grid<GridCenterPnt> {
    ...
    virtual void ModifyPnt(GridIndex k, GridCenterPnt &val) {
        val.IndNum = k;
        Grid<GridCenterPnt>::ModifyPnt(k, val);
    };
    virtual void AddPnt(GridIndex k, GridCenterPnt &val) {
        val.IndNum = k;
        Grid<GridCenterPnt>::AddPnt(k, val);
    };
};
...
struct GridCenterManipulator : public GridManipulatorStd<GridCenter> {
    ...
    virtual SpacePoint GetGridPnt(GridIndex k) {
        return grid->GetPnt(k).Coord;
    }
    virtual void SetGridPnt(SpacePoint newVal, GridIndex k) {
        grid->GetPnt(k).Coord = newVal;
    }
    virtual DataVector<SpacePoint, CopyStructSlow<SpacePoint>> GetGridMaskedPnt(
        DataVector<GridIndex> &mask);

    virtual void SetGridMaskedPnt(
        DataVector<SpacePoint, CopyStructSlow<SpacePoint>> &data,
        DataVector<GridIndex> &mask);

    //   SpacePoint GetOldPnt()                       {return store->OldGridKValue.Coord;}
    DataVector<GridIndex> GetNeibous(GridIndex k) {
        return grid->GetPnt(k).Neibous;
    }
};
...
struct TriangulatorSingleSquare : public TriangulatorInterface {
    ...
    ~TriangulatorSingleSquare(){};
    TriangulatorSingleSquare(SpaceBound &bnd, IndBound &ind) {
        SetRegion(bnd, ind);
    }
    void SetRegion(SpaceBound &bnd, IndBound &ind) {
        bound = bnd;
        index = ind;
    }
    ...
    virtual int save_data_state(FilterOut &so) {
        ...
        so << KeyDesc("bound") << bound << KeyDesc("index") << index;
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        TriangulatorInterface::read_data_state(si);
        si >> StdKey >> bound >> StdKey >> index;
        return 1;
    };
    SpaceBound bound;
    IndBound index;
    ...
    virtual GridManipulator *CreateGridManip() {
        return new GridCenterManipulator;
    };

    virtual MaskManipulatorGeneral *CreateMaskGen() {
        return new MaskManipulatorGeneral;
    }
    virtual MaskManipulatorStruct *CreateMaskStr() {
...
struct BoundaryCenter : public BoundaryInterfaceStd {
    ...
    inline void Init() {
        BoundaryInterfaceStd::Init();
        AreaStoreInterface *ar = dynamic_cast<AreaStoreInterface *>(get_StoredBy());
        triang = dynamic_cast<TriangulatorSingleSquare *>(ar->GetTriang());
    }

    void GetBoundary(GridBoundPnt &bnd);
    virtual int save_data_state(FilterOut &so) {
        BoundaryInterfaceStd::save_data_state(so);
        so << KeyDesc("triang") << (SavableClass *)triang;
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        BoundaryInterfaceStd::read_data_state(si);
        SavableClass *tmp;
        si >> StdKey >> tmp;
        triang = (TriangulatorSingleSquare *)tmp;
        return 1;
    };
};
