...
struct GridBound : Grid<GridBoundPnt> {
    ...
    virtual int save_data_state(FilterOut &so) {
        ...
        so << KeyDesc("centerind_ptr") << (SavableClass *)centerind_ptr;
        return 1;
    };
    ...
    virtual DataVector<GridIndex> MaskCenterShift(
...
struct GridVarBoundInd : public GridField<GridBoundIndPnt> {
    ...
    GridVarBoundInd(GridManipulator *stor = NULL, char *name = NULL)
        : GridField<GridBoundIndPnt>(stor, name) {
        gridbound_ptr = new GridBound(this);
        CreateGridData();
    }
...
struct GridBoundManipulator : public GridManipulatorStd<GridBound> {
    ...
    virtual SpacePoint GetGridPnt(GridIndex k) {
        return grid->GetPnt(k).Center;
    }
    virtual void SetGridPnt(SpacePoint newVal, GridIndex k) {
        grid->GetPnt(k).Center = newVal;
    }
