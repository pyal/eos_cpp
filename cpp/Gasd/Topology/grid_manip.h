...
struct GridManipulator : public NamedObjectInited {
    ...
    virtual int NumPnt(
        int CountDeleted = 0) = 0;   // if CountDeleted - return MaxInd of grid
    ...
    virtual int GridFrozen(
        int frozen = -1) = 0;   //{if (frozen!=-1) Frozen=frozen;return Frozen;};
...
struct FieldInterface : public NamedObject {
    ...
    virtual void ModifyPnt(
        GridIndex k) = 0;   //Only After - grid structure not changed !!!
