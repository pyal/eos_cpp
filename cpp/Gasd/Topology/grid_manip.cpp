...
DescribedClass *create_AreaStoreInterface_cd() {
    return new AreaStoreInterface;
}


static ClassDesc AreaStoreInterface_cd(
    typeid(AreaStoreInterface),
    "AreaStoreInterface",
    1,
    "Area",
    &create_AreaStoreInterface_cd);
...
MaskManipulatorGeneral *AreaStoreInterface::GetMaskGen(MaskManipulatorGeneral *newMask) {
    if(newMask != NULL)
        VarList.Add(newMask, MaskGen);
    return dynamic_cast<MaskManipulatorGeneral *>(VarList.GetPtr(MaskGen));
};
MaskManipulatorStruct *AreaStoreInterface::GetMaskStr(MaskManipulatorStruct *newMask) {
    if(newMask != NULL)
        VarList.Add(newMask, MaskStr);
    return dynamic_cast<MaskManipulatorStruct *>(VarList.GetPtr(MaskStr));
};
