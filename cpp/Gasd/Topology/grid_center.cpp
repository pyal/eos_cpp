...
DescribedClass *create_GridCenterManipulator_cd() {
    return new GridCenterManipulator;
}
static ClassDesc GridCenter_cd(typeid(GridCenter), "GridCenter");
static ClassDesc GridCenterManipulator_cd(
    typeid(GridCenterManipulator),
    "GridCenterManipulator",
    1,
    "Grid",
    &create_GridCenterManipulator_cd);

static ClassDesc GridCenterPnt_cd(typeid(GridCenterPnt), "GridCenterPnt");

static ClassDesc GridField_GridCenterPnt_Slow_cd(
    typeid(GridField<GridCenterPnt, CopyStructSlow<GridCenterPnt>>),
    "GridField_GridCenterPnt_Slow");

DescribedClass *create_TriangulatorSingleSquare_cd() {
    return new TriangulatorSingleSquare;
}
static ClassDesc TriangulatorSingleSquare_cd(
    typeid(TriangulatorSingleSquare),
    "TriangulatorSingleSquare",
    1,
    "Grid",
    &create_TriangulatorSingleSquare_cd);

//BoundaryCenter
DescribedClass *create_BoundaryCenter_cd() {
    return new BoundaryCenter;
}
static ClassDesc BoundaryCenter_cd(
    typeid(BoundaryCenter),
    "BoundaryCenter",
    1,
    "Grid",
    &create_BoundaryCenter_cd);
...
int GridCenterPnt::save_data_state(FilterOut &so) {
    ...
    so << KeyDesc("IndNum") << IndNum << KeyDesc("Coord") << Coord << KeyDesc("Volume")
       << Volume << KeyDesc("Neibous") << Neibous << KeyDesc("EndPnt\n");
    return 1;
}
int GridCenterPnt::read_data_state(FilterIn &si) {
    si >> StdKey >> IndNum >> StdKey >> Coord >> StdKey >> Volume >> StdKey >> Neibous >>
        StdKey;
    return 1;
}
    ...
    for(k = 0; k < data[From].Neibous.SetDim(); k++)
        if((i = data[From].Neibous[k]) != -1) {
            for(k1 = 0; k1 < data[i].Neibous.SetDim(); k1++)
                if(data[i].Neibous[k1] == From) {
                    data[i].Neibous[k1] = To;
                }
        }
    data[From].IndNum = To;
...
DataVector<SpacePoint, CopyStructSlow<SpacePoint>> GridCenterManipulator::
    ...
    DataVector<GridCenterPnt, CopyStructSlow<GridCenterPnt>> pnt =
        grid->GetMaskedPnt(mask);
    int k, N = pnt.SetDim();
...
void GridCenterManipulator::SetGridMaskedPnt(
