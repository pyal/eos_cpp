...
struct GridField : public FieldInterface //public GridVar<T,CpM>
 {
   GridField(GridManipulator *stor=NULL,char *name=NULL,int Local=0)
    {data.SetPolicy(50,-50,0,NULL);if (stor) stor->AddVar(this);
     if (name) set_obj_name(name);if (Local) unmanage();}
...
// To use in return statement
   ...
   inline virtual void MovePnt(GridIndex From,GridIndex To){data[To]=data[From];}

   inline virtual int save_data_state (FilterOut &so)  {FieldInterface::save_data_state(so); so<<KeyDesc("data")<<data;return 1;};
   inline virtual int read_data_state (FilterIn &si)  {FieldInterface::read_data_state(si); si>>StdKey>>data;return 1;};

   inline virtual void SetDefault(T &Def) {data.SetFill(&Def);}
   inline virtual void StoreChanged(){if (GetGridManip()) data.SetNumEl(GetGridManip()->NumPnt(1));}//else data.SetNumEl(0);}
...
struct Grid : public GridField< T,CpM >,public NamedObjectStoreAbstract 
 {
   Grid() :GridField< T,CpM  >(),Frozen(0){VarList.set_DefaultStorage(this);};

   virtual T& GetPnt(GridIndex k){if (k==-1) return OldGridKValue;return data[k];};


   virtual void AddPnt(GridIndex k){ ExEnv::err()<<" Grid::AddPnt(k) - useless? is it not ? "<<endl;abort();}
   ...
   virtual void ModifyPnt(GridIndex k){ ExEnv::err()<<" Grid::ModifyPnt(k) - useless? is it not ? "<<endl;abort();}
...
// AddPnt(k,val) ModifyPnt(k,val)
   ...
   inline GridManipulator* GetGridManip(){AreaStoreInterface *ret=dynamic_cast<AreaStoreInterface*>(get_StoredBy());if (ret==NULL) {fcout<<"Very bad. Grid::GetGridManip - is NULL\n";abort();};return ret->GetGridManip();}
...
// from NamedObjectStoreAbstract
   inline void DeleteObject(NamedObject *obj)       {CheckFrozen("DeleteObject");VarList.DeleteObject(obj);};
   inline  int Add (NamedObject* dat, int pos = -1) {CheckFrozen("Add");return VarList.Add(dat,pos);};
  ...
  private:
   ...
   inline void CheckFrozen(char *func){if (Frozen) {ExEnv::err()<<" Error in "<<func<<"  Grid Frozen. Unfrozing first "<<endl;GridFrozen(0);}}
...
struct BoundaryInterfaceStd : public BoundaryInterface
 {
   inline void Init() { AreaStoreInterface *ar=dynamic_cast<AreaStoreInterface*>(get_StoredBy());if (ar==NULL) {fcout<<"Very bad. GridManipulatorStd::Init Area - is NULL\n";abort();};grid_manip=ar->GetGridManip();}
...
// One point is boundary ( -1 - is a neibour ); bnd First,Second,Pos are already formed by GetPntBound
   ...
   inline virtual int save_data_state (FilterOut &so) {BoundaryInterface::save_data_state(so); so<<KeyDesc("grid_manip")<<(SavableClass*)grid_manip;return 1;};
   inline virtual int read_data_state (FilterIn &si)  {BoundaryInterface::read_data_state(si); SavableClass* tmp;si>>StdKey>>tmp;grid_manip=dynamic_cast<GridManipulator*>(tmp);return 1;};
...
//   inline virtual FieldInterface *GetGridField(){return store;}
  protected: T* grid; BoundaryInterface *bound; NamedStore *gvar; SparceArrayInd *ind; public:

   void Init();
   inline FieldInterface *GetGridField(){return grid;};
   inline BoundaryInterface *GetBound(){return bound;};


// Field Manipulator
   virtual inline int AddVar(FieldInterface *NewVar) {return grid->Add(NewVar);}
   virtual inline void DelVar(FieldInterface *NewVar){if (grid) grid->DeleteObject(NewVar);else {ExEnv::err()<<" Trying to delete var from grid==NULL in GridManipulatorStd\n";}}
   virtual inline FieldInterface* GetVar(char *name) {return (FieldInterface*)gvar->GetPtr(gvar->Name2Pos(name));}
   virtual inline FieldInterface* GetVar(int Num)    {return (FieldInterface*)gvar->GetPtr(Num);}
   virtual inline int GetVar(FieldInterface* get2var){return get2var->get_StoredByNum();}


   virtual int save_data_state (FilterOut &so) {so<<KeyDesc("Grid_ptr")<<(SavableClass*)grid<<KeyDesc("Bound_ptr")<<(SavableClass*)bound; return 1;};
   virtual int read_data_state (FilterIn &si)  {SavableClass *gr,*bn;si>>StdKey>>gr>>StdKey>>bn;grid=dynamic_cast<T*>(gr);bound=dynamic_cast<BoundaryInterface*>(bn);if (grid!=NULL) { gvar=&(grid->VarList);ind=gvar->GetInd();} return 1;};

// if CountDeleted - return MaxInd of grid
   virtual inline int NumPnt (int CountDeleted=0)      {return ind->Num(CountDeleted);};
   virtual inline GridIndex NextPnt(GridIndex i)       {return ind->Next(i);};

   virtual inline GridIndex StartPnt()                 {return ind->Next(-1);};
   virtual inline GridIndex StartEmptyIndex()          {return ind->NextEmp(-1);};
   virtual inline GridIndex NextEmptyIndex(GridIndex i){return ind->NextEmp(i);};

   virtual inline int GridFrozen(int frozen=-1)        { return grid->GridFrozen(frozen);};
   virtual inline void GetPntBound(GridIndex pnt,int pntpos,GridBoundPnt &bnd) {bound->GetPntBound(pnt,pntpos,bnd);};

   void PrintAllGridVars(FilterOut &out)
    {int cur=-1,i=0;NamedObject *tmp;
     ...
     while((cur=in_->Next(cur))>=0)
      {
       ...
       if ((tmp=gvar->GetPtr(cur))!=NULL) 
        out
           ...
           <<"Nref "<<(int)tmp->nreference()<<" "
           <<tmp->get_obj_name();else out<<" Bad - NULL - ptr in store ";
       out<<"\n";out.flush();
      }
    }
...
template <class T,class CpM>
void GridField< T, CpM>::SetMaskedPnt(DataVector<T, CpM> &ret, DataVector<GridIndex, CopyStructSlow<GridIndex> > &mask)
{
 int k,N=mask.SetDim();
  ...
  switch(act)
   {
    ...
    case (GridAction::DelPnt):
     if (!froze) {StoredPnt.Del(k);VarDelPnt(k);}
     else VarAddAction(act,k,To);break;
    case (GridAction::MovePnt) :
      StoredPnt.Add(To);
     if (!froze) {StoredPnt.Del(k);VarMovePnt(k,To);}
     else VarAddAction(act,k,To);break;
...
void Grid<T,CpM>::VarDelPnt(GridIndex i)
 {
  SparceArrayInd *var_ind=VarList.GetInd();
  int cur=-1;while ((cur=var_ind->Next(cur))>=0) ((FieldInterface*)VarList.GetPtr(cur))->DelPnt(i);
 };
template <class T,class CpM>
void Grid<T,CpM>::VarMovePnt(GridIndex From,GridIndex To)
 {
  SparceArrayInd *var_ind=VarList.GetInd();
  int cur=-1;while ((cur=var_ind->Next(cur))>=0) ((FieldInterface*)VarList.GetPtr(cur))->MovePnt(From,To);
 };
template <class T,class CpM>
void Grid<T,CpM>::VarModifyPnt(GridIndex i)
 {
  SparceArrayInd *var_ind=VarList.GetInd();
  int cur=-1;while ((cur=var_ind->Next(cur))>=0) ((FieldInterface*)VarList.GetPtr(cur))->ModifyPnt(i);
 };
...
void GridManipulatorStd<T>::Init()
 {
  ...
  if (ar==NULL) {fcout<<"Very bad. GridManipulatorStd::Init Area - is NULL\n";abort();};
  grid=dynamic_cast<T*>(ar->GetGridField());bound=ar->GetBound();
  if (grid==NULL) {fcout<<"Very bad. GridManipulatorStd::Init - grid is NULL\n";abort();};
  gvar=&(grid->VarList);ind=&(grid->StoredPnt);
 };
