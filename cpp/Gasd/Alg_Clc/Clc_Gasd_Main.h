...
struct GasdCalculator : SavableClass {
    ...
    virtual int save_data_state(FilterOut &so) {
        so << "WorkDim" << WorkDim << "AccuracySpace" << AccuracySpace << "NumPnt"
           << NumPnt;
        return !so.fail();
    }

    virtual int read_data_state(FilterIn &si) {
        si >> StdKey >> WorkDim >> StdKey >> AccuracySpace >> StdKey >> NumPnt;
        return !si.fail();
    }
...
struct Config_Gasd_ReadPar : Config_Gasd {
    ...
    Config_Gasd_ReadPar() : ReadIsGood(0), ReadTime(0) {
        strcpy(MaskStoredPoints, "StoredPoints");
    }
...
struct GasdCalculator_Simple1D : GasdCalculator {
    ...
    virtual void MakeTimeStp(
        ...
        GridVectorField<double> *Coord =
            (GridVectorField<double> *)grid_manip->GetVar("Coord");
    ...
    virtual void StartCalculation() {
        ...
        *X = *(shift->GetCenter()->GetX(0));
        ...
        for(k = 0; k < MaskAll.SetDim(); k++)
            U->GetPnt(MaskAll[k]) =
                sin((X->GetPnt(MaskAll[k]) / Length * NumPeriods + Phase) * 2 * M_PI) *
                    Amp +
                ShiftAmp;
