...
DescribedClass* create_GasdCalculator_cd(){return new GasdCalculator;}
DescribedClass* create_Config_Gasd_ReadPar_cd(){return new Config_Gasd_ReadPar;}

DescribedClass* create_GasdCalculator_Simple1D_cd(){return new GasdCalculator_Simple1D;}

static ClassDesc GasdCalculator_cd(typeid(GasdCalculator),
                 "GasdCalculator",1,"GasdCalculator_Category",
                 &create_GasdCalculator_cd);
static ClassDesc Config_Gasd_ReadPar_cd(typeid(Config_Gasd_ReadPar),
                 "Config_Gasd_ReadPar",1,"Config_Gasd_Category",
                 &create_Config_Gasd_ReadPar_cd);


static ClassDesc GasdCalculator_Simple1D_cd(typeid(GasdCalculator_Simple1D),
                 "GasdCalculator_Simple1D",1,"GasdCalculator_Category",
...
// AccuracySpace;
  ...
  area=new AreaStoreInterface;area->CreateArea(new TriangulatorSingleSquare(bound,index));
...
int Config_Gasd_ReadPar::ReadData(FilterIn &in,FilterOut &out)
 {
  if (NotToReadPar) {ReadIsGood=1;calc->StartCalculation();WriteRes(0,out);return 1;}
  ReadIsGood=0;
  GridManipulator *grid=stor->GetGridManip();
  if (!stor) {fcout<<" Config_Gasd_ReadPar::ReadData, stor is null\n";fcout.flush();return 0;}
  ...
  char tmp[256];in>>tmp>>ReadTime;
  for (k=0;k<Npnt;k++)
   {
    if (!ReadSpaceAsDouble) in>>coord[k];
    else {coord[k].SetDim(SpaceDim);for (k1=0;k1<SpaceDim;k1++) in>>coord[k][k1];}
    for (k1=0;k1<Nvar;k1++) in>>arr(k,k1);
   }
  if (in.fail()) return 0;
...
//fcout<<HEAD("Writing Begin")<<*grid->GetGridField()<<ENDL;
 if (!stor) {fcout<<" Config_Gasd_ReadPar::WriteRes, stor is null\n";fcout.flush();return ;}
  ...
  for (k=0;k<Nvar;k++) arr.SetColumn(k,((GridField<double>*)
                  (grid->GetVar(data_names[k].name)))->GetMaskedPnt(mask));
  for (k=0;k<Npnt;k++)
   {
    if (!ReadSpaceAsDouble) out<<coord[k];
    else {for (k1=0;k1<SpaceDim;k1++) out<<coord[k][k1];}
    for (k1=0;k1<Nvar;k1++) out<<arr(k,k1);
    out<<"\n";
   }
  out<<"EndIter"<<"\n";
...
int Config_Gasd_ReadPar::save_data_state (FilterOut &so)
 {
  ...
  for (int k=0;k<NumVar;k++) so<<data_names[k];
  so<<KeyDesc("\nSpaceDim")<<SpaceDim<<KeyDesc("ReadSpaceAsDouble")<<ReadSpaceAsDouble;
...
int Config_Gasd_ReadPar::read_data_state (FilterIn &si)
 {
  int NumVar,k;si>>StdKey>>NumVar>>StdKey;
  for (k=0;k<NumVar;k++) si>>data_names[k];
  si>>StdKey>>SpaceDim>>StdKey>>ReadSpaceAsDouble>>StdKey>>NotToReadPar>>StdKey;
  GetHuman(si,calc);
  if (calc!=NULL) 
   {
    calc->reader=this;stor=calc->CreateGrid();
    GridManipulator* grid=stor->GetGridManip();
    GridField<double> *tmp;
    for (k=0;k<NumVar;k++) tmp=new GridField<double>(grid,data_names[k].name);
   }
  return stor!=NULL;
 }
