...
struct Algorithm_TimeWrite : SavableClass
 {
  ...
  virtual double GetStartTime(double Time) {StartTime=Time;return StartTime;}
...
//  Algorithm_TimeWrite *time_ptr;
  ...
  virtual int SetParam(Algorithm_MainAction *m_ptr,Algorithm_TimeWrite *t_ptr,
                       FilterIn *in_ptr,FilterOut *out_ptr)
   {  main_ptr=m_ptr;time_ptr=t_ptr; return 1;}
...
struct Algorithm_MainAction_Gasd : Algorithm_MainAction
 {
  ...
  ~Algorithm_MainAction_Gasd(){Delete();}
  void Delete(){delete in_ptr;in_ptr=NULL;delete out_ptr;out_ptr=NULL;
...
struct Algorithm_TimeWrite_Gasd : Algorithm_TimeWrite
 {
  ...
  virtual double GetStartTime(double Time)
   {StartTime=Time;OldTimeWrite=Time;NewTimeWrite=Time+TimeWriteStp;
    NumIter=1;OldStp=1e50;NumWriteSteps=0;
    return StartTime;}
  virtual int GetNextTime(double &Time,double MaxTimeStp,double BoundWaveAmp);

  virtual int save_data_state (FilterOut &so)
   {so<<KeyDesc("StartTime")<<StartTime<<KeyDesc("EndTime")<<EndTime<<KeyDesc("TimeWriteStp")<<TimeWriteStp
      <<KeyDesc("StabilityCoef")<<StabilityCoef<<KeyDesc("IncreaseStepCoef")<<IncreaseStepCoef
      <<KeyDesc("BoundWaveCoef")<<BoundWaveCoef;
    return 1;};
  virtual int read_data_state (FilterIn &si)
   {si>>StdKey>>StartTime>>StdKey>>EndTime>>StdKey>>TimeWriteStp
      >>StdKey>>StabilityCoef>>StdKey>>IncreaseStepCoef
      >>StdKey>>BoundWaveCoef;
    GetStartTime(StartTime);
    return 1;};
    ~Algorithm_TimeWrite_Gasd(){};

  virtual void print(ostream& = ExEnv::out()) const
   {fcout<<" Algorithm_TimeWrite_Gasd \n Used to get next time for gasd.\n Standart time step calculation algorithm\n";
    fcout<<" Its format : "<<(SavableClass*)this<<"\n";}
 };
...
//  int AccuracySpace,AccuracyTime,CalculationMethod;
  virtual int IsGood(){return 1;};
  virtual SavableClass *CreateCopy(){return NULL;};
  virtual int ReadData(FilterIn &in,FilterOut &out){return 1;};
  virtual void MakeTimeStp(double TimeWas,double TimeNew,double &MaxTimeStp,
                           double &BoundWaveAmp)
   { BoundWaveAmp=0;MaxTimeStp=1e6; }
...
struct Algorithm_MakeStp_Gasd : Algorithm_MakeStp
 {
  ...
  virtual double GetStartTime(double time)
   { if (ChangeStart) time=gasd->GetStartTime(); return Algorithm_MakeStp::GetStartTime(time);};
  virtual int SetParam(Algorithm_MainAction *m_ptr,Algorithm_TimeWrite *t_ptr,
                       FilterIn *in_ptr,FilterOut *out_ptr)
   {  
...
//    *out_ptr<<"Config_Gasd\n"<<(SavableClass*)gasd;
    if (!gasd->ReadData(*in_ptr,*out_ptr)) return 0;
    ...
    while ( (!seek_eof(*in_ptr)) && (gasd->ReadData(*in_ptr,*out_ptr)) )
     { gasd_old<<gasd->CreateCopy();ChangeStart=1;}
    if (!gasd->IsGood()) if (!gasd_old->IsGood()) return 0;else gasd<<gasd_old->CreateCopy();
    return 1;
   }
