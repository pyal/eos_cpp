...
// returns Mask for the Base(without new boundary) on the new Grid
        ...
        virtual void IterateRegion(const TGridMaskedData &src, const TGridMaskedData &dst, void (*doit)(int , int )) = 0;
...
// returns Mask for the Base on the old Grid
    ...
    protected:
        virtual TGridMaskedData Add(TGridMaskBase *mask, const TGridMaskedData &mdata, int inPlace) = 0;
        virtual TGridMaskedData Add(TGridMaskBase *mask, double c, int inPlace) = 0;
        virtual TGridMaskedData Sub(TGridMaskBase *mask, const TGridMaskedData &mdata, int inPlace) = 0;
        ...
        virtual TGridMaskedData Abs(TGridMaskBase *mask, const TGridMaskedData &mdata) = 0;
   ...
   //double operator[](size_t k)const{ return Ptr[k];};
    ...
    public:
        TGridMaskedData(){};
        TGridMaskedData(const TGridMaskedData &right): Data(right.Data), Mask(right.Mask) {};
        TGridMaskedData (TGridVariablesBase* data, TGridMaskBase* mask) : Data(data), Mask(mask) {}
...
// to be done - moving by arrays....
        ...
        public:
            ...
            inline TIterator& Start(const TGridMaskedData &mask) {
                Low = Up = -1;
                Mask->SetMask(mask);
                if (!IsOk() || Mask->Mask->NumPoints() == 0 || !Mask->Mask->GetNextInterval(Low, Up, 0))
                    Mask->Clear();
                else
                    CurInd = Low;
                return *this;
            ...
            inline TIterator& Last(const TGridMaskedData &mask) {
                Low = Up = -1;
                Mask->SetMask(mask);
                if (!IsOk() || Mask->Mask->NumPoints() == 0 || !Mask->Mask->GetNextInterval(Low, Up, 1)) {
                    Mask->Clear();
                    return *this;
                }
                CurInd = Up - 1;
