    ...
    public:
        struct TRegData : SavableClass {
            struct TGridVar {
                Stroka Name;
                TGridVar() {};
                TGridVar(const char *name, double minval, double maxval):Name(name), MinVal(minval), MaxVal(maxval) {};
            ...
            TRegData(const char *str, SavableClass *ptr, int dim, const TGridVar &var) : SavableClass(), RegionGridSize(dim) {
            ...
            void SetData(Ref<TPolyRegion> reg) {
                ...
                if (RegVarNames.size() != RegVars.size())
                    throw info_except("RegVarNames.size<%i> != RegVars.size<%i>", RegVarNames.size(), RegVars.size());
                ...
                if (RegDoubles.size() != RegDoubleNames.size())
                    throw info_except("RegDoubles.size<%i> != RegDoubleNames.size<%i>", RegDoubles.size() != RegDoubleNames.size());
                ...
                for(list<TGridVar>::iterator it = GridVars.begin(); it != GridVars.end(); it++) {
                    reg->Grid.AddVar (it->Name, new NPolygon::TGridVar<double>(DataManip::MakeStepVector(RegionGridSize, it->MinVal, it->MaxVal, 0, 0)), TRegionBounds(0, 0), mask);
                }
            ...
            int save_data_state( FilterOut&so) {
                so<<" RegionGridSize "<<RegionGridSize<<SavableClass::EOLN()<<
                    " RegVars "<<RegVars<<SavableClass::EOLN()<<
                    " RegVarNames "<<RegVarNames<<SavableClass::EOLN()<<
                    " RegDoubles "<<RegDoubles<<SavableClass::EOLN()<<
                    " RegDoubleNames "<<RegDoubleNames<<SavableClass::EOLN()<<
                    " GridVars(NameMinValMaxVal) "<<GridVars;
               return 1; 
            };
            ...
            Stroka MakeHelp(){return "Stores object and its name";}
        };
        ...
        Ref<TPolyRegion> MakeRegion() {
            ...
            for(list<Ref<TRegData> >::iterator it = Childs.begin(); it != Childs.end(); it++){
        ...
        int save_data_state( FilterOut&so){
            ...
            so<<" DataFile "<<DataFile<<" RegionBoundarySize "<<RegionBoundarySize;
             return 1; 
        };
        int read_data_state(FilterIn&si) {
            Clear();
        ...
        Stroka MakeHelp(){return "Class for generation of calculation regions...";}

    };

    struct PolyRegIO {
        static TGridVariablesBase *GetVectorElement(const Stroka &name, TPolyRegion *reg) {
            size_t lftParent = (size_t)(-1), rgtParent = -1;
            if ((lftParent = name.find("[")) == (size_t)(-1) || (rgtParent = name.find("]")) == (size_t)(-1))
                return NULL;
            Stroka n(name, 0, lftParent);
            int i = atoi(~Stroka(name, lftParent + 1, rgtParent));
            TGridVectorVar<double> *vec = SavableClass::TestType<TGridVectorVar<double> >(reg->Grid.GetVar(n), "Write only vectors of double\n");
            return (*vec)[i];
        }
        static void SaveRegionData(ostream &out, Ref<TPolyRegion> region, const vector<Stroka> &vecStr, const TRegionBounds &bnds) {
            //vector<Stroka> vecStr = Str::SplitLine(varNames, 0);
            if (vecStr.size() == 0)
                return;
            vector<TGridMaskedData::TIterator> vecDat(vecStr.size());
            for (TPolyRegion::TShallowIterator iter = region->ShallowStart(); iter.IsOk(); iter.Next()) {
                for(size_t i = 0; i < vecStr.size(); i++) {
