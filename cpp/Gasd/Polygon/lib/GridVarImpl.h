                ...
                for(int curX = curL; curX < curU; curX++) {
                    if(wasX >= wasU - 1) {
                        if(!(wasMask->GetNextInterval(wasL, wasU, 0)))
                            throw info_except(
                                "Dims are different!!! maskWas:\n%s\nMaskNow:\n%s\n",
                                ~SavableClass::object2string(wasMask),
                                ~SavableClass::object2string(wasMask));
                        wasX = wasL;
                    } else
                        wasX++;
