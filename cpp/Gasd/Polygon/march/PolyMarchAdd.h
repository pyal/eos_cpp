...
namespace NPolygon {
    ...
    private:
        ...
        double Tmin, Tmax, TsplMisfit; // use for march calculation (not original int, beg, end)
        double Xsigma, Xcenter, Ysigma; // use for march calculation (not original int, beg, end)
        Stroka TimeDepFile;
        ...
        double Tint(double sx, double sMin, double sMax, double gMin, double gMax, CurveSpline &gSpl) {
            if (sx < sMin || sx > sMax)
                return 0;
...
//cout << sx << " " << spaceI << " " << 0 << "\n";
        ...
        void ClcAddition(TPolyRegion *reg, double curTime, double curStp) {
            ...
            double TimeCoef = Tint(curTime + 0.5 * curStp, Tmin, Tmax, MinReadT, MaxReadT, TimeDepSpl) * 
                curStp * TimeIntNormCoef * regCoef;
            TRegionBounds bndCent(0, -1);
            double *pos  = (double*)reg->Grid.GetMaskedData(bndCent, PosName).Start().GetElementPtr();
            double *ener = (double*)reg->Grid.GetMaskedData(bndCent, EnergyAddName).Start().GetElementPtr();
  ...
  //      }
		...
		//}
        int save_data_state( FilterOut&so) {
            so << " IntegralInt " << IntegralInt;// <<" Ysigma2Norm " << Ysigma2Norm <<" Xsigma2Norm " << Xsigma2Norm << SavableClass::EOLN();
            so << " TimeDepFile " << TimeDepFile <<" Tmin " << Tmin << " Tmax " << Tmax << " TsplMisfit " << TsplMisfit << SavableClass::EOLN();
            so << " Xsigma " << Xsigma <<" Xcenter " << Xcenter << " Ysigma " << Ysigma << SavableClass::EOLN();
            so << " PosName " << PosName <<" EnergyAddName " << EnergyAddName <<" RegCoefName " << RegCoefName << SavableClass::EOLN();
        ...
        int read_data_state(FilterIn&si) { 
            ...
            CurveSpline::SplineFromData(~TimeDepFile, TimeDepSpl, TsplMisfit, "Time dependence spline");
            ...
            tNorm *= (Tmax - Tmin);
            tNorm *= TGausFunc(0, Ysigma).Integral(0, YmeanL) *2; // / TGausFunc(0, Ysigma).Integral(0, 10 * Ysigma);
            TimeIntNormCoef = 1e-9 / tNorm * IntegralInt * 4637;//3.326 kJ for he if int 1e9;
            //TimeIntNormCoef = 1e-9 / tNorm * IntegralInt * 2.71;//3.326 kJ for he if int 1e9;
            //TimeIntNormCoef *= SpaceInt * TimeInt;
            return 1;
        }
        Stroka MakeHelp() {
            Stroka help = "Class for external energy input to the marcher scheme. Uses marcher for time step clc.";
    ...
    private:
		...
		Stroka EnerName, DensName, EOSName, PresName, MassName, SplDataName, PosName, VelName;
		double DensLowCoef, SplitMassCoef ;
		int InLim(double dens, double ener, MatterIO *mat, double &resDens, double &resE) {
...
//cout << " got in lim dens " << dens << " temp " << t << " lowD*coef " << d << " lowE " << e << " wasD " << resDens << " wasE " << resE << "\n";
		...
		void CheckBounds(TPolyRegion *src, TPolyRegion *dst, int LeftToRight) {
			TRegionBounds bndRgt(TRegionBounds::BNDBAD, 0), bndRgtMin(TRegionBounds::BNDBAD, -1),
						  bndLft(0, TRegionBounds::BNDBAD), bndLftMin(-1, TRegionBounds::BNDBAD), bndLftPlus(1, TRegionBounds::BNDBAD);
			double bndDens, bndE, setDens, setE;
			if (LeftToRight) {
				bndDens = *(double*)TGridMaskedData::TIterator(src->Grid.GetMaskedData(bndRgtMin, DensName)).GetElementPtr();
				bndE = *(double*)TGridMaskedData::TIterator(src->Grid.GetMaskedData(bndRgtMin, EnerName)).GetElementPtr();
				//bndDens = src->Grid.GetMaskedData(bndRgtMin, DensName);
				//bndE = src->Grid.GetMaskedData(bndRgtMin, EnerName);
				MatterIO *mat = SavableClass::TestType<MatterIO>(src->MapSavableVar[EOSName]);
				setDens = *(double*)TGridMaskedData::TIterator(dst->Grid.GetMaskedData(bndLft, DensName)).GetElementPtr();
				setE = *(double*)TGridMaskedData::TIterator(dst->Grid.GetMaskedData(bndLft, EnerName)).GetElementPtr();
	            ...
	            dst->Grid.GetMaskedData(bndLft, MassName) = 
					(dst->Grid.GetMaskedData(bndLftPlus, PosName) - dst->Grid.GetMaskedData(bndLft, PosName)) * setDens;
				CheckSplit(dst);
			} else {
		...
		void CheckSplit(TPolyRegion *reg) {
			...
			for(TGridMaskedData::TIterator it(reg->Grid.GetMaskedData(bndM, MassName)); it.IsOk(); ++it) {
		...
		void MakeSplit(TPolyRegion *reg, TGridMaskedData::TIterator splitCell, TGridMaskedData::TIterator lftJoin) {
			double splitMass = *(double*)splitCell.GetElementPtr(reg->Grid.GetVar(MassName)) * 0.5;
			double mLft = *(double*)lftJoin.GetElementPtr(reg->Grid.GetVar(MassName));
			double mRgt = *(double*)(lftJoin + 1).GetElementPtr(reg->Grid.GetVar(MassName));
			double joinedMass = mLft + mRgt;

			double splitDens = *(double*)splitCell.GetElementPtr(reg->Grid.GetVar(DensName));
			double joinedDens = joinedMass /
				                (mLft / *(double*)lftJoin.GetElementPtr(reg->Grid.GetVar(DensName)) +
								mRgt / *(double*)(lftJoin + 1).GetElementPtr(reg->Grid.GetVar(DensName)));

			double splitE = *(double*)splitCell.GetElementPtr(reg->Grid.GetVar(EnerName));
			double joinedE = (mLft * *(double*)lftJoin.GetElementPtr(reg->Grid.GetVar(EnerName)) +
								mRgt * *(double*)(lftJoin + 1).GetElementPtr(reg->Grid.GetVar(EnerName))) / joinedMass;

			double splitU = *(double*)splitCell.GetElementPtr(reg->Grid.GetVar(VelName));
			double joinedU = (mLft * *(double*)lftJoin.GetElementPtr(reg->Grid.GetVar(VelName)) +
								mRgt * *(double*)(lftJoin + 1).GetElementPtr(reg->Grid.GetVar(VelName))) / joinedMass;

			double splitP = *(double*)splitCell.GetElementPtr(reg->Grid.GetVar(PresName));
			double joinedP = (mLft * *(double*)lftJoin.GetElementPtr(reg->Grid.GetVar(PresName)) +
								mRgt * *(double*)(lftJoin + 1).GetElementPtr(reg->Grid.GetVar(PresName))) / joinedMass;

			double posLft = *(double*)splitCell.GetElementPtr(reg->Grid.GetVar(PosName));
			double posRgt = *(double*)(splitCell + 1).GetElementPtr(reg->Grid.GetVar(PosName));

			SplitJoin(reg, splitCell, lftJoin, MassName, splitMass, splitMass, joinedMass, 1);
			SplitJoin(reg, splitCell, lftJoin, DensName, splitDens, splitDens, joinedDens, 1);
			...
			SplitJoin(reg, splitCell, lftJoin, PosName , (posLft + posRgt) * 0.5, posRgt, joinedE, 0);
		}
		void SplitJoin(TPolyRegion *reg, TGridMaskedData::TIterator splitCell, TGridMaskedData::TIterator lftJoin, const Stroka &varName,
						double splitLft, double splitRgt, double joinedVal, int setJoined =  1) {
			TRegionBounds bndFull(-reg->Grid.GetBoundarySize(), reg->Grid.GetBoundarySize());
			TGridMaskedData::TIterator src(reg->Grid.GetMaskedData(bndFull, varName)), dst(src);
    ...
    public:
        ...
        void RebuildBounds(TPolyRegion *reg) {
			...
			int i = 1;
            for(TPolyRegion::TShallowIterator it = reg->ShallowStart(); it.IsOk(); it.Next(), i++) {
				if (i == SrcRegion)
					src = it.CurRegion();
				if (i == DstRegion)
					dst = it.CurRegion();
        ...
        int save_data_state( FilterOut&so) {
			so << " SrcRegion " << SrcRegion << " DstRegion " << DstRegion << " DensLowCoef " << DensLowCoef << SavableClass::EOLN();
			so << " EnerName " << EnerName << " DensName " << DensName << " EOSName " << EOSName << " PresName " << PresName;
			so << " MassName " << MassName << " PosName " << PosName << " VelName " << VelName << SavableClass::EOLN();
			so << " SplitMassCoef " << SplitMassCoef << " SplDataName " << SplDataName << SavableClass::EOLN();
			return TPolyMarchPlusBase::save_data_state(so);
        };
        int read_data_state(FilterIn&si) { 
            Stroka tmp;
			si >> tmp >> SrcRegion >> tmp >> DstRegion >> tmp >> DensLowCoef;
			si >> tmp >> EnerName >> tmp >> DensName >> tmp >> EOSName >> tmp >> PresName >> tmp >> MassName >> tmp >> PosName;
        ...
        Stroka MakeHelp() {
			Stroka help = "Class imitating  gas - liquid separation. Src,Dst regions are counted from 1. SplDataName - contains 3 spline - T_DensLow, T_DensHigh, T_ELow. Uses marcher for time step clc.\n";
            help += " Marcher is of the same type - NPolygon::RegionMarch_category.\n";
            //help += " Registered NPolygon::RegionMarch_category objects:\n\n~+\n" + SavableClass::HelpForCategory("NPolygon::RegionMarch_category");
            //help += "~-\n";
            return help;
