...
namespace NPolygon {
    ...
    struct TFluxPrepare {
        ...
        static void MakeBoundedPresSound(TPolyRegion *reg, const TRegionBounds &bndsClc, const TRegionBounds &bndsLimit,
                        const Stroka &dens, const Stroka &ener, const Stroka &eos, const Stroka &res, int resIsPres) {
            ...
            curPos = ClcEOSPressure(resPtr, enerPtr, dencPtr, reg, eos, curPos, sizeBase, resIsPres);
            if (sizeRgt > 0) {
                TPolyRegion *next = FindNextRegion(reg, 1);
                ClcEOSPressure(resPtr, enerPtr, dencPtr, next, eos, curPos, sizeRgt, resIsPres);
            }
        }
        static int ClcEOSPressure(double *resPtr, double *enerPtr, double *dencPtr, TPolyRegion *reg, const Stroka &eos, int begPos, int size,int resIsPres) {
            ...
            MatterIO *mat = SavableClass::TestType<MatterIO>(reg->MapSavableVar[eos]);
            if (resIsPres) 
                mat->Pressure(&resPtr[begPos - 1], &dencPtr[begPos - 1], &enerPtr[begPos - 1], size);
            else {
