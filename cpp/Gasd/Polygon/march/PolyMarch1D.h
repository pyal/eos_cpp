...
namespace NPolygon {
    ...
    //    virtual void ClcFlux(TPolyRegion *reg, const Stroka &uName, const Stroka &fName, double dX_dT_coef) {} = 0;
    //};
    //struct FluxClaculatorLargange : FluxCalculatorBase {
    //    FluxClaculatorLargange () : FluxCalculatorBase() {};
    //    virtual void InitRegion(TPolyRegion *reg, const Stroka &uName) {};
    //    virtual void ClcFlux(TPolyRegion *reg, const Stroka &uName, const Stroka &fName, double dX_dT_coef) {
    ...
    //        switch fluxType {
    //            case Base : return FluxByName["BASE"];
    //            case LaxFriedrich : return FluxByName["LF"];
    //            case Richtmyer : return FluxByName["RI"];
    //            case Force : return FluxByName["FORCE"];
    //        }
    ...
    //            TGridVariablesBase *uData = reg->Grid.GetMaskedData( uName, TRegGrid::AllData, 0).Data;
    //            reg->Grid.AddVar(FluxByName["BASE"], SavableClass::TestType<TGridVariablesBase>(uData->Duplicate()));
    //            reg->Grid.AddVar(FluxByName["LF"], SavableClass::TestType<TGridVariablesBase>(uData->Duplicate()));
    //            reg->Grid.AddVar(FluxByName["RI"], SavableClass::TestType<TGridVariablesBase>(uData->Duplicate()));
    //        }
    //    }
    //    virtual void ClcFlux(TPolyRegion *reg, const Stroka &uName, const Stroka &fName, double dX_dT_coef) {
    //        switch FluxType {
    ...
    //            break;
    //        default :
    //            throw info_except("Type <%i> is unknown. Possible types <LaxFriedrich Richtmyer Force>\n", FluxType);
    //            break;
    //        }
    //    }
    //    void ClcFluxLF(TPolyRegion *reg, const Stroka &uName, const Stroka &fName, double dX_dT_coef) {
    //        TGridMaskedData dst = reg->Grid.GetMaskedData(TRegGrid::Bound, 0, "fName");
    //        baseFlux->ClcFlux(reg, uName, FluxByName["BASE"], dX_dT_coef);
    //        TGridMaskedData fL = reg->Grid.GetMaskedData(TRegGrid::Center, 0, FluxByName["Base"]);
    //        TGridMaskedData fR = reg->Grid.GetMaskedData(TRegGrid::Center, 1, FluxByName["Base"]);
    ...
    //    }
    //    void ClcFluxRI(TPolyRegion *reg, const Stroka &uName, const Stroka &fName, double dX_dT_coef) {
    //        TGridMaskedData dst = reg->Grid.GetMaskedData(TRegGrid::Bound, 0, "fName");
    //        baseFlux->ClcFlux(reg, uName, FluxByName["BASE"], dX_dT_coef);
    //        TGridMaskedData fL = reg->Grid.GetMaskedData(TRegGrid::Center, 0, FluxByName["Base"]);
    //        TGridMaskedData fR = reg->Grid.GetMaskedData(TRegGrid::Center, 1, FluxByName["Base"]);
    ...
    //        dst = (fL + fR +  (uL - uR) / dX_dT_coef)* 0.5;
    //    }
    //    void ClcFluxForce(TPolyRegion *reg, const Stroka &uName, const Stroka &fName, double dX_dT_coef) {
    //        TGridMaskedData dst = reg->Grid.GetMaskedData(TRegGrid::Bound, 0, "fName");
    //        ClcFluxLF(reg, uName, FluxByName["LF"], dX_dT_coef);
    //        ClcFluxRI(reg, uName, FluxByName["RI"], dX_dT_coef);
    //        TGridMaskedData fLF = reg->Grid.GetMaskedData(TRegGrid::Center, 0, FluxByName["LF"]);
    //        TGridMaskedData fRI = reg->Grid.GetMaskedData(TRegGrid::Center, 0, FluxByName["RI"]);
    //        dst = (fLF + fRI)* 0.5;
    //    }
    ...
    //            TGridVariablesBase *rData = reg->Grid.GetMaskedData( baseRvar, TRegGrid::AllData, 0).Data;
    ...
    //    void ClcLimiter(TPolyRegion *reg, const Stroka &baseRvar, const Stroka &limiterVar, double courantNumber) {
    //        InitRegion(reg, baseRvar);
    //        reg->Grid.GetMaskedData(TRegGrid::Center, 1, DQName) = reg->Grid.GetMaskedData(TRegGrid::Center, 1, baseRvar);
    //        reg->Grid.GetMaskedData(TRegGrid::Center, 1, DQName) -= reg->Grid.GetMaskedData(TRegGrid::Center, 0, baseRvar);
    //        reg->Grid.GetMaskedData(TRegGrid::Center, 0, VarRl) = reg->Grid.GetMaskedData(TRegGrid::Center, -1, DQName) / 
    //                                                              reg->Grid.GetMaskedData(TRegGrid::Center,  0, DQName);
    //        reg->Grid.GetMaskedData(TRegGrid::Center, 0, VarRr) = reg->Grid.GetMaskedData(TRegGrid::Center,  1, DQName) / 
    //                                                              reg->Grid.GetMaskedData(TRegGrid::Center,  0, DQName);
    //        ClcNamedLimiter(reg, VarRr, courantNumber, SUPERBEE);
    //        ClcNamedLimiter(reg, VarRl, courantNumber, SUPERBEE);
    //        TGridMaskedData::TIterator rIt(reg->Grid.GetaskedData(varRr, TRegGrid::Center, 0)), 
    ...
    //            r = min(r, l);
    //        }
    //        reg->Grid.GetMaskedData(TRegGrid::Center, 0, limiterVar) = reg->Grid.GetMaskedData(TRegGrid::Center, 1, varRr);
    //    }
    //    void ClcNamedLimiter(TPolyRegion *reg, const Stroka &varR, double courantNumber, int limitType) {
    //        double phiG = (1 - courantNumber) / ( 1 + courantNumber );
    //        switch limitType  {
    //            case  SUPERBEE : 
    //                TGridMaskedData::TIterator varIt(reg->Grid.GetMaskedData(varR, TRegGrid::Center, 0));
    ...
    //    FlicLimiter Limiter
    //    void ClcFlux(TPolyRegion *reg, const Stroka &fluxLow, const Stroka &fluxHigh, double courantNumber, 
    //        const Stroka &baseRvar, const Stroka &resFlux) {
    //        Limiter.ClcLimiter(reg, baseRvar, "flicLim");
    //        reg->Grid.GetMaskedData(TRegGrid::Center, 0, resFlux) = reg->Grid.GetMaskedData(TRegGrid::Center, 0, fluxLow) + 
    //            reg->Grid.GetMaskedData(TRegGrid::Center, 0, "flicLim") * 
    //            (reg->Grid.GetMaskedData(TRegGrid::Center, 0, fluxHigh) -reg->Grid.GetMaskedData(TRegGrid::Center, 0, fluxLow));
    //    }
    //};
    struct TPolyMarchU2 : TPolyMarchRegionBase {
        ...
        void FillPressure(TPolyRegion *reg) {
            TGridMaskedData denc = reg->Grid.GetMaskedData(TRegionBounds(0, 0), "R");
            //TGridMaskedData pos = reg->Grid.GetMaskedData(TRegGrid::Bound, 0, "X");
            TGridMaskedData energy = reg->Grid.GetMaskedData(TRegionBounds(0, 0), "R");
            TGridMaskedData pressure = reg->Grid.GetMaskedData(TRegionBounds(0, 0), "P");
