...
namespace NPolygon {
    ...
    public:
        ...
        virtual void ClcFlux(TPolyRegion *reg, const Stroka &uName, const Stroka &fName, const Stroka &xName, const TRegionBounds &bnds, double dT) = 0;
    };
    ...
    struct FluxRI : FluxCalculatorBase {
        ...
        virtual void ClcFlux(TPolyRegion *reg, const Stroka &uName, const Stroka &fName, const Stroka &dxName, const TRegionBounds &bnds, double dT) {
            ...
            if (! reg->Grid.VarExists(TmpFluxName)) 
                reg->Grid.AddVar(TmpFluxName, SavableClass::TestType<TGridVariablesBase>(reg->Grid.GetVar(uName)->Duplicate()));
...
// given ( 0, 0 ) use (-1, 0)  -> (0, 0)
    struct FluxForce : FluxCalculatorBase {
        ...
        FluxForce(const Stroka &calculatedLF, const Stroka &calculatedRI) 
            : CalculatedRI(calculatedRI)
            , CalculatedLF(calculatedLF) {
        };
    ...
    struct FlicLimiter : SavableClass {
        ...
        FlicLimiter() : DQName("flicDQ"), VarRl("flicRl"), VarRr("flicRr"), VarTmp("flicTmp"){};
        int IsVector(TGridVar<double> *&var, TGridVectorVar<double> *&vec, TGridVariablesBase *dat) {
        ...
        void InitRegion(TPolyRegion *reg, const Stroka &baseRvar) {
            if (!reg->Grid.VarExists(DQName)) {
                ...
                reg->Grid.AddVar(DQName, SavableClass::TestType<TGridVariablesBase>(rData->Duplicate()));
                reg->Grid.AddVar(VarRr, SavableClass::TestType<TGridVariablesBase>(rData->Duplicate()));
                reg->Grid.AddVar(VarRl, SavableClass::TestType<TGridVariablesBase>(rData->Duplicate()));
...
// Result - (0,0)
        void ClcLimiter(TPolyRegion *reg, const Stroka &baseRvar, const Stroka &limiterVar, const Stroka &sound, const Stroka &dX, const TRegionBounds &bndsCenter, double tStp) {
            InitRegion(reg, baseRvar);
            TRegionBounds bndsPlus(bndsCenter.GetLft() - 1, bndsCenter.GetRgt() + 1);

            reg->Grid.GetMaskedData(bndsPlus, DQName)  = reg->Grid.GetMaskedData(bndsPlus, baseRvar);
            reg->Grid.GetMaskedData(bndsPlus, DQName) -= reg->Grid.GetMaskedData(bndsPlus - 1, baseRvar);
            reg->Grid.GetMaskedData(bndsCenter, VarRl)   = reg->Grid.GetMaskedData(bndsCenter - 1, DQName) / 
                                                                  reg->Grid.GetMaskedData(bndsCenter, DQName);
            reg->Grid.GetMaskedData(bndsCenter, VarRr) = reg->Grid.GetMaskedData(bndsCenter + 1, DQName) / 
                                                                  reg->Grid.GetMaskedData(bndsCenter, DQName);

            double courantNumber = 0;
            TGridVariablesBase *dxVar = reg->Grid.GetVar(dX); //*soundVar = reg->Grid.GetVar(sound), 
            for(TGridMaskedData::TIterator soundIt(reg->Grid.GetMaskedData(bndsCenter, sound)) ; soundIt.IsOk(); soundIt.Next())
                courantNumber = max<double>(courantNumber ,
                                *(double*)soundIt.GetElementPtr() * tStp / *(double*)soundIt.GetElementPtr(dxVar));

cout<<"Calculated CFL "<<courantNumber<<"\n";

            TGridVar<double> *varRlcalc, *varRrcalc;
            TGridVectorVar<double> *vecRlcalc, *vecRrcalc;
            reg->Grid.GetMaskedData(bndsCenter, limiterVar) = reg->Grid.GetMaskedData(bndsCenter, limiterVar) * 0 + 10;
            ...
            if (IsVector(varRrcalc, vecRrcalc, reg->Grid.GetVar(VarRr))) {
                for(size_t i = 0; i < vecRrcalc->Dimension(); i++) {
                    SetLimiter(reg, limiterVar, (*vecRrcalc)[i], SUPERBEE, bndsCenter, courantNumber);
                    SetLimiter(reg, limiterVar, (*vecRlcalc)[i], SUPERBEE, bndsCenter, courantNumber);
                }
            } else {
                    SetLimiter(reg, limiterVar, varRrcalc, SUPERBEE, bndsCenter, courantNumber);
                    SetLimiter(reg, limiterVar, varRlcalc, SUPERBEE, bndsCenter, courantNumber);
            }
        }
        void SetLimiter(TPolyRegion *reg, const Stroka &limName, TGridVariablesBase *varRcalc, 
                int limitType, const TRegionBounds &bndsCenter, double courantNumber) {
            ClcNamedLimiter(reg, VarTmp, varRcalc, SUPERBEE, bndsCenter, courantNumber);
            TGridVariablesBase *varTmp = reg->Grid.GetVar(VarTmp);
            
            for(TGridMaskedData::TIterator it(reg->Grid.GetMaskedData(bndsCenter, limName)); it.IsOk(); it.Next()) {
                double &r = *(double*)it.GetElementPtr();
                r = min<double>(r, *(double*)it.GetElementPtr(varTmp));
            }
        }

        void ClcNamedLimiter(TPolyRegion *reg, const Stroka &varResName, TGridVariablesBase *varR, int limitType, const TRegionBounds &bndsCenter, double courantNumber) {
            double phiG = (1 - courantNumber) / (1 + courantNumber);
            TGridVariablesBase *varRes = reg->Grid.GetVar(varResName);

            switch (limitType)  {
                case  SUPERBEE : 
                    for(TGridMaskedData::TIterator varIt(reg->Grid.GetMaskedData(bndsCenter, varR)) ; varIt.IsOk(); varIt.Next()) {
                    ...
                    break;
                default : throw info_except("Limiter type <%i> - unknown\n", limitType);
            }
        }
...
// Use baseRvar(- 1, +2), limiterVar(0,0)
// Result - (0,0)
        void ClcFlux(TPolyRegion *reg, const Stroka &fluxLow, const Stroka &fluxHigh, const Stroka &sound, const Stroka &dX,
            const Stroka &baseRvar, const Stroka &resFlux, const TRegionBounds &bndsCenter, double tStp) {
            Init(reg, fluxLow);
            ...
            reg->Grid.GetMaskedData(bndsCenter, resFlux) = reg->Grid.GetMaskedData(bndsCenter, fluxHigh); 
            reg->Grid.GetMaskedData(bndsCenter, resFlux) -= reg->Grid.GetMaskedData(bndsCenter, fluxLow);
            reg->Grid.GetMaskedData(bndsCenter, resFlux) *= reg->Grid.GetMaskedData(bndsCenter , "flicLim");
            reg->Grid.GetMaskedData(bndsCenter, resFlux) += reg->Grid.GetMaskedData(bndsCenter, fluxLow);
        }
    };
    struct ClcGasdStep {
    ...
    struct TPolyMarchU2 : TPolyMarchRegionBase {
        struct FluxClaculatorU2 : FluxCalculatorBase {
            ...
            int save_data_state( FilterOut&so) {
                so<<" SoundSpeed "<<SoundSpeed<<" NegativeSound "<<NegativeSound<<" LinearFlux "<<LinearFlux;
                return 1; 
            };
        ...
        TGridMaskedData::TIterator  itY = reg->Grid.GetMaskedData(TRegionBounds(-2, 2), "Y").Start();
        TGridMaskedData::TIterator  itX = reg->Grid.GetMaskedData(TRegionBounds(-2, 2), "X").Start();
        TGridMaskedData::TIterator  itE = reg->Grid.GetMaskedData(TRegionBounds(-2, 2), "X").Last();
        double X0 = *(double*)reg->Grid.GetMaskedData(TRegionBounds(), "X").Start().GetElementPtr();
        double X1 = *(double*)reg->Grid.GetMaskedData(TRegionBounds(), "X").Last().GetElementPtr();
        double Xm = 0.5*(X1-X0);
        for(; itX.IsOk() ; itX.Next(),itY.Next()) {
            *(double*)itY.GetElementPtr() = 5 + 5 * sin(2* M_PI / (X1 - X0) * (*(double*)itX.GetElementPtr() - X0));
...
// Result - Y(0,-1)
            ...
            ClcFlic.ClcFlux(reg, "FluxForce", "FluxRI", "C05", "dX", "Y", "FluxFlic", TRegionBounds(0, 0), tStp);
            SetDeltaFluxes(reg, "FluxVar FluxLF FluxRI FluxForce FluxFlic");

            reg->Grid.GetMaskedData(BndCenteredU, "Y") -= reg->Grid.GetMaskedData(BndCenteredU, MarchFlux) / 
        ...
        void SetDeltaFluxes(TPolyRegion *reg, const Stroka &fluxes) {
            ...
            for(size_t i = 0; i < vecF.size(); i++) {
                ...
                reg->Grid.GetMaskedData(BndCenteredU, vecF[i]) = reg->Grid.GetMaskedData(BndCenteredU + 1, vecF[i]) - 
        ...
        int save_data_state( FilterOut&so) {
            so<<" MarchFlux(FluxVar:FluxLF:FluxRI:FluxForce:FluxFlic) "<<MarchFlux<<" MarchCourant "<<MarchCourant;
    ...
    struct TPolyMarchDriverFluxU2 : TPolyMarchDriverFluxBase {
        ...
        virtual Stroka GetRvarName(TPolyRegion *reg, const Stroka &uName, const TRegionBounds &bnd) {return Yname;};
        virtual Stroka GetVarName()  {return Yname;};
    ...
    struct TPolyMarchDriver : TPolyMarchRegionBase {
        ...
        void Init(TPolyRegion *reg) {
            if (! reg->Grid.VarExists("FluxVar")) {
                ...
                reg->Grid.AddVar("FluxVar", SavableClass::TestType<TGridVariablesBase>(src.Data->Duplicate()));
                reg->Grid.AddVar("FluxLF", SavableClass::TestType<TGridVariablesBase>(src.Data->Duplicate()));
                reg->Grid.AddVar("FluxRI", SavableClass::TestType<TGridVariablesBase>(src.Data->Duplicate()));
                reg->Grid.AddVar("FluxForce", SavableClass::TestType<TGridVariablesBase>(src.Data->Duplicate()));
                reg->Grid.AddVar("FluxFlic", SavableClass::TestType<TGridVariablesBase>(src.Data->Duplicate()));
                reg->Grid.AddVar("dX", new TGridVar<double>());
                reg->Grid.AddVar("C05", new TGridVar<double>());
                reg->Grid.AddVar("TimedDeltaFlux", SavableClass::TestType<TGridVariablesBase>(src.Data->Duplicate()));
                BndCenteredU.Set(0, -1);
                BndBoundX.Set(0, 1);
                BndFluxU.Set(-1, 1);
                BndTime.Set(-1, 1);
...
// Use dX(-1,1) Y(-1,1)
        ...
        void MakeTimeStep(TPolyRegion *reg, double curTime, double tStp) {
            Stroka yName = ClcBaseFlux->GetVarName(), rName = ClcBaseFlux->GetRvarName(reg, yName, BndBoundX);
            ...
            ClcFlic.ClcFlux(reg, "FluxForce", "FluxRI", "C05", "dX", rName, "FluxFlic", TRegionBounds(0, 0), tStp);
            SetDeltaFluxes(reg, "FluxVar FluxLF FluxRI FluxForce FluxFlic");
...
//TGridVectorVar<double> *yVar = SavableClass::TestType<TGridVectorVar<double> >(reg->Grid.GetVar(yName));
            ...
            reg->Grid.GetMaskedData(BndCenteredU, "TimedDeltaFlux") = ((reg->Grid.GetMaskedData(BndCenteredU, MarchFlux) / 
                                                             reg->Grid.GetMaskedData(BndCenteredU, "dX")) * (-tStp));
        ...
        Stroka MakeHelp() {
            Stroka ret = " Basic class for time step calculation - constructing time step and modifed fluxes\n";
            ret += "For Sound and basic FLux calculation uses FluxCalculatorBase class.\n";
    ...
    struct TPolyMarchDriverFluxGasdLagrangeImitation : TPolyMarchDriverFluxBase {
        ...
        virtual Stroka GetRvarName(TPolyRegion *reg, const Stroka &uName, const TRegionBounds &bnd) {return PresName;};
        ...
        virtual void InitRegion(TPolyRegion *reg) {
            ...
            if (! reg->Grid.VarExists(CombiVar)) {
                TFluxPrepare::SetVectorVar(reg, DensName + " " + VelName + " " + EnerName + " " + PresName, CombiVar);
            }
        }
        ...
        void ClcSound(TPolyRegion *reg, const Stroka &uName, const Stroka &sName,const TRegionBounds &bnds) {
            TGridMaskedData sound(reg->Grid.GetMaskedData(bnds, sName));
            TGridVectorVar<double> *vec = SavableClass::TestType<TGridVectorVar<double> >(reg->Grid.GetVar(uName));
        ...
        int save_data_state( FilterOut&so) {
            so<<" SoundSpeed "<<SoundSpeed<<" NegativeSound "<<NegativeSound<<" LinearFlux "<<LinearFlux<<SavableClass::EOLN();
            so<<" Xname "<<Xname<<" DensName "<<DensName<<" VelName "<<VelName<<" EnerName "<<EnerName<<" PresName "<<PresName<<" EOSName "<<EOSName<<SavableClass::EOLN();
        ...
        int read_data_state(FilterIn&si) { 
            Stroka tmp;
            si>>tmp>>SoundSpeed>>tmp>>NegativeSound>>tmp>>LinearFlux;
            si>>tmp>>Xname>>tmp>>DensName>>tmp>>VelName>>tmp>>EnerName>>tmp>>PresName>>tmp>>EOSName;
            return 1;
        }
    ...
    struct TPolyMarchDriverFluxGasdLagrange : TPolyMarchDriverFluxBase {
        ...
        Stroka MassName, InvDencName, EnergySumName, SoundName, VelPrevName, CombiRVar, TmpVarName;
        ...
        void ClcSound(TPolyRegion *reg, const Stroka &uName, const Stroka &sName,const TRegionBounds &bnds) {
            ...
            Abs(reg->Grid.GetMaskedData(bnds, TmpVarName), (reg->Grid.GetMaskedData(bnds + 1, VelName) - reg->Grid.GetMaskedData(bnds, VelName)));
            reg->Grid.GetMaskedData(bnds, sName) += reg->Grid.GetMaskedData(bnds, TmpVarName);
