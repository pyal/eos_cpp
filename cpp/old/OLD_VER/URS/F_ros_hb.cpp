...
double IntExpLow(double a, double b, double d, double q, double x0, int One = 0) {
    if(!One)
        return -a * (IntExpInf(0, -b, d, q, x0) - IntExpInf(0, -b, d, q, 1));
}
double IntExpLow1(double a, double b, double d, double x0, int One = 0) {
    if(!One)
        return -a * (IntExpInf1(0, -b, d, x0) - IntExpInf1(0, -b, d, 1));
}
...
double IntPow6Inf(double a, double b, double d, double q, double x0) {
    ...
    return a / (d6 * 24) * ((cos(qx0) * qx0 * (2 - qx2) + sin(qx0) * (6 - qx2)) / x4 +
                            (0.5 * M_PI - SiQ) * q4);
}
...
double UnityIntegral() {
    ...
    if(d >= x0) {
        return (IntExpInf1(ExpIa, ExpIb, d, 1) * 6 -
                IntPow6Inf1(PowIa, PowIb, d, 1) * Alpha) *
               Epsilon / (Alpha - 6);
    }
    ...
    return (IntExpInf1(ExpIa, ExpIb, d, x0 / d) * 6 -
            IntPow6Inf1(PowIa, PowIb, d, x0 / d) * Alpha) *
               Epsilon / (Alpha - 6) +
           LowInt;
};
...
double IntExp6Correlation(double niu) {
    ...
    DInt +=
        //		InFinQunc10(DoubleIntegral,0.1*StndErr,StndErr,100,10,1000,Flag)
        RecInt(DoubleIntegral, 0.1 * StndErr, 25, StndErr) * Epsilon / (Alpha - 6);
    if(Flag != 0)
        cout << " Flag " << Flag << "\n";
