...
void CellFixed(RegionIO &Reg,double &PresDeriv)
  {
   ...
   M_l[Num]=0;M_l[0]=0;
   P_l[0]=10;P_l[Num]=10;
   P_l[0]=10;P_l[Num]=10;U_l[0]=U_l[1];U_l[Num]=U_l[Num-1];
   F_l[1]=0;F_l[Num]=0;
   for (k=2;k<Num;k++) 
			  {
...
//  Right - Calculating masses between points
   ...
   M_l[Num]=0;M_l[0]=0;
   M_l[1]*=0.5;M_l[Num]*=0.5;  // Now Added ?
  }; 
/*
...
//cout<<"StepRegion  beg    Coreleft() "<<coreleft()<<"\n";
   ...
   CalcPos(Reg,TimeStp,MinTimeStp);CalcMain(Reg,TimeStp);
			Reg.TimeCh(TimeStp);                
  };
