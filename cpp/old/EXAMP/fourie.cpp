...
{ double *tmp=new double[NumPnt+1];tmp[0]=NumPnt;}
...
//  double MainHarm=2*M_PI/(x[NumPnt]-x[1]);
  ...
  BaseVal=0;for (int k=1;k<=NumPnt;k++) BaseVal+=y[k];
 };
...
int main(int argc, char *argv[])
{
  ...
  for (int k=1;k<=SinCoef[0];k++) out<<x[k]<<FMT<<SinCoef[k]<<FMT<<CosCoef[k]<<"\n";

  delete x;delete y;delete CosCoef;delete SinCoef;
  return EXIT_SUCCESS;
}
