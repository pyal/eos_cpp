...
//  double Low,Up;
  ...
  static double xFunc(double x){if (CurRndStruct) return CurRndStruct->Clc(x)*CurRndStruct->YNorm;return 0;}
...
//  virtual double Rnd() { return X_from_Prob(double(rand())/RAND_MAX);};
  virtual double Rnd() { return X_from_Prob(RandomClass::Rnd(1));};

  virtual double X_from_Prob(double x)  { return Int.Evaluate(x*Norm);};
...
struct RndGaus:RndIntegral
{
  ...
  RndGaus(){Set(0,0);};
  RndGaus(double zero,double sigma){Set(zero,sigma);};
  void Set(double zero,double sigma){GausDistribSigma=sigma;GausDistribZero=zero;}
...
//cout<<x<<FMT<<exp(-sqr((x-GausDistribZero)/GausDistribSigma)/2)/sqrt(2*M_PI)/GausDistribSigma<<"\n";
	...
	if (dE>-40)	return exp(dE)/sqrt(2*M_PI)/GausDistribSigma;
	return 0;
  }
};
...
//  RndBoltsmanSinPower(){Set(0,1);};
  RndBoltsmanSinPower(double BoltCoef=0,double SinPower=1,double BoltCoef2=1):
                      RndIntegral(1){Set(BoltCoef,SinPower,BoltCoef2);};
  void Set(double BoltCoef,double SinPower,double BoltCoef2)
  {BoltsmanDistribCoef=BoltCoef;BoltsmanSinPower=SinPower;
  ...
  virtual double Clc(double x)
  {
    ...
    if (BoltsmanSinPower<0)
    {
      ...
      else r=BoltsmanDistribCoef*pow(fabs(cos(x*M_PI/180)),-BoltsmanSinPower);
    }
    else
    {
      if (BoltsmanSinPower==(int)BoltsmanSinPower) 
      r=BoltsmanDistribCoef*pow(sin(x*M_PI/180),BoltsmanSinPower);
      else r=BoltsmanDistribCoef*pow(fabs(sin(x*M_PI/180)),BoltsmanSinPower);
    }
//   if ( r>40) return 0;
   return exp(-r);
  }
...
//	  RelaxTime=1;
  ...
  void SetParamVals(VecCl &values){ParamVal=values;};
...
//	  double s=0,x=double(rand())/RAND_MAX;
	  ...
	  int i=0;
	  while ((x>s) && (i<NormTime[0])) {i++;s+=NormTime[i];}
	  CurrentState=i;
//cout<<x<<" RndStart "<<CurrentState<<"  "<<ParamVal[CurrentState]<<"\n";
  ...
  double RndTstp()
  {
	  if (!TimesGenerated) GenerateTimes(1);
