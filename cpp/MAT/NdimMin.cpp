...
int MinFind1DClass::GoodVec(vector<IndexedVal> &vec) {
    ...
    if((vec[1].ValInd() <= vec[0].ValInd()) && (vec[1].ValInd() <= vec[2].ValInd()))
        return 1;
    return 0;
}
...
double MinFind1DClass::CheckDifficult(double &StrtMisf, vector<IndexedVal> &vec) {
    ...
    if(ZeroStp(vec[2].ValCmp(), vec[0].ValCmp())) {
        for(int k = 0; k < (int)vec.size(); k++)
            vec1[k] = IndexedVal(vec[k].ValCmp(), vec[k].ValInd());
        sort(vec1.begin(), vec1.end());
        StrtMisf = vec1[0].ValCmp();
        return vec1[0].ValInd();
    }
    double x4m = (vec[0].ValCmp() + vec[1].ValCmp()) * 0.5;
    double y4m = Func(x4m);
    vec1[2] = IndexedVal(y4m, x4m);
    if(GoodVec(vec1))
        return ThreePointEst(StrtMisf, vec1);
    vec1 = vec;
    ...
    if(GoodVec(vec1))
        return ThreePointEst(StrtMisf, vec1);
    vec1[2] = IndexedVal(y4p, x4p);
    vec1[0] = IndexedVal(y4m, x4m);
    vec1[1] = vec[1];
    if(!GoodVec(vec1)) {
        cout << "Very bad?\n";
        StrtMisf = vec[1].ValInd();
        return vec[1].ValCmp();
    } else
        return ThreePointEst(StrtMisf, vec1);
}
double MinFind1DClass::ThreePointEst(double &StrtMisf, vector<IndexedVal> &vec) {
    ...
    x4 = FixedMin(
        ...
        vec[1].ValInd(),
        vec[2].ValInd());
    if((x4 < vec[0].ValCmp()) || (x4 > vec[2].ValCmp()))
        return CheckDifficult(StrtMisf, vec);
    //    if (stp very small...
    double y4 = Func(x4);
    if(y4 > vec[1].ValInd())
        return CheckDifficult(StrtMisf, vec);
...
double MinFind1DClass::LinMin(double StrtStpEst, double StrtPnt, double &StrtMisf) {
    ...
    double y0 = StrtMisf;
    vector<IndexedVal> vec(3);
    vec[0] = IndexedVal(y0, Pos);
    vec[1] = IndexedVal(Func(Pos + Stp), Pos + Stp);
    if(vec[1].ValInd() - vec[0].ValInd() > 0)
        Pos = Pos - Stp;
    else
        Pos += 2 * Stp;
    ...
    for(int k = 1; k <= MaxLinIter; k++) {
        if(GoodVec(vec))
            return ThreePointEst(StrtMisf, vec);
        if(ConstVec(vec))
            break;
...
//                             1/2 -----------------------------------------------
//                                  (-y3 + y2) x1 + (-y1 + y3) x2 + (y1 - y2) x3
double MinFind1DClass::FixedMin(
    double x1,
    double x2,
    double x3,
    double y1,
    ...
    if(fabs(denom) < MinDouble)
        denom = (denom < 0) ? -MinDouble : MinDouble;
    return 0.5 * ((-y3 + y2) * sqr(x1) + (-y1 + y3) * sqr(x2) + (y1 - y2) * sqr(x3)) /
           denom;
}


void Derive2DClass::Execute(
    const VecCl &Par,
    VecCl &Stp,
    MatrCl &D2,
    VecCl &D1,
    ...
    for(int k = 1; k <= Par.Dim(); k++) {
        for(int k1 = k; k1 <= Par.Dim(); k1++) {
            D2(k1, k) = (F2(k, k1) - F1[k1] - F1[k] + D0) / (s[k] * s[k1]);
            D2(k, k1) = D2(k1, k);
        }
    }
};
void Derive2DClass::Execute(const VecCl &Par, VecCl &Stp, VecCl &D1, double &D0) {
    ...
    for(int k = 1; k <= Par[0]; k++) {
        ...
        p[k] += dx;
        D1[k] = (Execute(p) - D0) / dx;
        p[k] = Par[k];
    }
};
void Derive2DHi2::Execute(
    const VecCl &Par,
    VecCl &Stp,
    MatrCl &D2,
    VecCl &D1,
    ...
    for(int k = 1; k <= N; k++)
        if(EigV[k] > MatrZer) {
            ...
            for(int k1 = 1; k1 <= N; k1++)
                ParMis[k1] += fabs(c * tmp[k1] * (Norm[k1]));
        }
    if(Sum > 0)
        ParMis = ParMis * (1. / Sum);
    return ParMis;
...
double GetParamsCorrelation::ParamsCorrelation(VecCl &Par, VecCl &Stp, VecCl &ParMis) {
    VecCl StpV, D1, EigV(Par.Dim());
    StpV = Stp;
    ParMis = Par * 0;
    StpV = Stp * 0 + MathEps05 * 100;
    double FoundMisf;
    if(Par.Dim() == 0) {
        FoundMisf = Deriv2->Execute(Par);
        return 0;
    }
    MatrCl D2;
    Deriv2->Execute(Par, StpV, D2, D1, FoundMisf);
...
struct FormMinDirections {
    ...
    void Execute(VecCl &StpEst, VecCl &NormVarC, double lambda, int Meth) {
        ...
        Res = Res * 0;
        for(int k = 1; k <= Dim(); k++)
            Res = Res + MatrCl::GetCol(StpDir, k) * StpEst[k];
        StpEst = Res;
        Method = NoLambda;
        Lambda = 0;
    }
    void Execute(MatrCl &StpDir, VecCl &StpEst, VecCl &StpOrder, VecCl &NormVarC) {
        ...
        vector<IndexedVal> vec;
        vec.reserve(Edelta.Dim());
        //vector<IndexedVal> vec(Edelta.Dim());
        //cout<<"Edelta\n"<<Edelta<<"EigV\n"<<EigV<<"D0 "<<D0<<"\n";
        for(int k = 1; k <= Dim(); k++) {
            double tmp = fabs(Edelta[k] * D0) > MathZer
                             ? fabs(
                                   sqr(Edelta[k]) /
                                   (max<double>(fabs(EigV[k]), MathZer05) * 4 * D0))
                             : 0;
            if((tmp > MathZer05) && (fabs(EigV[k]) > ErrorLQ)) {
                tmp = tmp * fabs(D0);
                StpEst[k] = Edelta[k] / EigV[k];
            } else {
                StpEst[k] = 0;
                tmp = 0;
            }
...
private:
    ...
    void NormalSolve_Unstable(
        ...
        VecCl ND1;
        ND1 = Cvt2Coord(D1, Norm);
        if(!ReduceLQ(ND2, StpDir, EigV.Ptr, ErrorLQ)) {
            NormalSolve_Symm(StpDir, EigV, Edelta, NormVarC);
            return;
        }
        MatrCl EigM;
        EigM = EigV;
        ...
        for(int k = 1; k <= n; k++) {
            for(int k1 = 1; k1 <= n; k1++)
                if((d = max<double>(fabs(m1(k, k1)), fabs(m2(k, k1)))) > M_Eps2)
                    ...
                    Mis = max<double>(Mis, fabs(m1(k, k1) - m2(k, k1)));
        }
        return Mis;
    }
};
...
#ifndef WCPP_NET
DescribedClass *Dummy_EigenVectMinim() {
    return new EigenVectMinim();
}
static ClassDesc EigenVectMinim_ClassDesc(
    typeid(EigenVectMinim),
    "minimN_eigen",
    1,
    "MDMin",
    Dummy_EigenVectMinim);
#else
static ClassDesc EigenVectMinim_ClassDesc(
    typeid(EigenVectMinim),
    "minimN_eigen",
...
void EigenVectMinim::FormStpNormDir(
    MatrCl &D2,
    VecCl &D1,
    double D0,
    MatrCl &StpDir,
    ...
    dir.Method = FormMinDirections::OptimLambda;
    dir.Lambda = LambdaCoef;
    dir.Execute(StpDir, StpEst, StpOrder, NormVarCoef);
    StpGain = dir.StpGain;
}

int EigenVectMinim::MakeRndMotion(
    double &HiStrt,
    VecCl &ParStrt,
    MatrCl &StpDir,
    VecCl &StpEst,
    ...
    int NumStp = 50;
    Ref<RndIntegral> rndStp = new RndGaus(0.5, 2);
    rndStp->Init(-4, 4, 100, 1e-2);
    RandWalk wlk(Tstrt, Tend, Astrt, Aend, NumStp, Deriv2, rndStp);
    MatrCl StpM = StpDir;
    for(int k = 1; k <= StpM.Dim(); k++)
        MatrCl::SetCol(StpM, k, MatrCl::GetCol(StpDir, k) * StpEst[k]);
    for(int k = 1; k <= StpM.Dim(); k++) {
        for(int k1 = 1; k1 <= StpM.Dim(); k1++)
            StpM(k, k1) *= NormVarCoef[k];
    }
...
double EigenVectMinim::MakeRndStp(
    double &HiStrt,
    VecCl &Par,
    VecCl &Stp,
    double StpEst,
    ...
    for(int k = 1; k <= Stp.Dim(); k++)
        StpDo[k] *= NormVarCoef[k];
    double T = StpGet / 8;
    Ref<RndIntegral> rndStp = new RndGaus(1, 2);
    rndStp->Init(-2, 4, 100, 1e-2);
    double Xstp, Xres = 0;
...
int EigenVectMinim::CheckModel(
    ...
    VecCl Stp = Par * 0;
    double Gain = 0;
    for(int k = 1; k <= StpGain.Dim(); k++)
        Gain += StpGain[k];
    for(int k = 1; k <= StpGain.Dim(); k++)
        Stp = Stp + MatrCl::GetCol(StpMatr, k) * StpEst[k];
    for(int k = 1; k <= Stp.Dim(); k++)
        Stp[k] *= NormVarCoef[k];
...
void EigenVectMinim::Execute(
    ...
    //}
    FormStpNormDir(
        D2, D1, HiStrt, StpDir, StpEst, StpOrder, NormVarCoef);   //+SetNormalCoef
    if((NumIter != -1) && (CheckModel(HiStrt, StpDir, StpEst, Par, NormVarCoef)))
        return;
    MultiDim2One Func(Deriv2);
    Min1D->SetFunc(MultiDim2One::Func1D);
    ...
    for(int k = 1; k <= D1.Dim(); k++) {
        ...
        if(fabs(Stp1D) > fabs(0.01 * StpEst[Ind]))
            StpEst[Ind] = Stp1D;
        else
...
int MinFindNDimClass::Execute(VecCl &Par, VecCl &Stp, int NumIter, double &FoundMisf) {
    if(Par.Dim() == 0) {
        FoundMisf = Minim->GetDeriv2()->Execute(Par);
        return 0;
    }
    VecCl StpV, Par0;
    StpV = Stp;
    ...
    for(int k = 1; k <= Stp.Dim(); k++)
        Stp[k] = max<double>(fabs(Par0[k] - Par[k]) * 10, 100 * MathEps05);
...
struct LambdaMin {
    ...
    static double Func(double log_lambda) {
        VecCl tmp = CurPtr->Par;
        return CurPtr->Clc(log_lambda + CurPtr->ZerLambda, tmp);
    }
    ...
    void SetPar(
        ...
        VecCl &par,
        double hi0,
        Derive2DClass *deriv2,
        MinFind1DClass *min1d,
        LMMinim *lmmin) {
        D2 = d2;
        D1 = d1;
        Par = par;
...
#ifndef WCPP_NET
DescribedClass *Dummy_LMMinim() {
    return new LMMinim();
}
static ClassDesc LMMinim_ClassDesc(
    typeid(LMMinim),
    "minimN_LM",
    1,
    "MDMin",
...
LMMinim::LMMinim()
    : EigenVectMinim(), Lambda(10, 20, 0.1), Optimized(FormMinDirections::OptimLambda){};
LMMinim::LMMinim(
    Derive2DClass *deriv2,
    MinFind1DClass *min1d,
    double errorlq,
    ...
    double eps2,
    LambdaData lambda)
    : EigenVectMinim(deriv2, min1d, errorlq, zeroeig, eps2, 2),
      Lambda(lambda),
...
void LMMinim::Execute(
    ...
    if(Lambda.CurCoef > Lambda.DivideVal) {
        ...
        Log_Lam = MinLamb->Execute(-1, HiStrt);
        Min.Restore();
    ...
    if(Lambda.CurCoef < 1e-15)
        Lambda.CurCoef = 10 * Lambda.DivideCoef;
};
...
void MapVars::SetMap(VecCl &start, VecCl &coder) {
    mapVar.clear();
    mapVar.reserve(coder.Dim());
    Start = start;
    for(int k = 1; k <= coder.Dim(); k++)
        if(coder[k] != 0)
            mapVar.push_back(k);
};
