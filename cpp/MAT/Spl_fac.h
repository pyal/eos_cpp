...
struct SplineXYIO : ISpline {
    ...
    SplineXYIO(){};
    virtual void SetLimits(double xu, double xl, double yu, double yl) {
        Xup = xu;
        Xlow = xl;
        Yup = yu;
        Ylow = yl;
    ...
    virtual void SetLimits(TData<real> &dat) {
        if(Stricmp(dat.InterfaceIOName(), "TTable") != 0) {
            cout << "SetLimits wrong data.\n";
            exit(0);
        }
        SetLimits(
            dat.D[0][dat.I[0] - 1], dat.D[0][0], dat.D[1][dat.I[1] - 1], dat.D[1][0]);
    };
    ...
    int save_data_state(FilterOut &out) {
        out.precision(12);
        out << "\nMisfit " << Delta << " Xup " << Xup << " Xlow " << Xlow << " Yup "
            << Yup << "  Ylow " << Ylow << "\n";
        out << " BSpline_XY_Coef \n";
        BSpl.SaveIni(out);
        return 1;
    ...
    int read_data_state(FilterIn &in) {
        char tmp[256];
        in >> tmp >> Delta >> tmp >> Xup >> tmp >> Xlow >> tmp >> Yup >> tmp >> Ylow >>
            tmp;
        BSpl.ReadIni(in);
        if(!in)
...
struct SplineXIO : ISpline {
    ...
    virtual int Generate(int Cont, map<Stroka, vector<double>> &vec) {
        ...
        if(vec.find("X") == vec.end() || vec.find("Y") == vec.end())
            throw info_except(
                "Have to define X Y (and can define W ) variables to generate 2D spline\n");
        dat.AddColumn(&vec["X"][0], (int)vec["X"].size());
        dat.AddColumn(&vec["Y"][0], (int)vec["Y"].size());
    ...
    virtual int Evaluate(map<Stroka, vector<double>> &vec) {
        if(vec.find("X") == vec.end() || vec.find("Y") == vec.end())
            throw info_except(
                "Have to define X Y (and can define W ) variables to evaluate 2D spline\n");
    ...
    virtual void SetLimits(double xu, double xl) {
        Xup = xu;
        Xlow = xl;
    };
    virtual void SetLimits(TData<real> &dat) {
    ...
    static double Integrate(double xFrom, double xTo, double misf, SplineXIO *spl) {
        ...
        StaticSpl = spl;
        double ret =
            Qags_IntSubInt(IntegSplClcFunc, xFrom, xTo, misf, misf, 10000, ResErr, Err);
        StaticSpl = oldSpl;
        return ret;
    }
...
struct RegridSpline : SplineXYIO {
    ...
    int save_generation_par(FilterOut &out) {
        out << "GenerationMisfit" << GenMisfit << "GenerationNumX" << GenNumX
            << "GenerationNumY" << GenNumY << SavableClass::EOLN();   //<<"\n";
        return !(!out);
    }
...
struct ExtendedRegridSpline : RegridSpline {
    ...
    virtual int Evaluate(double *x, double *y, double *z, int nx) {
        ...
        for(i = 0; i < nx; i++) {
            X[i] =
                EncodeVal(SetInLimit(x[i], BndXlow, BndXup), LogX, AddBeforeLogX, MulX);
            Y[i] =
                EncodeVal(SetInLimit(y[i], BndYlow, BndYup), LogY, AddBeforeLogY, MulY);
        }
    ...
    int save_data_state(FilterOut &out) {
        out << "LogX" << LogX << "LogY" << LogY << "LogZ" << LogZ << "AddBeforeLogX"
            << AddBeforeLogX << "AddBeforeLogY" << AddBeforeLogY << "AddBeforeLogZ"
            << AddBeforeLogZ << SavableClass::EOLN();
        out << "MulX" << MulX << "MulY" << MulY << "MulZ" << MulZ << SavableClass::EOLN();
        out << "MinX" << DecodeVal(Xlow, LogX, AddBeforeLogX, MulX) << "MaxX"
            << DecodeVal(Xup, LogX, AddBeforeLogX, MulX) << "MinY"
            << DecodeVal(Ylow, LogY, AddBeforeLogY, MulY) << "MaxY"
            << DecodeVal(Yup, LogY, AddBeforeLogY, MulY) << SavableClass::EOLN();
        return SplineXYIO::save_data_state(out);
    };
    int read_data_state(FilterIn &in) {
        char tmp[256];
        in >> tmp >> LogX >> tmp >> LogY >> tmp >> LogZ >> tmp >> AddBeforeLogX >> tmp >>
            AddBeforeLogY >> tmp >> AddBeforeLogZ;
            ...
            for(int i1 = 0; i1 < dat.I[i]; i1++) {
                if(!decodeIt) {
                    if(!CheckVal(dat.D[i][i1], l[i], a[i], m[i])) {
                        ...
                        throw info_except(
                            "Row %i min val is %f max val is %f AddVal is %f MulVal is %f.Cannot make log...\n",
                            i,
                            Min,
                            Max,
                            a[i],
...
struct CurveSpline : SplineXIO {
    int SplineOrder;
    CurveSpline(int SplOrder = 3) {
        SplineOrder = SplOrder;
    }
    //   real *Weight;
    virtual int Generate(
        int &NumX,
    ...
    void SaveIni(ostream &out) {
        ...
        if(!out)
            throw info_except(" Could not save class %s\n ", class_name());
    }
    static int SplineFromData(
        const char *file,
        CurveSpline &spl,
        double misf,
        const char *mes) {
        ...
        if(mes != NULL && (vec.size() != 2 || (nR = (int)vec[0].size()) < 3))
            throw info_except(
                "Got file <%s> with NumCol %i NumRow %i\nErrorMes:<%s>",
                file,
                vec.size(),
                nR,
                mes);
        if(vec.size() == 2 && (nR = (int)vec[0].size()) >= 3) {
            ...
            //double misf = 1e-2;
            int Gen =
                spl.Generate(NumInt, misf, 0, &vec[0][0], &vec[1][0], (int)vec[0].size());
            if(!Gen && mes)
                throw info_except(
                    "File <%s> Could not generate spline\nErrorMes:<%s>", file, mes);
            if(Gen)
                return 1;
        }
        return 0;
    ...
    Stroka MakeHelp() {
        Stroka ret =
            "ier=0  : normal return. the spline returned has a residual sum of squares fp such that abs(fp-s)/s <= tol with tol a relative tolerance set to 0.001 by the program. \n";
        ret +=
            "ier=-1 : normal return. the spline returned is an interpolating spline (fp=0). \n";
        ret +=
            "ier=-2 : normal return. the spline returned is the weighted least squares polynomial of degree k. in this extreme case fp gives the upper bound fp0 for the smoothing factor s. \n";
        ret +=
            "ier=1  : error. the required storage space exceeds the available storage space, as specified by the parameter nest.\n";
        ret +=
            "ier=2  : error. a theoretically impossible result was found during  the iteration proces for finding a smoothing spline with fp = s. probably causes : s too small. \n";
        ret +=
            "ier=3  : error. the maximal number of iterations maxit (set to 20  by the program) allowed for finding a smoothing spline  with fp=s has been reached. probably causes : s too small \n";
        ret +=
            "ier=10 : error. on entry, the input data are controlled on validity the following restrictions must be satisfied.  -1<=iopt<=1, 1<=k<=5, m>k, nest>2*k+2, w(i)>0,i=1,2,...,m  xb<=x(1)<x(2)<...<x(m)<=xe, lwrk>=(k+1)*m+nest*(7+3*k) \n";
        ret += "\n\n";
        return ret;
    }
};
struct CurveSplineEncode : CurveSpline {
    ...
    int save_generation_par(FilterOut &out) {
        ...
        out << "LogX" << LogX << "AddBeforeLogX" << AddBeforeLogX << "MulX" << MulX
            << SavableClass::EOLN();
        out << "LogY" << LogY << "AddBeforeLogY" << AddBeforeLogY << "MulY" << MulY
            << SavableClass::EOLN();
        return !(!out);
    }
...
struct CurveSpline_Noise : CurveSpline {
    ...
    virtual int save_generation_par(FilterOut &out) {
        out << " Mode " << Mode << " SplineOrder " << SplineOrder << " ModeVal "
            << ModeVal;
    ...
    Stroka MakeHelp() {
        Stroka ret =
            "|MD| = 1: Prior given value for p in VAL  (VAL.ge.ZERO). This is the fastest  use of GCVSPL, since no iteration  is performed in p.\n";
        ret += "|MD| = 2: Generalized cross validation. \n";
        ret +=
            "|MD| = 3: True predicted mean-squared error, with prior given variance in VAL.\n";
        ret +=
            "|MD| = 4: Prior given number of degrees of  freedom in VAL (ZERO.le.VAL.le.N-M). \n";
        ret += "Error return:\n";
