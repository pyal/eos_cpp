...
struct CurveAnalyzer{
    struct AtomCurve{
        ...
        void MakeModelB(double xLft, double xRgt, double yLft, double yRgt){
            ...
            B0 = (-6*A0*x0*x2+x2*y0*(2*x0+x2)+x0*y2*(x0+2*x2))/denom;
            B1 = ( 6*A0*(x0+x2)-2*(x2*(2*y0+y2)+x0*(y0+2*y2)))/denom;
        ...
        void ClcParams(int beg, int end, const vector<double> &xar, const vector<double> &yar, double maxmisfit){
            Set((xar[beg] + xar[end-1])*0.5, (yar[beg] + yar[end-1])*0.5, 0, 0, beg, end, xar[beg], xar[end-1], maxmisfit);
            if (beg == end - 1)
               return;
            double a0, a1, x0;
            ...
            Set(x0, a0, a1, sqrt(Delta2Model / modelmisfit.size()), beg, end, xar[beg], xar[end - 1], maxmisfit);
        }
    ...
    private:
        void Set(double x0, double a0, double a1, double sigma, int beg, int end, double xbeg, double xend, double maxmisfit){
            iBeg = beg;
            iEnd = end;
            xBeg = xbeg;
            xEnd = xend;
        ...
        vector<double> ClcDerivConst(double &x0, double &a0, double &a1, int beg, int end, const vector<double> &xar, const vector<double> &yar){
           int N = end - beg;
           double Mean = (N-1)*0.5;
           x0 = beg+Mean; 
...
//cout<<beg<<" "<<end<<" "<<a0<<" "<<a1<<"\n";
    ...
    struct AtomOpt{
        ...
        void SetDat(AtomCurve &fst, AtomCurve &sec, vector<double> &xar, vector<double> &yar){
            Fst = &fst;
            Sec = &sec;
            Xar = &xar;
            Yar = &yar;
        ...
        int OptBnd(AtomCurve &fst, AtomCurve &sec, vector<double> &xar, vector<double> &yar){
...
//cout<<lastres<<"\n";
            while (i < atoms.size() && AtomCurve::SameModel(Atoms[lastres], atoms[i], SameCoef/sqrt((double)StpL)) &&
                    atoms[i].iEnd-regbeg < MaxNumPnt){
            //while (i < atoms.size() && AtomCurve::SameModel(Atoms[lastres], atoms[i], MaxMisfit)){
                Atoms[lastres].ClcParams(Atoms[lastres].iBeg, atoms[i].iEnd, xar, yar, MaxMisfit);
                i++;
            }
//cout<<Atoms[lastres].iBeg<<" "<<Atoms[lastres].iEnd<<"\n";
            if (i >= atoms.size())
                break;
            Atoms.push_back(AtomCurve());
            Atoms[lastres + 1].ClcParams(atoms[i].iBeg, atoms[i].iEnd, xar, yar, MaxMisfit);
//cout<<lastres+1<<" plus\n";
//cout<<Atoms[lastres+1].iBeg<<" "<<Atoms[lastres+1].iEnd<<" plus\n";
            ...
            while (!optim.OptBnd(Atoms[lastres], Atoms[lastres+1], xar, yar)){
                if (i==atoms.size()-1){
                    ...
                    Atoms[lastres].ClcParams(Atoms[lastres].iBeg, atoms[i].iEnd, xar, yar, MaxMisfit);
                    i++;
                    Atoms[lastres + 1].ClcParams(atoms[i-1].iEnd, atoms[i].iEnd, xar, yar, MaxMisfit);
                }
            }
    ...
    vector< vector <double> > FormResult(const vector<double> &Xval, const vector<double> &Yval, Stroka &Desc){
