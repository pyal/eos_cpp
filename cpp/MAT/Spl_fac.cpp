...
static ClassDesc RegridSpline_ClassDesc(
    typeid(RegridSpline),
    "RegridSpline",
    1,
    "Spline_category",
    create<RegridSpline>);
static ClassDesc CurveSpline_ClassDesc(
    typeid(CurveSpline),
    "CurveSpline",
    1,
    "Spline_category",
    create<CurveSpline>);
static ClassDesc ExtendedRegridSpline_ClassDesc(
    typeid(ExtendedRegridSpline),
    "ExtendedRegridSpline",
    1,
    "Spline_category",
    create<ExtendedRegridSpline>);
static ClassDesc CurveSpline_Noise_ClassDesc(
    typeid(CurveSpline_Noise),
    "CurveSpline_Noise",
    1,
    "Spline_category",
    create<CurveSpline_Noise>);
...
int RegridSpline::Generate(
    ...
    int i[3] = {NumX, NumY, NumX * NumY};
    BSpl.SetDim(3, i);
    if(RD.I[2] != RD.I[1] * RD.I[0] || RD.I[1] < 2 || RD.I[0] < 2)
        throw info_except(
            "Bad dim of the input data DimZ(%i)!=DimX(%i)*DimY(%i) or DimX,DimY<2. Hint if DimY=1 then take into account that X=const each NumY times\n",
            RD.I[2],
            RD.I[0],
            RD.I[1]);
    real MisfR = Misf;
    integer NumXi = NumX, NumYi = NumY, Conti = Cont, px = 3, py = 3, DX = RD.I[0],
            DY = RD.I[1];
    integer SX, SY;

    integer NRWork = 4 + NumX * (DY + px * 2 + 5) + NumY * (2 * px + 5) +
                     (px + 1) * (DX + DY) + NumX + DY,
...
int CurveSpline::Generate(
    ...
    integer NumXs = NumX, Conti = Cont, NumXd = dat.I[Xcol], polynom_degree = SplineOrder,
            Result, NumXs_r;
    integer NRWork = NumXd * (polynom_degree + 1) + NumX * (3 * polynom_degree + 7),
            NIWork = NumX, *IWork;
    IWork = new integer[NIWork];
    ...
    if(Mis_ret == 0 || Misf < Mis_ret * 0.5 || IsNan(Mis_ret))
        throw info_except(
            "Could not make spline. Error %g have to be %g\n", Mis_ret, Misf);
    //Result *= 100;

    if(Result != 10 && Result != 1000) {
        BSpl.I[0] = NumX;
        BSpl.I[1] = NumX - polynom_degree - 1;
        Delta = Mis_ret;
    } else   //if (Result!=0)
    {
...
int CurveSpline_Noise::Generate(
    int &NumX,
    double &Misf,
    int Cont,
    TData<real> &dat,
