...
struct Cold_Vinet : NamedOneVarFunction {
    ...
    virtual int save_data_state(FilterOut &so) {
        so << " B0 " << B0 << " B0prime " << B0prime << " V0_cm3/mol " << V0
           << " MolVeight " << MolVeight << " ";
        return !(!so);
    }
...
struct Cold_TF_Kalitkin_P : NamedOneVarFunction {
    ...
    double KalitkinP(double Denc) {
        ...
        double Pc = 1. / 5 * pow(3 * sqr(M_PI), 2. / 3) * pow(ro_r, 5. / 3) -
                    13. / 36 * pow(3 / M_PI, 1. / 3) * pow(ro_r, 4. / 3);
        return Pc * M_Pau_GPa;
    }
...
struct Spl2DGenerator : ManyVarFunction {
    ...
    virtual int save_data_state(FilterOut &out) {
        out << " SplineFile " << SplFileName << " GenerateSpline " << GenerateSpline
            << " SplineDescr " << SplineDescr << " SplineClass " << SplName;
        Spl->save_generation_par(out);
        return 1;
    };
    virtual int read_data_state(FilterIn &in) {
        char tmp[256];
        in >> tmp >> SplFileName >> tmp >> GenerateSpline >> tmp >> SplineDescr >> tmp >>
            SplName;
        ...
        if(GenerateSpline == 0) {
            ...
            if(!in_)
                throw info_except(
                    "Wrong spline format. Spline file: %s\n", SplFileName.c_str());
        ...
        if(GenerateSpline == 2) {
            ...
            File::ReadFile(
                SplFileName.c_str(),
                dat,
                "Could not read data points for spline generation");
    ...
    ~Spl2DGenerator() {
        ...
        if(ret > 0)
            throw info_except(
                "Could not generate spline!!!. Return code is %i reason : \n", ret);
    ...
    Stroka MakeHelp() {
        ...
        for(ClassDesc *desc = ClassDesc::list_category_start("Spline_category"); desc;
            ...
            //continue;
            add += Stroka("For class <") + spl->class_name() +
                   "> input internal params are:[\n";
        ...
        res += add + ManyVarFunction::MakeHelp();
        return res;
    }
};
...
struct NoiseRemoval : ManyVarFunction {
    ...
    virtual int save_data_state(FilterOut &out) {
        out << " ResultFileName " << ResultFileName << " MaxMisfitVal " << MaxMisfitVal
            << " StartNumDiv " << StartNumDiv << " MaxNumPnt " << MaxNumPnt
            << " SameCoef " << SameCoef << " WriteDescr " << WriteDescr;
        return 1;
    };
    virtual int read_data_state(FilterIn &in) {
        char tmp[256];
        in >> tmp >> ResultFileName >> tmp >> MaxMisfitVal >> tmp >> StartNumDiv >> tmp >>
            MaxNumPnt >> tmp >> SameCoef >> tmp >> WriteDescr;
        if(!in)
            throw info_except("Could not read spline???\n");
    ...
    ~NoiseRemoval() {
        ...
        vector<vector<double>> model =
            CurveAnalyzer()
                .InitDat(Xval, Yval, StartNumDiv, MaxMisfitVal, MaxNumPnt, SameCoef)
...
struct NoiseWavelet : OneVarFunction {
    ...
    virtual int read_data_state(FilterIn &input) {
        char tmp[256];
        input >> tmp >> NoiseFile >> tmp >> NoiseXcol >> tmp >> NoiseYcol >> tmp >>
            WaveletType >> tmp >> Signal2NoiseRatio >> tmp >> UsePlusMethod;
        ...
        if(vec.size() <= (size_t)max<size_t>(NoiseYcol, NoiseXcol))
            throw info_except(
                "Bad input file Ncol <%i> Nrow<%i> but NoiseXcol <%i> NoiseYcol <%i>\n",
                vec.size(),
                vec[0].size(),
                NoiseXcol,
                NoiseYcol);
        ...
        if(!UsePlusMethod)
            Y = wave.Construct(
                &vec[NoiseYcol][0],
                int(vec[NoiseYcol].size()),
                WaveletType,
                Signal2NoiseRatio);
        else
            Y = wave.ConstructPlus(
                &vec[NoiseYcol][0],
                int(vec[NoiseYcol].size()),
                WaveletType,
                Signal2NoiseRatio);
    ...
    virtual int save_data_state(FilterOut &output) {
        output << " NoiseFile " << NoiseFile.c_str() << " NoiseXcol " << NoiseXcol
               << " NoiseYcol " << NoiseYcol << " WaveletType " << WaveletType;
        output << " Signal2NoiseRatio " << Signal2NoiseRatio << " UsePlusMethod "
               << UsePlusMethod;
        return 1;
    }
    Stroka MakeHelp() {
