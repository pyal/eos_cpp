...
struct InterfaceIO {
    ...
    virtual const char *InterfaceIOName() {
        return " InterfaceIO ";
    };

    virtual int ReadIni(istream &input) {
        return 1;
    };
...
template <class T>
extern int TTableTColumn(TData<T> &Res, TData<T> &dat);
template <class T>
extern int TTColumnTable(TData<T> &Res, TData<T> &dat);

// NumX 10 NumY 10 ExpX 0 ExptY 0
template <class T>
extern int ReadTableRow(istream &in, TData<T> *dat);
...
struct TTable : TData<T> {
    ...
    TTable() : TData<T>(3){};
    TTable(TData<T> &da_) : TData<T>(da_) {}
    //{ TData<T>::TData(da_);}
    ...
    int RowSet(int i, int RowNum, T *val) {
        ...
        if(RowNum == 1) {
            int strt = i * this->I[1];
            for(int k = 0; k < this->I[0]; k++)
                this->D[2][strt + k] = val[k];
        } else {
            for(int k = 0; k < this->I[0]; k++)
                this->D[2][k * this->I[1] + i] = val[k];
        }
        return 1;
    }
    void SaveIni(ostream &out);
...
template <class T>
TData<T> &TData<T>::operator=(TData<T> &dat) {
    //cout<<" TData= \n";
    if(Stricmp(dat.InterfaceIOName(), InterfaceIOName()) != 0)
        throw info_except(
            " Different types from %s to %s . Exiting\n",
            dat.InterfaceIOName(),
            InterfaceIOName());
    SetDim(dat.N, dat.I);
    int l = sizeof(T);
    for(int k = 0; k < N; k++)
        MemoryMove(dat.D[k], D[k], l * dat.I[k]);
    return *this;
};
#ifndef WCPP
template <class T>
void TData<T>::SetDim(int n, int *i0)
#else
template <class T>
void TData<T>::SetDim(int n, int *i0)   //=NULL)
#endif
{
    Delete();
    N = n;
...
template <class T>
void TData<T>::SaveIni(ostream &out) {
    out << "NumVar " << N << "\n";
    for(int k = 0; k < N; k++)
        out << " NumV[" << k + 1 << "]  " << I[k];
    out << "\n";
...
template <class T>
int TData<T>::ReadIni(istream &in) {
    char tmp[256];
    in >> tmp >> N;
    int *i = new int[N];
    for(int k = 0; k < N; k++)
...
template <class T>
void TTable<T>::SaveIni(ostream &out) {
    int k;
    out << "NumberX " << this->I[0] << " NumberY " << this->I[1] << "\n";
    for(k = 0; k < this->I[0]; k++)
        out << this->D[0][k] << "  ";
    out << "\n";
    for(k = 0; k < this->I[1]; k++)
        out << this->D[1][k] << "  ";
    out << "\n";
    for(k = 0; k < this->I[2]; k++) {
        out << FMT << this->D[2][k];
        if((double)(k + 1) / this->I[1] == (k + 1) / this->I[1])
            out << "\n";
    }
    out << "\n";
};
template <class T>
int TTable<T>::ReadIni(istream &in) {
    char tmp[256];
    int x, y, k;
    in >> tmp >> x >> tmp >> y;
    int i[3] = {x, y, x * y};
...
template <class T>
void TColumn<T>::SaveIni(ostream &out) {
    int i, k;
...
template <class T>
int TColumn<T>::ReadIni(istream &in) {
    int row, i, k, *a;
    char tmp[256];
    in >> tmp >> this->N >> tmp >> row;
    a = new int[this->N];
    for(k = 0; k < this->N; k++)
        a[k] = row;
    this->SetDim(this->N, a);
    delete a;
    for(k = 0; k < this->I[0]; k++) {
        for(i = 0; i < this->N; i++)
            in >> this->D[i][k];
...
template <class T>
int TTableTColumn(TData<T> &Res, TData<T> &dat) {
    if((Stricmp(dat.InterfaceIOName(), "TColumn") != 0) ||
       (Stricmp(Res.InterfaceIOName(), "TTable") != 0))
        throw info_except(
            "Wrong types in TTableTColumn %s != TColumn || %s !=TTable\n",
            dat.InterfaceIOName(),
            Res.InterfaceIOName());
    if((dat.N != 3) || (dat.I[0] != dat.I[1]) || (dat.I[0] != dat.I[2]))
        throw info_except(
            " Bad dimensions in TTableTColumn %i  %i %i (have to be same)\n",
            dat.I[0],
            dat.I[1],
            dat.I[2]);
    int k = 1, x = 0, y = 1, t = 0;
    while((y < dat.I[0]) && (fabs(dat.D[0][y] - dat.D[0][y - 1]) < MathZer))
        y++;
    if(int(dat.I[0] / y) != (double)dat.I[0] / y || y == 1) {
        dat.SaveIni(cout);
        throw info_except(
            " Bad y dimension in TTableTColumn %i and got num same x %i\n", dat.I[0], y);
    }
    x = dat.I[0] / y;
    int i[3] = {x, y, dat.I[0]};
    Res.SetDim(3, i);
...
template <class T>
int TTColumnTable(TData<T> &Res, TData<T> &dat) {
    if((Stricmp(dat.InterfaceIOName(), "TTable") != 0) ||
       (Stricmp(Res.InterfaceIOName(), "TColumn") != 0))
        throw info_except(
            " Wrong types in TTColumnTable %s %s \n",
            dat.InterfaceIOName(),
            Res.InterfaceIOName());
    if((dat.N != 3) || (dat.I[0] * dat.I[1] != dat.I[2]))
        throw info_except(
            " Bad dimensions in TTColumnTable %i * %i != %i\n",
            dat.I[0],
            dat.I[1],
            dat.I[2]);
    int i[3] = {dat.I[2], dat.I[2], dat.I[2]};
    Res.SetDim(3, i);
    int k, k1, t = 0;
    for(k = 0; k < dat.I[0]; k++) {
        for(k1 = 0; k1 < dat.I[1]; k1++) {
            Res.D[0][t] = dat.D[0][k];
            Res.D[1][t] = dat.D[1][k1];
            Res.D[2][t] = dat.D[2][t];
            t++;
        }
    }
...
template <class T>
int ReadTableRow(istream &in, TData<T> *dat) {
    if(Stricmp(dat->InterfaceIOName(), "TTable") != 0) {
        cout << " ReadTableRow work only with TTable. Wrong.";
        return 0;
    }
    char tmp[256];
    ...
    if(((ExpX) && (Xu * Xl <= 0)) || ((ExpY) && (Yu * Yl <= 0)))
        if(Xu * Xl <= 0) {
            cout << "Different Singns in exp format. Exiting";
            return 0;
        }
    ...
    if(ExpX) {
        for(k = 0; k < i0; k++)
            dat->D[0][k] = ExpSX * exp(dat->D[0][k]);
    }
    if(ExpY) {
        for(k = 0; k < i1; k++)
            dat->D[1][k] = ExpSY * exp(dat->D[1][k]);
    }
    for(k = 0; k < i1 * i0; k++)
        dat->D[2][k] = 0;
    return 1;
...
template <class T>
void TData<T>::AddColumn(T *column, int Num) {
    int *i = new int[N + 1];
    if(I != NULL)
...
template <class T>
void TData<T>::SetColumn(T *column, int Num, int Col) {
    if((Col >= N) || (Col < 0))
        throw info_except(
            "Cannot set column <%i> Total number of columns <%i>\n", Col, N);
    delete D[Col];
    I[Col] = Num;
    D[Col] = new T[Num];
    MemoryMove(column, D[Col], sizeof(T) * Num);
};

//static int SortCmp(const void *v1,const void *v2)
template <class T>
int TData<T>::SortCmp(const void *v1, const void *v2) {
    //   int i1=*((int*)v1),i2=*((int*)v2);
    T *t1 = &(CurDat->D[SortColumn][*((int *)v1)]),
      *t2 = &(CurDat->D[SortColumn][*((int *)v2)]);
    if(*t1 > *t2)
        return 1;
    if(*t1 == *t2)
        return 0;
...
template <class T>
void TData<T>::SortData(int SortCol) {
    if(N <= 0)
        return;
    int k, Num = I[0];
