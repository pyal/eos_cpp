...
//    :L2OSourceCal(new ManyVarFunc2OneVar), L2OSourceExp(new PolynomFunc), L2ODetector(new ManyVarFunc2OneVar), InterFilter(new PolynomFunc), ZeroFilterSignal(1e-5), WaveLength(500), SourceTemp(2700), Opacity(1), SingleLine(1), CalibrationSignal(0.6){ }
    ...
    virtual int save_data_state( FilterOut&so) { 
        so<<" L2OSourceCal "<<L2OSourceCal<<" L2OSourceExp "<<L2OSourceExp<<" L2ODetector "<<L2ODetector<<" InterFilter "<<InterFilter;
        so<<" ZeroFilterSignal "<<ZeroFilterSignal;
        so<<" WaveLength "<<WaveLength<<" SourceTemp "<<SourceTemp<<" Opacity "<<Opacity;
	...
	virtual int read_data_state(FilterIn&si){ 
        char tmp[256];
        si>>tmp>>L2OSourceCal>>tmp>>L2OSourceExp>>tmp>>L2ODetector>>tmp>>InterFilter;
    ...
    void SetCalib(){
        ...
        if (SingleLine){
            Calibration1Const = (exp(0.01438775220950165/(WaveLength*1e-9*SourceTemp))-1)/L2OSourceCal->Calculate(WaveLength);
            return;
        } 
...
private:
    ...
    void FindIntegralBegEnd(double &intbeg, double &intend, OneVarFunction *f1, OneVarFunction *f2){
    ...
    double IntegrateInt(double temp, CurveSpline *l2osource, double integralcoef){
        ...
        double ResErr;
        int Err;
        return Qags_IntSubInt(IntenValStatic, IntegralBeg, IntegralEnd, 1e-6, 1e-6, 300, ResErr, Err);
    }
// =============================
    ...
    double MisfVal(double temp){
        return CalibrationSignal / CalibrationIntConst * IntegrateInt(temp, &ExpTract, ColorTempCoef) - ExpSigMisf;
    }
    ...
    double FindExpT2GivenLength(double signal){
        ...
        return  0.01438775220950165/(WaveLength*1e-9*log(1+CalibrationSignal*cal1/(signal*Opacity)));
        //return  0.01438775220950165/(WaveLength*1e-9*log(1+CalibrationSignal*Calibration1Const/(signal*Opacity)));
    }
    void MakeSpline(CurveSpline &spl, OneVarFunction *f1, OneVarFunction *f2, OneVarFunction *f3){
...
struct ColorTemp:ManyVarFunction{
    ...
    virtual double Calculate(const map<Stroka, double> &exp, const Stroka &name){
        ...
        if (chan < 0 || chan >= (int)Chanels.size())
            throw info_except("Unknown name %s. Can clc names: [ Opacity Temperature Misfit T_0 T_n ]. Number of channels %i\n", name.c_str(), Chanels.size());
        return FindTemp(FoundOpacity, exp, chan);
    }
    Stroka MakeHelp(){
        Stroka str = "Input brightness chanels. Calculate opacity to minimize temperature misfit. Can clc names: [ Opacity Temperature Misfit T_0 T_n ]\n";
        return str;
    }
    double FindTemp(double opac, const map<Stroka, double> &exp, int chan){
        ...
        if (it == exp.end())
            throw info_except("Cannot calculate channel <%i> Ch_%i - is not defined\n", chan, chan);
...
//cout<<opac<<" "<<sqrt(s2/N - sqr(mean))<<" "<<mean<<"\n";
    ...
    void FindOpacity(const map<Stroka, double> &exp, double &foundtemp, double &foundopacity, double &foundmisfit){
        foundopacity = 0.5;
        ExpSignal = &exp;
        CurPtr = this;
        double resMis;
