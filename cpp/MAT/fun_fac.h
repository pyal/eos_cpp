...
struct NamedOneVarFunction : OneVarFunction {
    ...
    int KnownClcName(const Stroka &name, int Break = 0){
        ...
        if (Break && !ret)
            throw info_except("Unknown clc name <%s>. Known names are <%s>\n",name.c_str(), GetNames().c_str());
        return ret;
    }
...
struct ManyVarFunction : SavableClass{
   ...
   virtual int save_data_state(FilterOut &out){return 1;}
   virtual int read_data_state(FilterIn &in) {return 1;}
};
...
struct OneVarFormula:OneVarFunction{
    ...
    void InitProgram(){
        ...
        prg.GenerateDouble(0,"Y");prg.GenerateDouble(0,"X");
        prg.Read();
...
struct ManyVarFunc2OneVar:OneVarFunction{
    ...
    virtual int save_data_state(FilterOut&so){
        so<<" ManyVarFunc "<<Func<<" InVarName "<<InVarName<<" OutVarName "<<OutVarName;
        so<<" FixedParameters { ";
        for(map<Stroka, double>::iterator it = FixedParameters.begin();it != FixedParameters.end();it++){
            char tmp[256];
...
struct IFunctionConverter : SavableClass {
    ...
    virtual void Encode(const vector<double> &xVec,vector<double> &yVec, const char *errMes = NULL) {
    ...
    virtual void Decode(const vector<double> &xVec,vector<double> &yVec, const char *errMes = NULL) {
        yVec.resize(xVec.size());
        for(size_t i = 0; i < xVec.size(); i++) 
   ...
   vector<double> MakeEncodedStepVector(double minX, double maxX, double numX, int centered) {
