...
struct VecCl {
    ...
    void Copy2Double(double *dst) {
        if(dst[0] != Ptr[0]) {
            cout << " Incompatible type in VecCl::Copy2Double\n";
            return;
        }
    ...
    VecCl ExtractSubVector(int i, int j) {
        VecCl ret(j - i + 1);
        MemoryMove(&Ptr[i], &ret.Ptr[1], sizeof(double) * (j - i + 1));
        return ret;
    }
    ...
    void SetDim(int NewDim) {   //double *pnt=Vec.Ptr;
        if((Ptr == NULL) || (NewDim != Dim())) {
            if(Ptr != NULL)
                delete Ptr;
            if(NewDim < 0)
                throw info_except(
                    "Cannot create vector with negative dimensions: %i\n", NewDim);
            if(NewDim) {
                Ptr = new double[NewDim + 1];
                Ptr[0] = NewDim;
            } else
                Ptr = NULL;
        }
    ...
    static VecCl ComponentMul(const VecCl &fst, const VecCl &sec) {
        if(fst.Dim() != sec.Dim())
            throw info_except(
                "Dims are different fst.Dim=%i sec.Dim=%i\n", fst.Dim(), sec.Dim());
        VecCl res = fst;
...
struct MatrCl {
    ...
    void GetDimMN(int &X, int &Y) const {
        X = DimX;
        Y = DimY;
    };
    ...
    void SetMatr(int DX, int DY, double **matr = NULL) {
        if((matr == NULL) && (Addr != NULL) && (DimX == DX) && (DimY == DY))
            return;
        DeleteMatr();
        if(matr == NULL)
            NewMatr(DX, DY);
