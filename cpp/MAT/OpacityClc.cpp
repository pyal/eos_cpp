...
static ClassDesc BrigtnesTemp_ClassDesc(typeid(BrigtnesTemp),"BrigtnesTemp",1,"OneVarFunction_category",create<BrigtnesTemp>);
static ClassDesc ColorTemp_ClassDesc(typeid(ColorTemp),"ColorTemp",1,"ManyVarFunction_category",create<ColorTemp>);

static ClassDesc PlancEmission_ClassDesc(typeid(PlancEmission),"PlancEmission",1,"ManyVarFunction_category",create<PlancEmission>);
...
double BrigtnesTemp::PlancEmisE(double Temp, double Lambda){
    ...
    if (earg>300)
        return 0;
    return 1.19104/pow(Lambda, 5)/(exp(0.0143878/tl)-1)*1e11;// Joule/Second/nanoMeter^3
}
