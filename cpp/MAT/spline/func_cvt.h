...
struct TExpConverter : IFunctionConverter {
    TExpConverter() : IFunctionConverter(), LogX(1), MulX(1), AddX(0) {}
    TExpConverter(int logX, double mulX, double addX)
        : IFunctionConverter(), LogX(logX), MulX(mulX), AddX(addX) {}
    ~TExpConverter();   //{}
    ...
    virtual double Encode(double x, const char *errMes = NULL) {
        ...
        if(arg < MathZer) {
            if(errMes)
                throw info_except(
                    "Encoding x %f with MulX %f AddX %f Log arg is %f\n",
                    x,
                    MulX,
                    AddX,
                    arg);
...
struct TAutoExpConverter : TExpConverter {
    TAutoExpConverter() : TExpConverter(), AutoMin(1), AutoVal(1e-3) {
        //        , AutoSearchMin(0) , AutoSearchMax(0)
    }
    virtual int save_data_state(FilterOut &so) {
        TExpConverter::save_data_state(so);
        so << "AutoMin_yesno " << AutoMin << " AutoVal "
           << AutoVal;   // << " AutoSearchMin " << AutoSearchMin << " AutoSearchMax " << AutoSearchMax;
        return 1;
    }
    virtual int read_data_state(FilterIn &si) {
        ...
        Stroka tmp;
        si >> tmp >> AutoMin >> tmp >>
            AutoVal;   // >> tmp >> AutoSearchMin >> tmp >> AutoSearchMax;
        return 1;
    }
    ...
    static double Quality(double x) {
        ...
        double sum = 0,
               step = CurPtr->MinimXvector[CurPtr->MinimXvector.size() - 1] -
                      CurPtr->MinimXvector[0] / (CurPtr->MinimXvector.size() - 1);
        step = fabs(step);
