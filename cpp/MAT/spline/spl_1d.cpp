...
static ClassDesc TSpline1DCalculator_ClassDesc(
    typeid(TSpline1DCalculator),
    "TSpline1DCalculator",
    1,
    "SplineCalculator_category",
    create<TSpline1DCalculator>);
static ClassDesc TSpline1DGenerator_ClassDesc(
    typeid(TSpline1DGenerator),
    "TSpline1DGenerator",
    1,
    "SplineGenerator_category",
    create<TSpline1DGenerator>);
...
int TSpline1DCalculator::Evaluate(
    ...
    integer polynom_degree = SplineOrder, Num = dat[0].size(), SX = SplineCoefs[0].size(),
            ier;
    splev_(
        &SplineCoefs[0][0],
        &SX,
...
int TSpline1DGenerator::Generate(vector<vector<double>> &dat1, int Cont) {
    ...
    vector<real> weight = DataManip::MakeStepVector(dat[1].size(), 1, 1, 0, 0), minV,
                 maxV;
    ...
    integer NumXs = NumX, Conti = Cont, NumXd = dat[0].size(),
            polynom_degree = SplineOrder, Result, NumXs_r;
