...
static ClassDesc TSpline2DCalculator_ClassDesc(
    typeid(TSpline2DCalculator),
    "TSpline2DCalculator",
    1,
    "SplineCalculator_category",
    create<TSpline2DCalculator>);
static ClassDesc TSpline2DGenerator_ClassDesc(
    typeid(TSpline2DGenerator),
    "TSpline2DGenerator",
    1,
    "SplineGenerator_category",
    create<TSpline2DGenerator>);
...
int TSpline2DCalculator::Evaluate(
    const vector<vector<double>> &dat,
    vector<double> &res) {
    if(dat.size() != 2 || dat[0].size() == 0)
        throw info_except("Bad dimention of the dat file\n");
    real rx, ry, rz;
    integer lwk = 8, liw = 2, ier, n1 = 1, ny = 1, SX = SplineCoefs[0].size(),
            SY = SplineCoefs[1].size();
    vector<real> wk(lwk);
    ...
    for(int k = 0; k < (int)dat[0].size(); k++) {
        ...
        bispev_(
            ...
            &ier);
        if(ier != 0)
            throw info_except("BadSplineResult(%i) x(%f) y(%f) z(%f)\n", ier, rx, ry, rz);
        res[k] = rz;
    }
    return ier;
};
...
int TSpline2DGenerator::Generate(vector<vector<double>> &dat, int Cont) {
    ...
    if(dat.size() != splineDim + 1)
        throw info_except(
            "Bad grid size() is %i and SplineDim is %i\n", dat.size(), splineDim);
    ...
    //cout<<"data \n";for(size_t i = 0; i < dat[0].size(); i++) { for(size_t j = 0; j < 3; j++) cout<<dat[j][i] << " ";cout<<"\n"; }

    //vector<vector<double> > dat1;DataManip::XYTable2Planar(dat1, tab);
    //cout<<"\n\n\n\ndata1 \n";for(size_t i = 0; i < dat[0].size(); i++) { for(size_t j = 0; j < 3; j++) cout<<dat[j][i] << " ";cout<<"\n"; }
    integer NumXi = NumX, NumYi = NumY, Conti = Cont, px = 3, py = 3, DX = tab[0].size(),
            DY = tab[1].size();
    integer SX, SY;
    ...
    regrid_(
        ...
        &NIWork,
        &Result);
    if(SumSquareResiduals == 0 || GenerationMisfit < SumSquareResiduals * 0.5 ||
    ...
    spline->SetData(
        ~SplineName, ~SplineDescr, minV, maxV, SumSquareResiduals, splineCoefs);
    return Result;
}
