...
struct TEncodedSplineCalculator : ISplineCalculator {
    ...
    TEncodedSplineCalculator(ISplineCalculator *spl, const vector<Ref<IFunctionConverter> > &cvtVec) 
        : ISplineCalculator() 
        , SplineClc(spl)
        , CvtVec(cvtVec) {
        ClcDeriveVector();
    ...
    virtual int  Evaluate (const vector<double> &pnt, double &res) {
        if (pnt.size() + 1 != CvtVec.size()) 
            throw info_except("Bad pnt(%d) - evaluate(%d) dims\n", pnt.size(), CvtVec.size());
    ...
    virtual int  Evaluate (const vector<vector<double> > &dat, vector<double> &res) {
        if (dat.size() + 1 != CvtVec.size()) 
            throw info_except("Bad dat(%d) - evaluate(%d) dims\n", dat.size(), CvtVec.size());
...
private:
    ...
    void SetDatInBound(vector<double> &addBnd, vector<double> &inDat, const vector<double> &dat, int col) {
        vector<double> minV = GetBndMin(), maxV = GetBndMax();
...
struct TEncodedSplineGenerator : ISplineGenerator {
    ...
    virtual int  Generate(vector<vector<double> > &dat, int cont = 0) {
        if (dat.size() != CvtVec.size()) 
            throw info_except("Bad dat(%i) - converter(%i) dims, have to be %i\n", dat.size(), CvtVec.size(), GetSplineDim() + 1);
        vector<double> eVec;
    ...
    void MakeGrid(vector<vector<double> > &grid, const vector<int> &numPnt, const vector<double> &bndMin, const vector<double> &bndMax) {
        vector<double> eVec;
        grid.clear();
        if (numPnt.size() != bndMin.size() || numPnt.size() != bndMax.size() || numPnt.size() + 1 != CvtVec.size())
            throw info_except("Bad sizes num(%d) min(%d) max(%d) cvt(%d)\n", numPnt.size(), bndMin.size(), bndMax.size(), CvtVec.size());
        for(size_t i = 0; i < CvtVec.size() - 1; i++) {
            VecCl vec = VecCl::MakeStepVector(numPnt[i], CvtVec[i]->Encode(bndMin[i]), CvtVec[i]->Encode(bndMax[i]), 0);
            CvtVec[i]->Decode(vec.Copy2Vector(), eVec, "grid generation");
            grid.push_back(eVec);
        }
    }
    virtual void MakeGrid(vector<double> &grid, int axisNum, int numPnt, double bndMin, double bndMax) {
        vector<double> eVec;
        //grid.clear();
    ...
    int read_data_state(FilterIn &in) {
        ...
        if (!SplineGen || GetSplineDim() + 1 != CvtVec.size())
            throw info_except("Bad Spline generator or bad cvt vec dim(%i) have to be %i\n", CvtVec.size(), GetSplineDim() + 1);
        return 1;
    }
    int save_data_state(FilterOut &out) {
        out << " FunctionConverterX..Z { " <<SavableClass::ppEOLN();
        for(int i = 0; i < (int)CvtVec.size() -1; i++)
            out << ~(Stroka(" CvtX_") + Stroka::Int2Str(i) + " ") << CvtVec[i] << SavableClass::EOLN();
