...
struct IndexedVal : RefCount {
    ...
    IndexedVal(double valueind, double valuecmp)
        : ValueInd(valueind), ValueCmp(valuecmp){};
...
struct MinFind1DClass : RefCount {
    ...
    void SetConst(
        double minstep,
        double funcerror,
        double linearmulcoef,
        int maxliniter,
        int maxsearchiter) {
        MinStep = max<double>(minstep, M_Eps2);
        FuncError = max<double>(funcerror, M_Eps2);
        MaxLinIter = maxliniter;
        MaxSearchIter = maxsearchiter;
...
private:
    ...
    int ZeroStp(double x1, double x2) {
        return ((CurSearchIter++ > MaxSearchIter) || (fabs(x1 - x2) < 2 * (MinStep))) ? 1
                                                                                      : 0;
    }
};
struct DeriveFuncClass : RefCount {
    virtual double Execute(const VecCl &Par) {
        return 0;
    };
};
...
struct Derive2DClass : DeriveFuncClass {
    ...
    double Execute(const VecCl &Par) {
        return DeriveFuncClass::Execute(Par);
    };
};
...
struct Derive2DHi2 : Derive2DClass {
    ...
    double Execute(const VecCl &Par) {
        ...
        if(Res.CheckInfinity(1 / MathZer))
            throw info_except(
                "Got infinity... Paraqmeters are : %s", Par.ToString().c_str());
...
struct EigenVectMinim : MultiDimMinimizer {
    ...
    int BadWlk;
    void SetFunc(MinFind1DClass *min1d, Derive2DClass *deriv2) {
        Min1D = min1d;
        Deriv2 = deriv2;
    }
    EigenVectMinim();
