...
extern int tnbc_(
    ...
    doublereal *,
    integer *,
    int (*)(integer *n, doublereal *x, doublereal *f, doublereal *g),
    doublereal *,
    doublereal *,
    integer *);
...
doublereal FuncSub(real *x) {
    return FuncStatic(*x);
};
int FuncSubNa6(real *x, integer *numfun, integer *np, real *funvls) {
    for(long k = 0; k < *numfun; k++) {
        for(long k1 = 0; k1 < *np; k1++)
            funvls[k * (*np) + k1] = FuncStatic(x[k1]);
    }
    return 0;
};
...
double Qainf_OscilIntInf(
    ...
    real *a = new real[wrksub], *b = new real[wrksub];
    a[0] = From;
    b[0] = FromAssimpt;
    ...
    nw = numfun * 5 + 1 + (wrksub << 2) * numfun + wrksub * 3 + (emax + 5) * (emax + 5) +
         numfun * 38;
    ...
    integer key, restar = 0;
    key =
        (Method == OscillMethod_Euler) ? 0 : (Method == OscillMethod_EulerModif) ? 1 : 2;
    ...
    sqainf_(
        &numfun,
        reinterpret_cast<U_fp>(&FuncSubNa6),
        &period,
        &gamma,
        a,
        b,
    ...
    delete[] b;
    if(ifail) {
        cout << " Qainf_OscilIntInf : error " << ifail << " ResErr " << abserr
             << " NumEval " << neval << "\n";
        cout.flush();
    };
    Err = ifail;
    ResErr = abserr;
    NumberEvaluations = neval;
...
double Qawf_FourierIntInf(
    ...
    delete work;
    if(ier != 0) {
        cout << " Qawf_FourierIntInf : error " << ier << " ResErr " << abserr << "\n";
        cout.flush();
    };
    ResErr = abserr;
    Err = ier;
...
double Qawo_FourierIntAB(
    ...
    double Omega,
    int ToMulOnCos) {
    real a = From, b = To, epsabs = ErrorAbs, epsrel = ErrorRel, abserr, result,
         omega = Omega;
    integer integr = (ToMulOnCos) ? 1 : 2, neval, ier;
    ...
    delete work;
    if(ier != 0) {
        cout << " Qawf_FourierIntAB : error " << ier << " ResErr " << abserr << "\n";
        cout.flush();
    };
    ResErr = abserr;
    Err = ier;
...
double Qage_OscilInt(
    ...
    int &Err) {
    int Dir;
    if(CheckLim(From, To, ResErr, Err, Dir))
        return 0;
    ...
    delete iord;
    if(ier != 0) {
        cout << " Qage_OscilInt : error " << ier << " ResErr " << abserr << "\n";
        cout.flush();
    };
    ResErr = abserr;
    Err = ier;
...
double Qag_OscilInt(
    ...
    delete iwork;
    if(ier != 0) {
        cout << " Qag_OscilInt : error " << ier << " ResErr: " << abserr << "\n";
        cout.flush();
    };
    Err = ier;
    ResErr = abserr;
...
double Qagi_InfinInt(
    ...
    delete iwork;
    if(ier != 0) {
        cout << " Qagi_InfinInt : error " << ier << " ResErr: " << abserr << "\n";
        cout.flush();
    };
    Err = ier;
    ResErr = abserr;
...
double Qagie_InfinInt(
    ...
    delete iord;
    if(ier != 0) {
        cout << " Qagie_InfinInt : error " << ier << " ResErr " << abserr << "\n";
        cout.flush();
    };
    ResErr = abserr;
    Err = ier;
...
double Qags_IntSubInt(
    ...
    FuncStatic = tmpFunc;
    if(ier != 0) {
        cout << " Qags_IntSubInt : error " << ier << " ResErr: " << abserr << "\n";
        cout.flush();
    };
    Err = ier;
    ResErr = abserr;
...
int Fzero(
    ...
    if(iflag == 3)
        cout
            << " Fzero; Flag " << iflag << " X " << b << " Func(X) " << f(b) << "\n"
...
double UncMin(
    ...
    void (*StaticUncMinGradClcUser_)(double *x, double *grad, int n, double &func) =
        StaticUncMinGradClcUser;
    int UseStdFunc = 0, ToDeleteStp = 0;

    integer msglvl = 1, maxit = MaxIter, maxfun = MaxFun;
    doublereal eta = LineSearchEta, stepmx = MaxLineSearchStp, accrcy = FunAccur,
               xtol = ResErr;


    if(GradClc == NULL) {
        UseStdFunc = 1;
        StaticUncMinFuncClc = FuncClc;
        if(GradStp == NULL) {
            GradStp = new double[N];
            ToDeleteStp = 1;
            for(int k = 0; k < N; k++)
                if(fabs(x_res[k]) < 1e-30)
                    GradStp[k] = 0.01;
                else
                    GradStp[k] = 0.01 * x_res[k];
            StaticUncMinGradStp = GradStp;
        }
        StaticUncMinGradStp = GradStp;
    } else
        StaticUncMinGradClcUser = GradClc;


    integer N_ = N, Error_ = Error, WorkLen_ = 14 * N;
    doublereal ret_ = est_sol, *WorkArr_ = new doublereal[WorkLen_],
               *ResGrad_ = new doublereal[N_];
    doublereal *x_res_ = new doublereal[N_];
    Double2LongD(N, x_start, x_res_, 1);
    ...
    if(BoundMin == NULL) {
        if(UseStdFunc)
            lmqn_(
                ...
                WorkArr_,
                &WorkLen_,
                reinterpret_cast<U_fp>(SubstUncMinFuncStd)   //);
                ,
                &msglvl,
                &maxit,
                &maxfun,
            ...
            lmqn_(
                ...
                WorkArr_,
                &WorkLen_,
                reinterpret_cast<U_fp>(SubstUncMinFuncUser)   //);
                ,
                &msglvl,
                &maxit,
                &maxfun,
        ...
        doublereal *BoundMin_ = new doublereal[N_];
        Double2LongD(N, BoundMin, BoundMin_, 1);
        doublereal *BoundMax_ = new doublereal[N_];
        Double2LongD(N, BoundMax, BoundMax_, 1);
        if(UseStdFunc)
            tnbc_(
                &Error_,
                &N_,
    ...
    //int k;
    if((Error_ != 0) &&
       ((fabs(double(ResGrad_[0])) > 1) || (fabs(double(ResGrad_[N - 1])) > 1))) {
