/* qawoe.f -- translated by f2c (version 20000118).
	...
	elist, integer *iord, integer *nnlog, integer *momcom, real *chebmo)
{
    ...
    integer k;
    real small, erro12, a1, a2, b1, b2, defab1, defab2, oflow, width;
    integer ierro, ktmin;
    real uflow;
    integer nrmax, nrmom;
    logical noext;
...
/* Computing MAX */
    ...
    if (*integr != 1 && *integr != 2 || *epsabs <= (float)0. && *epsrel < 
	    dmax(r__1,(float)5e-15) || *icall < 1 || *maxp1 < 1) {
	*ier = 6;
    }
    if (*ier == 6) {
	goto L999;
...
/*           --------------- */
    ...
    *abserr = oflow;
    ...
    iroff3 = 0;
    ktmin = 0;
    small = (r__1 = *b - *a, dabs(r__1)) * (float).75;
    nres = 0;
    numrl2 = 0;
    extall = FALSE_;
    if ((r__1 = *b - *a, dabs(r__1)) * (float).5 * domega > (float)2.) {
	goto L10;
    }
    numrl2 = 1;
    extall = TRUE_;
    rlist2[0] = *result;
L10:
    if ((r__1 = *b - *a, dabs(r__1)) * (float).25 * domega <= (float)2.) {
	extall = TRUE_;
    }
    ksgn = -1;
...
/*           subroutine qc25f). */
	...
	small *= (float).5;
	if (width * (float).25 * domega > (float)2.) {
	    goto L140;
	}
	extall = TRUE_;
	goto L130;
