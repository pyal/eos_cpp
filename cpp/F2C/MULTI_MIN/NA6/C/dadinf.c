/* dadinf.f -- translated by f2c (version 20000118).
...
/* Subroutine */ int dadinf_(integer *numfun, U_fp funsub, doublereal *period,
	 doublereal *gamma, doublereal *a, doublereal *b, integer *wrksub, 
	integer *emax, integer *minpts, integer *maxpts, doublereal *epsabs, 
	doublereal *epsrel, integer *restar, integer *lenw, integer *key, 
	doublereal *result, doublereal *abserr, integer *neval, integer *nsub,
	 integer *ifail, logical *done, doublereal *values, doublereal *
	errors, doublereal *greate, doublereal *c__, doublereal *d__, 
	doublereal *u, doublereal *e, doublereal *t, doublereal *ct, 
	doublereal *exterr, doublereal *beta, doublereal *unew, doublereal *
	uerr, doublereal *work2, integer *worst, integer *list, integer *
	upoint)
{
    ...
    integer jj;
    doublereal epmach;
    extern /* Subroutine */ int dgaint_(doublereal *, doublereal *, integer *,
	     U_fp, logical *, logical *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, integer *, doublereal *, doublereal *,
	     doublereal *, integer *, doublereal *, doublereal *);
    integer vacant;
    extern /* Subroutine */ int dexinf_(integer *, doublereal *, doublereal *,
	     integer *, integer *, doublereal *, doublereal *, integer *, 
	    integer *, doublereal *, doublereal *, doublereal *, doublereal *,
	     doublereal *, doublereal *, doublereal *);
    static doublereal maxeer;
    integer update, sbrgns;
    extern /* Subroutine */ int dtrint_(integer *, integer *, doublereal *, 
	    integer *, integer *);
    integer pointr, one, top;

/* ***BEGIN PROLOGUE DADINF */
...
/*   DADINF uses extrapolation on a sum of integrals over subintervals */
...
/*            IFAIL = +1 if MAXPTS was too small for DADINF */
/*                      to obtain the required accuracy. In this case */
/*                     DADINF returns values of RESULT with estimated */
/*                     absolute errors ABSERR. */
/*            IFAIL = -1 if EMAX was too small for DADINF */
/*                      to obtain the required accuracy. In this case */
/*                     DADINF returns values of RESULT with estimated */
/*                     absolute errors ABSERR. */
...
/* ***ROUTINES CALLED DTRINT, DGAINT, DEXINF, D1MACH. */
/* ***END PROLOGUE DADINF */
...
/* ***FIRST EXECUTABLE STATEMENT DADINF */
...
/*   Apply the rule procedure  over the two first intervals. */
	...
	i__2 = *numfun;
	for (i__ = 1; i__ <= i__2; ++i__) {
	    values[i__ + values_dim1] = 0.;
/* L30: */
	    errors[i__ + errors_dim1] = 0.;
	}
	greate[1] = 0.;
	worst[1] = 1;
	one = 2;
	...
	dgaint_(&a[i__], &b[i__], numfun, (U_fp)funsub, &done[1], &more, &
...
/* L70: */
	...
	index = i__;
	dtrint_(&c__2, &index, &greate[1], &list[1], &i__);
/* L80: */
    }
...
/*   Finally RESULT will be initialized. */
    ...
    update = -1;
    i__2 = numu - 1;
    dexinf_(numfun, gamma, &cc, key, &i__2, &t[1], &ct[1], &update, emax, &u[
...
/*   Remove the top element from the heap.(The value of K does not matter */
	    ...
	    pointr = sbrgns + 2;
	    dtrint_(&c__1, &sbrgns, &greate[1], &list[1], &k);
...
/*   Apply basic rule. */
	...
	i__2 = ndiv;
	for (i__ = 1; i__ <= i__2; ++i__) {
	    dgaint_(&a[index], &b[index], numfun, (U_fp)funsub, &done[1], &
...
/*   UPDATE is used to signal an extrapolation step. */
	...
	i__2 = numu - 1;
	dexinf_(numfun, gamma, &cc, key, &i__2, &t[1], &ct[1], &update, emax, 
...
/*   are finished. MORE will signal if there is more to do. */
	...
	i__2 = *numfun;
	for (j = 1; j <= i__2; ++j) {
	    if (abserr[j] <= *epsrel * (d__1 = result[j], abs(d__1)) || 
		    abserr[j] <= *epsabs) {
		done[j] = TRUE_;
	    } else {
		more = TRUE_;
...
/*   Store results in heap. */
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
	    jj = sbrgns + i__;
	    dtrint_(&c__2, &jj, &greate[1], &list[1], &index);
	    index = sbrgns + i__ + 1;
/* L160: */
	}
	sbrgns += ndiv;
