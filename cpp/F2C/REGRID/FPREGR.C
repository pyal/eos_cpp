/* FPREGR.f -- translated by f2c (version 19970805).
...
/* Subroutine */ int fpregr_(integer *iopt, real *x, integer *mx, real *y, 
	integer *my, real *z__, integer *mz, real *xb, real *xe, real *yb, 
	real *ye, integer *kx, integer *ky, real *s, integer *nxest, integer *
	nyest, real *tol, integer *maxit, integer *nc, integer *nx, real *tx, 
	integer *ny, real *ty, real *c__, real *fp, real *fp0, real *fpold, 
	real *reducx, real *reducy, real *fpintx, real *fpinty, integer *
	lastdi, integer *nplusx, integer *nplusy, integer *nrx, integer *nry, 
	integer *nrdatx, integer *nrdaty, real *wrk, integer *lwrk, integer *
	ier)
{
    ...
    real fpms;
    integer ifsx, ifsy, nplx, nply, mynx, i__, j, l;
    real p, f1, f2, f3;
    integer nminx, nmaxx, nmaxy, nminy, k3;
    real p1, p2, p3;
    integer mm, lq;
    real rn;
    extern doublereal fprati_(real *, real *, real *, real *, real *, real *);
    extern /* Subroutine */ int fpgrre_(integer *, integer *, integer *, 
	    ...
	    real *, real *, real *, real *, real *, real *, real *, integer *,
	     integer *), fpknot_(real *, integer *, real *, integer *, real *,
	     integer *, integer *, integer *, integer *);
    integer mk1, nrintx, nrinty, kx1, kx2, ky1, ky2;
    real acc, one;
    integer lax, lay, lbx, lby, lri, mpm, nxe, nye, nxk, lsx, lsy, ich1, ich3;
    real con1, con4, con9;
...
/*  find the least-squares spline sinf(x,y) and calculate for each kno
t */
/*  interval tx(j+kx)<=x<=tx(j+kx+1) (ty(j+ky)<=y<=ty(j+ky+1)) the sum
 */
/*  of squared residuals fpintx(j),j=1,2,...,nx-2*kx-1 (fpinty(j),j=1,
2, */
...
/*  if nx=nxe and ny=nye we cannot further increase the number of knot
s */
...
/*  add a new knot in the x-direction */
	    fpknot_(&x[1], mx, &tx[1], nx, &fpintx[1], &nrdatx[1], &nrintx, 
		    nxest, &c__1);
/*  test whether we cannot further increase the number of knots in
 the */
/*  x-direction. */
	    if (*nx == nxe) {
		goto L250;
