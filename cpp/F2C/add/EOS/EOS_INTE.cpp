   ...
   if (stricmp(name,"MatterABu")==0) 
     ...
     {eos->Pressure=PressureSpl;eos->Sound=SoundSpl;eos->Temperature=TemperatureSpl;
      eos->ReadIni=ReadSpl;eos->DeleteIni=DeleteSpl;strcpy(eos->name,"MatterSpl");}
       ...
       else return 0; /*{ cout<<" EOSReadStruct; There is no such function \n"<<name<<"\n";return 0; }*/
...
void main(int argc, char *argv[])
{
	...
	if (!EOSRead(NumMatter,in)) exit(0);
	double D,E,P,T,S;
	printf("Density   Energy  Pressure   Temperature   Sound  Number \n");
	while (!feof(in))
	{
		fscanf(in,"%lf %lf %i",&D,&E,&N);
		P=EOSPressure(D,E,N);S=EOSSound(D,E,N);T=EOSTemperature(D,E,N);
