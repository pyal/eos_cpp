...
#include "lib/data/VecRes.h"
    ...
    Symbol(
        ...
        void *ptr_,
        int (*defn_)())
        : name(name_), type(type_), u(val_, ptr_, defn_, str_) {
        sym_ptr = this;
    }
    ...
    struct InDat {
        ...
        InDat(const InDat &in) {
            *this = in;
        };
        InDat &operator=(const InDat &in) {
            val = in.val;
            ptr = in.ptr;
            defn = in.defn;
            str = in.str;
        ...
        InDat(double val_, void *ptr_, int (*defn_)(), const char *str_)
            : val(val_), str(str_), ptr(ptr_), defn(defn_){};
    };
    InDat u;
    friend ostream &operator<<(ostream &out, const Symbol &sym) {
        out << " SymbolType " << GetTypeName(sym.type) << " Name< " << sym.name
            << " > U.Val " << sym.u.val << " U.Str< " << sym.u.str << " > ";
        return out;
    }
...
struct CalculatorVars : RefCount {
    ...
    ZUBR_CALCULATOR_ZUBR_STYPE AddVar(
        const char *name,
        int type,
        double val,
        const char *str,
...
struct Datum {
    ...
    friend FilterOut &operator<<(FilterOut &out, const Datum &dat) {
        out << " Datum.Val " << dat.val << " Datum.sym ";
        *((ostream *)(&out)) << dat.sym << " ";
        return out;
    }
    friend FilterIn &operator>>(FilterIn &in, const Datum &dat) {
        return in;
    }
};
...
struct CalculatorProgam : RefCount {
    ...
    static inline CalculatorProgam *GetCurPtr() {
        return Ptr;
    };
