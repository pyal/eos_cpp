...
struct DataBase:DescribedClass{
	...
	virtual int MakeOperation(Operation oper, DataBase* res, DataBase *op1, DataBase *op2){return 0;};
	virtual Stroka ToStr(){return Stroka("NODATA");}
	friend ostream& operator<<(ostream &out, DataBase& dat){out<<dat.ToStr().c_str()<<" ";}
};
template<class T>
struct DataStd<T>:DataBase{
	...
	virtual int MakeOperation(Operation oper, DataBase* res, DataBase *op1, DataBase *op2){
		DataStd<T>* o1,*o2,*r;
		if (!Convert(op1,o1) || !Convert(op2,o2) || !Convert(res,r))
			return 0;
		if (oper==DataBase::Pls){ r->v=o1->v+o2->v;return 1;}
		if (oper==DataBase::Mns){ r->v=o1->v-o2->v;return 1;}
		if (oper==DataBase::Div){ r->v=o1->v/o2->v;return 1;}
