...
struct DataConstr : RefCount {
    DataConstr(
        const DataSimpleStorage::Pos &dat_,
        TypeDef *Type_,
        int Spec_,
        const char *name)
...
struct TypeDef : RefCount {
    ...
    struct FuncDef : RefCount {
        Ref<ArgDef> Arg;
        Ref<FuncStruct> Func;
        FuncDef(Ref<ArgDef> arg, Ref<FuncStruct> func) : Arg(arg), Func(func){};
    };
    ...
    Ref<FuncDef> CheckOperArgs(Ref<ArgDef> arg_def, Stroka &Oper) {
        ...
        if(funcs.empty())
            return 0;
        for(list<Ref<FuncDef>>::iterator iter = funcs.begin(); iter != funcs.end();
            iter++)
    ...
    Stroka OperationsDefined() {
        ...
        for(map<Stroka, list<Ref<FuncDef>>>::iterator iter = Funcs.begin();
            ...
            iter++) {
            Res += iter->first + " : \n";
            for(list<Ref<FuncDef>>::iterator it = iter->second.begin();
                it != iter->second.end();
                it++) {
                Res += "\tArgs: ";
                for(list<Ref<DataConstr>>::iterator i = (*it)->Arg->dat.begin();
                    i != (*it)->Arg->dat.end();
                    i++) {
                    if((*i)->Type)
                        Res += (*i)->Type->TypeName + " ";
...
private:
    int SameArgs(Ref<ArgDef> arg1, Ref<ArgDef> arg2) {
        ...
        do {
            if(!DataConstr::SameType(
                   iter1 != arg1->dat.end() ? *iter1 : NULL,
                   iter2 != arg2->dat.end() ? *iter2 : NULL))
                return 0;
...
struct TypeDefStorage : TypeDef {
    ...
    void AddTypeDef(Ref<TypeDef> type) {
        ...
        for(map<Stroka, list<Ref<FuncDef>>>::iterator iter = type->Funcs.begin();
            iter != type->Funcs.end();
            iter++) {
            for(list<Ref<FuncDef>>::iterator it = iter->second.begin();
                it != iter->second.end();
...
struct FuncArgs : RefCount {
    ...
    FuncArgs(DataSimpleStorage::Pos r, const list<DataSimpleStorage::Pos> &d)
        : dat(d), res(r){};
};
...
struct InstrSimpleProgr : InstrSimple {
    InstrSimpleProgr() {
        ProgrSimpleData = new DataSimpleStorage;
    }
...
struct ConstructProgram : RefCount {
    ...
    void Clear() {
        progr = new InstrSimpleProgr;
        data = new DataConstrStorage;
    }
    int AddFunc(DataConstr *dat1, DataConstr *dat2, Stroka Oper, Ref<DataConstr> &res) {
        list<DataConstr *> lst;
        lst.push_back(dat1);
        lst.push_back(dat2);
    ...
    int AddFunc(const list<DataConstr *> &dat, Stroka Oper, Ref<DataConstr> &res) {
        ...
        if(!func) {
            ...
            int k = 0;
            char tmp[256];
            for(list<DataConstr *>::const_iterator it = dat.begin(); it != dat.end();
                it++, k++)
                Error += Stroka("\tdat") + Stroka(Itoa(k, &tmp[0], 10)) + ": " +
                         data->GetTypeName(*it) + " \n";
        ...
        progr->ProgrSimpleInstr.push_back(
            new InstrSimple(new FuncArgs(res->dat, dst), func->Func, Oper));
        return 1;
    };
