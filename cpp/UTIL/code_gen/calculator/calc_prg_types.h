...
struct StdClcTypes : DescribedClass {
    ...
    static void AddStdTypes(TypeDefStorage *stor) {
        Ref<StdClcTypes> ptr;
        for(ClassDesc *desc = ClassDesc::list_category_start("StdClcTypes_category");
            desc;
            desc = ClassDesc::list_category_next(desc, "StdClcTypes_category")) {
            ptr << desc->create();
            if(ptr)
...
struct StdClcDouble : StdClcTypes {
    ...
    struct Operations : FuncStruct {
        ...
        void Function(Ref<FuncArgs> args, Stroka Oper) {
            ...
            double *d1 = (double *)DataSimpleStorage::GetData(*it);
            it++;
...
struct StdClcString : StdClcTypes {
    ...
    struct Operations : FuncStruct {
        ...
        void Function(Ref<FuncArgs> args, Stroka Oper) {
            list<DataSimpleStorage::Pos>::iterator it = args->dat.begin();
            Ref<Stroka> d1 = *(Ref<Stroka> *)DataSimpleStorage::GetData(*it);
            it++;
            if(Oper == Stroka("print")) {
                cout << d1->c_str() << "\n";
                return;
