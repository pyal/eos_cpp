...
// to program storage, instructions are already generated.
	...
	static inline Ref<DataConstr> AddFunc(DataConstr *fst){
        ...
        if (fst->Type->TypeName!=Stroka("undef"))
            zubr_calculator_zubr_error((Stroka("Bad func name type:")+fst->Type->TypeName).c_str());
	...
	static inline Ref<DataConstr> DefineVar(DataConstr* fst){
        ...
        Ref<DataConstr> res;
        for(list<DataConstr*>::iterator i = p->arg_lst_stack.top().begin();i!=p->arg_lst_stack.top().end();i++)
            res = DefineVar(fst,*i);
        p->arg_lst_stack.pop();
        return res;
    }
	static inline Ref<DataConstr> DefineVar(DataConstr* fst, DataConstr *sec){
        ...
        if (sec->Type->TypeName!=Stroka("undef"))
            zubr_calculator_zubr_error((Stroka("Bad var name:")+sec->Type->TypeName).c_str());
        if (fst->Type->TypeName==Stroka("undef"))
            zubr_calculator_zubr_error((Stroka("Bad type name:")+fst->Type->TypeName).c_str());
    ...
    void AddStdVars(){
        ...
        GenerateDouble(M_PartDist1Bor_is_VolIn_cm3_mol,"M_PartDist1Bor_is_VolIn_cm3_mol");
	...
	static inline Ref<DataConstr> GenerateDouble(double val,const char *name=NULL){
		Ref<DataConstr> d = GetCurPtr()->Progr.data->GenerateVar(new 
            DataConstr(NULL,GetCurPtr()->Progr.data->GetTypeDef(Stroka("double"))->Type,0,name));
	...
	static inline Ref<DataConstr> GenerateUndef(const Stroka &str){
        return GetCurPtr()->Progr.data->GenerateVar(new DataConstr(NULL,GetCurPtr()->Progr.data->GetTypeDef(Stroka("undef"))->Type,0,str.c_str()));
	}
    static inline CalculatorProgam *GetCurPtr(CalculatorProgam *newPtr=NULL){
        if (newPtr){
            Ptr_Old = Ptr;Ptr = newPtr;
        }
        return Ptr;
    };
