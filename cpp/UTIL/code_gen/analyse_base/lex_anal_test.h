...
struct TestCase_Manip : TestCase_Buffered {
    ...
    TestSuite *CreateTest() {
        ...
        suite->AddTest(
            Test_Freeze_Restore_Freeze_Unfreeze, "Test_Freeze_Restore_Freeze_Unfreeze");
        suite->AddTest(
            Test_Freeze_Restore_Freeze_Restore, "Test_Freeze_Restore_Freeze_Restore");
        suite->AddTest(
            Test_Freeze_Restore_Undo_Freeze_Restore,
            "Test_Freeze_Restore_Undo_Freeze_Restore");
        suite->AddTest(
            Test_Freeze_Restore_Undo_Freeze_UnFreeze,
            "Test_Freeze_Restore_Undo_Freeze_UnFreeze");
        suite->AddTest(
            Test_Freeze_Restore_Freeze_UnFreeze_End,
            "Test_Freeze_Restore_Freeze_UnFreeze_End");
        suite->AddTest(
            Test_Freeze_Restore_Freeze_Restore_End,
            "Test_Freeze_Restore_Freeze_Restore_End");
        suite->AddTest(
            Test_Freeze_Restore_Undo_Freeze_Restore_End,
            "Test_Freeze_Restore_Undo_Freeze_Restore_End");
        suite->AddTest(
            Test_Freeze_Restore_Freeze_Undo_Restore_End,
            "Test_Freeze_Restore_Freeze_Undo_Restore_End");
        suite->AddTest(
            Test_Freeze_Restore_Freeze_Undo_Restore_Empty,
            "Test_Freeze_Restore_Freeze_Undo_Restore_Empty");
        suite->AddTest(
            Test_Freeze_Restore_Freeze_UnToStr, "Test_Freeze_Restore_Freeze_UnToStr");
        suite->AddTest(
            Test_Freeze_Restore_Freeze_GetUnget_UnToStr,
            "Test_Freeze_Restore_Freeze_GetUnget_UnToStr");
    ...
    static TestCase_Manip *Restore(DescribedClass *ptr) {
        ...
        //info_except
        if(!cur)
            throw(info_except("ptr is not TestCase_Manip"));
        return cur;
    }
    ...
    static void Test_Freeze_Restore_Freeze_Unfreeze(TestCase *ptr) {
        ...
        for(int k = 21; k <= 33; k++)
            if((ch = cur->manip->get()) != cur->buf[k])
                throw(info_except(string("Trying to get for ind ")
                                      .append(Itoa(k, tmp, 10))
                                      .append(" found error; char in buf <")
                                      .append(1, cur->buf[k])
                                      .append("> and read char <")
                                      .append(1, ch)
                                      .append(">\n")
                                      .c_str()));
    ...
    static void Test_Freeze_Restore_Freeze_UnToStr(TestCase *ptr) {
        ...
        if(str != str1)
            throw(info_except(string("Unfrozen String differs src:<")
                                  .append(str1)
                                  .append("> result one is:<")
                                  .append(str)
                                  .append(">\n")
                                  .c_str()));
        ...
        if(str != str1)
            throw(info_except(
                string(
                    "Unfrozen String differs (this is the and of buffer test getstring src:<")
                    .append(str1)
                    .append("> result one is:<")
                    .append(str)
                    .append(">\n")
...
struct TestCase_Lex_Analyser : TestCase_Buffered {
    ...
    static TestCase_Lex_Analyser *Start(DescribedClass *ptr, const char *str) {
        ...
        if(!cur)
            throw(info_except("ptr is not TestCase_Lex_Analyser"));
    ...
    static void CheckRes(Lex_Result *fst, Lex_Result *sec) {
        if(Lex_Result::Equals(fst, sec))
            ...
            return;
        if(fst == NULL)
            throw(info_except(string("Lex result returned NULL and have to return \n")
                                  .append(sec->ToString())
                                  .c_str()));
        if(sec == NULL)
            throw(info_except(string("Lex result returned ")
                                  .append(fst->ToString())
                                  .append(" and have to return NULL ")
                                  .c_str()));
        throw(info_except(string("Lex result returned ")
                              .append(fst->ToString())
                              .append(" and have to return  ")
                              .append(sec->ToString())
                              .c_str()));
    };
    static void Test_String(TestCase *ptr) {
        TestCase_Lex_Analyser *cur = Start(
            ptr,
            "\"Start\"\t\"Another\034\" \t\'\035\'\t\n\
\"aa\
a\naa\"");
        ...
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0, 0, "Another\034", Lex_Result::String);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0, 0, "\035", Lex_Result::Char);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0, 0, "aaa\naa", Lex_Result::String);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        CheckRes(res, NULL);
    }
    static void Test_Number(TestCase *ptr) {
        TestCase_Lex_Analyser *cur =
            Start(ptr, "123 0x3a 022 022.\t1e-1 1.\n 1e23 1.e1 1e 1.e");
        Lex_Result *res, tobe;
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(123, 0, "", Lex_Result::NumberInt);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0x3a, 0, "", Lex_Result::NumberInt);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(022, 0, "", Lex_Result::NumberInt);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0, 22, "", Lex_Result::NumberDouble);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0, 0.1, "", Lex_Result::NumberDouble);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0, 1, "", Lex_Result::NumberDouble);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0, 1e23, "", Lex_Result::NumberDouble);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0, 10, "", Lex_Result::NumberDouble);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(1, 0, "", Lex_Result::NumberInt);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0, 0, "e", Lex_Result::Identifier);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(0, 1, "", Lex_Result::NumberDouble);
        CheckRes(res, &tobe);
    ...
    static void Test_Token(TestCase *ptr) {
        ...
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(1, 0, "+", Lex_Result::Token);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(2, 0, "-", Lex_Result::Token);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(4, 0, "word", Lex_Result::Token);
        CheckRes(res, &tobe);
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(3, 0, "wor", Lex_Result::Token);
        CheckRes(res, &tobe);
        ...
        res = cur->lex_an->Analyse();
        tobe.GenerateResult(5, 0, "word", Lex_Result::Token);
        CheckRes(res, &tobe);
