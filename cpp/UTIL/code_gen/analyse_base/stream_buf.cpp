...
void StreamMemStorage::PopMark() {
    if(pos.empty())
        cout << "Stoping to write empty mem\n";
    pos.pop();
    if((pos.empty()) && (!ReadingStorage))
        clear();
}
void StreamMemStorage::ReturnToMark(
    int &line,
    int &pos_,
    int &Ch,
    StreamUngetStorage &undo) {
    if(pos.empty()) {
        cout << "Starting read from empty storage\n";
        throw;
    }
    StreamPosition p = pos.top();
    pos.pop();
    ReadingPos = p.it;
    line = p.Line;
    pos_ = p.Pos;
...
string StreamManip_Base::UnfreezeToStr() {
    ...
    string ret = "";
    while((upper != mem.GetCurrentPos())) {
        ret += (char)get();
    }
    if(LastGetChar == EOF)
        ret.resize(max<int>(ret.length(), 1) - 1);
    //	if ((was_undo.active()) && (ret!=""))
    if((was_undo.active()) && (ret.length() != 0)) {
        undo.get(LastGetChar);
        unget();
        ret.resize(max<int>(ret.length(), 1) - 1);
    }
...
void StreamManip_Base::ReadStream() {
    if(undo.get(LastGetChar))
        return;
    if(mem.ReadNext(LastGetChar))
        return;
    if(!in_ptr)
