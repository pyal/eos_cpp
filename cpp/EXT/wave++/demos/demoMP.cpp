...
int main(int argc, char *argv[]) {
    ...
    {                                  //functions if desired
        ...
        std::vector<real> Gcoef, Gcoef1;
        Interval f_approx(0, dim - 1), f_approx1(0, dim - 1);
        RealGabor Gtemp, Gtemp1;
        Partition Part(dim, 2.0);

        error = RunShiftGaborMP(
            max_iter, epsilon, CleanSignal, Part, RecSignal, Rf, G, Gcoef);
        std::cout << "error for Shift, a = 2 : " << error << std::endl;

        error1 =
            RunFFTGaborMP(max_iter, epsilon, CleanSignal, RecSignal1, Rf1, G1, Gcoef1);
        std::cout << "error for FFT"
                  << " : " << error1 << std::endl;

        outfile << "********* INPUT_MAKER " << inp << " *********" << std::endl;
        outfile << "number of iterations performed " << G.size() << "  error = " << error
                << std::endl;
        for(int z = 0; z < G.size(); z++) {
            ...
            outfile << "\t(" << Gtemp.s << ", " << Gtemp.u << ", " << Gtemp.v << ", "
                    << Gtemp.w << ")" << std::endl;
            outfile << "\t\tcoeficient: " << coef1 << std::endl;
            outfile << "\t(" << Gtemp1.s << ", " << Gtemp1.u << ", " << Gtemp1.v << ", "
                    << Gtemp1.w << ")" << std::endl;
