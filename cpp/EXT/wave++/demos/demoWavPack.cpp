...
real Factor = (1.0 + sqrt(2 * log((double)MAXLEVEL * DIM)));   //D&J Best Wavelet (BWB)
...
int main() {
    ...
    const int iterations = 17;   //as many as wavelet filters
    ...
    G[4].Set(d10doqf, 0, 9);
    H[4].Set(d10soqf, 0, 9);
    G[5].Set(d12doqf, 0, 11);
    H[5].Set(d12soqf, 0, 11);
    G[6].Set(d14doqf, 0, 13);
    H[6].Set(d14soqf, 0, 13);
    G[7].Set(d16doqf, 0, 15);
    H[7].Set(d16soqf, 0, 15);
    G[8].Set(d18doqf, 0, 17);
    H[8].Set(d18soqf, 0, 17);
    G[9].Set(d20doqf, 0, 19);
    H[9].Set(d20soqf, 0, 19);
    G[10].Set(c06doqf, 0, 5);
    H[10].Set(c06soqf, 0, 5);
    G[11].Set(c12doqf, 0, 11);
    H[11].Set(c12soqf, 0, 11);
    G[12].Set(c18doqf, 0, 17);
    H[12].Set(c18soqf, 0, 17);
    G[13].Set(c24doqf, 0, 23);
    H[13].Set(c24soqf, 0, 23);
    G[14].Set(c30doqf, 0, 29);
    H[14].Set(c30soqf, 0, 29);
    G[15].Set(b18doqf, 0, 17);
    H[15].Set(b18soqf, 0, 17);
    G[16].Set(v24doqf, 0, 23);
    H[16].Set(v24soqf, 0, 23);
    ...
    for(i = 0; i < DIM; i++)
        NoisySignal[i] = CleanSignal[i] + Noise_level * (real)gennor(0, 1);
    fstream out("test", ios::out);
    //for(k=16; k< iterations; k++)
    k = 16;
    {
        ...
        Analysis(
            ...
            H[k],
            G[k],
            ConvDecPer);   //Wavelet packet analysis for CleanSignal
        ...
        for(i = CleanSignal.beg; i <= CleanSignal.end; i++)
            out << i << " " << CleanSignal[i] << " " << RecSignal[i] << " "
                << NoisySignal[i] << "\n";
        bwbErrors[k] = BB_rms;

        std::cout << "The levels for BB are: " << std::endl;
        for(i = 0; i < BB.num_of_levels; i++)
            std::cout << BB.levels[i] << "\t";
        std::cout << std::endl;
        stop();
        std::cout << "The Relative Mean Square Error for estimate k= " << k << std::endl;
        std::cout << bwbErrors[k] << std::endl;
