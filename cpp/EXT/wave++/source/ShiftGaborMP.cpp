...
void getOptimalShiftGabor(
    const Interval &f,   // signal
    ...
    for(integer j = Part.s.beg; j <= Part.s.end; j++) {
        ...
        SI.Set(beg, end);
        codv = cos(ddv);
        sidv = sin(ddv);   // used for uodating CO, SI
        Pptr = P.origin + beg;
        COptr = CO.origin + beg;
        SIptr = SI.origin + beg;
        ...
        *SIptr++ = sin(beg * ddv);
        ...
        codvlms = cos(ddv * lmshift);
        sidvlms = sin(ddv * lmshift);
        codvdu = cos(ddv * ddu);
        sidvdu = sin(ddv * ddu);
        covlms = covdu = 1;
        sivlms = sivdu = 0;
