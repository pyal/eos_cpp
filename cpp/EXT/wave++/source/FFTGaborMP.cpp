...
real RunFFTGaborMP(
    ...
    for(int iter = 0; iter < max_iter; iter++) {
        ...
        Gptr = Gtemp.Sample.origin;
        fptr = f_approx.origin;
        Rfptr = Rf.origin;
        ...
        error = sqrt(error);
        if(error < epsilon) {
            std::cout << "Desired precision achieved in " << iter + 1 << " iterations. "
                      << std::endl;
            break;
        }
    }
...
void getOptimalFFTGabor(const Interval &f) {
    ...
    for(; s > 1; s >>= 1) {
        ...
        for(; u <= dim - 4 * s; u += du)
            proces2(
                8 * s, 8 * s - 1, f.origin + u - 4 * s);   // using FFT calculate a,b,..
        //and update product and coef for all Gabores with fixed s and u.
        // i.e., go thtough all frequences
...
void proces1(const integer &N, const real *fptr) {
    ...
    complexWavelet *E1, *E2, *O1, *O2;
    E1 = (complexWavelet *)calloc(N, sizeof(complexWavelet));
    assert(E1);
    E2 = (complexWavelet *)calloc(N, sizeof(complexWavelet));
    assert(E2);
...
void update() {
    ...
    if(Pnorm2 > 1.0e-16 && Qnorm2 > 1.0e-16 && absval(a1) > 1.0e-16) {
        product = (a * a1 + b * b1) /
