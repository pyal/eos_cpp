...
#define fftbitrev(out,in,q)  bitrevd(out,in,q,sizeof(complexWavelet))
#define fftbrinpl(x,q)       bitrevi(x,q,sizeof(complexWavelet))
...
void
  bitrevd(
	  void *out_,		/* Pointer to base of the output array. */
	  const void *in_,	/* Pointer to base of the input array. */
	  int   q,		/* # of index bits, or elements of `in[]'. */
	  int   size)		/* Number of bytes in an `in[]' element.  */
{
  int u, n;
  char *out = (char *)out_, *in = (char *)in_ ;
...
void
  bitrevi(
	  void *x_,		/* Pointer to the input/output array. */
...
{
  int b, j, k, N, N1, M;
  complexWavelet  *fptr1, *fptr2, tmp;
  const complexWavelet *Wptr;
	   ...
	   int M)		/* for n=0,1,2,...,|M|-1. */
{
  ...
  M = abs(M);
  W = (complexWavelet *)malloc(M*sizeof(complexWavelet));
      ...
      int q)			/* point DFT of the input `f[]' */
{
  ...
  fhat = (complexWavelet *)calloc(N, sizeof(complexWavelet));
  ...
  /* Generate the complexWavelet exponential vector `Omega_{N/2}': */
  if(inverse)
