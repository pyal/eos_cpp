...
QMF::QMF(const QMF &Rhs) : beg(Rhs.beg), end(Rhs.end), center(Rhs.center), dev(Rhs.dev) {
    if(Rhs.coef) {
...
void QMF::Set(const real *f, integer alpha, integer omega) {
    ...
    if(energy > 0) {
        ...
        for(integer j = 1; j <= (end - beg) / 2; j++) {
            real y = 0;
            for(integer k = j + beg; k <= end - j; k++)
                y += k * coef[k - j] * coef[k + j];
            dev += sgn * y;
            sgn = -sgn;
        }
...
void GPQMF::periodize(real *fq, integer q) {
    ...
    while(j < q) {
        *(pointer = fq + index) = 0;
        for(counter = beg + j; counter <= end; counter += q)
            *pointer += coef[counter];
        index = ((index + 1) % q);
        j++;
    }
}
