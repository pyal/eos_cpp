...
void Analysis(
    ...
    for(integer i = 0; i < A.dim; i++)
        A.origin[i] = In.origin[In.beg + i];
    integer L, B;
    for(L = 0; L < A.maxlevel; L++) {
        for(B = 0; B < (1 << L); B++) {
            con_dec(A.block_start(L, B), A.left_child(L, B), A.block_length(L), H);
            con_dec(A.block_start(L, B), A.right_child(L, B), A.block_length(L), G);
        }
    }
}
...
void Synthesis(
    ...
    {                                      // from maxlevel-1 up to 0
        ...
        {
            adj_con_dec(
                A.left_child(L, B), A.block_start(L, B), A.block_length(L + 1), H);
            adj_con_dec(
                A.right_child(L, B), A.block_start(L, B), A.block_length(L + 1), G);
        }
    }
...
void Analysis(
    ...
    for(L = 0; L < A.maxlevel; L++) {
        for(B = 0; B < (1 << L); B++) {
            ...
            alpha = ICH(parent->beg + H.beg);
            omega = IFH(parent->end + H.end);
            child->Set(alpha, omega);
            con_dec(*parent, *child, H);
            child++;   // child = A.right_child(L, B);
            alpha = ICH(parent->beg + G.beg);
            omega = IFH(parent->end + G.end);
            child->Set(alpha, omega);
