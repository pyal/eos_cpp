...
HedgePer::HedgePer(const integer &DIM, const integer &n, const integer *Levs)
    : dim(DIM), num_of_levels(n) {
    ...
    if(levels_consistent()) {
        ...
        std::cout << "Attempt to declare a hedge with incosistent levels" << std::endl;
        exit(1);
    }
    integer p = maxlevel();
    ((void)(p));
...
//////////////////////////////////////////////////////////////////////////////
HedgePer::HedgePer(const HedgePer &Rhs) : dim(Rhs.dim), num_of_levels(Rhs.num_of_levels) {
...
bool HedgePer::levels_consistent() const {
    if(levels != 0) {
        ...
        for(i = 1; i < num_of_levels; i++) {
            k = p - levels[i];
            if((sum >> k) << k != sum)
                return false;   //sum divisible by 2^k?
            sum += 1 << k;      // sum += 2^k
            if(sum > (1 << p))
                return false;
        }
...
HedgeAper::HedgeAper(const integer &n, const integer *Levs) : num_of_levels(n) {
