...
typedef real (*costFun)(
    const real *block,
    const integer &blocklength,
    const real &sigma,
    const integer &signaldim);
...
real BestBasisHelp(integer *Levs, integer &n, Node<real> *ptr, const integer &L);
// utility

void ExtractBestBasis(const ArrayTreePer &A, HedgePer &H, costFun F, const double &sigma);
...
void GetCosts(const ArrayTreeAper &A, Tree &B, costFun F, const real &sigma);
