...
ostream &operator<<(ostream &output, const Interval &I) {
    if(I.origin) {
        ...
        std::cout << "Interval empty. No output produced." << std::endl;
    return output;   // enables std::cout << x << y
}
...
istream &operator>>(istream &input, Interval &I) {
    if(I.origin) {
        ...
        std::cout << "cannot input into empty interval." << std::endl;
    return input;
}   // enables cin >> x >> y
...
ostream &operator<<(ostream &output, const HedgePer &H) {
    if(H.origin) {
        ...
        std::cout << "HedgePer empty. No output produced." << std::endl;
    return output;
}
...
void PrintLevelsToFile(const HedgePer &H, const char *filename) {
    if(H.levels) {
        ...
        for(integer i = 0; i < H.num_of_levels; i++)
            glug << H.levels[i] << std::endl;
    } else
        std::cout << "Hedge levels empty. No output produced." << std::endl;
}
...
ostream &operator<<(ostream &output, const HedgeAper &H) {
    if(H.root) {
        ...
        std::cout << "HedgeAper empty. No output produced." << std::endl;
    return output;
}

///////////////////////////////////////////////////////////////////////////////
istream &operator>>(istream &input, HedgeAper &H) {
    for(integer j = 0; j < H.num_of_levels; j++)
        input >> H.root[j];
    return input;
}   // enables cin >> x >> y
...
ostream &operator<<(ostream &output, const ArrayTreePer &A) {
    if(A.origin) {
        ...
        std::cout << "ArrayTreePer empty. No output produced." << std::endl;
    return output;
}
...
ostream &operator<<(ostream &output, const ArrayTreeAper &A) {
    if(A.root) {
        ...
        std::cout << "ArrayTreeAper empty. No output produced." << std::endl;
    return output;
}
