...
//wavelettype = 0..16 ; best 16?
    static real OracCost( const real *data,  const integer &n, const real &sigma,const integer &k){
        real cost=0;
    ...
    std::vector<double> &Construct(double *signal, int len, int wavelettype, double signalnoiseration){
        //int MAXLEVEL = 15;
        ...
        Analysis(NoisySignal, W, H[wavelettype], G[wavelettype], ConvDecPer);//Wavelet packet analysis for NoisySignal
    ...
    std::vector<double> &ConstructPlus(double *signal, int len, int wavelettype, double signalnoiseration){
