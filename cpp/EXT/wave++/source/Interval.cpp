...
Interval::Interval(const integer &alpha, const integer &omega, const real *data)
    : origin(0) {
    if(data == 0)
        Set(alpha, omega);
    else
        Set(alpha, omega, data);
}
...
const Interval &Interval::operator+=(const Interval &I) {
    if(I.origin)   // if I empty, return without any changes
    {
        if(beg == I.beg && end == I.end) {
            for(integer j = beg; j <= end; j++)
                origin[j] += I.origin[j];
        } else {
            Interval Temp(minimum(beg, I.beg), maximum(end, I.end));
            integer j;
            for(j = beg; j <= end; j++)
                Temp.origin[j] += origin[j];   // add  *this
            for(j = I.beg; j <= I.end; j++)
                origin[j] += I.origin[j];   // add  I
            *this = Temp;
        }
    }
    return *this;
...
void Interval::Set(integer alpha, integer omega, const real *data) {
    if(origin)
        delete[](origin + beg);   // if redefining the existing interval
...
Interval Absval(const Interval &I) {
    ...
    if(I.origin) {
        ...
        for(integer i = I.beg; i <= I.end; i++)
            Temp.origin[i] = absval(I.origin[i]);
    } else
