...
struct UrsCurve_BrightTempClc : URS_Curve::ClcVar {
    ...
    virtual Ref<SavableClass> GetRes(const Stroka &SubName) {
        ...
        if(it == Results.end())
            throw info_except(
                "Variable <%s> for some reason was not calculated\n", SubName.c_str());
    ...
    virtual int save_data_state(FilterOut &so) {
        so << " VarSignalName " << VarSignalName << " OpacityDifCalib2Measure "
           << OpacityDifCalib2Measure << " WaveLength " << WaveLength
           << " InterFilterFormName " << InterFilterFormName << " DetectorEmissivityName "
           << DetectorEmissivityName << " CalibTemp " << CalibTemp << " CalibSignal "
           << CalibSignal << " CalibEmissivityName " << CalibEmissivityName
           << " UseFixedCalibEmissivity " << UseFixedCalibEmissivity << " ExtraFiberNorm "
           << ExtraFiberNorm << " ExtraSignalNorm " << ExtraSignalNorm;
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        char tmp[256];
        si >> tmp >> VarSignalName >> tmp >> OpacityDifCalib2Measure >> tmp >>
            WaveLength >> tmp >> InterFilterFormName >> tmp >> DetectorEmissivityName >>
            tmp >> CalibTemp >> tmp >> CalibSignal >> tmp >> CalibEmissivityName >> tmp >>
            UseFixedCalibEmissivity >> tmp >> ExtraFiberNorm >> tmp >> ExtraSignalNorm;
        return 1;
    };
    virtual Stroka MakeHelp() {
        Stroka res =
            "Class for calculation brightness temperautre using calibration data.\nKnown SubNames are: Temperature, DetectorSensitivity, Plank, IntegTemp\n";
        res +=
            "Physical model: calibration <CalibSignal> done: using the light with temperature <CalibTemp> \n        with fixed emissivity <UseFixedEmissivity> or emissivity is taken from <CalibEmissivityName> 2D spline \n           (line,temp -> emissivity) calculated from 2D data by urs_curve <UrsCurve_SplConstr> method\n";
        res += "Measurements done for the line <WaveLength> or \n";
        res +=
            "variable <IntegTemp> - will calculate integral over detector sensibility (during calibration and measurement)\n";
        res +=
            "During integration will use <DetectorEmissivityName> 2 column file with [line emissivity] format\n";
        res +=
            "Experimental signal is multiplied by <OpacityDifCalib2Measure> variable\n";
        res +=
            "<InterFilterName> - currently is not working ( models non zero length filtering of the signal... )\n";
...
protected:
    virtual void ClcNewStp(int CurStp, const Stroka &SubName) {
        ...
        if(SubName == "DetectorSensitivity")
            Results[SubName] =
                DetectorEmissivitySpl.Evaluate(fabs(GetVar(VarSignalName)));
        if(SubName == "Plank")
    ...
    double OpacityDifCalib2Measure, WaveLength, CalibTemp, CalibSignal, ExtraSignalNorm,
        ExtraFiberNorm;   //nm<<"DetectorEmissivityName"<<DetectorEmissivityName<<
    double UseFixedCalibEmissivity;
    Stroka InterFilterFormName, DetectorEmissivityName, CalibEmissivityName,
        VarSignalName;
    ...
    void InitSens() {
        ...
        if(UseFixedCalibEmissivity < 0) {
            ReadSplineXY(
                CalibEmissivityName.c_str(),
                CalibEmissSpl,
                CalibTemp,
                "Init Calib emissivity");
        ...
        if(WaveLength > 0) {
            ...
            ReadSpline(
                InterFilterFormName.c_str(), InterFilterSpl, "Init InterFilterSpl");
            if(UseFixedCalibEmissivity < 0)
    ...
    double IntenVal(double lambda) {
        ...
        double intence =
            DetectorEmissivitySpl.Evaluate(lambda) * PlancEmisE(IntegTemp, lambda);
    ...
    double IntegrateInt(double temp, CurveSpline *sourceemis, CurveSpline *interfilter) {
        ...
        return Qags_IntSubInt(
            IntenValStatic, IntegMinL, IntegMaxL, 1e-6, 1e-6, 100, ResErr, Err);
    }
    ...
    double FindExpT2GivenLength(double signal) {
        ...
        return 0.01438775220950165 /
               (WaveLength * 1e-9 *
                log(1 + CalibSignal * CalibConst / (signal * OpacityDifCalib2Measure)));
    }
    ...
    double MisfSigAdd(double temp) {
        return OriginalSignal -
               OriginalAddCalibConst * (PlancEmisE(temp, WaveLength) * ExtraFiberNorm +
                                        ExtraSignalNorm * IntegrateInt(temp, NULL, NULL));
    }
    ...
    double FindExpT2GivenLengthAdd(double signal) {
        ...
        if((res = Fzero(MisfSigAddStatic, 100, 1e6, GuessMisf, 1e-6, 1e-6, 1000)) > 3)
            cout << " Bad FindExpT2GivenLengthAdd Fzero return " << res << " found val "
                 << GuessMisf << " found mis " << MisfSigAdd(GuessMisf) << "\n";
        return GuessMisf;
    }
