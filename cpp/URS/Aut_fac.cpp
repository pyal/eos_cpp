...
void SetCurTim(One_Cell &Par, double Tim) {
    Par.CurTime = Tim;
};
...
//    double Ulast = Par.Sh0.Med.Velo + Par.Sh1.Med.Velo + Par.Sh2.Med.Velo;
    ...
    Pos2 = 0;
    if(Par.Sh1.Dvel != 0)
        Pos1 = (Par.Sh1.Time - Ct) * (Par.Sh1.Dvel - Par.Sh2.Med.Velo);
    if(Par.Sh2.Dvel != 0)
        Pos2 = (Par.Sh2.Time - Ct) * (Par.Sh2.Dvel + Par.Sh1.Med.Velo);
    L = Par.Length + (Ct - Par.Sh0.Time) * dU;
    Pos1 = L - Pos1;
    Pos2 = max<double>(0, Pos2);
    Pos1 = min<double>(L, Pos1);
    Pos0 = Par.Position + (Ct - Par.Sh0.Time) * Par.Sh0.Med.Velo;
};
void ControlBound(One_Cell &Par) {
    // Form Positions, TimeStp
    double Pos1, Pos2, Pos0, L;
    ClcPos(Par, Pos0, Pos1, Pos2, L);
    Par.Sh0.Time = Par.CurTime;
    Par.Position = Pos0;
    Par.Sh1.Pos = Pos1;
    Par.Sh2.Pos = Pos2;
    Par.Sh0.Pos = L;
    ...
    if(Tim >= Par.Sh1.Time) {
        Par.Sh1.Dvel = 0;
        Par.Sh1.Time = MathInf;
        Par.Sh1.Med = Par.Sh1.Med * 0;
    }
    if(Tim >= Par.Sh2.Time) {
        Par.Sh2.Dvel = 0;
        Par.Sh2.Time = MathInf;
...
void AddNewWave(One_Cell &Par, Shock_Par Res, int Fst) {
    ...
    //   if ((Tnew<=Told) || (Norm(Par.Sh2.Med)<sqrt(MathZer)))
    {
        ...
        Par.Sh2.Time =
            L / max<double>(StndErr, (Par.Sh2.Dvel + Par.Sh1.Med.Velo)) + Par.CurTime;
        Par.Sh1.Time =
            (L - Pos2) / max<double>(StndErr, (Par.Sh1.Dvel - Par.Sh2.Med.Velo)) +
            Par.CurTime;
        if(Norm(Par.Sh1.Med) < sqrt(MathZer))
            Par.Sh1.Time = MathInf;
        if(!Fst)
            ExchangeBounds(Par);
        return;
...
int Interact(One_Cell &Fst, One_Cell &Sec, MatterIO *MFst, MatterIO *MSec, double Tim) {
    ...
    if(MFst != NULL) {
        ControlBound(Fst);
        Par1 = Fst.Sh0.Med + Fst.Sh2.Med + Fst.Sh1.Med;
    }
    if(MSec != NULL) {
        ControlBound(Sec);
        Par2 = Sec.Sh0.Med;
    ...
    if((MFst != NULL) && (MSec != NULL)) {
        ...
        AddNewWave(Fst, Res1, 1);
        AddNewWave(Sec, Res2, 0);
        return 1;
    }
    if(MSec == NULL) {
...
Shock_Par SetMeanPar(One_Cell &Par, Int_Par *Set) {
    ...
    if(Set == NULL) {
        ...
        ret.Med = (tmp.Sh0.Med * tmp.Sh0.Pos + tmp.Sh1.Med * (tmp.Sh0.Pos - tmp.Sh1.Pos) +
                   tmp.Sh2.Med * (tmp.Sh0.Pos - tmp.Sh2.Pos)) *
                  (1 / tmp.Sh0.Pos);
...
void OneInteract(AutIO &dat, int k) {
    int MFst, MSec;
    MFst = GetNumMatter(k, dat);
    MSec = GetNumMatter(k + 1, dat);
    if(!Interact(
           dat.Data[k],
           dat.Data[k + 1],
           dat.Matter[MFst],
...
void ClcTim(double TimeClc, AutIO &dat) {
    ...
    do {
        ...
        if(MinT >= TimeClc) {
            ...
            int Fst = MinK;
            if(MinT == dat.Data[MinK].Sh2.Time)
                Fst--;
            dat.CurTime = MinT;
            OneInteract(dat, Fst);
            dat.CurTime = MinT;
...
int AutIO::ReadIni(istream &input) {
    ...
    if(NumMatter > MaxNumMatter - 2) {
        cout << " NumMatter " << NumMatter << " Is more then MaxNumMat " << MaxNumMatter
             << " exiting\n";
        exit(0);
    }
...
void AutIO::ReadPar(istream &input) {
    if((NumPnt <= 1) || (NumPnt >= MaxNumPoints - 2)) {
    ...
    if((i < NumPnt) || (NumPnt == 0)) {
        cout << "RegionIO::ReadIni; Entered only part of points NumPnt i " << NumPnt
             << "   " << i << "\n";
        if(i != 0)
            InterfaceIODelete();
        BadPar = 1;
