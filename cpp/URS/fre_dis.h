...
struct PureDis_StrictClc : RefCount {
    struct Param : RefCount {
        ...
        Param(const Param &par)
            : Denc(par.Denc),
              ...
              RelatNumbers(par.RelatNumbers),
              RelatVolumesCoefs(par.RelatVolumesCoefs),
              FreeEPure(par.FreeEPure){};
    ...
    double ClcVolCoef(const Param &par) {
        double Coef = par.Denc * (par.RelatNumbers * par.RelatVolumesCoefs) /
    ...
    VecCl FreeEVectorCor(const Param &par) {
        ...
        VecCl pure =
            VecCl::ComponentMul(
                VecCl::ComponentMul(par.FreeEPure, SubstVeights), par.RelatNumbers) /
            (par.RelatNumbers * SubstVeights);
    ...
    VecCl FreeENumDeriv(const Param &par_) {
        ...
        return FreeENumDeriv(
            par, VecCl::ComponentDiv(FreeEVectorCor(par) * M0, par.RelatNumbers));
    }
    ...
    VecCl FreeENumDeriv(const Param &par_, const VecCl &FreeELogNumCor_) {
        ...
        for(int k = 1; k <= ret.Dim(); k++) {
            ret[k] = kt_Mole * (1 - par.RelatVolumesCoefs[k] * SumN_V0) +
                     ...
                     par.Pressure * (SubstVeights[k] / par.Denc -
                                     VMulCoef * par.RelatVolumesCoefs[k]);
        }
    ...
    VecCl GetMixtureCorrection(const Param &par) {
        ...
        for(int k = 1; k <= SubstVeights.Dim(); k++) {
            ...
            Cor[k] = par.RelatNumbers[k] *
                     log(max<double>(
                         par.RelatNumbers[k] * par.RelatVolumesCoefs[k] / V0, MathZer));
        }
        Cor = Cor * kt_Mole / M0;
        return Cor;
    }
...
struct PureDis_Minim_Base : SavableClass {
    virtual VecCl Do(
        ...
        PureDis_StrictClc *FreeEMixt,
        const VecCl &ChemCoefs) = 0;
};
...
struct PureDis_MinimNum : PureDis_Minim_Base {
    PureDis_MinimNum()
        : PureDis_Minim_Base(),
          StartGuess(0.5),
          MaxVal(-log(M_MinDouble2)),
          MinVal(log(M_MinDouble2)),
          ErrorAbs(M_Eps2),
          ErrorRel(M_Eps2),
          MaxIter(50){
    ...
    int CheckLarge(PureDis_StrictClc::Param *par, double &Guess) {
        ...
        double Coef = (par->FreeEPure[1] * veight[1] - par->FreeEPure[2] * veight[2]) /
                      (M_Rconst * par->Temp * M0);
        //        if (fabs(Coef)<100){
        //            Guess = 0.5;
        //            return 0;
        //        }
    ...
    virtual VecCl Do(
        ...
        const VecCl &RelatVolumesCoefs,
        const VecCl &FreeE_NoMixt,
        PureDis_StrictClc *ClcFreeEMixt,
        const VecCl &chemcoefs) {
        curPar = new PureDis_StrictClc::Param(
            Denc, Temp, Pressure, RelatNumbers, RelatVolumesCoefs, FreeE_NoMixt);
        ...
        if(4 < (err = Fzero(
                    ExecuteStatic, MinVal, MaxVal, Guess, ErrorAbs, ErrorRel, MaxIter))) {
            cout
                << " Bad FZero PureDis_MinimNum in PureDis_MinimNum::PureDis_MinimNum \nfound X "
...
private:
    VecCl ClcRelatNumbers(const VecCl &ChemCoefs, double ReactProp) {
        ...
        for(int k = 1; k <= RelN.Dim(); k++)
            RelN[k] = (ChemCoefs[k] > 0) ? ChemCoefs[k] * ReactProp
                                         : ChemCoefs[k] * (ReactProp - 1);
        return RelN;
    }
...
struct PureDis_Minim1Pres : PureDis_Minim_Base {
    ...
    int save_data_state(FilterOut &so) {
        so << " MinVal " << MinVal << " MaxVal " << MaxVal << " MaxDenc " << MaxDenc
           << " FixedVolumeCoef " << FixedVolumeCoefs;
        if(!so)
    ...
    virtual VecCl Do(
        ...
        if(4 < (err = Fzero(
                    ...
                    MaxIter))) {
            cout
                << " Bad FZero PureDis_Minim1Pres in SingleDiss_MinimFunc::PureDis_Minim1Pres \nfound X "
                << Decode(Guess) << " Mis " << ExecuteStatic(Guess) << " err " << err
                << "\n";
        }
        StaticPtr = OldPtr;
...
private:
    ...
    VecCl ChemCoefs;
    Ref<PureDis_StrictClc> PureDisClc;
    double StartGuess, MinVal, MaxVal, ErrorAbs, ErrorRel, MaxIter, FixedVolumeCoefs,
        MaxDenc;
...
private:
    ...
    VecCl GetPressures(double VolCoef) {
        ...
        VecCl pres(2);
        for(int k = 1; k <= pres.Dim(); k++) {
            pres[k] = PureDisClc->GetChemSubst()[k - 1]->Pressure(
                min(SubstDenc[k], MaxDenc), curPar->Temp);
        }
        VecCl res = MinimNum;
        res[1] = pres[res[1]];
        res[2] = pres[res[2]];
    ...
    VecCl ClcResult(double VolCoef) {
        curPar->RelatVolumesCoefs[MinimNum[2]] =
            curPar->RelatVolumesCoefs[MinimNum[1]] * VolCoef;
...
struct DisMixture_Energy : RefCount {
    ...
    double FreeECor(const VecCl &FreeEPure, const VecCl &RelatNumbers, double Temp) {
        ...
        for(int k = 1; k <= SubstVeights.Dim(); k++) {
            ret += Mas[k] * (FreeEPure[k] + GetMixtureCorrection(
                                                RelatNumbers[k], Temp, SubstVeights[k]));
        }
        return ret;
    }


    static double GetMixtureCorrection(
        double RelatNumber,
        double Temp,
        double SubstVeight) {
...
struct Dis_RelatNum_MinimFunc_Base : SavableClass {
    virtual VecCl FindRelatNumbers(
        const VecCl &FreeE_NoMixt,
        double Temp,
        const VecCl &RelatNumbers,
        const VecCl &veights) = 0;
};
...
struct Dis_RelatNum_MinimFunc_Single : Dis_RelatNum_MinimFunc_Base {
    ...
    VecCl FindRelatNumbers(
        ...
        double Guess =
            (RelatNumbers[1] <= 0 || RelatNumbers[1] >= 1) ? 0.5 : RelatNumbers[1];
        Guess = Encode(Guess);
...
struct PureDis_FindE : DisFindMinimE_Base {   //
    ...
    PureDis_FindE()
        : MinPres(new PureDis_Minim1Pres),
          FixedNumVolCoefs(0){};   //:DisFindMinimE_Base():SavableClass(){}
    virtual void Init(
        const VecCl &substveights,
        const DataVector<Ref<InterfaceFreeEIO>, CopyStructRef<Ref<InterfaceFreeEIO>>>
            &chemsubst,
        ...
        Params = new PureDis_StrictClc::Param(
            0, 0, 0, startsubstcoef, relatvolumecoef, relatvolumecoef);
        //SubstVeights = substveights;
        //ChemSubst = chemsubst;
        //SubstMixtures = substmixtures;
        RelatVolumeCoef = relatvolumecoef;
    ...
    virtual VecCl GetResultVolumeCoef() {
        return ResultVolumeCoefs;
    }
    virtual VecCl GetResultPressures() {
        return ResultPressures;
    }
    virtual VecCl GetResultFreeEPure() {
        return ResultFreeEPure;
    }
    virtual VecCl GetResultFreeECorr() {
        return ResultFreeECorr;
    }
    virtual double ClcFreeE(double Denc, double Temp) {
        ...
        Params->RelatVolumesCoefs =
            Params->RelatVolumesCoefs / Params->RelatVolumesCoefs[1];
        double err = 1;
        int numIt = 0;
        VecCl Pres;
        ...
        if(FixedNumVolCoefs) {
            VecCl RelNum(2);
            RelNum = RelNum * 0 + 1;
            Params->RelatVolumesCoefs = MinPres->Do(
                Denc,
                Temp,
                0,
                RelNum,
        ...
        while(numIt++ < 15 && err > M_Eps2 * 10) {
            ...
            Params->RelatNumbers = MinNum.Do(
                ...
                Params->FreeEPure,
                MixtE,
                MatrCl::GetRow(ChemReact, 1));
            ResultPressures = MixtE->PressureVector(*Params);
            err = sqrt(
                      (ResultPressures - ResultPressures[1]) *
                      (ResultPressures - ResultPressures[1])) /
                  sqrt(ResultPressures * ResultPressures);
            if(FixedNumVolCoefs)
                break;
        }
        ...
        if(Mist > 1e-5) {
            ...
            cout << "	 LastFreeE	" << MixtE->FreeEMixt(*Params) << " : "
                 << MixtE->FreeEVectorPure(*Params) << "\n";
        }
#endif
    ...
    Stroka MakeHelp() {
        Stroka res =
            "Class for finding dissosiation fraction + relat volumes of fractions. Use log(Vi/V0) correction\n";
        return res;
    }
...
private:
    ...
    VecCl ResultRelatMasses, ResultVolumeCoefs, ResultPressures, ResultFreeEPure,
        ResultFreeECorr;
    void CheckStart() {
        if(ChemReact.DimX != 1 || ChemReact.DimY != 2)
            throw info_except(
                "Bad dims: DimX %i DimY %i\n", ChemReact.DimX, ChemReact.DimY);
    }
};
...
struct DisFindMinimE : DisFindMinimE_Base, DisSamePresFunc {
    ...
    DisFindMinimE()
        : DisFindMinimE_Base(),
          DisSamePresFunc(),
          MinimizeVolumes(1),
          MinimRelatNum(new Dis_RelatNum_MinimFunc_Single){};
    ...
    double ClcFreeE(double Denc, double Temp) {
        ...
        if(Mist > 1e-5) {
            cout << "Denc	" << Denc << " Temp	" << Temp << " Mistake " << Mist << "\n";
            cout << "Relat pres "
                 << Pressures(Denc, Temp, RelatNumbers, RelatVolumesCoefs);
            cout << "Relat volume	" << RelatVolumesCoefs;
            cout << "Relat Numbers " << RelatNumbers;
            cout << "	 LastFreeE	" << resFreeE << " : "
                 << FreeEClc->FreeEVectorPure(Denc, Temp, RelatNumbers, RelatVolumesCoefs)
                 << "\n";
        }
#endif
    ...
    virtual VecCl GetResultVolumeCoef() {
        return LstClcRelatVolumesCoefs /
               sqrt(LstClcRelatVolumesCoefs * LstClcRelatVolumesCoefs);
    }
    virtual VecCl GetResultRelatMasses() {
        return FreeEClc->ClcMassCoef(LstClcRelatNumbers);
    }
    virtual VecCl GetResultPressures() {
        return Pressures(
            LstClcDenc, LstClcTemp, LstClcRelatNumbers, LstClcRelatVolumesCoefs);
    }
    virtual VecCl GetResultFreeEPure() {
        return FreeEClc->FreeEVectorPure(
            LstClcDenc, LstClcTemp, LstClcRelatNumbers, LstClcRelatVolumesCoefs);
    }
    virtual VecCl GetResultFreeECorr() {
        return FreeEClc->FreeEVectorCor(
            LstClcDenc, LstClcTemp, LstClcRelatNumbers, LstClcRelatVolumesCoefs);
    }
    ...
    int read_data_state(FilterIn &si) {
        ...
        if(!si || !MinimRelatNum)
            throw info_except(
                "Could not read MinimizeVolumes=%i or MinimRelatNum\n", MinimizeVolumes);
        return DisFindMinimE_Base::read_data_state(si);
    }
    int save_data_state(FilterOut &so) {
        so << " MinimizeVolumes " << MinimizeVolumes << " MinimRelatNum "
           << MinimRelatNum;
        if(!so)
            throw info_except("Could not save class\n");
    ...
    Stroka MakeHelp() {
        Stroka res =
            "Class for finding dissosiation fraction + relat volumes of fractions. \nFor dissosiation fraction clc is used Dis_RelatNum_MinimFunc_category class.\nCurrently in the system are present " +
            SavableClass::HelpForCategory("Dis_RelatNum_MinimFunc_category");
        return res;
    }
...
private:
    ...
    double MinimDissFreeE(
        ...
        VecCl FePure =
            FreeEClc->FreeEVectorPure(Denc, Temp, RelatNumbers, RelatVolumesCoefs);
        RelatNumbers = MinimRelatNum->FindRelatNumbers(
            FePure, Temp, RelatNumbers, FreeEClc->Veights());
    ...
    void SetLstClc(
        ...
        double Denc,
        double Temp) {
        LstClcRelatNumbers = RelatNumbers;
        LstClcRelatVolumesCoefs = RelatVolumesCoefs;
        LstClcDenc = Denc;
        LstClcTemp = Temp;
    };
    int MinimizeVolumes;
...
struct FreeEDis : FreeEIOFind {
    ...
    Stroka MakeHelp() {
        ...
        res +=
            "Additionaly this class can calculate FreeE - total freE; M_i - mass of i-th component ; V_i - volume of i-th component ; P_i - pressure of i-th component ; Fp_i - FreeE pure of i-th component ; ; Fc_i - FreeE corrected(no entropy) of i-th component\n";
        res +=
            "Calculate dissosiation of matters - adding to the free E of pure mat - entropy mixture addition. Pure free E calc under assumption that relat denc of matters are the same. For minim of E used base class DisFindMinimE_Base. Currently in the system we have classes of this category:\n";
