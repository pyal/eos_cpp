...
struct UrsCurve_Calculator : URS_Curve::ClcVar {
    ...
    virtual Ref<SavableClass> GetRes(const Stroka &SubName) {
        ...
        for(int k = 0; k < OutputCurveNames.SetNumEl(); k++)
            if(SubName == OutputCurveNames[k])
                return new SavableDouble(*(double *)DataSimpleStorage::GetData(
                    prg.GetVar(OutputClcNames[k])->dat));
    ...
    virtual int read_data_state(FilterIn &si) {
        char tmp[256];
        si >> tmp >> tmp;
        InputCurveNames.Release();
        InputClcNames.Release();
        OutputCurveNames.Release();
        OutputClcNames.Release();
        si >> tmp;
        while(Stricmp(tmp, "}") != 0) {
            InputCurveNames.push(Stroka(tmp));
            si >> tmp;
            InputClcNames.push(Stroka(tmp));
            si >> tmp;
            if(Stricmp(tmp, ";") != 0)
        ...
        if(Stricmp(tmp, "{") != 0)
            throw info_except(
                " Could not read name, wrong delimiter %s - have to be { \n", tmp);
        si >> tmp;
        while(Stricmp(tmp, "}") != 0) {
            OutputCurveNames.push(Stroka(tmp));
            si >> tmp;
            OutputClcNames.push(Stroka(tmp));
            si >> tmp;
            if(Stricmp(tmp, ";") != 0)
                throw info_except(
                    " Could not read name, wrong delimiter %s - have to be ; \n", tmp);
