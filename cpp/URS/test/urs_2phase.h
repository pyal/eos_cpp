...
struct UrsCurve_EOS_PT_Equilib : URS_Curve::ClcVar {
    UrsCurve_EOS_PT_Equilib()
        : URS_Curve::ClcVar(),
          ...
          NameMatterLowP("MatLowP"),
          NameMatterHghP("MatHghP"){};
    virtual void ClcStart(URS_Curve *Data) {
        ...
        MatLowP =
            dynamic_cast<EOS_Savable *>((SavableClass *)Dat->GetVar(NameMatterLowP));
        MatHghP =
            dynamic_cast<EOS_Savable *>((SavableClass *)Dat->GetVar(NameMatterHghP));
        if(!MatLowP || !MatHghP || !Temp)
            throw info_except(
                "Could not get MatLowP?%i MatHghP?%i Temperature?%i..\n",
                MatLowP == NULL,
                MatHghP == NULL,
                Temp == NULL);
        if(!MatHghP->Mat || !MatLowP->Mat)
            throw info_except(
                "Matter's MatHghP?%i or MatLowP?%i - matter - poorly defined.\n",
                MatHghP->Mat == NULL,
                MatLowP->Mat == NULL);
    };

    virtual int save_data_state(FilterOut &so) {
        so << "NameTemp" << NameTemp.c_str() << "NameMatterHghP" << NameMatterHghP.c_str()
           << "NameMatterLowP" << NameMatterLowP.c_str();
        return 1;
    };
...
struct UrsCurve_PT_Bnd_Constructor : UrsCurve_EOS_PT_Equilib {
    ...
    virtual void ClcStart(URS_Curve *Data) {
        ...
        if(!dynamic_cast<MatterFreeE *>((MatterIO *)MatLowP->Mat) ||
           !dynamic_cast<MatterFreeE *>((MatterIO *)MatHghP->Mat))
            throw info_except(
                "UrsCurve_FreeE works only with MatterFreeE base class. %s or %s  is wrong type\n",
                MatLowP->Mat->class_name(),
                MatHghP->Mat->class_name());
    ...
    virtual int save_data_state(FilterOut &so) {
        ...
        so << "ClcError" << ClcError << "FindPDencFrom" << FindPDencFrom << "FindPDencTo"
           << FindPDencTo << "FindGLowPres" << FindGLowPres << "FindGHghPres"
           << FindGHghPres << "StartPres" << StartPres;
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        UrsCurve_EOS_PT_Equilib::read_data_state(si);
        char tmp[256];
        si >> tmp >> ClcError >> tmp >> FindPDencFrom >> tmp >> FindPDencTo >> tmp >>
            FindGLowPres >> tmp >> FindGHghPres >> tmp >> StartPres;
        return 1;
    };
    ...
    virtual Stroka MakeHelp() {
        ...
        char tmp[256];
        res += Stroka("Class for FreeE Bnd calculation. Have ") +
               Itoa(NumClcNames, tmp, 10) + " internal vars for output:";
        for(map<Stroka, int>::iterator it = ClcVarNames.begin(); it != ClcVarNames.end();
            it++)
            res += it->first + " ";
        res += Stroka(".\n");
...
protected:
    ...
    static double DeltaPres(double Denc) {
        return FindPres - FindMat->Pressure(Denc, CurTemp);
    }

    static double StaticClcError, StaticDencLow, StaticDencHgh, StaticDencFrom,
        StaticDencTo;
    ;
    ...
    static double DeltaG(double Pres) {
        ...
        if(4 < (err = Fzero(
                    ...
                    500))) {
            cout
                << " Bad FZero PressureLow in UrsCurve_PT_Bnd_Constructor::DeltaG \nfound Denc "
        ...
        if(4 < (err = Fzero(
                    ...
                    500))) {
            cout
                << " Bad FZero PressureHgh in UrsCurve_PT_Bnd_Constructor::DeltaG \nfound Denc "
                << StaticDencHgh << " Mis " << DeltaPres(StaticDencHgh) << " err " << err
                << "\n";
        }
        //cout<<" found Denc "<<StaticDencHgh<<" Mis "<<DeltaPres(StaticDencHgh)<<" err "<<err<<"\n";
        double GHgh =
            StaticFreeLowPStat->FreeE(StaticDencLow, CurTemp) +
            StaticFreeLowPStat->Pressure(StaticDencLow, CurTemp) / StaticDencLow;
        double GLow =
            StaticFreeHghPStat->FreeE(StaticDencHgh, CurTemp) +
            StaticFreeHghPStat->Pressure(StaticDencHgh, CurTemp) / StaticDencHgh;
    ...
    void ClcBndPnt() {
        ...
        if(4 <= (err = Fzero(
            ...
            cout << " Bad FZero DeltaG in UrsCurve_PT_Bnd_Constructor::ClcBndPnt \nTemp "
                 << CurTemp << " found Pres " << LastPres << " Mis " << DeltaG(LastPres)
                 << " err " << err << "\n";
        }
        ...
        eos_res[ClcVarNames["EnergyLowP_T"]] =
            new SavableDouble(StaticFreeLowPStat->Energy(StaticDencLow, CurTemp));
        eos_res[ClcVarNames["EnergyHghP_T"]] =
            new SavableDouble(StaticFreeHghPStat->Energy(StaticDencHgh, CurTemp));
    }
};
...
struct UrsCurve_PT_Bnd_Binary : URS_Curve::ClcVar {
    ...
    UrsCurve_PT_Bnd_Binary()
        : URS_Curve::ClcVar(),
          ...
          NameLowD("MatLowD"),
          NameHghD("MatHghD"),
          NameFileToGenerate("HghLow_bnd.bin"),
          ClcError(1e-6){};
    virtual void ClcStart(URS_Curve *Data) {
        URS_Curve::ClcVar::ClcStart(Data);
        ...
        if(!LowD || !HghD || !Temp)
            throw info_except(
                "Could not get LowD?%i HghD?%i Temperature?%i..\n", !LowD, !HghD, !Temp);
        T.SetNumEl(0);
        H.SetNumEl(0);
        L.SetNumEl(0);
    };
    ...
    virtual void ClcEnd() {
        ...
        Ref<FreeELiqSol_Bnd> bnd = FreeELiqSol_Bnd::Generate(
            (double *)H, (double *)L, (double *)T, H.SetNumEl(), ClcError);
        FreeELiqSol_Bnd::SaveFile(NameFileToGenerate.c_str(), bnd.pointer());
    };

    virtual int save_data_state(FilterOut &so) {
        so << "NameTemp" << NameTemp.c_str() << "NameHghD" << NameHghD.c_str()
           << "NameLowD" << NameLowD.c_str() << "NameFileToGenerate"
           << NameFileToGenerate.c_str() << "ClcError" << ClcError;
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        char tmp[256];
        si >> tmp >> NameTemp >> tmp >> NameHghD >> tmp >> NameLowD >> tmp >>
            NameFileToGenerate >> tmp >> ClcError;
        return 1;
    };
    virtual Stroka MakeHelp() {
        Stroka res = URS_Curve::ClcVar::MakeHelp();
        res += Stroka(
            "Binary Bnd file generator - stores results for boundary in the binary format...\n");
...
protected:
    ...
    virtual void ClcNewStp(int CurStp, const Stroka &SubName) {
        if(!FirstVarStpClc(this))
            return;
        T.push(GetVar(NameTemp));
        L.push(GetVar(NameLowD));
        H.push(GetVar(NameHghD));
    }
};
...
struct UrsCurve_PT_UnstBnd_Clc : URS_Curve::ClcVar {
    ...
    virtual void ClcStart(URS_Curve *Data) {
        ...
        Ref<URS_Curve::ClcVar> Temp;
        Temp = Dat->GetClcVar(NameTemp);
        EOS_Savable *MatFreeEEnv =
            dynamic_cast<EOS_Savable *>((SavableClass *)Dat->GetVar(NameMatter));
        ...
        if(!MatUnst || !Temp)
            throw info_except(
                "Could not get MatUnst?%i MatFreeE?%i EOS_Savable?%i Temperature?%i..\n",
                MatUnst == NULL,
                MatFree == NULL,
                MatFreeEEnv == NULL,
                Temp == NULL);
    ...
    virtual Ref<SavableClass> GetRes(const Stroka &SubName) {
        ...
        if(Results.find(SubName) == Results.end()) {
            Stroka Names;
            for(map<Stroka, double>::iterator it = Results.begin(); it != Results.end();
                it++)
                Names += it->first + " ";
            throw info_except(
                "Cannot calculate variable %s\nKnown names are :%s\n",
                SubName.c_str(),
                Names.c_str());
        }
    ...
    virtual int save_data_state(FilterOut &so) {
        ...
        so << "Dmin" << Dmin << "Dmax" << Dmax << "NumPnt" << NumPnt << "NumSplPnt"
           << NumSplPnt << "Misfit" << Misfit;
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        char tmp[256];
        si >> tmp >> NameTemp >> tmp >> NameMatter;
        si >> tmp >> Dmin >> tmp >> Dmax >> tmp >> NumPnt >> tmp >> NumSplPnt >> tmp >>
            Misfit;
        return 1;
    };
    Stroka GetVars() {
...
protected:
    ...
    virtual void ClcNewStp(int CurStp, const Stroka &SubName) {
        if(StpDone == CurStp)
            return;
        Results = MatterStable::TwoPhaseBoundary::ClcBnd(
            GetVar(NameTemp), Dmin, Dmax, NumPnt, NumSplPnt, MatUnst, Misfit);
        StpDone = CurStp;
    }
};
...
struct UrsCurve_PT_UnstBnd_Binary : URS_Curve::ClcVar {
    ...
    virtual void ClcEnd() {
        if(DataMap.begin()->second.size() <= 1)
            return;
        Stroka desc = MatterStable::TwoPhaseBoundary::SetBnd(
            DataMap, ResFileName, SplMisfit, NumSplPnt);
...
protected:
    ...
    virtual void ClcNewStp(int CurStp, const Stroka &SubName) {
        if(StpDone == CurStp)
            return;
        StpDone = CurStp;
        for(map<Stroka, vector<double>>::iterator it = DataMap.begin();
            it != DataMap.end();
            it++)
...
struct UrsCurve_PT_clc : URS_Curve::ClcVar {
    ...
    virtual void ClcStart(URS_Curve *Data) {
        ...
        if(!Matter || !Pres || !Temp)
            throw info_except(
                "Could not get Matter<%s>?%i Pressure<%s>?%i Temperature<%s>?%i..\n",
                NameMatter.c_str(),
                !Matter,
                NamePres.c_str(),
                !Pres,
        ...
        if(!dynamic_cast<MatterFreeE *>((MatterIO *)Matter->Mat))
            throw info_except(
                "UrsCurve_PT_clc works only with MatterFreeE base class. %s is wrong type\n",
    ...
    virtual Ref<SavableClass> GetRes(const Stroka &SubName) {
        ...
        if(Results.find(SubName) == Results.end())
            throw info_except(
                "Cannot calculate var <%s>. Class can calculate only <%s>\n",
                SubName.c_str(),
                GetVars().c_str());
        return new SavableDouble(Results[SubName]);
    }
    ...
    virtual int save_data_state(FilterOut &so) {
        so << "NameTemp" << NameTemp.c_str() << "NamePres" << NamePres.c_str()
           << "NameMatter" << NameMatter.c_str() << "MinDenc" << MinDenc << "MaxDenc"
           << MaxDenc << "ClcError" << ClcError;
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        char tmp[256];
        si >> tmp >> NameTemp >> tmp >> NamePres >> tmp >> NameMatter >> tmp >> MinDenc >>
            tmp >> MaxDenc >> tmp >> ClcError;
        return 1;
    };
...
protected:
    ...
    map<Stroka, double> Results;
    Stroka GetVars() {
        return " FreeG FreeE Dencity Eneregy ";
    }
    virtual void ClcNewStp(int CurStp, const Stroka &SubName) {
        if(!FirstVarStpClc(this))
            return;
    ...
    double FindDenc(double Pres, double Temp) {
        ...
        int err;
        if(4 <
           (err = Fzero(DeltaP, MinDenc, MaxDenc, StatDenc, ClcError, ClcError, 500))) {
            cout << " Bad FZero in UrsCurve_PT_clc::DeltaG \nfound Denc " << StatDenc
                 << " Mis " << DeltaP(StatDenc) << " err " << err << "\n";
        }
        CurPtr = Old;
        return StatDenc;
    }
    ...
    static double DeltaP(double Denc) {
        double y =
            log(max<double>(1e-200, CurPtr->FreeClc->Pressure(Denc, CurPtr->StatTemp)) /
                CurPtr->StatPres);
