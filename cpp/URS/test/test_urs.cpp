...
void WriteResource(int ResourceNum,const char *FileName){
    ...
    int k1=0;
    while (k1<size) {if (dat[k1]==13) {k1++;continue;} out.write(&dat[k1],1);k1++;}
}
void TestCase_Urs_Static::Text_Func_Out(ResourcesStruct *resources,int Length,const char *name)
{
    for (int k=0;k<Length;k++) 
    {
        ...
        cout<<nam_cfg.c_str()<<"         ";cout.flush();
        if (system(Stroka(Stroka("urs_curve ") + nam_cfg + " >> " + TestExeOutput).c_str())==0){
            cout<<"OK\n";
            if (DeleteGoodResults)
                {unlink(nam_cfg.c_str());unlink(nam_test.c_str()); unlink(nam_res.c_str());}
        } else {
...
int main( int argc, const char *argv[] )
{
	...
	Time_struct Time;
    try {
        NRef::CommandLineExe Cmd("Usage: test_urs [Params_Key_Spec] [/][-]key  \n without parameters - standart test\n");
...
//        Cmd.MainHelp += GenerateDetailedHelp();
        ...
        if (Cmd.Get("AllResults").Exist) TestCase_Urs_Static::DeleteGoodResults=0;
