...
struct URS_Curve : SavableClass {
    ...
    struct Output : SavableClass {
        ...
        virtual void OutStart(int numIter, URS_Curve *data) {
            ...
            for(list<Stroka>::iterator it = OutNames.begin(); it != OutNames.end();
                it++) {
                Ref<ClcVar> clc = Dat->GetClcVar(*it);
                if(!clc)
                    throw info_except(
                        "variable %s is not of ClcVar type...\n", it->c_str());
                out << it->c_str() << "\t";
            }
            out << "\n";
        }
    ...
    void Do() {
        ...
        for(map<Stroka, Ref<SavableClass>>::iterator it = Vars.begin(); it != Vars.end();
            it++) {
    ...
    Stroka MakeHelp() {
        Stroka res;
        res +=
            Stroka("Class for drawing gasd curves.\n") +
...
struct UrsCurve_StepClc : URS_Curve::ClcVar {
    UrsCurve_StepClc()
        : MinVal(0), MaxVal(100), NumDivStp(100), LogScale(0), NumSame(1), Centered(0){};
    virtual void ClcStart(URS_Curve *data) {
        URS_Curve::ClcVar::ClcStart(data);
        int NumStp = Dat->NumIter;
        if(NumSame * NumDivStp == 0 || NumStp % (NumSame * NumDivStp) != 0)
            throw info_except(
                " NumStp %i is not product of NumSame %i and NumDivStp %i \n",
                NumStp,
                NumSame,
                NumDivStp);
    };
    ...
    virtual int read_data_state(FilterIn &si) {
        char tmp[256];
        si >> tmp >> MinVal >> tmp >> MaxVal >> tmp >> NumDivStp >> tmp >> LogScale >>
            tmp >> NumSame >> tmp >> Centered;
        return 1;
    };
    virtual Stroka MakeHelp() {
...
protected:
    virtual void ClcNewStp(int curStp, const Stroka &subName) {
        ...
        double mi = LogScale ? log(MinVal) : MinVal, ma = LogScale ? log(MaxVal) : MaxVal,
               stp = (ma - mi) / max<double>(nDiv, 1);
        if(Centered)
            mi += 0.5 * stp;
        Res = mi + stp * cur;
        if(LogScale)
...
struct UrsCurve_EOS_RoT : URS_Curve::ClcVar {
    ...
    virtual void ClcStart(URS_Curve *data) {
        ...
        if(!Mat || !denc || !temp)
            throw info_except(
                "Could not get Matter?%i Denc?%i or Temperature?%i..\n",
                Mat == NULL,
                denc == NULL,
                temp == NULL);
        if(!Mat->Mat)
    ...
    virtual int save_data_state(FilterOut &so) {
        so << "NameDenc" << NameDenc.c_str() << "NameTemp" << NameTemp.c_str()
           << "NameMatter" << NameMatter.c_str();
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        char tmp[256];
    ...
    Stroka MakeHelp() {
        Stroka res =
            "Class for FreeE Ro_T calculations. Input pars: Dencity, Temperature\n";
        return res;
    }
...
struct UrsCurve_EOS_RoE : URS_Curve::ClcVar {
    ...
    virtual int save_data_state(FilterOut &so) {
        so << "NameDenc" << NameDenc.c_str() << "NameEnergy" << NameEner.c_str()
           << "NameMatter" << NameMatter.c_str();
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
...
#define NumClcNames 3
    ...
    virtual Ref<SavableClass> GetRes(const Stroka &SubName) {
        ...
        return new SavableDouble(
            Mat->Mat->OutputValClc(GetVar(NameDenc), GetVar(NameEner), SubName));
    }
    virtual Stroka MakeHelp() {
        char tmp[256];
        Stroka res = UrsCurve_EOS_RoE::MakeHelp();
        res +=
            Stroka("Have ") + Itoa(NumClcNames, tmp, 10) + " internal vars for output:";
        for(int k = 0; k < NumClcNames; k++)
            res += ClcNames[k] + " ";
        res += ".\n";
        return res;
...
private:
    virtual Ref<SavableClass> ClcName(int k) {
        if(!eos_res[k]) {
            MatStat = Mat->Mat;
            eos_res[k] =
                new SavableDouble(eos_clc[k](GetVar(NameDenc), GetVar(NameEner)));
        }
        return eos_res[k];
    }
};
...
#define NumClcNames 6
    ...
    virtual void ClcStart(URS_Curve *Data) {
        ...
        if(!dynamic_cast<MatterFreeE *>((MatterIO *)Mat->Mat))
            throw info_except(
                "UrsCurve_FreeE works only with MatterFreeE base class. %s is wrong type\n",
                Mat->Mat->class_name());
    ...
    virtual Ref<SavableClass> GetRes(const Stroka &SubName) {
        ...
        //FreeStat = FreeClc;
        return new SavableDouble(
            FreeClc->OutputValClc(GetVar(NameDenc), GetVar(NameTemp), SubName));
...
protected:
    ...
    static double ClcPressure(double r, double t) {
        return FreeStat->Pressure(r, t);
    }
    static double ClcSound(double r, double t) {
        return MatStat->Sound(r, FreeStat->Energy(r, t));
    }
    static double ClcEnergy(double r, double t) {
        return FreeStat->Energy(r, t);
    }
    static double ClcFreeE(double r, double t) {
        return FreeStat->FreeE(r, t);
    }
    static double ClcFreeG(double r, double t) {
        return FreeStat->FreeE(r, t) + FreeStat->Pressure(r, t) / r;
    }
...
private:
    virtual Ref<SavableClass> ClcName(int k) {
        if(!eos_res[k]) {
            ...
            FreeStat = FreeClc;
            eos_res[k] =
                new SavableDouble(eos_clc[k](GetVar(NameDenc), GetVar(NameTemp)));
        }
        return eos_res[k];
    }
};
...
struct UrsCurve_EOS_FindE : UrsCurve_EOS_RoT {
    ...
    virtual Ref<SavableClass> GetRes(const Stroka &SubName) {
        if(SubName != "Energy" && SubName != "FindE")
            throw info_except(
                "Cannot calculate var %s. Use to clc Enery only\n", SubName.c_str());
        CheckStp(SubName, this);
        return new SavableDouble(Res);
        //		throw info_except("Cannot calculate variable %s\n",SubName.c_str());
    }
...
protected:
    virtual void ClcNewStp(int CurStp, const Stroka &SubName) {
        if(!FirstVarStpClc(this))
            return;
        Res = Mat->Mat->Energy(GetVar(NameDenc), GetVar(NameTemp));
    }
};
...
struct UrsCurve_EOS_Isoentrope : UrsCurve_EOS_Ro {
    ...
    virtual Stroka MakeHelp() {
        Stroka res =
            "Get Dencity  var and StartE StartU - result - \n" +
            Stroka(
...
struct UrsCurve_EOS_Hugoniot : UrsCurve_EOS_Ro {
    ...
    virtual int save_data_state(FilterOut &so) {
        UrsCurve_EOS_Ro::save_data_state(so);
        so << "StartDenc" << Start.Denc << "StartEner" << Start.Ener << "StartPres"
           << Start.Pres << "StartVel" << Start.Vel << "PressureDependece"
           << PressureDependece << "RecalcPres" << RecalcPres;
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        UrsCurve_EOS_Ro::read_data_state(si);
        ...
        char tmp[256];
        si >> tmp >> StartDenc >> tmp >> StartEner >> tmp >> StartPres >> tmp >>
            StartVel >> tmp >> PressureDependece >> tmp >> RecalcPres;
        Start = HugoniotClass::HugoniotData(StartVel, StartPres, StartDenc, StartEner, 0);
        return 1;
    };
    virtual Stroka MakeHelp() {
        Stroka res =
            "From Pressure var(or Dencity if  PressureDependence=1) and Start (E,Ro,U,P) params make hugoniot. Write also if we have to recalculate start Pressure(RecalcPres=1)\nResult for output - Hugoniot: Energy Velocity ShockVelocity Pressure Dencity.\n";
        return res;
    }
...
struct UrsCurve_FileReader : URS_Curve::ClcVar {
    ...
    UrsCurve_FileReader()
        : URS_Curve::ClcVar(), DefaultValue(0), in(0), FileName("input.dat") {
        DataNames.push("X");
        DataNames.push("Y");
    };
    virtual Ref<SavableClass> GetRes(const Stroka &SubName) {
    ...
    virtual Stroka MakeHelp() {
        Stroka res =
            "Class for reading of external file. Input file name and names for columns\n";
        return res;
    }
    ...
    virtual void ClcNewStp(int CurStp, const Stroka &SubName) {
        ...
        while(File::GetLine(in, str)) {
            ...
            if(line.size() < (size_t)CurLine.SetNumEl()) {
                cout << "Bad line its length is " << line.size() << " and have to be "
                     << CurLine.SetNumEl() << "Line is:\n"
                     << str << "\n";
                continue;
            }
            int Bad = 0;
...
struct UrsCurve_XY_Test : URS_Curve::ClcVar {
    struct Result : SavableClass {
        Result(double dat1, double dat2, int toprint = 1)
            : Dat1(dat1), Dat2(dat2), ToPrint(toprint){};
        double Dat1, Dat2;
        int ToPrint;
        virtual int save_data_state(FilterOut &so) {
            if(ToPrint)
    ...
    virtual void ClcStart(URS_Curve *Data) {
        ...
        if(!Y1 || !Y2)
            throw info_except(
                "Could not get Y1 or Y2 curve. Y1 %s Y2 %s\n",
                NameY1.c_str(),
                NameY2.c_str());
        CurPnt = 0;
        CurError = 0;
    ...
    virtual void ClcEnd() {
        ...
        if(CheckMaxSingleError) {
            if(MaxError > MaxMeanError) {
                throw info_except(
                    "MaxError is too big!!!. Error is %13.6e and have to be %13.6e\n",
                    MaxError,
                    MaxMeanError);
                return;
            }
        } else if(CurError / CurPnt > MaxMeanError)
            throw info_except(
                "MeanError is too big!!!. Error is %13.6e and have to be %13.6e\n",
                CurError / CurPnt,
                MaxMeanError);
    };

    virtual int save_data_state(FilterOut &so) {
        so << "NameY1" << NameY1.c_str() << "NameY2" << NameY2.c_str() << "MaxMeanError"
           << MaxMeanError << "CheckMaxSingleError" << CheckMaxSingleError;
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        char tmp[256];
        si >> tmp >> NameY1 >> tmp >> NameY2 >> tmp >> MaxMeanError >> tmp >>
            CheckMaxSingleError;
        return 1;
    };
    virtual Stroka MakeHelp() {
        Stroka res =
            "Class for testing the result. Output Name = None - not to print anything, only test\n";
        res += "If CheckMaxSingleError==0:\n";
        res +=
            "If Sum{|X1-X2|/max(|X1|,|X2|)}/NumPnt>MaxMeanError - throw an exception. \n";
...
protected:
    ...
    virtual void ClcNewStp(int CurStp, const Stroka &SubName) {
        ...
        double CurMis =
            fabs(GetVar(NameY1) - GetVar(NameY2)) /
            max<double>(
                max<double>(fabs(GetVar(NameY1)), fabs(GetVar(NameY2))), M_MinDouble2);
        CurError += CurMis;
...
struct UrsCurve_HugCheck : UrsCurve_EOS_Ro {
    ...
    virtual int save_data_state(FilterOut &so) {
        UrsCurve_EOS_Ro::save_data_state(so);
        so << "StartDenc" << Start.Denc << "StartEner" << Start.Ener << "StartPres"
           << Start.Pres << "StartVel" << Start.Vel << "FinPres" << FinPres;
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        UrsCurve_EOS_Ro::read_data_state(si);
...
struct UrsCurve_SplConstr : URS_Curve::ClcVar {
    ...
    UrsCurve_SplConstr()
        : URS_Curve::ClcVar(),
          NameX("X_Temperature2FreeESpl"),
          NameY("Y_Dencity2FreeESpl"),
          NameZ("Z_FreeE2FreeESpl"),
          spl(new ExtendedRegridSpline()),
          ResSplineName("XY_Z_spline.spl"),
          SplineDescription("Zero_Spline"){};
    virtual void ClcStart(URS_Curve *Data) {
        URS_Curve::ClcVar::ClcStart(Data);
        Ref<URS_Curve::ClcVar> x, y, z;
        x = Dat->GetClcVar(NameX);
        y = Dat->GetClcVar(NameY);
        z = Dat->GetClcVar(NameZ);
        if(!x || !y || !z)
            throw info_except(
                "Could not get X or Y or Z curve. X %s Y %s Z %s\n",
                NameX.c_str(),
                NameY.c_str(),
                NameZ.c_str());
    };
    ...
    virtual void ClcEnd() {
        ...
        if(dat.I[2] == 0)
            throw info_except(
                "There is no data in the input file n1<%i> n2<%i> n3<%i> datN<%i>\n",
                dat.I[0],
                dat.I[1],
                dat.I[2],
                dat.N);
    ...
    virtual int save_data_state(FilterOut &so) {
        so << "NameX" << NameX.c_str() << "NameY" << NameY.c_str() << "NameZ"
           << NameZ.c_str() << "ResSplineName" << ResSplineName.c_str()
           << "Raw_SplineDescription_Str" << SplineDescription.c_str();
        spl->save_generation_par(so);
        return 1;
    };
    virtual int read_data_state(FilterIn &si) {
        char tmp[256];
        si >> tmp >> NameX >> tmp >> NameY >> tmp >> NameZ >> tmp >> ResSplineName >>
            tmp >> SplineDescription;
        spl->read_generation_par(si);
        return 1;
    };
    virtual Stroka MakeHelp() {
        Stroka res =
            "Class for generating  XY_Z spline.\nResult spline file can be used by MatterFreeSpl - the same format\n";
        res +=
        ...
        res +=
            "Ideally - create new class - for storing - retrieving FreeSpl file - now it is stored by this class, used by MatterFreeSpl class...\n";
        return res;
    }
...
struct UrsCurve_VarStorage : URS_Curve::ClcVar {
    ...
    virtual Ref<SavableClass> GetRes(const Stroka &SubName) {
        ...
        if(Dat->CurItet >= vec->SetNumEl())
            throw info_except(
                "Cannot calculate index %i for variable %s num el is only %i\n",
                Dat->CurItet,
                SubName.c_str(),
                vec->SetNumEl());
    ...
    virtual int save_data_state(FilterOut &so) {
        so << " Variables { ";
        for(map<Stroka, Ref<ResizeVector<double>>>::iterator it = Vars.begin();
            it != Vars.end();
            it++)
            so << it->first << " ";
        so << " } ";
    ...
    virtual void SetCurRes(const Stroka &SubName, SavableClass *cl) {
        ...
        if(!doub)
            throw info_except(
                "Cannot set the variable of type %s it have to be SavableDouble\n",
                cl->class_name());
    ...
    virtual void OutStep() {
        for(list<Out>::iterator it = OutNames.begin(); it != OutNames.end(); it++) {
            ...
            if(dat) {
                ...
                UrsCurve_VarStorage *stor = dynamic_cast<UrsCurve_VarStorage *>(
                    (SavableClass *)Dat->GetVar(it->SetName));
                if(!stor)
                    throw info_except(
                        "variable %s is not of UrsCurve_VarStorage type...\n",
                        it->SetName.c_str());
...
struct UrsCurve_InOut : URS_Curve::ClcVar {
    ...
    virtual void ClcStart(URS_Curve *Data) {
        ...
        for(map<Stroka, Stroka>::iterator it = InputVar_Mat.begin();
            it != InputVar_Mat.end();
            it++) {
    ...
    virtual Stroka MakeHelp() {
        Stroka res =
            "Class for any EOS calculation (any number of input data and output data - depending on Matter...)\n";
        return res;
    }
...
struct UrsCurve_OneVarFunction : URS_Curve::ClcVar {
    UrsCurve_OneVarFunction()
        : URS_Curve::ClcVar(), NameX("X"), Func(new Cold_Vinet), StartIntPnt(1){};
    virtual void ClcStart(URS_Curve *Data) {
        URS_Curve::ClcVar::ClcStart(Data);
    ...
    virtual Ref<SavableClass> GetRes(const Stroka &SubName) {
        ...
        if(SubName == "Val")
            ...
            throw info_except(
                "Cannot calculate variable %s. Calculate [Val, Derivative, Integral]\n",
                SubName.c_str());
    }
    virtual Stroka MakeHelp() {
        Stroka res =
            "Get X  var and StartIntPnt  - result - \n" +
...
struct UrsCurve_NamedOneVarFunction : URS_Curve::ClcVar {
    UrsCurve_NamedOneVarFunction()
        : URS_Curve::ClcVar(), NameX("X"), Func(new Cold_Vinet){};
    virtual void ClcStart(URS_Curve *Data) {
        URS_Curve::ClcVar::ClcStart(Data);
        Ref<URS_Curve::ClcVar> X;
        X = Dat->GetClcVar(NameX);
    ...
    virtual Stroka MakeHelp() {
        Stroka res =
            "Get X  var  - result - \n" +
            Stroka(
                "NamedOneVarFunction result. Input - NamedOneVarFunction_category function\n") +
...
struct UrsCurve_ManyVarFunction : URS_Curve::ClcVar {
    ...
    virtual Stroka MakeHelp() {
        Stroka res =
            "Input X  vars  - result - arbitrary...\n" +
            Stroka(
                "ManyVarFunction result. Input - ManyVarFunction_category function\n") +
