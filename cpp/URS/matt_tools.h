...
struct FindSubstanceIO : virtual SavableClass {
    ...
    FindSubstanceIO(const char *file, const char *subst)
    ...
    int SetInput(FilterIn *in) {
        in->OpenBuf(
            DataSource::GenerateUnicName(FileName.c_str()).c_str(),
            1,
            DataSource::Disk,
            0,
            FileName.c_str());
    ...
    void SetOutput(FilterOut *out) {
        ...
        while(!seek_eof(O)) {
            ...
            if(Sub == Substance)
                throw info_except(
                    "Substance %s already present in file %s",
                    Substance.c_str(),
                    FileName.c_str());
            O.getline(tmp, 1024);
        }
    ...
    virtual int save_data_state(FilterOut &output) {
        output << " FileName " << FileName << " Substance " << Substance << " ";
        return !(!output);
    }
};
