...
int IsentropeClass::IsentropePres(double FromDenc,double FromEnergy,double ToPres,
...
int IsentropeClass::NextPressure(double &NextDenc,double &NextEnergy,double &NextPres,
...
HugoniotClass::HugoniotData HugoniotClass::HugDencClc(
        const HugoniotClass::HugoniotData &beg_pnt, double res_denc){
...
void HugoniotClass::SetHugPresClcVar(const HugoniotClass::HugoniotData &beg_pnt, double res_pres,
...
//cout<<" dv_est "<<dv_est<<"\n";
    ...
    if (k>=MaxIter){
        k = 0; dv_est = dv_est0; dP_est = dP_est0;
        while (dP_start*dP_est>0 && k++<MaxIter){
            dv_est*=1.05;
            dP_est = MisHugPres_v(dv_est);
        }
...
HugoniotClass::HugoniotData HugoniotClass::HugPresClc_v(
        const HugoniotClass::HugoniotData &beg_pnt, double res_pres){
...
//    CurPtr = this;
    ...
    int res;
    if (3<=(res=Fzero(HugoniotClass::MisHugPres_vs,MindV,MaxdV,dv,M_Eps2,M_Eps2,10000))){
                    cout <<" Error, could not find HugDenc ZeroFzeroIt\n";
                    cout<<" Error: "<<res<<"\n";
                    cout<<"Found dv "<<dv<<"\n";
    }
...
HugoniotClass::HugoniotData HugoniotClass::HugPresClc(
    ...
    MD_p = res_pres;
    MD_e = beg_pnt.Ener;
    double MinA = sqrt(fabs(MD_dP)*MD_v1)*1.000001, MaxA = 10+3*MinA, A = 1.4*MinA;
    CurPtr = this;
...
//    }
    ...
    if (3<=(res=Fzero(HugoniotClass::MisHugPres_Ds,MinA,MaxA,A,M_Eps2,M_Eps2,10000))){
                    cout <<" Error, could not find HugDenc ZeroFzeroIt\n";
                    cout<<" Error: "<<res<<"\n";
