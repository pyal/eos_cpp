...
struct ManyVarSplGenerator : ManyVarFunction {
    ...
    virtual int save_data_state(FilterOut &out) {
        out << " SplineFile " << SplFileName << " GenerateSpline " << GenerateSpline
            << " SplineDescr " << SplineDescr << " SplineOrder " << SplineOrder
            << " SplineClass " << SplName;
    ...
    Stroka MakeHelp() {
        Stroka res =
            "Class for generating and calculating splines -> y(x). Do make spline define <X> <Y> and may <W>\n Currently present Splines:" +
...
struct NoiseRemoval : ManyVarFunction {
    ...
    virtual int save_data_state(FilterOut &out) {
        out << " ResultFileName " << ResultFileName << " MaxMisfitVal " << MaxMisfitVal
            << " StartNumDiv " << StartNumDiv << " MinNumDiv " << MinNumDiv;
        return 1;
    };
    virtual int read_data_state(FilterIn &in) {
        char tmp[256];
    ...
    NoiseRemoval()
        : ResultFileName("exp.dat"), StartNumDiv(100), MaxMisfitVal(1), MinNumDiv(5) {}
    ...
    ~NoiseRemoval() {
        ...
        Stroka descr;
        CurveAnalyzer anal;
        anal.InitDat1(Xval, Yval, StartNumDiv, MaxMisfitVal, MinNumDiv, SameCoef);
    ...
    Stroka MakeHelp() {
        Stroka res =
            "Class for noise removing from experiment signal. Define <X> <Y> and may <W>\n" +
