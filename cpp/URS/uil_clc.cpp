...
void ClcMassInBody(Body &WorkBody) {
    if(UilWorkDim == 1) {
        for(int i = 1; i < Num - 1; i++)
            M_[i] = Ro_[i] * (R_[i + 1] - R_[i]);
    } else {
        for(int i = 1; i < Num - 1; i++)
            M_[i] = Ro_[i] * fabs(pow(R_[i + 1], UilWorkDim) - pow(R_[i], UilWorkDim)) /
                    UilWorkDim;
    }
...
void CellFixed(Body &WorkBody) {
    ...
    if(UilWorkDim == 1) {
        for(k = 2; k < Num; k++) {
            if(WorkBody.UnfixAll || (fabs(U_[k] - U_[k + 1]) > 1e-4) ||
               (fabs(P_[k + 1] - P_[k]) > 11)) {
                F_[k] = 0;
                F_[k + 1] = 0;
                F_[k - 1] = 0;
        ...
        for(k = 2; k < Num; k++) {
            if(WorkBody.UnfixAll || ((fabs(U_[k]) + fabs(U_[k + 1]) > 1e-4) ||
                                     (fabs(P_[k + 1] - P_[k]) > 11))) {
                F_[k] = 0;
                F_[k + 1] = 0;
                F_[k - 1] = 0;
...
void CalcTStpVisc(
    ...
    if(Ro_[1] == 0)
        return;
    int k;
    double Coef, *Sound = new double[Num + 3], *L = new double[Num + 3],
                 *tmp = new double[Num + 3];
    ...
    if(UilWorkDim == 1) {
        for(k = 1; k <= Num; k++)
            if((!F_[k]) && (M_[k] + M_[k - 1] > MathZer))
                AddU[k] = 2 * (P_[k] - P_[k - 1]) / (M_[k] + M_[k - 1]);
            else
                AddU[k] = 0;
        ...
        for(k = 1; k <= Num; k++)
            if((!F_[k]) && (M_[k] + M_[k - 1] > MathZer)) {
                if(UilWorkDim == 2)
                    R = fabs(R_[k] + R_[k - 1]) * 0.5;
                else
                    R = sqr(R_[k] + R_[k - 1]) * 0.25;   // - not so, rough approximation
                AddU[k] = 2 * (P_[k] - P_[k - 1]) * R / (M_[k] + M_[k - 1]);
            } else
                AddU[k] = 0;
