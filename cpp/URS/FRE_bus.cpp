...
static ClassDesc FreeEBushman_ClassDesc(
    typeid(FreeEBushman),
    "FreeEBushman",
    1,
...
static ClassDesc FreeCold_V_ClassDesc(
    typeid(FreeCold_V),
    "FreeCold_V",
    1,
    "OneVarFunction_category",
    create<FreeCold_V>);
static ClassDesc FreeColdR_V_ClassDesc(
    typeid(FreeColdR_V),
    "FreeColdR_V",
    1,
    "OneVarFunction_category",
    create<FreeColdR_V>);
static ClassDesc TetaExp_Sigma_ClassDesc(
    typeid(TetaExp_Sigma),
    "TetaExp_Sigma",
    1,
    "OneVarFunction_category",
    create<TetaExp_Sigma>);
static ClassDesc BfuncE_T_ClassDesc(
    typeid(BfuncE_T),
    "BfuncE_T",
    1,
    "OneVarFunction_category",
...
static ClassDesc HeatCapacity_SigmaT_ClassDesc(
    typeid(HeatCapacity_SigmaT),
    "HeatCapacity_SigmaT",
    1,
    "TwoVarFunction_category",
    create<HeatCapacity_SigmaT>);
...
static ClassDesc TetaL_SigmaLT_Short_ClassDesc(
    typeid(TetaL_SigmaLT_Short),
    "TetaL_SigmaLT_Short",
    1,
    "TwoVarFunction_category",
    create<TetaL_SigmaLT_Short>);
static ClassDesc FreeM_SigmaMT_ClassDesc(
    typeid(FreeM_SigmaMT),
    "FreeM_SigmaMT",
    1,
    "TwoVarFunction_category",
    create<FreeM_SigmaMT>);
static ClassDesc HeatCapacityE_SigmaT_ClassDesc(
    typeid(HeatCapacityE_SigmaT),
    "HeatCapacityE_SigmaT",
    1,
    "TwoVarFunction_category",
    create<HeatCapacityE_SigmaT>);
static ClassDesc GammaE_SigmaT_ClassDesc(
    typeid(GammaE_SigmaT),
    "GammaE_SigmaT",
    1,
    "TwoVarFunction_category",
    create<GammaE_SigmaT>);
...
FreeEBushman::FreeEBushman()
    : FreeEIOFind("material.cfg", "Ni_Bushman"),
      Denc0c(9.03342),
      Denc0(8.92857),
      Denc0m(7.83),
      Zconst(28),
...
double FreeCold_V::Calculate(double sigma) {
    double ret = 0;
    for(int k = 1; k <= Num; k++)
        ret += ColdCoef[k] * (pow(sigma, k / 3.) - 1) / k;
    return 3 * ret;
}
...
int FreeCold_V::read_data_state(FilterIn &input) {
    ...
    input >> tmp >> Num;
    if(Num > 20) {
        cout << " Error reading FreeCold_V. Read Num pow coef (" << Num
             << ") is more then 20.\n";
        return 0;
    }
    for(int k = 1; k <= Num; k++)
        input >> tmp >> ColdCoef[k];
...
double FreeColdR_V::Calculate(double sigma) {
    return (Ac * pow(sigma, M_) / M_ + Bc * pow(sigma, N_) / N_ +
            Cc * pow(sigma, L_) / L_) +
           Esub_Mul_Denc0c;
}
...
int FreeColdR_V::read_data_state(FilterIn &input) {
    char tmp[256];
    input >> tmp >> Ac >> tmp >> Bc >> tmp >> M_ >> tmp >> N_ >> tmp >> Esub_Mul_Denc0c;
    Cc = -Ac - Bc;
...
int FreeColdR_V::save_data_state(FilterOut &output) {
    output << " Ac " << Ac << " Bc " << Bc << " m " << M_ << " n " << N_
           << " Esub_Mul_Denc0c " << Esub_Mul_Denc0c;
    return 1;
};
...
double TetaExp_Sigma::Calculate(double x) {
    ...
    double ret = Teta0 * pow(x, 2. / 3) *
                 exp((Gamma0 - 2. / 3) * (sqr(Bcoef) + sqr(Dcoef)) / Bcoef *
...
int TetaExp_Sigma::read_data_state(FilterIn &input) {
    char tmp[256];
    input >> tmp >> Teta0 >> tmp >> Gamma0 >> tmp >> Bcoef >> tmp >> Dcoef;
    return 1;
}
int TetaExp_Sigma::save_data_state(FilterOut &output) {
    output << " Teta0 " << Teta0 << " Gamma0 " << Gamma0 << " Bcoef " << Bcoef
           << " Dcoef " << Dcoef;
    return 1;
}
...
int HeatCapacity_SigmaT::read_data_state(FilterIn &input) {
    char tmp[256];
    input >> tmp >> Sigma_a >> tmp >> Temp_a;
    return 1;
}
int HeatCapacity_SigmaT::save_data_state(FilterOut &output) {
    output << " Sigma_a " << Sigma_a << " Temp_a " << Temp_a;
    return 1;
}
...
double TetaL_SigmaLT::Teta(double sigL) {
    ...
    double ret =
        Teta0 * exp(Coef * (Gamma0 - 2. / 3) * (sqr(Bcoef) + sqr(Dcoef)) / Bcoef *
...
double TetaL_SigmaLT::Calculate(double sigL, double temp) {
    ...
    double B2 = (1 - A3) * A1 / (A1 - A2), B1 = -A2 * B2 / A1;
    if(sigL < 1)
        Coef = B1 * pow(sigL, A1) + B2 * pow(sigL, A2) + A3;
...
int TetaL_SigmaLT::read_data_state(FilterIn &input) {
    char tmp[256];
    input >> tmp >> Temp_sa >> tmp >> Temp_ca >> tmp >> sigL_sigC >> tmp >> PowRelease >>
        tmp >> PowX0 >> tmp >> PowCoef >> tmp >> PowPow >> tmp >> Teta0 >> tmp >>
        Gamma0 >> tmp >> Bcoef >> tmp >> Dcoef;
    return 1;
}
int TetaL_SigmaLT::save_data_state(FilterOut &output) {
    output << " Temp_sa " << Temp_sa << " Temp_ca " << Temp_ca << " sigL_sigC "
           << sigL_sigC << " PowRelease " << PowRelease << " PowX0 " << PowX0
           << " PowCoef " << PowCoef << " PowPow " << PowPow << " Teta0 " << Teta0
           << " Gamma0 " << Gamma0 << " Bcoef " << Bcoef << " Dcoef " << Dcoef;
    return 1;
}
...
double TetaL_SigmaLT_Short::Calculate(double sigL, double temp) {
    //     double Coef=1;
    double ret =
        Temp_sa * (Temp_ca * Teta(sigL) + temp) * pow(sigL, 2. / 3) / (Temp_ca + temp);
    return ret;
};
int TetaL_SigmaLT_Short::read_data_state(FilterIn &input) {
    char tmp[256];
    input >> tmp >> Temp_sa >> tmp >> Temp_ca >> tmp >> Teta0 >> tmp >> Gamma0 >> tmp >>
        Bcoef >> tmp >> Dcoef;
    return 1;
}
int TetaL_SigmaLT_Short::save_data_state(FilterOut &output) {
    output << " Temp_sa " << Temp_sa << " Temp_ca " << Temp_ca << " Teta0 " << Teta0
           << " Gamma0 " << Gamma0 << " Bcoef " << Bcoef << " Dcoef " << Dcoef;
    return 1;
}
...
double FreeM_SigmaMT::Calculate(double sigM, double temp) {
    ...
    return 3 * MulCoef * (Cm + 2. * Am / 5 * (pow(sigM, PowAm) - 1)) +
           3 * (Bm - Cm) * temp;
}
int FreeM_SigmaMT::read_data_state(FilterIn &input) {
    char tmp[256];
    input >> tmp >> Temp_m0 >> tmp >> Am >> tmp >> Bm >> tmp >> Cm >> tmp >> PowAm;
    return 1;
}
int FreeM_SigmaMT::save_data_state(FilterOut &output) {
    output << " Temp_m0 " << Temp_m0 << " Am " << Am << " Bm " << Bm << " Cm " << Cm
           << " PowAm " << PowAm;
    return 1;
}
