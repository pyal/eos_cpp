    ...
    struct MixtureFreeEClc : SavableClass {
        ...
        int save_data_state(FilterOut &so){
            ...
            for (size_t i = 0; i < Names.size(); i++) 
                so<<SavableClass::EOLN()<<" Matter "<<Names[i] << " Mass "<<Masses[i];
        ...
        Stroka MakeHelp(){
            Stroka res = "Class for calcution free of 2 matters mixture : Temp, MeanDenc, VolCoefs are given...";
            return res;
        }
        ...
        vector<double> ClcFreeEMixt(ClcDissociation::MatterCalculator *mats) {
            ...
            for (size_t iMat = 0; iMat < nums.size(); iMat++) 
                fMix[iMat] = nums[iMat] * mats->MolWeights[iMat] * kT * log(vols[iMat]/v0);
        ...
        void SetStartParams(ClcDissociation::MatterCalculator *mats) {
            ...
            if (Matters->MolWeights.size() != Names.size())
                throw info_except("Wrong sizes: Matters->MolWeights:%i and Names:%i\n", Matters->MolWeights.size(), Names.size());
            Matters = mats;
            ...
            Matters->NumCoefs = VecCl::ComponentDiv(VecCl(massesInt), VecCl(Matters->MolWeights)).Copy2Vector();
        }
    protected:
