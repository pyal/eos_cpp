...
struct MatterStable: MatterIO
  {
	...
	struct TwoPhaseBoundary:SavableClass{
		...
		double Stability(double Dencity,double Temperature){
			if ( !In_Lim(Temperature,Tmin,Tmax) || !In_Lim(Dencity,Dmin,Dmax)) return 0; 
			double DencL=Dl_T.Evaluate(Temperature),DencR=Dr_T.Evaluate(Temperature);
			double DencSL=DSl_T.Evaluate(Temperature),DencSR=DSr_T.Evaluate(Temperature);
			if  ( (In_Lim(Dencity,Dmin,DencL)) || (In_Lim(Dencity,DencR,Dmax)) ) return 0;
			if (In_Lim(Dencity,DencSL,DencSR)) return 1; // MayBeOnlyStable
			if (In_Lim(Dencity,DencL,DencSL)) return (Dencity-DencL)/(DencSL-DencL);
		...
		int InTwoPhase(double Dencity,double Temperature){
			if ( !In_Lim(Temperature,Tmin,Tmax) || !In_Lim(Dencity,Dmin,Dmax)) return 0; 
			double DencL=Dl_T.Evaluate(Temperature),DencR=Dr_T.Evaluate(Temperature);
			if  (In_Lim(Dencity,DencL,DencR))  return 1;
			return 0;
			};
		...
		virtual int read_data_state(FilterIn &input)
			...
			E_T.ReadIni(input);P_T.ReadIni(input);dPdT_T.ReadIni(input);
			Dr_T.ReadIni(input);Dl_T.ReadIni(input);
			DSl_T.ReadIni(input);DSr_T.ReadIni(input);
			return 1;
			};
		virtual int save_data_state( FilterOut &output)
			{
			output.precision(12);
			output<<" Binodal_Tmin "<<Tmin<<" Tmax "<<Tmax<<" Denc_min "<<Dmin<<" Denc_max "<<Dmax<<"\n";
			E_T.SaveIni(output);P_T.SaveIni(output);dPdT_T.SaveIni(output);
			Dr_T.SaveIni(output);Dl_T.SaveIni(output);
			DSl_T.SaveIni(output);DSr_T.SaveIni(output);
			return 1;
			};
// Construction of the boundary from the EOS....
        static map<Stroka, double> ClcBnd(double Temp, double Dmin, double Dmax, double NumPnt, double NumSplPnt, FreeEIO *Mat, double Misfit);
        static Stroka SetBnd(map<Stroka, vector<double> > &data, const Stroka &res_file_name, double SplMisfit, double NumSplPnt);
        static vector<Stroka> GetBndNames(){
            vector<Stroka> ret(8);
            ret[0] = "E_T";ret[1] = "P_T";ret[2] = "dPdT_T";ret[3] = "Dr_T";ret[4] = "Dl_T";
            ret[5] = "DSr_T";ret[6] = "DSl_T";ret[7] = "T";
            return ret;
        };
    private:
        static Stroka MakeSpline(CurveSpline &spl,int Nspl,double Misf,double *y,double *x,int NumX, const char *err_name);
        ...
        static double IntegralFunc_PV_Int_Static(double v){return TwoPhaseBoundaryStaticPtr->IntegralFunc_PV_Int(v);}
        double IntegralFunc_PV_Int(double v); 
        static double ZeroFunc_PV_Int_Static(double pres){return TwoPhaseBoundaryStaticPtr->ZeroFunc_PV_Int(pres);}
   ...
   double DencityMax()  {return MatUnstable->DencityMax();};

   double E_,D_,L_T,L_E,L_D,Zero_Temp;
   static MatterStable* Current;
   static double DeltaFree(double T)    {   return Current->Energy(Current->D_,T)-Current->E_; }
  ...
  double Temperature(double Dencity,double Energy)
    {
     ...
     L_D=Dencity;L_E=Energy;L_T=T;
     if ( !Bnd.InTwoPhase(Dencity,T) ) return T;
     double T0=L_T;
     E_=Energy;D_=Dencity;
     MatterStable *Old = Current;
     Current=this;
     int err;
     FixedUnstable=1;
     if (4<(err=Fzero(DeltaFree,10,1e6,T0,StndErr*0.01,StndErr*0.01,200)) )
   ...
   double Pressure(double Dencity,double Energy)
    {
     double T=MatUnstable->Temperature(Dencity,Energy);
     if ( !Bnd.InTwoPhase(Dencity,T) ) return MatUnstable->Pressure(Dencity,Energy);
//cout<<"Wau!!!!!!!!! Dencity "<<Dencity<<" Energy "<<Energy<<" T "<<T<<"\n";
     double t=Temperature(Dencity,Energy);
     return Bnd.P_T.Evaluate(t);
    };
   MatterStable(double Zer_T=100){MatUnstable=new MatterABu;L_T=1000;L_D=0;L_E=0;Zero_Temp=Zer_T;strcpy(TwoPhaseName,"TwoPhaseName");};
   virtual int save_data_state( FilterOut &output) 
    {
...
//     fstream in(TwoPhaseName,ios::in);Bnd.ReadIni(in);in.close();
 ...
 //   virtual void MakeHelp(FilterTextOut&out){
    ...
    virtual Stroka MakeHelp(){
        ...
        res += Stroka(" TwoPhaseName file - is a binary file... Help for it is not available....\n");
        return res;
	}
...
struct MatterBinodalSpinodal:virtual MatterIO
  {
   ...
   double Energy(double Dencity,double Temperature)
     {
     if (InputValStorage["MatterStable"]<1) return MatStable->MatUnstable->Energy(Dencity,Temperature);
     return MatStable->Energy(Dencity,Temperature);
     };
   double Temperature(double Dencity,double Energy)
    {
     if (InputValStorage["MatterStable"]<1) return MatStable->MatUnstable->Temperature(Dencity,Energy);
     return MatStable->Temperature(Dencity,Energy);
    }                                          

   double Pressure(double Dencity,double Energy)
    {
     if (InputValStorage["MatterStable"]<1) return MatStable->MatUnstable->Pressure(Dencity,Energy);
     return MatStable->Pressure(Dencity,Energy);
    };

   MatterBinodalSpinodal():MatterIO(),MatStable(new MatterStable) {SetInternalNames();};
   virtual int save_data_state( FilterOut &output) {
   ...
   virtual double Pressure(double Dencity,double Energy)
    {
     ...
     if (ret==-100) return Liquid->Pressure(Dencity,Energy);
     if (ret==-200) return Solid->Pressure(Dencity,Energy);
     return ret;
    }
//   virtual double Sound(double Dencity,double Energy);
   ...
   {
     double ret=Bnd.LiqPressure(Dencity,Energy);
     if (ret==-100) return Liquid->Temperature(Dencity,Energy);
     if (ret==-200) return Solid->Temperature(Dencity,Energy);
     return Bnd.Temperature(Dencity);
   };
   virtual double DencityCold(){ return Solid->DencityCold();}
   virtual double DencityMax(){ return Solid->DencityMax();}
   virtual double Energy(double Dencity,double Temperature)
   {
     double Temp=Bnd.Temperature(Dencity);
     if (Temperature>Temp) return Liquid->Energy(Dencity,Temperature);
     if (Temperature<Temp) return Solid->Energy(Dencity,Temperature);
     return  Liquid->Energy(Dencity,Temperature);
   };


   MatterLiqSol():BndFile("BndFileName") {Solid=new MatterABu;Liquid=new MatterABu;};
   virtual int save_data_state( FilterOut &output) 
    {
...
//		double low = Rhgh_T.Evaluate(Temp), up = Rlow_T.Evaluate(Temp);
		...
		if (low>up) {double tmp=low;low=up;up=tmp;}
...
//cout<<Temp<<" "<<low<<" "<<up<<" "<<Denc<<" "<<a<<"\n";
	...
	void GetLowUp(double &low,double &up,double Temp){
		low = Rlow_T.Evaluate(Temp); up = Rhgh_T.Evaluate(Temp);
		if (low>up) {
			double tmp=low;low=up;up=tmp;
		}
	}
	virtual int save_data_state(FilterOut&so){
		so<<"NameBndSplFile"<<NameFile;return 1;
	};
	virtual int read_data_state(FilterIn&si){
		char tmp[256];si>>tmp>>NameFile;ReadFile(NameFile.c_str(),this);
		return 1;
	}
	...
	static FreeELiqSol_Bnd *Generate(double *Rhgh, double *Rlow, double *T, int Num, double Err){
		FreeELiqSol_Bnd *res = new FreeELiqSol_Bnd;
		int NumRes=Num;
		if (!res->Rhgh_T.Generate(NumRes, Err, 0, T, Rhgh, Num))//int &NumX,double &Misf,int Cont,double *x,double *y,int n,
			throw info_except("could not generate spline for Hgh denc\n");
		NumRes=Num;
		if (!res->Rlow_T.Generate(NumRes, Err, 0, T, Rlow, Num))//int &NumX,double &Misf,int Cont,double *x,double *y,int n,
...
struct FreeELiqSol:FreeEIOFind {
	FreeELiqSol():FreeEIOFind("material.cfg","FreeELiqSol_Bushman_Mg"),MatHghTemp(new FreeEIdeal),MatLowTemp(new FreeEIdeal),
...
//		return x*MatHghTemp->FreeE(low,Temp)+(1-x)*MatLowTemp->FreeE(up,Temp);
    ...
    void ReadSubstancePar(istream &in){
        char tmp[256];
        in>>tmp;MatHghTemp << SavableClass::Read(in);
        in>>tmp;MatLowTemp << SavableClass::Read(in);
		in>>tmp;Bnd  << SavableClass::Read(in);
		if (!MatHghTemp || !MatLowTemp || !Bnd)
			throw info_except("Could not register MatHghTemp?%i or MatLowTemp?%i or Bnd?%i\n",!MatHghTemp,!MatLowTemp,!Bnd);
    };
    void SaveSubstancePar(ostream &out){
		out<<" MatHghTemp ";SavableClass::Save(out,MatHghTemp);
		out<<" MatLowTemp ";SavableClass::Save(out,MatLowTemp);
		out<<" Bnd ";SavableClass::Save(out,Bnd);
    }
	virtual Stroka MakeHelp(){
        Stroka res = FreeEIOFind::MakeHelp();
		res += Stroka(" To generate bnd class - Make bounds by urs_curve;\nBounds generation config:\n");
	...
	virtual double OutputValClc(const map<Stroka,double> &Input, const Stroka &Output){
		...
		if (CheckPresence(OutputNames, Output, ptr) && ptr!=NULL){
            if (Output=="Low_M")
                return DisLowM(CheckPresence(Input, "Dencity"), CheckPresence(Input, "Temperature"));
            if (Output=="Hgh_M")
                return 1-DisLowM(CheckPresence(Input, "Dencity"), CheckPresence(Input, "Temperature"));
		}
