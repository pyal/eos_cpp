...
int InHead(
    ...
    double &PresDerivCoef,
    double &EndTime,
    Ref<ExternalEnergyInput> &energyAdd) {
    char tmp[256];
    in >> tmp >> CurTime >> tmp >> TimeStp >> tmp >> TimeWrite >> tmp >> PresDerivCoef >>
        tmp >> EndTime;
    if((!in) && (seek_eof(in)))
        return 0;
...
Region *GetRegion(
    ...
    input >> tmp;
    OutHead(
        output, *Reg, CurTime, TimeStp, TimeWrite, PresDerivCoef, EndTime, energyAdd, 1);
    ...
    try {
        ...
        while((!(CurTime >= EndTime)) && (!BreakSignal)) {
            ...
            while((!(CurTime >= NewTimeWrite)) && (!BreakSignal)) {
                NewTimeWrite = min(
                    OldTimeWrite + TimeWrite / (1 + PresDeriv * PresDerivCoef), EndTime);
                if(x < 100) {
                    x++;
                    CurStp = 0.5 * log10((double)x) * TimeStp;
                } else
                    CurStp = TimeStp;
