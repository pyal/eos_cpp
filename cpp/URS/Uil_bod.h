...
//   	void SetBnd(double M,double P,double V,int F)  {Mass=M;Pressure=P; Velocity=V; Free=F;}
   	BndStruct(){Free=1;Mass=0;Pressure=ZeroPressure;};
 };
...
struct Body:RefCount
{
   ...
   {NumPnt=NumP;NumIntPar=NumI;Viscouse=Visc;Viscouse2=Visc2;Matter=Mat;FstCalcP=1;FstCalcS=1;};
   Body(){Matter=NULL;NumPnt=0;IterCalcS=0;NumSkipS=10;UnfixAll = 0;}
   Body(int NPnt,MatterIO *Mat,double Vis,double Vis2,const DataVector<Stroka> &IntParNames)
    {Matter=NULL;NumPnt=0;IterCalcS=0;NumSkipS=10;Create(NPnt,Mat,Vis,Vis2,IntParNames);UnfixAll = 0;}
   void DeleteBody();
   void Create(int NPnt,MatterIO *Mat,double Vis,double Vis2,const DataVector<Stroka> &IntParNames);
   ~Body(){DeleteBody();};
   ...
   void SetBoundMU(Body *Fst,Body *Lst)
     {
      ...
      if ( (Fst==NULL)  || (Fst->Mass[1]<=MathZer))
        {BndFst.Free=1;BndFst.Mass=0;BndFst.Velocity=Velocity[1];}
      else 
       {
		      N1=Fst->NumPnt-1;
		      BndFst.Free=0;
      ...
      if ( (Lst==NULL)  || (Lst->Mass[1]<=MathZer)) 
        {BndLst.Free=1;BndLst.Mass=0;BndLst.Velocity=Velocity[NumPnt-1];}
      else 
       {
      		N1=1;
	      	BndLst.Free=0;
      ...
      if ((UilWorkDim!=1) && (Fst==NULL)) 
        {BndFst.Free=0;BndFst.Mass=1e10;BndFst.Velocity=0;}
	     int N=NumPnt;
   	  Mass[0]=BndFst.Mass;Velocity[0]=BndFst.Velocity;
	     Mass[N]=BndLst.Mass;Velocity[N]=BndLst.Velocity;
    };
   ...
   void SetBoundPv(Body *Fst,Body *Lst)
     {
	     int N1;
      if ( (Fst==NULL) || (Fst->Mass[1]<=MathZer)) 
       {BndFst.Pressure=ZeroPressure;BndFst.Pos=2*Pos[1]-Pos[2];}
      else 
       {
		      N1=Fst->NumPnt-1;
      ...
      if ( (Lst==NULL)  || (Lst->Mass[1]<=MathZer)) 
       {BndLst.Pressure=ZeroPressure;BndLst.Pos=2*Pos[NumPnt-1]-Pos[NumPnt-2];}
      else 
       {
      		N1=1;
      ...
      if ((UilWorkDim!=1) && (Fst==NULL)) 
        {BndFst.Pressure=Pressure[1];BndFst.Pos=2*Pos[1]-Pos[2];}
	     int N=NumPnt;
   ...
   void SetBoundU(Body *Fst,Body *Lst)
     {
      if ((Lst!=NULL) && (Lst->Mass[1]>MathZer)) {BndLst.Velocity=Lst->Velocity[1];Velocity[NumPnt]=BndLst.Velocity;}
      if ((Fst!=NULL) && (Fst->Mass[1]>MathZer)) {BndFst.Velocity=Fst->Velocity[Fst->NumPnt-1];Velocity[0]=BndFst.Velocity;}
      if ((UilWorkDim!=1) && (Fst==NULL)) { BndFst.Velocity=0;Velocity[0]=0;}
    };
