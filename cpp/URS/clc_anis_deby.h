...
namespace ClcIonisation {
    ...
    struct MathAnisIon {
        ...
        double MolVeight;
        void Init(
            const vector<double> &ionpot,
            const vector<double> &gfactor,
            const vector<int> &n_num,
            const vector<int> &l_num,
        ...
        vector<double> ClcIonNum() {
            ...
            do {
                ...
                if(!Fzero(
                       ...
                       Err,
                       1000))
                    cout << "Fzero bad - result is x<" << DeltaElDenc
                         << "> y(0)=" << DeltaNumEStatic(DeltaElDenc) << "\n";
                CurPtr = OldPtr;
    ...
    private:
        ...
        void SetClcConst(double denc, double temp) {
            ...
            for(size_t i = 0; i < clcCoef3.size(); i++) {
                clcCoef3[i] =
                    (sqr(N_num[i]) - L_num[i] * (L_num[i] - 1)) / (2 * N_num[i]);
        ...
        void SetIterConst_(double ElNum, double El2Num) {
            ...
            if(CalcMethod == 0) {
                for(size_t i = 0; i < iterPot.size(); i++)
                    iterPot[i] += clcLc1[i] * LambdaI + clcLc2[i] * sqr(LambdaI) +
                                  clcLc3[i] * pow(LambdaI, 3);
            }
            iterElFree = Electron->ClcFreeE(iterElNum * clcDenc, clcTemp);
            iterElDeriv = Electron->ClcFreeEDeriv(iterElNum * clcDenc, clcTemp);
            cout << " New freeE " << iterElFree / MolVeight << " Denc "
                 << iterElNum * clcDenc * MolVeight << " Temp " << clcTemp;   //<<"\n";
        }
        ...
        double ClcLogRelat(int i1, int i2) {
            ...
            double res = (clcLc0[i1] - clcLc0[i2]) +
                         (i1 - i2) * (iterElFree + iterElNum * clcDenc * iterElDeriv);
            ...
            if(i1 != i2)
                cout << " Delta "
                     << (i1 - i2) * (iterElFree + iterElNum * clcDenc * iterElDeriv) /
                            M_Rconst
                     << " IonLev " << -IonPot[i2] / M_Rconst << "\n";
            if(CalcMethod < 2) {
                res += (-1.5 * sqrt(iterEl2Num) * (clcCoef1 + pow(clcCoef2, 3)) -
                        0.5 * clcCoef2 * Lc1sum / sqrt(iterEl2Num) -
                        Lc2sum * sqr(clcCoef2)) *
                       IonNumMul;
                res +=
                    (clcLc1[i1] - clcLc1[i2]) * sqrt(iterEl2Num) * clcCoef2 +
