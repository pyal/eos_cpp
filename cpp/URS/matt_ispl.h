...
private:
    ...
    static double ClcSpl(
        ISplineCalculator *spl,
        double x,
        double y,
        const char *err_mes = NULL) {
        vector<double> xV(2);
        double z;
        xV[0] = x;
        xV[1] = y;
    ...
    static void ClcSplV(
        ...
        if(!(err = spl->Evaluate(dat, resV)) && err_mes)
            throw info_except(
                "Could not evaluate spline for x = %s\n y = %s\n res = %i\nDescription : %s",
                ~DataManip::SaveTextVector(dat[0]),
                ~DataManip::SaveTextVector(dat[1]),
                err,
                err_mes);
...
public:
    ...
    double Sound(double Dencity, double Energy) {
        ...
        double Pp = Pressure(r + dr, Energy + P * dr / (r * r));
        Pp -= P;
        if(Pp < 0)
            Pp = 0.1;
    ...
    static double MisE(double E) {
        if(Current->Tspl == NULL)
            return 0;
        double res =
            ClcSpl(Current->Tspl, Current->E2FT_Converter->Decode(E), Current->St_Denc);
    ...
    double Energy(double Dencity, double Temperature) {
        ...
        if(!Fzero(MisE, E2FTmin, E2FTmax, E0, E2FTeps, E2FTeps, 1000)) {
            cout << " Bad Fzero    in MatterSpl::Energy Not Zero\nfound Encode(E) " << E0
                 << " E0 " << E2FT_Converter->Decode(E0) << " Mis " << MisE(E0) << "\n";
        }
        Current = Old;
    ...
    void ReadSplFile() {
        ...
        if((!sp) || (seek_eof(sp)))
            throw info_except(
                " There is no such MatterSpline file named %s\n", SplFile.c_str());
        ...
        sp >> tmp >> SoundStep >> tmp >> E2FTmin >> tmp >> E2FTmax >> tmp >> E2FTzero >>
            tmp >> E2FTeps;
    ...
    void SaveSplFile() {
        ...
        out << " SoundStep " << SoundStep << " E2FTmin " << E2FTmin << " E2FTmax "
            << E2FTmax << " E2FTzero " << E2FTzero << " E2FTeps " << E2FTeps
            << SavableClass::EOLN();
    ...
    TMatterSpl &SetParams(
        const Stroka &splFile,
        const Stroka &splineDesciption,
        ISplineCalculator *pspl,
        ISplineCalculator *tspl,
        double dencCold = 1e-6,
        double dencMax = 100,
        IFunctionConverter *e2FT_Converter = new TExpConverter(1, 1, 150),
        double soundStep = StndErr * 10,
        double e2FTmin = 0,
        double e2FTmax = 100,
        double e2FTzero = 5,
        double e2FTeps = StndErr * 0.01) {
