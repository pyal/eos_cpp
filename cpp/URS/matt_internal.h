...
struct Matter2State:MatterIO {
   ...
   virtual Stroka MakeHelp(){
       ...
       res += Stroka(" InputName: TransTime - is positive - Matter is in 1 state waiting time abs(TimeToWait)\n"); 
       res += Stroka("            TransTime - is negative - Matter is in 2 state waiting time abs(TimeToWait)\n");
       return res;
   }
   ...
   void ClcArray(double *P,double *Dencity,double *Energy,int Num, const Stroka &What);

   virtual int save_data_state( FilterOut&output){
      output<<"Pres_ElseDenc "<<Pres_ElseDenc<<" Pressure1_2 "<<MinP<<" TimeToWait "<<TimeToWait<<" Substances \n";
	  output<<"   "; SavableClass::Save(output,M1);
	  output<<"   "; SavableClass::Save(output,M2);
	  return 1;
   };
    ...
    void CheckInput(){
	 ...
	 virtual void ChangeInputVal(double Dencity,double Energy,double Time){ 
        ...
        if (InputValStorage["TransTime"]>=0)
            if (Val>MinP) InputValStorage["TransTime"]+=Time;
...
struct MatterSum:MatterIO {
    ...
    void ClcDenc(double Dencity,double Energy,double alpha, double &d1,double &d2)
			  {d1=alpha*(1+VolProp12)/(alpha+1);d2=(1+VolProp12)/(VolProp12*(alpha+1));}
    double Pressure(double Dencity,double Energy);
    double Sound(double Dencity,double Energy);
    double Temperature(double Dencity,double Energy);
...
struct Matter2StateProb:MatterIO {
    ...
    virtual Stroka MakeHelp(){
        Stroka res = MatterIO::MakeHelp();
        res += Stroka(" InputName: TransTime - is positive - Matter is in 1 state waiting time Prob...\n");
        res += Stroka("            TransTime - is negative - Matter is in 2 state waiting time Prob...\n");
        return res;
    }
    double Pressure(double Dencity,double Energy);
    double Sound(double Dencity,double Energy);
    ...
    virtual int save_data_state( FilterOut&output){
        output<<" TransDenc "<<TransDenc<<" Prob0 "<<Prob0<<" Prob1 "<<Prob1<<" Substances \n";
	    output<<"   "; SavableClass::Save(output,M1);
	    output<<"   "; SavableClass::Save(output,M2);
	    return 1;
    };
