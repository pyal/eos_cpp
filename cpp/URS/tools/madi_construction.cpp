...
struct MakeConstruction {
    ...
    MakeConstruction(const char *name) {
        ReadParam(name);
    };
    ...
    struct Result : SavableClass {
        ...
        static void Clc(
            ...
            res->U_P = DataVector<double, CopyStructFast<double>>::MakeStepVector(
                Discr, Ufr, Uto, 1);
    ...
    struct Clc {
        ...
        void GetPar(double u, double &pinter, double &rinter) {
            ...
            cout << " Dinter " << Dinter << " dU " << dU << " Unew " << u << " Uold "
                 << Uinter << "\n";
    ...
    static Clc clc;
    static double MisP(double u) {
        return clc.MisP(u);
    }
    static double PressureMat(double u) {
        return clc.PressureMat(u);
    }
    static double PressureInter(double u) {
        return clc.PressureInter(u);
    }
    void OutputRes() {
...
double MakeConstruction::Result::Ufr, MakeConstruction::Result::Uto,
    MakeConstruction::Result::Discr;
