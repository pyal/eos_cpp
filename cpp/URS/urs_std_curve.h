...
public:
    IsentropeClass(MatterIO *mat) : Mat(mat), OldPtr(CurPtr) {
        CurPtr = this;
    };   //
    void SetNewMat(MatterIO *mat) {
        Mat = mat;
    };
    virtual ~IsentropeClass() {
        Finish();
    };
    void Finish() {
        CurPtr = OldPtr;
...
private:
    ...
    static double IsoentropeDeriv(double d, double e) {
        return CurPtr->Mat->Pressure(d, e) / sqr(d);
    }
    // Calculating dU
...
struct HugoniotClass {
    HugoniotClass(MatterIO *mat) : Mat(mat), Pcritical(-0.01e5), NumHugIteration(10) {}
    struct HugoniotData {
...
private:
    void SetHugPresClcVar(
        const HugoniotData &beg_pnt,
        double res_pres,
        double &dv,
        double &v1,
        double &MinV,
