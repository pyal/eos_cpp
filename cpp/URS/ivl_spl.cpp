...
void BlackBox(map<Stroka, Stroka> par) {
    ...
    Ref<IBlackBoxBase> blackBox = SavableClass::TestType<IBlackBoxBase>(
        objVector[0], "Have to define BlackBox class");
    blackBox->DoIt();
}
void Show(map<Stroka, Stroka> par) {
    ...
    for(size_t i = 0; i < points.size(); i++)
        pnt[atoi(~points[i])] =
            File::open(~(par["Out"] + "." + points[i]), "w", "can not open out file");
...
int main(int argc, const char *argv[]) {
    ...
    try {
        ...
        Cmd.Add(
            BlackBox,
            "blackbox",
            "Run black box object ",
            "ConfigFile par.cfg File format [ BlackBox BlackBoxObject ]");
        Cmd.Add(
            Show,
            "show",
            "get given points for all times from the gasd dat file;Can define TimeName - string with time definition(default is CurTime)",
            "In STDIN input(dat) file\nOut STDOUT base output name (point name is added)\nPnt 1:2:3 points to get");
