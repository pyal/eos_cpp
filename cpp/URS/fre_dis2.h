...
namespace ClcDissociation {
    ...
    struct NamedMatter : RefCount {
        ...
        NamedMatter(const NamedMatter &mat) {
            Init(mat.FreeMat, mat.MolWeight, mat.Name.c_str(), mat.StartProportion, 
                mat.StartVolumeCoef, mat.SubstMixtures, mat.DeriveConst);
        }
        void Init(Ref<InterfaceFreeEIO> freemat, double molweight, const char *name,
            double startproportion, double startvolumecoef, double substmixtures, double derivconst){
            MolWeight = molweight;
            FreeMat = freemat;
            Dencity = MathZer;
            Temperature = -1;
	    ...
	    void Save(ostream &out) {
		    out<<" Name "<<Name.c_str()<<" MolWeight "<<MolWeight<<" StartProportion "<<StartProportion;
            out<<" StartVolumeCoef "<<StartVolumeCoef<<" SubstMixtures "<<SubstMixtures<<" DeriveConst "<<DeriveConst;
            out<<" MatterFreeE ";
    ...
    private:
        ...
        void CheckParams(double denc, double temp){
            ...
            //    return ;
            if (fabs(denc - Dencity) < 0.001 * DeriveConst * Dencity && fabs(temp - Temperature) < 0.1 * StndErr)
                return ;
            Dencity = denc;
            Temperature = temp;
    ...
    struct ChemEquation : RefCount {
	    ChemEquation() {
            SubstNames.push_back("Mat1");SubstNames.push_back("Mat2");
            //SubstNums.push_back(0);SubstNums.push_back(1);
            SubstCoefs.push_back(1);SubstCoefs.push_back(1);
        };
	    ChemEquation(const ChemEquation &eq) : SubstNames(eq.SubstNames), SubstCoefs(eq.SubstCoefs), NumExt2NumInt(eq.NumExt2NumInt){};
	    int  Read(istream &in) {
...
//cout<<"WAU mas "<<mas<<" NumCoefs "<<NumCoefs<<" MolWeights "<<MolWeights;
      ...
      //              break;
                ...
                if (Name2Int.Str2Num(sbst.GetName().c_str(), 1) != Substs.size())
                    throw info_except("Duplicate? name <%s> found\n", sbst.GetName().c_str());
      ...
      //              break;
        ...
        void Save(ostream &out) {
            ...
            out<<" } "<<SavableClass::EOLN()<<" EnterListOfVectors_Vector_SubsName_SubsCoef { ";
            SavableClass::IncIdent();
...
cout<<"Shifting AddPressure...\n";
    ...
    struct Equilibrium2NumberBase : EquilibriumBase {
        ...
        int save_data_state(FilterOut &so){
            ...
            SavableClass::IncIdent();
            for (size_t i = 0; i < 2; i++) 
                so<<SavableClass::EOLN()<<" Matter "<<Names[i] << " ClasterSize "<<ClasterSizes[i];
            SavableClass::DecIdent();
            so<<EOLN()<<"}"<<SavableClass::EOLN();
            if (!so)
                throw info_except("Could not save class\n");
        ...
        Stroka MakeHelp(){
            Stroka res = "Class for finding numbers of dis atoms of 2 matters : Temp, MeanDenc, VolCoefs are given...";
            return res;
        }
...
// To be called after FindEquilib
        vector<double> ClcFreeEMixt(MatterCalculator *mats) {
            ...
            vector<double> fMix(2);// = (VecCl((int)mats->Substs.size()) * 0).Copy2Vector();

            for (size_t iClast = 0; iClast < NumInt2NumExt.size(); iClast++) {
                int iMat = NumInt2NumExt[iClast];
                fMix[iMat] = nums[iMat] * mats->MolWeights[iMat] * kT * log(vols[iMat]/v0) / ClasterSizes[iClast];
            }
...
//        }
        ...
        void SetMatterNumbers(double logX) {
            ...
            ChemEquation &eq = Matters->ChemEqs[0];
            vector<double> numC = (VecCl((int)Matters->MolWeights.size()) * 0).Copy2Vector();
    ...
    struct Equilibrium2Number : Equilibrium2NumberBase {
        ...
        virtual void FindEquilib(MatterCalculator *mats) {
		    ...
		    StaticPtr = this;
            if (4<(err = Fzero(FindMisfitLogXStatic, clcCage[0], clcCage[1], logXguess, ErrorAbs, ErrorRel, MaxIter))) {
                cout<<" Bad FZero Equilibrium2Number \nfound X "
                    <<logXguess<<" Mis "<<FindMisfitLogX(logXguess)<<" err "<<err<<"\n";
            }
            StaticPtr = OldPtr;
...
// UUUU
            ...
            for (int iClast = 0; iClast < 2; iClast++) {
                ...
                double miu = Matters->MolWeights[iMat];
                double clastRT = kT / ClasterSizes[iClast];
                mis += eq.GetSubstCoef(iClast) * (clastRT + pres[iMat] * miu / dencs[iMat] - 
                    clastRT * vols[iMat] * N0 / (V0 * Matters->NumCoefs[iMat]) + fE[iMat] * miu + clastRT * log(vols[iMat]/V0));
            }
            return mis;
        }
...
//            int MaxIter = 100;
		    ...
		    StaticPtr = this;
            logXguess = Fmin(FindMisfitLogXStatic, clcCage[0], clcCage[1], logXguess, ErrorAbs, resFreeE);
            StaticPtr = OldPtr;
            SetMatterNumbers(logXguess);
            LastLogX = logXguess;
        }
    ...
    struct FreeEDisStdIO : FreeEIOFind {
	    ...
	    virtual double OutputValClc(const map<Stroka, double> &Input, const Stroka &Output){
		    ...
		    if (CheckPresence(OutputNames, Output, ptr) && ptr!=NULL){
                ...
                int k1 = MattersClc->Name2Int.Str2Num(Stroka(Output,0,Output.length()-2).c_str(), 0);
			    int k2 = MattersClc->Name2Int.Str2Num(Stroka(Output,0,Output.length()-3).c_str(), 0);
        ...
        Stroka MakeHelp(){
            ...
            res += "Additionaly this class can calculate for MatName component: MatName_M (mass)  MatName_D (dencity) MatName_V (volumeCoefs) MatName_P (pressure) MatName_Fp (FreeE of  pure component) MatName_Fc (FreeE corrected(entropy?+-) of component)  \n";
            res += "Calculate dissosiation of matters - adding to the free E of pure mat - entropy mixture addition. Pure free E calc under assumption that relat denc of matters are the same. \n";
            return res;
        }
    protected:
...
cout<<" Zer lgX "<<NumMinim.GetLastLogX()<<" val "<<NumMinim.ClcFreeE(MattersClc)<<"\n";
            //PresMinim.FindEquilib(MattersClc);
            NumMinimMin.SetGuess(NumMinim.GetLastLogX());
            NumMinimMin.FindEquilib(MattersClc);
cout<<" Min lgX "<<NumMinimMin.GetLastLogX()<<" val "<<NumMinimMin.ClcFreeE(MattersClc)<<"\n";
        ...
        double FreeEvar(double Dencity, double Temperature) {
            ...
            while (VolCoefMisfit < VecCl::ModMax(numCoefsOld - numCoefs) && numIter++ < maxNumIter);
            if (numIter >= maxNumIter) {
                cout<<" Bad result, numCoefsMisfit "<<VecCl::ModMax(numCoefsOld - numCoefs)<<"\n";
...
//cout<<VecCl::ModMax(volCoefsOld - volCoefs)<<"\nvC "<<volCoefs<<" vCold "<<volCoefsOld<<" numC "<< numCoefs<<" numCold "<<numCoefsOld;
            ...
            while (VolCoefMisfit < VecCl::ModMax(volCoefsOld - volCoefs) && numIter++ < 30);
            if (numIter >= 100) {
                cout<<" Bad result, volCoefsMisfit "<<VecCl::ModMax(volCoefsOld - volCoefs)<<"\n";
