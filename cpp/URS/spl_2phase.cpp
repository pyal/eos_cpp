...
void SetDatP_DT(TData<double> &dat,MatterIO *Mat,double AddT,double AddD,int GetPnt)
  {
   ...
   if (GetPnt) { Matt_spl::ReadData(NameBin,DatBin);strcpy(NameBin,"tmp_bin1.dat"); }
   else 
     {
      double IntPar;
      Matt_spl::SetIntParInit(IntPar,Mat);
      DataRegister("TColumn",DatBin);*DatBin=dat;
      int k,Num=DatBin->I[0];
      for (k=0;k<Num;k++) 
        {
         DatBin->D[0][k]-=AddT;DatBin->D[1][k]-=AddD;
...
//cout<<" n1 "<<k<<" l "<<Num<<" ";
   ...
   dat=*DatBin;delete DatBin;
   fstream outB(NameBin,ios::out);DataStore(outB,&dat);outB.close();
  };

struct BinodalPnt
{
  BinodalPnt(int Num) 
    {BinL=new double[Num+1];BinR=new double[Num+1];SpinL=new double[Num+1];
     SpinR=new double[Num+1];Temp=new double[Num+1];Pres=new double[Num+1];
     BinRE=new double[Num+1];dPdT=new double[Num+1];
     Tot=Num;
    };
  ~BinodalPnt()
    {delete BinL;delete BinR;delete SpinL;delete SpinR;delete Temp;delete Pres;
     delete BinRE;delete dPdT;}
...
{ Matt_spl::StdOut()<<" MakeSpline Control "<<err_name<<"\n";
  ...
  { double tmp=(x[k]+x[k+1])/2;Matt_spl::StdOut()<<x[k]<<" "<<y[k]<<" "<<spl.Evaluate(x[k])<<"\n"<<
...
for (int k=0;k<NumX-1;k++) Matt_spl::StdOut()<<x[k]<<" "<<y[k]<<"\n";
  Matt_spl::StdOut()<<"\n"<<flush;
...
double ZeroFunc_Misf,ZeroFunc_Spl_Vmin,ZeroFunc_Spl_Vmax,ZeroFunc_SplRes_Bmin,ZeroFunc_SplRes_Bmax;
...
//IntegralFunc_PV_Int(Smin-Ds);IntegralFunc_PV_Int(Smin);
  Fzero(IntegralFunc_PV_Int,Smin-Ds,Smin,Vmin,ZeroFunc_Misf*0.1,ZeroFunc_Misf*0.1,100);
if (ZeroFunc_ErrorM) cout<<"\n\nFor Pres "<<pres<<" \nMinV "<<Vmin<<" Misf "<<IntegralFunc_PV_Int(Vmin)<<"\n"<<flush;
//IntegralFunc_PV_Int(Smax);IntegralFunc_PV_Int(Smax+Ds);
  Fzero(IntegralFunc_PV_Int,Smax,Smax+10000*Ds,Vmax,ZeroFunc_Misf*0.1,ZeroFunc_Misf*0.1,100);
if (ZeroFunc_ErrorM) cout<<"MaxV "<<Vmax<<" Misf "<<IntegralFunc_PV_Int(Vmax)<<"\n"<<flush;
...
cout<<" T "<<Global_Binodal_Temp<<" MisPmin "<<MisPmin<<" MisPmax "<<MisPmax<<"\n";

return Mat->FreeE(1/Vmin,Global_Binodal_Temp)-Mat->FreeE(1/Vmax,Global_Binodal_Temp)+
        pres*(Vmin-Vmax);
//  return Qags_IntSubInt(IntegralFunc_PV_Int,Vmin,Vmax,ZeroFunc_Misf,
//                        ZeroFunc_Misf,100,ResErr,Err);//-PresInt;
}
void GenerateAllBnd(double *Pres,double *Denc,int NumD,double T,BinodalPnt &BinArr,int BinInd, 
                    int NumSplD,double Misf,int From,int To,MatterIO *Mat)
{
ZeroFunc_ErrorM=0;
  ...
  int k;
  double *Vol=new double[NumD];for (k=0;k<NumD;k++) Vol[k]=1/Denc[k];
...
void FindSpinEst(double *Pres,double *Denc,int NumD,double T,BinodalPnt &BinArr,int BinInd,
                 int &Good, int NumSplD,double Misf,MatterIO *Mat)
{
...
//cout<<k<<" "<<Pres[k]<<" "<<Denc[k]<<"\n";
    if (Pres[k]<Pres[k-1]) 
      {if (!FoundReg) {FoundReg=1;BinArr.SpinL[BinInd]=Denc[k-1];From=k-1;}
//       else if (!Cont) 
//        {cout<<" Error in GenerateTwoPhaseBnd FindSpinEst two phases for one density found.";
//       cout<<"Exiting.";FoundReg=0;break;}//exit(0);}
       Cont=1;
      }
    else { if ((FoundReg) && (Cont) ) {BinArr.SpinR[BinInd]=Denc[k];To=k;};Cont=0;}
  }
//cout.flush();
//  if ( (FoundReg) && (Cont) ) 
...
//TColumn<double> tmp_dat;tmp_dat=dat;DataStore(StdOut,&tmp_dat);StdOut.flush();
     FindSpinEst(&dat.D[2][Cur],dat.D[1],NumD,dat.D[Tind][k-1],BinArr,k,Cont,NumSplD,Misf,Mat);
     if ( (FoundReg) && (!Cont) ) { BinArr.To=k-1;Good=1;break;}
     if ( (Cont) && (!FoundReg) ) { BinArr.From=k;FoundReg=1;}
     Cur+=NumD;
    }
  if (!Good) 
    { 
     cout<<"GenerateTwoPhaseBnd FormBinodalPnt warning "; 
     if (!FoundReg) { cout<<" Two Pahse region not found. Exiting.";return 0;}//exit(0);}
...
//  double Emin=BinArr.BinRE[I0],Emax=BinArr.BinRE[I1];
  BinArr.Tmax=Tmax;BinArr.Tmin=Tmin;
  BinArr.Dmax=Dmax;BinArr.Dmin=Dmin;
...
//  MakeSpline(Bnd.Dl_T,PntSpl,Misf,&BinArr.Temp[Beg],&BinArr.BinL[Beg],Num,"Dl_T",1);
  MakeSpline(Bnd.P_T,PntSpl,Misf*0.0001,&BinArr.Pres[Beg],&BinArr.Temp[Beg],Num,"P_T",1);
  MakeSpline(Bnd.Dr_T,PntSpl,Misf,&BinArr.BinR[Beg],&BinArr.Temp[Beg],Num,"Dr_T",1);
  MakeSpline(Bnd.Dl_T,PntSpl,Misf,&BinArr.BinL[Beg],&BinArr.Temp[Beg],Num,"Dl_T",1);
  MakeSpline(Bnd.E_T,PntSpl,Misf,&BinArr.BinRE[Beg],&BinArr.Temp[Beg],Num,"E_T",1);
  MakeSpline(Bnd.dPdT_T,PntSpl,Misf*1e-6,&BinArr.dPdT[Beg],&BinArr.Temp[Beg],Num,"dPdT_T",1);

  MakeSpline(Bnd.DSl_T,PntSpl,Misf,&BinArr.SpinL[Beg],&BinArr.Temp[Beg],Num,"DSl_T",1);
  MakeSpline(Bnd.DSr_T,PntSpl,Misf,&BinArr.SpinR[Beg],&BinArr.Temp[Beg],Num,"DSr_T",1);
...
int GenerateTwoPhaseBnd(istream &row,MatterIO *Mat)
  {
   ...
   row>>tmp>>res_name>>tmp>>NumSplD>>tmp>>NumSplT>>tmp>>Misf>>tmp>>AddT>>tmp>>AddD>>tmp>>GetPnt;
...
double AddT_Static,AddD_Static,Misf_Static,dPdT_Static,FoundC_Static,
       FoundA_Static,Dmelt_Static;
int NumSplD_Static;
...
//   double Cnew=2./3*(-DeltaP/(Bus->Rconst*Tmelt*1e-3*Dmelt)-2*Anew);
   double Cnew=2./3*(-DeltaP/(Bus->Rconst*Tmelt*1e-3*Dmelt)-6./5*Anew*Melt->PowAm);
   FoundC_Static=Cnew;

   Melt->Am=Anew;Melt->Bm=Cnew;Melt->Cm=Cnew;
...
cout<<" AIter "<<Anew<<" dPdT got "<<BinArr.dPdT[1]<<" RightBnd "<<BinArr.BinR[1]<<"\n";

   TTable<double> dat1=*dat_Static;
   Tmelt=dat1.D[0][0];double dT=0.01*Tmelt;
   for (int i=0;i<dat1.I[0];i++) dat1.D[0][i]=Tmelt+dT;
...
{
    FreeEBushman *Bus=(FreeEBushman *)Mat_Static->FreeEPtr.pointer();
    FreeM_SigmaMT *Melt=dynamic_cast<FreeM_SigmaMT *>((TwoVarFunction*)Bus->FreeM.pointer());
//   FreeM_SigmaMT *Melt=(FreeM_SigmaMT *)Bus->FreeM;
   Bus->Denc0m=NewD;
   double A=Melt->Am;
...
void FindBushCoefABC(istream &row,MatterIO  *Mat_In,double dPdT,
                     double SeekFrom,double SeekTo,double StpShow)
{
...
cout<<" FindBushCoefABC Pres "<<Mat_Static->Pressure(Dmelt,Emelt)<<"\n";
   ...
   dat.I[0]=1;dat.D[0][0]=Tmelt;dat.I[2]=dat.I[1];
   SetDatP_DT(dat,Mat, AddT, AddD,0);
   AddT_Static=AddT;AddD_Static=AddD;Misf_Static=Misf;
   NumSplD_Static=NumSplD;
   dat_Static=&dat;
   dPdT_Static=dPdT;

   double A=( dynamic_cast<FreeM_SigmaMT *>((TwoVarFunction*)((FreeEBushman *)Mat_Static->FreeEPtr.pointer())->FreeM ) )->Am;
//   double A=( (FreeM_SigmaMT *)((FreeEBushman *)Mat_Static->FreeEPtr)->FreeM )->Am;
   A=(SeekFrom+SeekTo)/2;
   Fzero(dPdT_Difference,SeekFrom,SeekTo,A,Misf_Static*0.1,Misf_Static*0.1,100);
   FoundA_Static=A;
fstream Ftmp(res_name,ios::out);
for (double cur=SeekFrom;cur<=SeekTo;cur+=StpShow) Ftmp<<cur<<FMT<<dPdT_Difference(cur)+dPdT_Static<<"\n";
...
{
  LiquidSolidBndDat(){Tot=0;}
  LiquidSolidBndDat(int Num) {GenerateStruct(Num);}
  void GenerateStruct(int Num)
    {T=new double[Num+1];Esol=new double[Num+1];Eliq=new double[Num+1];
     Psol=new double[Num+1];Pliq=new double[Num+1];D=new double[Num+1];
     Tot=Num;
    };
  ~LiquidSolidBndDat(){DeleteStruct();}
  void DeleteStruct()
    { if (Tot>0)
     { delete T;delete Esol;delete Eliq;delete Pliq;delete Psol;delete D;
       T=NULL;Esol=NULL;Eliq=NULL;Pliq=NULL;Psol=NULL;D=NULL;Tot=0;}
    };
  void SetDim(int Num){DeleteStruct();GenerateStruct(Num);};
...
int FormSolLiqBound(TData<double> &dat,LiquidSolidBndDat &LiqSolBnd,MatterIO *MatSol,
                    MatterIO *MatLiq,double AddT,double AddD,int GetPnt,double Misf)
{
...
// First - Temperature, Second - Dencity
   ...
   for (int k=0;k<NumD;k++)
    {
     ...
     if (3<Fzero(DeltaFreeEnergy_Static,Tlow,Tup,ResT,Misf*0.1,Misf*0.1,500))
   ...
   LiqSolBnd.Dmin=dat.D[Dind][0];LiqSolBnd.Dmax=dat.D[Dind][NumD-1];
   return 1;
};
...
void SaveSolLiqBound(char *res_name,LiquidSolidBndDat &LiqSolBnd,int PntSpl,double Misf)
{
	MatterLiqSol::LiquidSolidBnd Bnd;
  int Num=LiqSolBnd.Tot;
  MakeSpline(Bnd.Eliq_D,PntSpl,Misf,&LiqSolBnd.Eliq[0],&LiqSolBnd.D[0],Num,"Eliq_D",1);
  MakeSpline(Bnd.Esol_D,PntSpl,Misf,&LiqSolBnd.Esol[0],&LiqSolBnd.D[0],Num,"Esol_D",1);
  MakeSpline(Bnd.Pliq_D,PntSpl,Misf,&LiqSolBnd.Pliq[0],&LiqSolBnd.D[0],Num,"Pliq_D",1);
  MakeSpline(Bnd.Psol_D,PntSpl,Misf,&LiqSolBnd.Psol[0],&LiqSolBnd.D[0],Num,"Psol_D",1);
...
int GenerateSolLiqPhaseBnd(istream &row,MatterIO *MatSol)
  {
   ...
   if ( !(!(MatLiq << SavableClass::Read(row))) ) { cout<<"Could not register MatLiq. Exit.\n";exit(0);}
   row>>tmp>>res_name>>tmp>>NumSplD>>tmp>>NumSplT>>tmp>>Misf>>tmp>>AddT>>tmp>>AddD>>tmp>>GetPnt;
   if (!ReadTableRow(row,(TData<real>*)&dat)) { cout<<" Could not read param.";exit(0);}
