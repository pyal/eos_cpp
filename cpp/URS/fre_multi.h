...
struct FreeESumMatter:FreeEIOFind{
    ...
    virtual ~FreeESumMatter(){EraseMat();}
    void EraseMat(){
        //for (size_t k=0;k<FreeVect.size();k++) 
        //    delete FreeVect[k];
        FreeVect.clear();FreeCoef.clear();
    }
    double FreeE(double Denc,double T) {
        double ret=0;
        for (size_t k=0;k<FreeVect.size();k++) 
            if (fabs(FreeCoef[k])>MathZer) ret+=FreeVect[k]->FreeE(Denc,T)*FreeCoef[k];
        return ret;
    }
...
//cout<<" FreeESumMatter::ReadSubstancePar start read matter \n";
        do{
            ...
            if (!(f<<SavableClass::Read(in)))
                throw info_except("Could not convert matter n=%i to InterfaceFreeEIO\n",FreeCoef.size());
...
struct SingleDiss_MinimFunc:RefCount{
    ...
    SingleDiss_MinimFunc(const VecCl &veights)
        :Veights(veights){
            if (Veights.Dim()!=2)
                throw info_except(" ReactCoef_DissMinimFunc -> works only in 2 dims. Trying to set init dim=%i\n",Veights.Dim());
    };
    double FindDissFstDenc(const VecCl &FreeE_NoMixt, double Denc, double Temp, double Guess=0){
//cout<<"FreeEStat "<<FreeE_NoMixt;
        ...
        ChemCoef *= pow(DencA,Veights[2]-Veights[1])*pow(Veights[1],-Veights[2])*pow(Veights[2],Veights[1]);
...
//cout<<" par "<<par<<" x "<<x<<" res "<<ret<<"\n";
	...
	static double GetMixtureFreeE(const VecCl &FreeE_NoMixt, double Denc, double Temp, const VecCl &Veights, double DissFstDenc){
		double R1 = DissFstDenc, R2 = Denc - DissFstDenc, R0 = Denc;
		double ret = ((FreeE_NoMixt[1]+GetMixtureCorrection(R1,Temp,Veights[1]))*R1+
					  (FreeE_NoMixt[2]+GetMixtureCorrection(R2,Temp,Veights[2]))*R2)/R0;
...
//Used by RelatVolumeMinimFunc
   	virtual VecCl ChemMulti_FindPressures(double Denc, double Temp, const VecCl &RelatNumbers, const VecCl &RelatVolumesCoefs)=0;
...
struct RelatVolumeMinimFunc:RefCount{
    ...
    RelatVolumeMinimFunc(ChemMultiMinimBase *freeclc, double denc, double temp,
    ...
    void Execute(VecCl &par, VecCl &res){
        ...
        par_ = Decode(par_);
        res = FreeClc->ChemMulti_FindPressures(Denc, Temp, RelatNumbers, par_)+ToAddPressure;
        res = EncodeRes(res);
...
// They are not used...
	...
	VecCl ClcFreeEStatic(double Denc, double Temp, const VecCl &RelatNumbers, const VecCl &RelatVolumesCoefs){
		VecCl SubstDenc = ClcSubstDenc(Denc,  RelatNumbers, RelatVolumesCoefs);
		...
		for(int k=1;k<=SubstVeights.Dim();k++){
			...
			ret[k] -= SingleDiss_MinimFunc::GetMixtureCorrection(SubstDenc[k],Temp,SubstVeights[k]);
		}
		return ret;			
	}
	VecCl ClcFreeEStatic_Test(double Denc, double Temp, const VecCl &RelatNumbers, const VecCl &RelatVolumesCoefs){
		VecCl SubstDenc = ClcSubstDenc(Denc,  RelatNumbers, RelatVolumesCoefs);
		VecCl MassCoef = ClcMassCoef(RelatNumbers);
		VecCl ret = SubstVeights*0;
		for(int k=1;k<=SubstVeights.Dim();k++){
...
//cout<<" difference "<<test-DissDenc;
		return MatrDiss_MinimFunc::GetMixtureFreeE(FeS, Denc, Temp, SubstVeights, DissDenc);
    };
    //double ToAddPressure;
...
if (Mist>1e-5){
    ...
    cout<<"Relat pres "<<ChemMulti_FindPressures(Denc, Temp, RelatNumbers, RelatVolumesCoefs);
    cout<<"Relat volume	"<<RelatVolumesCoefs;
    cout<<"Relat Numbers "<<RelatNumbers;
    cout<<"	 LastFreeE	"<<resFreeE<<" : "<<ClcFreeEStatic_Test(Denc, Temp,	RelatNumbers, RelatVolumesCoefs)<<"\n";
}
#endif
		ResultRelatMasses = ClcMassCoef(RelatNumbers);
        ResultVolumesCoefs = RelatVolumesCoefs/RelatVolumesCoefs[1];
        ResultPressures = ChemMulti_FindPressures(Denc, Temp, RelatNumbers, RelatVolumesCoefs);
        return resFreeE;
    }
...
//#endif
    ...
    void ReadSubstancePar(istream &in) {
		char tmp[256];
		in>>tmp;FreeMinim <<SavableClass::Read(in);
		in>>tmp>>tmp;
		NamedSubst sbst;
		SubstMap.clear();
		ChemEqs.clear();
...
struct FreeEChemMatterDiss:FreeEIOFind{
	...
	//	ClcStartParams();
	//}
    FreeEChemMatterDiss():FreeEIOFind("material.cfg","MatterChemH2Diss"),
        FinalState(new FreeEIdeal()),StartState(new FreeEIdeal()),FinalCoef(new OneVarFormula()){};
    double FreeE(double Denc,double T) {
...
//cout<<Denc<<" "<<coef<<"\n";
    ...
    void ReadSubstancePar(istream &in) {
        char tmp[256];
        in>>tmp;StartState << SavableClass::Read(in);
        in>>tmp;FinalState << SavableClass::Read(in);
        in>>tmp;FinalCoef  << SavableClass::Read(in);
        if (!FinalState || !in || !FinalCoef || !StartState)
            throw info_except("Could not read final state or final coef\n");
    }
    
    void SaveSubstancePar(ostream &out) {
        out<<" StartState ";SavableClass::Save(out,StartState);
        out<<" FinalState ";SavableClass::Save(out,FinalState);
        out<<" FinalCoef "; SavableClass::Save(out,FinalCoef);
    }
    Stroka MakeHelp(){
