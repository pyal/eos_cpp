...
public:
    ...
    void ReadSubstancePar(istream &in){
        char tmp[256];
        in>>tmp>>NumElPerAtom>>tmp>>MolVeight>>tmp>>Gfactor>>tmp>>ElMass_ElseNuclear>>tmp>>OutBoltzman>>tmp>>ColdCurve>>tmp>>ColdCurveTempCor;
    }
    void SaveSubstancePar(ostream &out) {
        out<<" NumElPerAtom "<<NumElPerAtom<<" MolVeight "<<MolVeight<<" Gfactor "<<Gfactor<<" ElMass_ElseNuclear "<<ElMass_ElseNuclear<<" OutBoltzman "<<OutBoltzman<<" ColdCurve "<<ColdCurve<<" ColdCurveTempCor "<<ColdCurveTempCor;
    }
    virtual Stroka MakeHelp() {
        return Stroka("EOS of gas of electrons in solid body. Strict formulas. Can calculate FermiTemp of e-. Also calculate FreeE of electrons(or atoms - if !ElMass_ElseNuclear) in the Boltzman assumption(take into account that we put Z electrons - as free - not one.... Gfactor have to be 2 - for electrons or what ever for atoms. If we set ColdCurve - we calculate FreeE at zero temperature. Always for electrons set Gfactor=2 - or will be errors. ColdCurveTempCor = 1 - we calculate low temperature corrections to the free energy ..\n") + FreeEIOFind::MakeHelp();
    };
    double FreeE(double Denc,double T){
        ...
        return kt*(FermiTemp-8/(3*sqrt(M_PI)*neLambda)*SpecFunc::FermiDiracInt(SpecFunc::Ip1_5, FermiTemp));
    }
	...
	virtual double OutputValClc(const map<Stroka,double> &Input, const Stroka &Output){
        ...
        if (Output==Stroka("BoltzFree")){
            return ClcFreeEBoltzman(CheckPresence(Input, "Dencity"), CheckPresence(Input, "Temperature") );
        }
...
private:
    ...
    double FindFermiInTemp(double FermiInt1_2_Val){
        ...
        if (3<(stat=Fzero(MisFermiInTemp,log(Current_FermiInt_AddVal-50),21,Guess,StndErr*0.001,StndErr*0.001,100000))) 
        
        { 
            double Misf =  MisFermiInTemp(Guess);
            if (fabs(Misf)>StndErr*0.001){
                double res = exp(Guess)-Current_FermiInt_AddVal;
                cout <<" Error, could not find FermiTemp ZeroFzeroIt Temp "<<Test_Temp<<" Denc "<<Test_Denc<<" res "<<res<<" mis "<<Misf<<" Fzero_status "<<stat<<"\n";
            }
       }
        //if (Guess>1000){
...
//cout<<"Ln[x] "<<log_x<<" x "<<x<<" int "<<Int<<" mis "<<ret<<" val "<<Current_FreeEElectronStrict->Current_FermiInt1_2_Val<<"\n";
    ...
    double ClcFreeEBoltzman(double Denc, double T){
        double kt=M_Rconst/MolVeight*T*NumElPerAtom;
        return -kt*(log(1/ClcNeLambda(Denc/2, T))+1);//  Denc/2 - to get right Gfactor for freeE bolt...
    }
    ...
    double ClcNeLambda(double Denc, double T){
        ...
        double LambdaCoef = 2*M_PI*M_PlankCros_K*M_C/sqrt(2*M_PI*Mass*M_eV_K*T);
...
// Tested by freeE
    ...
    double FermiColdLimit(double Denc){
        double C0 = sqrt(2.)*pow(M_Mass_Electron_eV*M_eV_K/sqr(M_C),1.5)/pow(M_PlankCros_K,3)/sqr(M_PI);
...
// in Landau formulas  g=2 is already included
    ...
    double ColdCurveLimit(double Denc){
        double Coef = 3*pow(3*sqr(M_PI),2./3)*sqr(M_PlankCros_K*M_C)/(10*M_Mass_Electron_eV*M_eV_K)*M_eV_kJ/M_eV_K;
        double ret = Coef*M_Na*NumElPerAtom/MolVeight*pow(M_Na*ClcNe(Denc),2./3);
        return ret;
    }
    double ColdCurveTempCorr(double Denc, double T){
        double Coef = M_Rconst*M_Mass_Electron_eV*M_eV_K/(sqr(M_PlankCros_K*M_C)*pow(3*M_Na,2./3))*pow(M_PI,2./3)/4;
// in Landau formulas  g=2 is already included
