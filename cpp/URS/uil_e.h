...
struct ExternalEnergyInput : OneVarFunction {
    ...
    void ClcAddition(Region *Reg, double CurTime, double CurStp) {
        double TimeCoef = ClcXint(CurTime + 0.5 * CurStp, MinT, MaxT, TimeMin, TimeMax, TimeDep) * 
        ...
        for(map<int, double>::iterator it = RegNum2Coef.begin(); it != RegNum2Coef.end();it++) {
            ...
            for(size_t i = 1; i <= body->NumPnt - 1; i++)
                ...
                body->Energy[i] += ClcXint(body->Pos[i], MinX, MaxX, SpaceMin, SpaceMax, SpaceDep) * TimeCoef * it->second;
        }
//exit(1);

    }
    inline double ClcXint(double x, double gMin, double gMax, double sMin, double sMax, CurveSpline &gSpl) {
        if (x < sMin || x > sMax)
            return 0;
...
//cout<<x<<" "<<shift<<" "<<spaceI<<" "<<gMin<<" "<<gMax<<" "<<sMin<<" "<<sMax<<"\n";
    ...
    virtual int read_data_state(FilterIn& input){
        char tmp[256];
        input>>tmp>>TimeDepName>>tmp>>SpaceDepName>>tmp>>TimeInt>>tmp>>SpaceInt>>tmp>>SpaceMin>>tmp>>SpaceMax;
        input>>tmp>>TimeMin>>tmp>>TimeMax;
        input>>tmp;
        SavableClass::ExeptionCheck(input, "{", "ExternalEnergyInput - expected begin of matter_coefs map...\n");
        RegNum2Coef.clear();
        ...
        CurveSpline::SplineFromData(~TimeDepName, TimeDep, 1e-7, "Time dependence spline");
        CurveSpline::SplineFromData(~SpaceDepName, SpaceDep, 1e-7, "Space dependence spline");
    ...
    virtual int save_data_state(FilterOut&output){
        output<<" TimeDepName "<<TimeDepName<<" SpaceDepName "<<SpaceDepName
            <<" TimeInt "<<TimeInt<<" SpaceInt "<<SpaceInt<<" SpaceMin "<<SpaceMin<<" SpaceMax "<<SpaceMax
    ...
    Stroka MakeHelp(){
        Stroka res = "Clc Energy input for time. TimeInt - given in number of beam 300Mev particles (10^9) gives energy of helium (3.326kJ). So for helium RegNum2Coef = 1, TimeInt - number of particles, for other matters - RegNum2Coef - define relative to helium absorption, SpaceInt * Int(x) - have to give 1 for the point of interest...\n";
