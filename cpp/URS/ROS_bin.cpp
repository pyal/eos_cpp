...
void ClcYHs(MixtureComposition &mixt,double *y,double &niu)
 {
  double o3=1./3;
  double niu1,niu2,alpha;mixt.get_Niu(niu1,niu2);alpha=mixt.get_Alpha();
...
//     niu21_=pow(arg2,o3);
  ...
  y[2]=(niu1*niu2+niu21_*(niu1-2*niu2)+niu12_*(niu2-2*niu1))/sqr(niu);
  y[3]=pow(pow(alpha*sqr(niu1),o3)+pow((1-alpha)*sqr(niu2),o3),3)/sqr(niu);
 }
// for unity mass
...
//  return kt*
  return  (1.5*(1+y[1]-y[2]-y[3])+(3*y[2]+2*y[3])/(1-niu)+(y[3]-1)*log(1-niu)+
               1.5*(1-y[1]-y[2]-y[3]/3)/sqr(1-niu)-3);
 }
...
double CorBinMinFunc(double *niu_get,int n_nop)
  {
   double niu[2];niu[0]=niu_get[0];niu[1]=niu_get[1];
niu[0]=exp(niu[0]);niu[1]=exp(niu[1]);
   if ((niu[0]<0) || (niu[1]<0) ||(niu[0]+niu[1]>=1-StndErr))  return (fabs(niu[0])+fabs(niu[1])+1)*1e6;
   MixtureComposition *mixt=BinRoss6Min->mixt;mixt->set_Niu(niu[0],niu[1]);
...
//   SmoothLimitUp=StndErr;SmoothLimitLo=StndErr*2;
   ...
   if (alpha>sqr(StndErr))   Diameter1=pow(6*niu[0]/(BinRoss6Min->S_DensDia*alpha*M_PI*C),1./3);//*2.238515;// To obtain S_Diameter in Bor radius
   if (1-alpha>sqr(StndErr)) Diameter2=pow(6*niu[1]/(BinRoss6Min->S_DensDia*(1-alpha)*M_PI*C),1./3);//*2.238515;// To obtain S_Diameter in Bor radius
   if (Diameter1>sqr(StndErr))  Int1=IntExp6Correlation(mixt->Exp6[0],niu[0],Diameter1);
...
void SetLargeNiuStep(double *niu) {niu[0]=-5;niu[1]=-5;}
...
//MinFindNDimClass
  ...
  Ref<MultiDimMinimizer> Minimizer=new EigenVectMinim(D2,min1d,1e-8,1e-15,FuncError,2);
  ...
  TmpPar[1]=niu[0];TmpPar[2]=niu[1];
  TmpStp[1]=min(fabs(niu[0]-low[0]),fabs(niu[0]-up[0]));TmpStp[1]=0.5;
  TmpStp[2]=min(fabs(niu[1]-low[1]),fabs(niu[1]-up[1]));TmpStp[2]=0.5;
  int Cont=1,MaxIt=5,NumT=MaxIt,k;
  if (LargeNiuStep(niu)) minFunc->CorCoef(0.4);
  else minFunc->CorCoef(0.02);
...
//  {SetLargeNiuStep(niu);ClcMinNiu_New(niu,low,up,MinE,FuncAccur);return;}
  if ( (LargeNiuStep(niu))) 
  {niu[0]=TmpPar[1]*0.948340978;niu[1]=TmpPar[2]*1.0579653;ClcMinNiu_New(niu,low,up,MinE,FuncAccur);return;}
//  {niu[0]=TmpPar[1]*1.0579653;niu[1]=TmpPar[2]*0.952340978;ClcMinNiu_New(niu,low,up,MinE,FuncAccur);return;}

  if ((k>0) )
  {
    minFunc->CorCoef(0.0);
    TmpStp=TmpStp*0+0.2;Cont=1;
...
//      for (k=1;((k<=NumT)&&(Cont));k++) Cont=MinNd.Execute(TmpPar,TmpStp,-1,MinE);
  ...
  niu[0]=TmpPar[1];niu[1]=TmpPar[2];
//cout<<" NumIter "<<k<<" niu "<<niu[0]<<" "<<niu[1]<<" MinE "<<MinE<<"\n";
}
void ClcMinNiu(double *niu,double *low,double *up,double &MinE,double FuncAccur)
{
  double ResFuncVal;
   FixedPar=niu[1];MinimizePar=0;niu[0]=Fmin(MinimizeFunc,low[0],up[0],niu[0],1e-2,ResFuncVal);
   FixedPar=niu[0];MinimizePar=1;niu[1]=Fmin(MinimizeFunc,low[1],up[1],niu[1],1e-3,ResFuncVal);
   FixedPar=niu[1];MinimizePar=0;niu[0]=Fmin(MinimizeFunc,low[0],up[0],niu[0],1e-4,ResFuncVal);
   FixedPar=niu[0];MinimizePar=1;niu[1]=Fmin(MinimizeFunc,low[1],up[1],niu[1],1e-4,ResFuncVal);
   int err;
   double ResErr=sqrt(FuncAccur),MaxFun=2,MaxIter=2;
   double stp[2];stp[0]=stp[1]=FuncAccur*10;
   ResErr=0.0002;
   NashMin_Mach_Epsilon=FuncAccur;
   MinE=UncMin(err,2,&niu[0],&niu[0],MinE,
               CorBinMinFunc,&stp[0],NULL,
               FuncAccur,ResErr,MaxFun,MaxIter,
               low,up,10,0.25 );

}
int CheckInLim(MixtureComposition &mixt,double Denc,double T,double AccurDA,double AccurT)
{
//return 0;
   if ( (fabs(mixt.L_Denc-Denc)<AccurDA*Denc) && (fabs(mixt.L_T-T)<AccurT) 
         && (fabs(mixt.L_alpha-mixt.alpha)<AccurDA) ) return 1;
   return 0;
}
double FreeBinaryCor(MixtureComposition &mixt,double Denc,double T)
{
   if (mixt.ClcInteractPart==0) return 0;
   if (CheckInLim(mixt,Denc,T,0.001*StndErr,0.5)) return mixt.L_FreeE;
   ...
   double niu[2],low[2],up[2];low[0]=low[1]=1e-5;//stp[2]
