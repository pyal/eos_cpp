...
static ClassDesc Matter2State_ClassDesc(
    typeid(Matter2State),
    "Matter2State",
    1,
    "Matter_category",
    create<Matter2State>);
static ClassDesc MatterSum_ClassDesc(
    typeid(MatterSum),
    "MatterSum",
    1,
    "Matter_category",
    create<MatterSum>);
static ClassDesc Matter2StateProb_ClassDesc(
    typeid(Matter2StateProb),
    "Matter2StateProb",
    1,
    "Matter_category",
    create<Matter2StateProb>);
...
void Matter2State::ClcArray(
    ...
    for(int k = 1; k <= Num; k++)
        if(InputVectorStorage["TransTime"][k] >= 0) {
            D1.push_back(Dencity[k]);
            E1.push_back(Energy[k]);
        } else {
            D2.push_back(Dencity[k]);
            E2.push_back(Energy[k]);
        }
    P1.resize(D1.size());
    P2.resize(D2.size());
...
void Matter2State::ChangeInputVal(double *Dencity, double *Energy, int Num, double Time) {
    double *P = new double[Num + 1];
    //double a,a0;
    //a0 = InputVectorStorage["TransTime"][1];
    if(Pres_ElseDenc)
        Pressure(P, Dencity, Energy, Num);
    else
        MemoryMove(Dencity, P, sizeof(double) * (Num + 1));
    for(int k = 1; k <= Num; k++) {
        if(InputVectorStorage["TransTime"][k] >= 0)
            if(P[k] > MinP)
                InputVectorStorage["TransTime"][k] += Time;
            else
                InputVectorStorage["TransTime"][k] = M_MinDouble;
        else if(P[k] < MinP)
            InputVectorStorage["TransTime"][k] -= Time;
        else
            InputVectorStorage["TransTime"][k] = -M_MinDouble;
        ...
        if(fabs(InputVectorStorage["TransTime"][k]) > TimeToWait)
            InputVectorStorage["TransTime"][k] =
                MTSign(InputVectorStorage["TransTime"][k]) * (-M_MinDouble);
...
void MatterSum::ChangeInputVal(double *Dencity, double *Energy, int Num, double Time) {
    double *P = new double[Num + 1];
    if(Pres_Denc)
        Pressure(P, Dencity, Energy, Num);
    else
...
void MatterSum::Pressure(double *P, double *Dencity, double *Energy, int Num) {
    ...
    for(k = 1; k <= Num; k++) {
        ...
        if(alpha == 0)
            ...
            ClcDenc(Dencity[n], Energy[n], alpha, d1, d2);
            P[n] = (P1[n] + alpha * P2[n]) / (1 + alpha);
        }
        n++;
    }
...
double MatterSum::Pressure(double Dencity, double Energy) {
    ...
    return (M1->Pressure(d1, Energy) + alpha * M2->Pressure(d2, Energy)) / (1 + alpha);
};
double MatterSum::Sound(double Dencity, double Energy) {
    ...
    double d1, d2;
    ClcDenc(Dencity, Energy, alpha, d1, d2);
    return (M1->Sound(d1, Energy) + alpha * M2->Sound(d2, Energy)) / (1 + alpha);
};
...
double MatterSum::Temperature(double Dencity, double Energy) {
    ...
    double d1, d2;
    ClcDenc(Dencity, Energy, alpha, d1, d2);
    return (M1->Temperature(d1, Energy) + alpha * M2->Temperature(d2, Energy)) /
           (1 + alpha);
};
...
void Matter2StateProb::ClcArray(
    double *P,
    double *Dencity,
    double *Energy,
    int Num,
    ...
    P1.resize(D1.size());
    P2.resize(D2.size());
    double *p1 = &P1[0], *p2 = &P2[0], *d1 = &D1[0], *d2 = &D2[0], *e1 = &E1[0],
           *e2 = &E2[0];
    p1--;
    p2--;
    e1--;
...
void Matter2StateProb::ChangeInputVal(
    double *Dencity,
    double *Energy,
    int Num,
    double Time) {
    ...
    for(int k = 1; k <= Num; k++) {
        if(InputVectorStorage["TransTime"][k] >= 0)
            if(Dencity[k] > TransDenc)
                InputVectorStorage["TransTime"][k] += Time;
            else
                InputVectorStorage["TransTime"][k] = M_MinDouble;
        else if(Dencity[k] < TransDenc)
            InputVectorStorage["TransTime"][k] -= Time;
        else
            InputVectorStorage["TransTime"][k] = -M_MinDouble;
