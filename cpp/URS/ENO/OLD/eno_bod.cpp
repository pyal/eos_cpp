...
int Body::ReadData(istream &input) {
    ...
    while((!seek_eof(input)) && (EnterStr(input, &d[0], NumR + 1, 1) == 0))
        ;
    if(seek_eof(input)) {
        cout << "RegionIO::ReadData; NoPoints ";
        delete d;
        return 0;
    }
    do {
    ...
    if((i < NumPnt - 1) || (NumPnt == 0)) {
        cout << "Body::ReadData; Entered only part of points NumPnt i " << NumPnt << "   "
             << i << "\n";
        return 0;
    }
    return 1;
...
void Body::SaveData(ostream &output) {
    ...
    while(i < NumPnt - 1) {
        ...
        d[4] = Energy[i];
        if(NumIntPar > 0)
            MemoryMove(IntPar[i], &d[6], sizeof(double) * NumIntPar);
...
#endif
    MemoryMove(Pressure, PresL, sizeof(double) * (NumPnt + 2));
    FstCalcP = 0;
    for(k = 1; k < NumPnt; k++)
        Pressure[k] = max<double>(Pressure[k] * 1e5, 10);
};
...
void Body::Sound(double *S) {
    ...
    if(FstCalcS)
        ...
        if(IterCalcS++ < NumSkipS) {
            MemoryMove(SounL, S, sizeof(double) * (NumPnt + 2));
            return;
        }
        IterCalcS = 0;
...
#endif
    MemoryMove(S, SounL, sizeof(double) * (NumPnt + 2));
    FstCalcS = 0;
};
