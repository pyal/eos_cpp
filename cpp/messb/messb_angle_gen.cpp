...
int RndBaseTimeGenerator::Rnd(double &Rnd_Time) {
    if(CurTime - FinalTime > MathZer)
        return 0;
    Rnd_Time = CurTime;
    double tmp = GetNextTime();
    if(fabs(CurTime - FinalTime) < MathZer)
        CurTime = FinalTime + 1e5;
    else
        CurTime = min(CurTime + tmp, FinalTime);
    return 1;
}
...
void RndAngleGenerator::Rnd(VecCl &Rnd_Angle, VecCl &Rnd_Time) {
    ...
    for(int k = 1; k <= Rnd_Time.Dim(); k++)
        Rnd_Angle[k] = Angle->Rnd();
}


void RndBlumeNumGenerator::Rnd(VecCl &Rnd_Num, VecCl &Rnd_Time) {
    RndBaseTimeGenerator::Rnd(Rnd_Time);
    Rnd_Num.SetDim((int)Ax_vec.size());
    for(int k = 1; k <= (int)Ax_vec.size(); k++)
        Rnd_Num[k] = Ax_vec[k - 1];
    Ax_vec.clear();
}
double RndBlumeNumGenerator::GetNextTime() {
    ...
    while(ret < fabs(FinalTime - StartTime)) {
        ...
        if((NewAx = rnd_iter.Rnd()) != CurAx)
            break;
    }

    if(Ax_vec.size() == Ax_vec.capacity())
        Ax_vec.reserve(Ax_vec.size() * 0.3 + 50);
    Ax_vec.push_back(NewAx);
    return ret;
}
void Rnd2AngleGenerator_Base::LinkAngles(
    vector<VecCl> &Angles,
    VecCl &Angles_Time,
    VecCl &ToAdd,
    VecCl &ToAdd_Time) {
    if(Angles.size() == 0) {
        Angles.push_back(ToAdd);
        Angles_Time = ToAdd_Time;
        return;
    }
    if(ToAdd_Time.Dim() == 0)
        return;
    ...
    while(1) {
        if(pMet.size() == pMet.capacity())
            pMet.reserve(pMet.size() * 0.3 + 50);
        if(fabs(Angles_Time[ang_pos + 1] - ToAdd_Time[add_pos + 1]) < MathZer) {
            ang_pos++;
            add_pos++;
            pMet.push_back(3);
        } else if(Angles_Time[ang_pos + 1] < ToAdd_Time[add_pos + 1]) {
            ang_pos++;
            pMet.push_back(1);
        } else {
            add_pos++;
    ...
    for(int k = 1; k <= N; k++) {
        if(pMet[k - 1] == 3) {
            add_pos++;
            ang_pos++;
            resT[k] = Angles_Time[ang_pos];
        } else if(pMet[k - 1] == 2) {
            add_pos++;
            resT[k] = ToAdd_Time[add_pos];
        } else {
            ang_pos++;
            resT[k] = Angles_Time[ang_pos];
        }
        for(int k1 = 0; k1 < Na; k1++)
            resA[k1][k] = Angles[k1][max<double>(1, ang_pos)];
...
void Rnd2AngleAxGenerator::Rnd(VecCl &Rnd_Teta, VecCl &Rnd_Phi, VecCl &Rnd_Time) {
    ...
    Rnd_Teta = Angles[0];
    for(int k = 1; k <= Rnd_Time.Dim(); k++) {
        Rnd_Phi[k] += PhiAx[Angles[2][k]];
        Rnd_Teta[k] += TetaAx[Angles[2][k]];
    }
}
...
HamTimeGenerator::HamTimeGenerator(HamData *ham, int Num, double Tto) {
    ...
    if(Num <= 0)
        dat_vec.push_back(new HamTimeStepData(ham, Tfrom, Tto));
    ReStartSeq();
}

int HamTimeGenerator::GetNextHam(HamTimeStepData *&stpdat) {
    if(CurPos >= (int)dat_vec.size())
        return 0;
...
void HamTimeAverageGenerator::ReGenerate() {
    ...
    while(AddNewHam(ham, time)) {
        if(dat_vec.size() == dat_vec.capacity())
            dat_vec.reserve(dat_vec.size() * 0.3 + 50);
        dat_vec.push_back(new HamTimeStepData(ham, Tlast, time));
        Tlast = time;
    }
    ReStartSeq();
};
...
void HamTimeAverageRndWalk::ReGenerate() {
    ...
    VecPos = 0;
    if(OutAngles) {
        cout << " HamTimeAverageRndWalk::ReGenerate() - angles\n";
        for(int k = 1; k <= Rnd_Time[0]; k++)
            cout << Rnd_Time[k] << " " << Rnd_Teta[k] << " " << Rnd_Phi[k] << "\n";
    }

    HamTimeAverageGenerator::ReGenerate();
}
int HamTimeAverageRndWalk::AddNewHam(HamData *&ham, double &TimeTo) {
    if(VecPos >= Rnd_Time.Dim() - 1)
        return 0;
    VecPos++;
    TimeTo = Rnd_Time[VecPos + 1];
    double H = Ham->H;
    if(HmulCos)
        Ham->H = H * cos(Rnd_Teta[VecPos] * M_PI / 180);
