...
struct RndBaseTimeGenerator : RefCount {
    ...
    RndBaseTimeGenerator(double starttime, double finaltime)
        : StartTime(starttime), FinalTime(finaltime) {
        Restart();
    };
...
struct RndBlumeNumGenerator : RndBaseTimeGenerator {
    RndBlume rnd_iter;
    RndBlumeNumGenerator(
        VecCl &axProb,
        double timeDisp,
        double starttime,
        double finaltime)
        : RndBaseTimeGenerator(starttime, finaltime), rnd_iter(axProb, axProb) {
        VecCl ParamVal = axProb * 0;
        for(int k = 1; k <= axProb.Dim(); k++)
            ParamVal[k] = k;
        rnd_iter.SetParamVals(ParamVal);
        rnd_iter.GenerateTimes(timeDisp);
        rnd_iter.RndStart();
    }
...
struct Rnd2AngleAxGenerator : Rnd2AngleGenerator_Base {
    ...
    VecCl TetaAx, PhiAx;
    Rnd2AngleAxGenerator(
        RndBlumeNumGenerator *ax,
        Rnd2AngleGenerator_Base *angles,
        VecCl &tetaax,
        VecCl &phiax)
...
struct HamTimeStepData : RefCount {
    Ref<HamData> Ham;
    double Tfrom, Tto;
    HamTimeStepData(HamData *ham, double tfrom, double tto)
        : Ham(ham), Tfrom(tfrom), Tto(tto){};
};
...
struct HamTimeGenerator : RefCount {
    ...
    HamTimeGenerator(HamTimeStepData *dat) {
        dat_vec.clear();
        dat_vec.push_back(dat);
        ReStartSeq();
    }
    ...
    int GetHamNum() {
        return (int)dat_vec.size();
    }
    double GetTotalEnd() {
        return dat_vec[dat_vec.size() - 1]->Tto;
    }
...
struct HamTimeAverageRndWalk : HamTimeAverageGenerator {
    HamTimeAverageRndWalk(
        Rnd2AngleGenerator_Base *ang,
        HamData *ham,
        double gamma,
        int hmulcos,
        int outangles)
...
struct RndBoltsman_GausWalk : RndFunc {
    ...
    RndBoltsman_GausWalk(RndGaus *rndgaus, RndBoltsmanSinPower *rndbolt)
        : rndGaus(rndgaus), rndBolt(rndbolt) {
        CurAngle = rndBolt->Rnd();
    };
    double Rnd() {
        ...
        if(rnd < p1 / max<double>(MathZer, p1 + p2))
            CurAngle -= stp;
        else
            CurAngle += stp;
        return CurAngle;
