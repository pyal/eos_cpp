...
struct FAArrAdd:FAArr
  {
   FAArrAdd(){name=strdup("sum_func");};
   ~FAArrAdd(){DeleteFAArr();};
...
struct FAArrSet:FAArr
  {
   ...
   FAArrSet()
     { name=strdup("set_func");MaxIter=0;CurIter=0;NumFunc=0;
       CorFuncConst0=1;CorFuncExpConst=3;};
   void DeleteFAArrSet();
   ~FAArrSet(){ DeleteFAArrSet();};
...
struct FARoll:FASingleIO
  {
   ...
   char NameInt[30];
   double *XParamPnt,*YParamPnt;
   FARoll():FASingleIO(){delete name;name=strdup("Roll_func");XParamPnt=NULL;YParamPnt=NULL;};
   ~FARoll(){delete XParamPnt;delete YParamPnt;
   XParamPnt=NULL;XParamPnt=NULL;};//FASingleIO::~FASingleIO(); - done automatically !!!
   ...
   virtual void GetInPar(istream &input)
     { input>>NParam       >>NameInt       ;FASingleIO::GetInPar(input);
       InputXYParam(); };
   void InputXYParam(); 
   void DrawHelp(ostream &out){out<<" No help for function "<<name<<" yet\n";}
