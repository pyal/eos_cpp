...
double MeanAngleConstructor_2D_3D::GetMeanCos(double Teta_Max)
{
  ...
  for (int k=2;k<=Phi[0];k++)
    T[k]=T[k-1]+Qags_IntSubInt(IntegralPhiInt,Phi[k],Phi[k-1],Misf,Misf,MaxNumInt,IntResMis,IntErr);

  Phi_T_Spl=new CurveSpline;
  Phi_T_Spl->Generate(NumSpl,Misf,0,&T[1],&Phi[1],Phi[0]);
  //Misf=1e-5;
  double ret=1/T[Phi.Dim()]*
    Qags_IntSubInt(IntegralCosInt,0,T[Phi.Dim()],Misf,Misf,MaxNumInt,IntResMis,IntErr);
  delete Phi_T_Spl;
...
{
  VecCl res(x[0]),cur(x[0]);res=res*0;Res[0]=x[0];
  VecCl tmp=res;
  double Teta_H,Int_teta,H=Ham->H,SumInt=0;
  if (OutPhi) cout<<"   MeanTetaCos Int_Teta TetaCur\n";
  int i=0;
  TetaGen->ResetSerie();
  while (TetaGen->GetNextProb(Teta_H,Int_teta))
  {
    ...
    if (OutPhi) cout<<MeanCos<<" "<<Int_teta<<"\n";
...
MagnonFieldStd::MagnonFieldStd
    (double js_const,double temp,double epow,double minen,double statistic,
     double qcoef1,double qcoef0,double tpow):
    JS_const(js_const),Temp(temp),Epow(epow),MinEn(minen),Statistic(statistic),
    QCoef1(qcoef1),QCoef0(qcoef0),Tpow(tpow)
{
  if (_isnan(QCoef1)) QCoef1=0;
  if (_isnan(QCoef0)) QCoef0=M_Eps;
  if (_isnan(JS_const)) JS_const=1/M_Eps2;
  if (!In_Lim(JS_const,M_Eps2,1./M_MinDouble2)) JS_const=1/M_Eps2;
  if (!In_Lim(QCoef0,M_Eps2,1./M_MinDouble2)) QCoef0=M_Eps2;
  NormalizeCoef=0;Clc(min(GetMaxX(),89));NormalizeCoef=-ExpFactorMax;
...
double MagnonFieldStd::Clc(double x)
{
  ...
  if ( (_isnan(ret)) || (!_finite(ret)) || (ret<0) ) ret=0;
//cout<<ret<<" ";
  return ret;
}
...
{
  if (fabs(x)>90)  return 0;
  double Phi=x*M_PI/180;
  double y0=(1-cos(Phi)),y=pow(y0,Epow);
  if (y<fabs(MinEn)) {ExpFactorMax=Beta()/Temp*QCoef0;return 0;}
...
//  double ret=NormalizeCoef*pow(y,1.5)/(exp(ExpFactor)+Statistic);
  double ret=pow(y,1.5)/(exp(ExpFactor+NormalizeCoef)+Statistic*exp(NormalizeCoef));
...
//  if (y0<fabs(MinEn)) return 0;
  if (y0<fabs(MinEn)) {ExpFactorMax=Beta()/Temp*QCoef0;return 0;}
  double ExpFactor=Beta()/Temp*(y0+QCoef1*sqrt(y0)+QCoef0);
  ExpFactorMax=ExpFactor;
//   if ( r>40) return 0;
//  return NormalizeCoef*y/(exp(ExpFactor)-1);
//  double ret=NormalizeCoef*y0*Phi/(exp(ExpFactor)+Statistic);
  double ret=y0*Phi/(exp(ExpFactor+NormalizeCoef)+Statistic*exp(NormalizeCoef));
  return ret;
}
