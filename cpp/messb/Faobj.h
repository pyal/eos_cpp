...
struct FASingleIO : FAIO {
    Stroka GetParName(int n) {
        if(n < 1 || n > GetNumPar())
            throw info_except(
                " cannot get name of parameter %i for func %s\n ", n, Func->name);
    ...
    virtual double Clc(double *par, double Xval) {
        SetPar(par);
        return Func->Clc(Par, Xval);
    };
    virtual void Clc(double *Xval, double *Res) {
        Func->Clc(Par, Xval, Res);
    };
...
protected:
    ...
    int GetPrefixNum(Stroka &name) {
        ...
        if(n == 0)
            throw info_except(
                " number is not found in %s number is %s func name is %s\n",
                name.c_str(),
                num.c_str(),
                res.c_str());
        name = res;
