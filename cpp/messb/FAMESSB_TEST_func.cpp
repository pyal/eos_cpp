...
#ifndef WCPP_NET
DescribedClass *Dummy_FAZerHQs_Gamma() {
    return new FAZerHQs_Gamma();
}
static ClassDesc FAZerHQs_Gamma_ClassDesc(
    typeid(FAZerHQs_Gamma),
    "HQ_Gamma_poly",
    1,
    "FAZer_Test",
    Dummy_FAZerHQs_Gamma);
#else
static ClassDesc FAZerHQs_Gamma_ClassDesc(
    typeid(FAZerHQs_Gamma),
    "HQ_Gamma_poly",
...
FAZerHQs_Gamma::FAZerHQs_Gamma() {
    ...
    helppar[0] = strdup(
        "Calculation of spectra of poycristal Fe. Main axis - Fix, H//Fix_z, QS - is turned (Gamma,Teta params) ");
...
void FAZerHQs_Gamma::ClcBase(double *par, double *Xval, double *Res) {
    ...
    double H = par[3], Qs = par[4], IS = par[5], W = par[6], Teta = par[7],
           Gamma = par[8], Eta = par[9], Phi = par[10];
    Solver_H_Z_poly(new Ham_Qs_Gamma(HamData(Teta, Phi, H, Qs, Eta, IS, W), Gamma))
        .Solve(Xval, Res);
...
//  =======================================================
#ifndef WCPP_NET
DescribedClass *Dummy_FAZerQsH() {
    return new FAZerQsH();
}
static ClassDesc FAZerQsH_ClassDesc(
    typeid(FAZerQsH),
    "QsH_poly",
    1,
    "FAZer_Test",
...
FAZerQsH::FAZerQsH() {
    ...
    NumPar = 9;
    helppar[0] = strdup(
        "Calculation of spectra of poycristal Fe. Main axis - Fix in Qs basis ( Fix ), H - is turned (H_Teta H_Phi) ");
    ...
    namepar[7] = strdup("  Eta    ");
    valpar[7] = 0;
    helppar[7] = strdup("Qs Eta (xy assimetry) (Vxx-Vyy)/Vzz  ");
    namepar[8] = strdup("  H_Teta ");
    valpar[8] = 0;
    helppar[8] = strdup("Teta Angle between QS_Z and H  Teta rotation around Fix_y ");
    namepar[9] = strdup("  H_Phi  ");
    valpar[9] = 0;
    helppar[9] = strdup(
        "Phi Angle - rotation around QS_Z -so final H - result of two operations - turn by teta around Fix_y  plus turn by Phi around Fix_Z.  ");
};
...
void FAZerQsH::ClcBase(double *par, double *Xval, double *Res) {
    ...
    Ref<HamData> ham =
        new HamHTurned_Z_Qs(Ham_Qs_Eiler(0, 0, H, Qs, Eta, IS, W), H_Teta, H_Phi);
    ...
    ContiniousRelaxationSolver(
        new HamTimeGenerator(ham, 10, 100),
        new ContiniousRelaxationStepMultiSolver(
            new TimeShiftGenerator(10, 20), new ContiniousRelaxationStepSolver()))
        .Solve(Xval, Res);
};
...
#ifndef WCPP_NET
DescribedClass *Dummy_FAZerHQsAver() {
    return new FAZerHQsAver();
}
static ClassDesc FAZerHQsAver_ClassDesc(
    typeid(FAZerHQsAver),
    "hq_aver",
    1,
    "FAZer_Test",
...
FAZerHQsAver::FAZerHQsAver() {
    ...
    delete namepar[3];
    delete helppar[3];
    namepar[3] = strdup("Hexternal ");
    valpar[3] = 500;
    helppar[3] = strdup("External magnetic field");

    namepar[i + 1] = strdup("  I_Hx_c  ");
    valpar[i + 1] = 0;
    helppar[i + 1] = strdup(
        " H magnetic anisotropy tenzor main Hx amplitude, direction coinced with Fixed axis system");
    namepar[i + 2] = strdup("  I_Hy_c  ");
    valpar[i + 2] = 0;
    helppar[i + 2] = strdup(
        " H magnetic anisotropy tenzor main Hy amplitude, direction coinced with Fixed axis system");
    namepar[i + 3] = strdup("  I_Hz_c  ");
    valpar[i + 3] = 0;
    helppar[i + 3] = strdup(
        " H magnetic anisotropy tenzor main Hz amplitude, direction coinced with Fixed axis system");
    namepar[i + 4] = strdup("   Hc     ");
    valpar[i + 4] = 0;
    helppar[i + 4] = strdup(" Internal magnetic field amplitude");
    namepar[i + 5] = strdup(" TetaV_H  ");
    valpar[i + 5] = 0;
    helppar[i + 5] = strdup(" Angle between Hext and viewing direction");
    namepar[i + 6] = strdup(" Add_H    ");
    valpar[i + 6] = 0;
    helppar[i + 6] = strdup(
        "If Add=1 - Hmag on the nucleus is calculated according to the formula: Hext - random direction of external field, Hc - amplitude of internal field, then: Hi[i]=I_H[i]_c  Hi=Hi+Hext  Hmod=sqrt(Hi*Hi)   Hi=Hi*(Hc/Hmod)+Hext - it means that internal magnetic field is turning to the direction of vector Hext+I_H . \t\tIf Add=0 then  Hi[i]=H_I[i]_c*Hext[i]   Hmod=sqrt(Hi*Hi)   Hi=Hi*(Hc/Hmod)+Hext  (defined in FormHSum procedure ). <internal magnetic field is turned to direction  H_I*Hext, where H_I - tensor, Hext - vector>");
    namepar[i + 7] = strdup("FineAver  ");
    valpar[i + 7] = 0;
    helppar[i + 7] = strdup(
        "Number of angles used in summation  is FineAver^2/2 ( aproximately ) (in one sector) ");
    namepar[i + 8] = strdup(" Sum8     ");
    valpar[i + 8] = 0;
    helppar[i + 8] = strdup(
        "Sum8=0 - make random angles only for one sphere sector. Sum8=1 - makes random angles for in all 8 sphere sectors ( 8 times more calculations )");
    NumPar = i + 8;
};
