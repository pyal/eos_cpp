...
static ClassDesc TestCase_Famessb_Static_ClassDesc(
    typeid(TestCase_Famessb_Static),
    "TestCase_Famessb_Static",
    1,
    "TestCase_Famessb_Static_category",
    create<TestCase_Famessb_Static>);
...
struct MinDistStruct {
    ...
    V2D_XY Pnt;
    MinDistStruct(){};
    void Init(VecCl &x, VecCl &y, V2D_XY &pnt) {
        SrcX = x;
        SrcY = y;
        Pnt = pnt;
    }
    double GetDist(VecCl &x, VecCl &y, V2D_XY &pnt) {
        Init(x, y, pnt);
        return GetDist();
    }
    double GetDist() {
        ...
        for(int k = 1; k <= x.Dim() - 1; k++) {
            ...
            double Dist2 = 0, Dist;
            if(dVn2 > MathZer)
                Dist2 = sqr(V2D_XY::VectorMul(V1, V2)) / dVn2;
            if(2 * Dist2 + dVn2 >= V1n2 + V2n2)
                Dist = sqrt(Dist2);
            else
                Dist = sqrt(min(V1n2, V2n2));
            if(Dist < MinDist)
                MinDist = Dist;
        }
        return MinDist;
    }
    double GetLength(VecCl &x, VecCl &y) {
        Init(x, y, V2D_XY(0, 0));
        return GetLength();
    }
    double GetLength() {
...
int TestCase_Famessb_Static::GetHi(string &name, double &Hi, string &Err) {
    ...
    if((!res_f) || (seek_eof(res_f))) {
        Err.append("Results were not generated for test ");
        return 0;
    }
    res_f.close();
    TData<double> *InArr;
    ...
    VecCl x(N), y(N);
    MemoryMove(InArr->D[0], &x.Ptr[1], sizeof(double) * N);
    ...
    for(int k = 1; k <= N; k++) {
        ...
        test += Clc.GetDist(
            x.ExtractSubVector(i, j),
            y.ExtractSubVector(i, j),
            V2D_XY(InArr->D[0][k - 1], InArr->D[2][k - 1]));
    }
    maxY = Clc.GetLength(x, y);
    delete InArr;
    if(num == 0) {
        Err.append("There is no data for test ");
        return 0;
    }
    Hi = test / maxY / num;
    Hi *= 100;
    if(PrintDiscrepencyEstimations)
        cout << " Hi " << Hi << " MaxY " << maxY << " n " << num << "\n";
    return 1;
}
int TestCase_Famessb_Static::Test_External_Func(
    ...
    for(int k = 0; k < Length; k++) {
        string nam_cfg(resources[k].Name);
        nam_cfg.append(".cfg");
        string nam_dat(resources[k].Name);
        nam_dat.append(".dat");
        string nam_res(resources[k].Name);
        nam_res.append(".res");
        string nam_bak(resources[k].Name);
        nam_bak.append(".bak");
        ...
        cout << "                      " << nam_cfg.c_str() << "\n";
        cout.flush();
        system(string("famessb ")
                   .append(nam_cfg)
                   .append(" /o /v>>")
                   .append(TestExeOutput)
                   .c_str());
        double test;
        if(!GetHi(nam_res, test, Err))
            Err.append(resources[k].Name).append("\n");
        else {
            char tmp[256], tmp1[256];
            if(test > resources[k].err) {
                Err.append("Error ")
                    .append(gcvt(test, 7, &tmp[0]))
                    .append(" is too big for test ")
                    .append(resources[k].Name)
                    .append(" It have to be ")
                    .append(gcvt(resources[k].err, 7, &tmp1[0]))
                    .append("\n");
            } else {
                //          cout<<"Test "<<resources[k].Name<<" is OK. Misf "<<test<<" have to be "<<resources[k].err<<"\n";
                if(DeleteGoodResults) {
                    unlink(nam_dat.c_str());
...
void TestCase_Famessb_Static::Text_Func_Out(
    ResourcesStruct *res,
    int ResLen,
    const char *name) {
    string err, ResErr, Message;
    Message.assign(" Test is BAD!!!  : ")
        .append(name)
        .append("\n");   //throw(TestExeption("OK"));
    if(Test_External_Func(res, ResLen, err))
        Message.assign(" Test is OK  : ")
            .append(name)
            .append("\n");   //throw(TestExeption("OK"));
    else
        ResErr.append("Error in ")
            .append(name)
            .append("function: \n    ")
            .append(err)
            .append("\n");
    cout << Message.c_str();
    if(ResErr.length() != 0)
        cout << string("Error in external functions: \n    ")
                    .append(ResErr)
                    .append("\n")
                    .c_str();
};
...
int main(int argc, const char *argv[]) {
    ...
    try {
        CommandLineExe Cmd(
            "Usage: famessb_test [Params_Key_Spec] [/][-]key  \n without parameters - standart test\n");
        ...
        if(Cmd.Get("PrintDiscrepency").Exist)
            TestCase_Famessb_Static::PrintDiscrepencyEstimations = 1;
        if(Cmd.Get("AllResults").Exist)
            TestCase_Famessb_Static::DeleteGoodResults = 0;
