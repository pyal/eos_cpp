...
struct MagnonMeanField:RndIntegral
{
  ...
  virtual double GetMaxOrderedQ(){return 0;};
  virtual double GetMinOrderedQ(){return 0;};
};


struct TetaGenerator:RefCount
{
 TetaGenerator(MagnonMeanField *rndgenerator,int numhtetaint,double normcoef=1, int clcstartparams = 1): RndGenerator(rndgenerator),
                NumHTetaInt(numhtetaint),NormCoef(normcoef)
  {
  ...
  {
    if (CurNum>=NumHTetaInt) {ResetSerie();return 0;}
...
//    Int_Teta=RndGenerator->Clc(Teta_H);
  ...
  double GetDisorderedInt(){return NormCoef*RndGenerator->GetDisorderedInt();}
  void ResetSerie(){CurNum=0;}
  int GetNumInt(){return NumHTetaInt;}
protected:
...
struct TetaGenerator_Paper9:TetaGenerator{
    TetaGenerator_Paper9(MagnonMeanField *rndgenerator,int numhtetaint,double normcoef=1)
        : TetaGenerator(rndgenerator, numhtetaint, normcoef, 0){
        VecCl Q = VecCl::MakeStepVector(NumHTetaInt, RndGenerator->GetMinOrderedQ(), RndGenerator->GetMaxOrderedQ(), 1);
        TetaStp=Q;
        TetaProb=Q;
...
struct MeanAngleConstructor:RefCount
{
  ...
  virtual double GetMeanCos(double Teta_Max){ return cos(M_PI*Teta_Max/180);}
};
...
struct MeanAngleConstructor_2D_3D:MeanAngleConstructor
{
  ...
  MeanAngleConstructor_2D_3D(double frequency,double coef2d_3d):Frequency(frequency),Coef2D_3D(coef2d_3d){};
  static double IntegralPhiInt(double x){return Cur->Frequency/sqrt(fabs(cos(Cur->IntegralAngle)-cos(x)));};
  static double IntegralCosInt(double x){return cos(Cur->Phi_T_Spl->Evaluate(x));}
...
struct TetaAverageSolver:Solver
{
  ...
  TetaAverageSolver(HamData *ham,TetaGenerator *tetagen,MeanAngleConstructor *meanh,
    int outphi):Ham(ham),TetaGen(tetagen),OutPhi(outphi),MeanH(meanh){};
...
struct TetaAverageSolver_Advanced:TetaAverageSolver
{
  ...
  double RelDisordInt;
  TetaAverageSolver_Advanced(HamData *ham,TetaGenerator *tetagen,MeanAngleConstructor *meanh,
    int outphi,HamData *hamdisord,double reldisordint):
    TetaAverageSolver(ham,tetagen,meanh,outphi),HamDisord(hamdisord),
      RelDisordInt(reldisordint){};
  void Solve(double *x,double *Res)
  {
    TetaAverageSolver::Solve(x,Res);Res[0]=x[0];
    VecCl tmp(x[0]),cur(x[0]);tmp=Res;
...
struct FieldBoltsmanSinPower:MagnonMeanField
{
  ...
  virtual double GetExpFactor(){return 0;}
  double Clc(double x){return Bolt.Clc(x);}
  virtual double GetDisorderedInt(){return 0;};
};
struct MagnonFieldStd:MagnonMeanField
{
  MagnonFieldStd(double js_const,double temp,double epow,double minen,double numneib,
                    double qcoef1,double qcoef0,double tpow);
  virtual double GetExpFactor(){return NormalizeCoef;}
  virtual double GetDisorderedInt(){return 0;};
...
//  double GetShiftedX(double x);
  double GetMaxX();
  double Beta(){return JS_const/pow(Temp,2*Tpow);};
...
{
  MagnonFieldOptic(double js_const,double temp,double statistic,double t_pow,
    double q_a0,double q_a1,double e_a0,double e_a1):NumInt2EstMax(100),
    JS_const(js_const),Temp(temp),Statistic(statistic),T_pow(t_pow),
    E_a0(q_a0),E_a1(q_a1),Q_a0(q_a0),Q_a1(q_a1)
  {
    GetMaxX();
  }
  virtual double GetDisorderedInt(){return 0;};
  double Clc(double x){if (!GoodPar(x)) return 0;return Distrib(x);}
  ...
  double q(double x)      {return 0.5*(-Q_a1+q_sqrt(x)); }
  double q_sqrt(double x) {return sqrt(4*(Energy(x)-Q_a0)+sqr(Q_a1));}
  int GoodPar(double x)   {return (Energy(x)-Q_a0<0)? 0:1; }
  double dq(double x)
  {
  ...
  {
    return sqr(q(x))*dq(x)/
      (exp(JS_const/Temp*Energy(x)+ExpFactor)+Statistic*exp(ExpFactor));
  }
  double GetMaxX() // + Set ExpFactor
  {
    ...
    {
      if (!GoodPar(Est[k])) continue;
      double x=Est[k],tmp=log(sqr(q(x))*dq(x))-JS_const/Temp*Energy(x);
      if (tmp>max_val) {max_val=tmp;k_max=k;}
    }
...
{
  MagnonFieldAccustic(double js_const,double temp,double statistic,double t_pow,
    double q_a0,double q_a1,double e_a0,double e_a1,
    double e_max,double q_max,double q_min,double dcosmax):NumInt2EstMax(100),
      JS_const(js_const),Temp(temp),Statistic(statistic),T_pow(t_pow),
      E_a0(e_a0),E_a1(e_a1),Q_a0(q_a0),Q_a1(q_a1),
      E_max(e_max),Q_max(q_max),Q_min(q_min),dCosMax(dcosmax)
  {
    Temp=fabs(Temp);E_max=fabs(E_max);Q_max=fabs(Q_max);Q_min=fabs(Q_min);
    GetMaxX();
  }
  virtual double GetDisorderedInt()
  {
    double Qbnd=q_bound();
    if (Qbnd>Q_max) return 0;
    CurPtr=this;

  //  YNorm=100/RecInt(RndIntegral::xFunc,x_low,x_up,Misf);Misfit=StndErr*0.1;
    int IntErr,IntNumInt=10000;double IntResMis;
    double Misf=1e-15;
    double IntOrdered=Qags_IntSubInt(MagnonFieldAccustic::IntegralFunc,
                                      Q_min,Qbnd,Misf,Misf,IntNumInt,IntResMis,IntErr);
    //double IntDisOrdered=Qags_IntSubInt(MagnonFieldAccustic::IntegralFunc,
    //                                  Qbnd,Q_max,Misf,Misf,IntNumInt,IntResMis,IntErr);
    double IntDisOrdered=Qags_IntSubInt(MagnonFieldAccustic::IntegralFunc,
                                      Qbnd,min(2*Qbnd,Q_max),Misf,Misf,IntNumInt,IntResMis,IntErr)+
                         Qags_IntSubInt(MagnonFieldAccustic::IntegralFunc,
                                        min(Q_max,2*Qbnd),Q_max,Misf,Misf,IntNumInt,IntResMis,IntErr);
...
//cout<<" Q_min "<<Q_min<<" Qbnd "<<Qbnd<<" Q_max "<<Q_max<<" Coef "<<ret<<"\n";
  ...
  double Energy_Q(double x){return sqr(x)+Q_a1*x+Q_a0;}
  double q(double x)      
  {
    double Phi=x*M_PI/180,CosX=cos(Phi);
    double ret=( (1-CosX)+E_a0*pow(1-CosX,E_a1) );
    if (ret<0) cout<<" very bad q<0\n";
    //if (T_pow>0) return fabs(ret)*E_max*pow(Temp,-T_pow);
    return fabs(ret)*E_max*((T_pow>0)?pow(Temp,-T_pow):1);
  }
  double q_bound() {return dCosMax*E_max*((T_pow>0)?pow(Temp,-T_pow):1);}
  int GoodPar(double x)   {return ((q(x)<min(Q_max,q_bound())&& (q(x)>Q_min)))? 1:0; }
  double dq(double x)
  {
    double Phi=x*M_PI/180,CosX=cos(Phi),SinX=fabs(sin(Phi));
    if (E_a1!=0) return SinX*(1+E_a0*E_a1*pow(1-CosX,E_a1-1));
    return SinX;
  }
  double Distrib(double x)
  {
...
//    x=x*E_max*((T_pow>0)?pow(Temp,-T_pow):1);
    return sqr(x)/(exp(JS_const/Temp*Energy_Q(x)+ExpFactor)+Statistic*exp(ExpFactor));
  }
...
{
  MagnonFieldPaper(double js_const,double temp,double statistic,
//      double t_pow,
    double q_a0,double q_a1,
//      double e_a0,double e_a1,
    double Rconst,
    double q_max,double q_min,double dcosmax):NumInt2EstMax(100),
      JS_const(js_const),Temp(temp),Statistic(statistic),
      Q_a0(q_a0),Q_a1(q_a1),
      R(Rconst),Q_max(q_max),Q_min(q_min),dCosMax(dcosmax)
  {
    Temp=fabs(Temp);R=fabs(R);Q_max=fabs(Q_max);Q_min=fabs(Q_min);
    GetMaxX();
  }
  virtual double GetDisorderedInt()
  {
//    return 0;
  ...
  //  YNorm=100/RecInt(RndIntegral::xFunc,x_low,x_up,Misf);Misfit=StndErr*0.1;
    int IntErr,IntNumInt=10000;double IntResMis;
    double Misf=1e-15;
    double IntOrdered=Qags_IntSubInt(MagnonFieldPaper::IntegralFunc,
                                      Q_min,Qbnd,Misf,Misf,IntNumInt,IntResMis,IntErr);
    //double IntDisOrdered=Qags_IntSubInt(MagnonFieldAccustic::IntegralFunc,
    //                                  Qbnd,Q_max,Misf,Misf,IntNumInt,IntResMis,IntErr);
    double IntDisOrdered=Qags_IntSubInt(MagnonFieldPaper::IntegralFunc,
                                      Qbnd,min(2*Qbnd,Q_max),Misf,Misf,IntNumInt,IntResMis,IntErr)+
                         Qags_IntSubInt(MagnonFieldPaper::IntegralFunc,
                                        min(Q_max,2*Qbnd),Q_max,Misf,Misf,IntNumInt,IntResMis,IntErr);
    double Coef=(IntOrdered<M_MinDouble2)?1:IntDisOrdered/IntOrdered;
...
//cout<<" Q_min "<<Q_min<<" Qbnd "<<Qbnd<<" Q_max "<<Q_max<<" Coef "<<ret<<"\n";
  ...
  virtual double q(double x)      
  {
    ...
    if (!Fzero(MagnonFieldPaper::ZeroFunc,From,To,Guess,1e-8,1e-8,1000)) cout<<" Fzero error\n";
    return Guess;
  }
  double FindCos;
...
Coef = 1;
  ...
  virtual double q_bound() {return dCosMax/(R*Temp);}
  ...
  double Denom(double qu)  { return 1./(exp(JS_const*Energy_Q(qu)/Temp+ExpFactor)+Statistic*exp(ExpFactor)); }
  double Distrib(double x) { 
      double qu = q(x); 
...
//      cout<<exp(JS_const*Energy_Q(qu)/Temp+ExpFactor)<<" "<<Statistic*exp(ExpFactor)<<"\n";
  ...
  double StDens_Q(double x){ return sqr(x)*Denom(x); }
  static double IntegralFunc(double x){return CurPtr->StDens_Q(x);} // integrating by q
  static double ZeroFunc(double x){return CurPtr->MisfQ(x);} // integrating by q
  static MagnonFieldPaper* CurPtr;

  int GoodPar(double x)   {double qu = q(x);return ((qu<min(Q_max,q_bound())&& (qu>Q_min)))? 1:0; }
  double GetMaxX(){
...
//Temp = temp;
  ...
  static double IntegralFuncPlus(double x){return CurPtrPlus->ClcIntegralInt(x);}
...
//cout<<q<<" "<<Add+qrt*(AlphaPar-1)-FindCos<<"\n";
  ...
  static double ZeroFuncPlus(double x){return CurPtrPlus->MisfQPlus(x);} // integrating by q
  ...
  virtual double q(double x){
    ...
    if (!Fzero(MagnonFieldPaperPlus::ZeroFuncPlus,From,To,Guess,1e-8,1e-8,1000)) cout<<" Fzero error\n";
    return Guess;
  }
  virtual double dq(double q,double tet){
      ...
      if (1/qrt<10){
          ...
          double up = -sqr(e*qrt)+sqr(a*qrt)*(AlphaPar-1)+e*a*(1-qrt*(qrt*(AlphaPar-2)+AlphaPar));
...
//      return -SinX/der;
  ...
  virtual double GetDisorderedInt(){
    ...
    double Misf=1e-15;
    double IntOrdered=Coef*Qags_IntSubInt(MagnonFieldPaperPlus::IntegralFuncPlus,
                                      Q_min,Qbnd,Misf,Misf,IntNumInt,IntResMis,IntErr);
    return (1-IntOrdered);
  }
...
struct MagnonField_Paper9:MagnonMeanField {
  ...
  MagnonField_Paper9(double temp,double statistic,
    double q_a0,double q_a1, double q_a2, double Rconst,
    double q_max,double q_min,double dcosmax):
      Temp(temp),Statistic(statistic),Q_a0(q_a0),Q_a1(q_a1),Q_a2(q_a2),
      R(Rconst),Q_max(q_max),Q_min(q_min),dCosMax(dcosmax){
    Temp=fabs(Temp);R=fabs(R);Q_max=fabs(Q_max);Q_min=fabs(Q_min);
    ExpFactor = 0;
  }
  virtual double GetDisorderedInt()
  {
    ...
    int IntErr,IntNumInt=10000;double IntResMis;
    double Misf=1e-15;
    double IntFull=Qags_IntSubInt(MagnonField_Paper9::IntegralFunc,
                                      0,Q_max,Misf,Misf,IntNumInt,IntResMis,IntErr);
    double IntDisOrdered=Qags_IntSubInt(MagnonField_Paper9::IntegralFunc,
                                      Qbnd,Q_max,Misf,Misf,IntNumInt,IntResMis,IntErr);
    if (IntFull<MathZer)
        return 1;
  ...
  double Energy_Q(double qu){return Q_a2*sqr(qu)+Q_a1*qu+Q_a0;}
  double Denom(double qu)  { return 1./(exp(Energy_Q(qu)/Temp+ExpFactor)+Statistic*exp(ExpFactor)); }
  ...
  virtual double GetMaxOrderedQ(){return min(Q_max, q_bound());};
  virtual double GetMinOrderedQ(){return Q_min;};




  static MagnonField_Paper9 *CurPtr;
  static double IntegralFunc(double x){return CurPtr->ClcIntegral(x);}
  double ClcIntegral(double q){ return fabs(sqr(q)*Denom(q));}
