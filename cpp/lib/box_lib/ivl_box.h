...
struct MatterConverterSplIO : IBlackBoxBase {
    ...
    int save_data_state(FilterOut &so) {
        so << " IvlParam " << IvlParam << SavableClass::EOLN() << " Pspl ";
        SavableClass::IncIdent();
        Pspl->save_generation_par(so);
        so << SavableClass::mmEOLN() << " Tspl ";
        SavableClass::IncIdent();
        Tspl->save_generation_par(so);
        so << SavableClass::mmEOLN() << " MisfitFindE " << MisfitFindE << " NumEGridPnt "
           << NumEGridPnt << " NumVGridPnt " << NumVGridPnt << " EOSspl " << EOSspl;
        return 1;
    };
    int read_data_state(FilterIn &si) {
        ...
        SavableClass *o1;
        si >> tmp >> o1 >> tmp;
        Pspl->read_generation_par(si);
        si >> tmp;
        Tspl->read_generation_par(si);
    ...
    Stroka MakeHelp() {
        ...
        ret +=
            " Uses 1d splines to find t_e p_t for given v, 2d-splines to construct eos\n";
...
private:
    ...
    void SaveSpline() {
        MatterCaloricSpl save(
            EOSspl, 1 / IvlParam->MaxVol, 1 / IvlParam->MinVol, Pspl, Tspl);
        save.SaveSplFile();
    }
           ...
           err != 4) {
            if(fabs(EnergyMisfitStatic(LastT)) > 10 * MisfitFindE) {
                cout << " Bad FZero FindPres in MatterSplConverter \nfound Temp " << LastT
                     << " Mis " << EnergyMisfitStatic(LastT) << " err " << err << "\n";
                VecCl tT(100), tV(100), tE(100);
                tT = VecCl::MakeStepVector(100, IvlParam->MinT, IvlParam->MaxT, 0);
                tV = tV * 0 + SearchVol;
