...
struct IIvlParametersBase : SavableClass {
    ...
    int save_data_state( FilterOut&so) {
        so << " pTVname " << pTVname << " eTVname " << eTVname << SavableClass::EOLN() <<
            " NumVolPnt " << NumVolPnt << " NumTempPnt " << NumTempPnt << SavableClass::EOLN() <<
...
// To be removed
    ...
    void CreateIvlData(TColumn<double> &datP, TColumn<double> &datE) {
        ...
        std::vector<double> temp = DataManip::MakeStepVector(NumTempPnt, MinT, MaxT, 1, 0);
        std::vector<double> vol = DataManip::MakeStepVector(NumTempPnt, MinVol, MaxVol, 1, 0);
...
struct TIvlParametersSplIO : IIvlParametersBase {
    ...
    int save_data_state( FilterOut&so) {
        so << " SplE_TV ";SavableClass::IncIdent(); SplE_TV->save_generation_par(so);so << SavableClass::mmEOLN() <<
              " SplP_TV ";SavableClass::IncIdent(); SplP_TV->save_generation_par(so);SavableClass::DecIdent(); 
        return 1; 
    };
    int read_data_state(FilterIn&si){ 
        Stroka tmp;
        SplE_TV = new ExtendedRegridSpline();
        SplP_TV = new ExtendedRegridSpline();
        si >> tmp;SplE_TV->read_generation_par(si); 
        si >> tmp; SplP_TV->read_generation_par(si);
        return 1;
    };
...
struct TIvlDataReader : SavableClass {
    ...
    int save_data_state( FilterOut&so) {
        so << "XFile " << XFile << " YFile " << YFile << " ZFile " << ZFile << SavableClass::EOLN();
        return 1;
    }
    int read_data_state(FilterIn&si){ 
        Stroka tmp;
    ...
    void ReadFiles() {
        ...
        if (X.size() * Y.size() != Z.size() || X.size() == 0)
            throw info_except("[%s]X(%i) * [%s]Y(%i) != [%s]Z(%i)\n", ~XFile, X.size(), ~YFile, Y.size(), ~ZFile, Z.size());
    }
...
private:
    ...
    void Append(vector<vector<double> > &z_er, const vector<double> &z, const vector<double> &e, int iVol) {
    ...
    void GenSpline(Ref<ISplineGenerator> spl, vector<double> &x, vector<double> &y, const char *errMess) {
    ...
    void Clc_pt(const vector<double> &e, vector<double> &p, vector<double> &t, int iVol) {
        ...
        GenSpline(T_e_spl, e_t, t_t, ~(Stroka("E_t for volume ") + Stroka::Int2Str(iVol)));
        GenSpline(P_t_spl, t_t, p_t, ~(Stroka("P_t for volume ") + Stroka::Int2Str(iVol)));
        Ref<ISplineCalculator> teClc = T_e_spl->GetCalculator(), ptClc = P_t_spl->GetCalculator();
        for(size_t i = 0; i < e.size(); i++) {
...
public:
    TIvlTableDataReconstruct()
        : P_t_spl(new TEncodedSplineGenerator)
        , T_e_spl(new TEncodedSplineGenerator) {
    };
...
struct TIvlTable2Spl : IBlackBoxBase {
    ...
    virtual void DoIt() {
        ...
        TMatterSpl mat;
        SplDescription += " minD " + Stroka(minP[1]) + " maxD " + Stroka(maxP[1]) + " minT " + Stroka(minT[2]) + " maxT " + Stroka(maxT[2]) + " minE " + Stroka(minT[0]) + " maxE " + Stroka(maxT[0]) + " ";
        mat.SetParams(SplFile, SplDescription, Pspl->GetCalculator(), Tspl->GetCalculator(), minP[1], maxP[1]);
        mat.SaveSplFile();
    }
    int save_data_state( FilterOut&so) {
