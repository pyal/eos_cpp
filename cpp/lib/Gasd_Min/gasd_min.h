...
struct FAZerColdExp6Curve : FAZerBase {
    ...
    void InputInternPar(istream &input) {
        ...
        if(!FreeE)
            throw info_except(" Could not convert matter to freeE\n");
        ...
        if(!(dynamic_cast<FreeERossH2_Minim *>((InterfaceFreeEIO *)Check->FreeVect[0])))
            throw info_except(
                " First matter in FreeESumMatter is not FreeERossH2_Minim\n");
    };
    //FreeERossH2 *Mat;
...
struct FAZerCurveMain : FAZerBase {
    FAZerCurveMain()
        : FAZerBase(), XVarName("MinVar.X"), YVarName("MinVar.Y"), UrsVarName("UrsVar") {
    ...
    void ClcBase(double *par, double *Xval, double *Res) {
        ...
        UrsCurve_VarStorage *stor =
            dynamic_cast<UrsCurve_VarStorage *>((SavableClass *)Curve->GetVar(VarName));
        if(!stor)
            throw info_except(
    ...
    void OutputInternPar(ostream &output) {
        ...
        out << " XVarName " << XVarName << " YVarName " << YVarName << " UrsVarName "
            << UrsVarName << " Curve \n";
    ...
    void InputInternPar(istream &input) {
        ...
        if(!VarConstructor)
            throw info_except(
                "Could not create class named %s (may be not FAZerCurveBase base class?)\n",
                ClassName.c_str());
