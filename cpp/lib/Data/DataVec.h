...
struct CopyStructFast {
    static void CopyArray(T *src, T *dst, int Num) {
        MemoryMove(src, dst, sizeof(T) * Num);
    }
    static void DeleteArray(T *src, int Num) {
        delete[] src;
    }
};
template <class T>
...
public:
    ...
    DataVector<T, CopyStructMethod> &operator=(
        const DataVector<T, CopyStructMethod> &vec);
    DataVector<T, CopyStructMethod> &operator=(T *vec);
    inline int SetDim() const {
        return Dim;
    }
    ...
    friend inline int operator==(
        const DataVector<T, CopyStructMethod> &vec1,
        const DataVector<T, CopyStructMethod> &vec2) {
        if(vec1.Dim != vec2.Dim)
            return 0;
        for(int k = 0; k < vec1.Dim; k++)
            if(vec1[k] != vec2[k])
                return 0;
        return 1;
    }
    ...
    inline T &operator[](int k) {
        ...
        if(k >= SetDim() || k < 0)
            throw info_except(
                "Trying to get element %i for array with dim %i\n", k, SetDim());
#endif
        return data[k];
    };
    ...
    DataVector<T, CopyStructMethod> operator+(
        const DataVector<T, CopyStructMethod> &coef);
    DataVector<T, CopyStructMethod> operator-(
        const DataVector<T, CopyStructMethod> &coef);

    static DataVector<T, CopyStructMethod> MakeStepVector(
        int Num,
        const T &low,
        const T &up,
        int centered);
...
DataVector<T1, CopyStructFast<T1>> &ConvertVectors(
    DataVector<T, CopyStructFast<T>> &src,
    DataVector<T1, CopyStructFast<T1>> &dst) {
    int k, N = src.SetDim();
    dst.SetDim(N);
    for(k = 0; k < N; k++)
        dst[k] = src[k];
...
DataVector<T, CopyStructMethod>::DataVector(int Num, T *Val) {
    ...
    data = new T[Num];
    if(Val != NULL)
        CopyStructMethod::CopyArray(
            Val, data, Dim);   //MemoryMove(Val,data,sizeof(T)*Dim);
}

template <class T, class CopyStructMethod>
DataVector<T, CopyStructMethod>::DataVector(const DataVector<T, CopyStructMethod> &vec)
    : data(NULL), Dim(0) {   //operator=(vec);
    SetDim(vec.Dim);
    CopyStructMethod::CopyArray(
        vec.data, data, vec.Dim);   //MemoryMove(vec.data,this->data,sizeof(T)*vec.Dim);
}
...
template <class T, class CopyStructMethod>
DataVector<T, CopyStructMethod> &DataVector<T, CopyStructMethod>::operator=(
    const T &vec) {
...
DataVector<T, CopyStructMethod> &DataVector<T, CopyStructMethod>::operator=(
    ...
    SetDim(vec.Dim);
    CopyStructMethod::CopyArray(
        vec.data, data, vec.Dim);   //MemoryMove(vec.data,data,sizeof(T)*vec.Dim);
    return *this;
}
...
void DataVector<T, CopyStructMethod>::Resize(int NewDim) {
    ...
    CopyStructMethod::CopyArray(
        old_dat,
        data,
        min(old_dim, NewDim));   //MemoryMove(old_dat,data,sizeof(T)*min(Dim,NewDim));
...
void DataVector<T, CopyStructMethod>::SetData(T *dat, int From, int To) {
    ...
    if(To == -1)
        To = Dim;
    if(To > Dim) {
        cout << "Very bad in DataVector::SetData().\n";
        cout.flush();
        abort();
    }
    CopyStructMethod::CopyArray(
        dat, data, To - From);   //MemoryMove(dat,data,sizeof(T)*(To-From));
}

template <class T, class CopyStructMethod>
DataVector<T, CopyStructMethod>::operator T *() const {
    return data;
}
...
DataVector<T, CopyStructMethod> DataVector<T, CopyStructMethod>::operator/(
    ...
    for(int k = 0; k < ret.Dim; k++)
        ret.data[k] = data[k] / coef;
    return ret;
}
...
*/
template <class T, class CopyStructMethod>
T DataVector<T, CopyStructMethod>::operator*(
    const DataVector<T, CopyStructMethod> &coef) {
    T ret = 0;
    if(Dim != coef.Dim) {
        cerr << "dimensions are different in operator vec1*vec2\nvec1.Dim=" << Dim
             << " vec2.Dim=" << coef.Dim << " return 0 \n";
        return ret;
    }
    for(int k = 0; k < Dim; k++)
        ret = ret + data[k] * coef.data[k];
    return ret;
}
template <class T, class CopyStructMethod>
DataVector<T, CopyStructMethod> DataVector<T, CopyStructMethod>::operator+(
