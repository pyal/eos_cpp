...
static ClassDesc NamedStore_cd(typeid(NamedStore),"NamedStore",1,"",&create_NamedStore_cd);

//ResizeVector<T,CpM>
//DescribedClass* create_NamedStore_cd(){return new NamedStore;}
static ClassDesc ResizeVector_Ref_NamedObject_cd(typeid(ResizeVector<Ref<NamedObject>,
                                                 CopyStructRef<Ref<NamedObject> > >),"ResizeVector_Ref_NamedObject");
...
void NamedStore::StoreChanged()
 { 
  SparceArrayInd* ind_p=GetInd ();int cur=-1; 
  while ((cur=ind_p->Next(cur))>=0) GetPtr(cur)->StoreChanged();
 };
