...
{
    //      DataArray ();
    DataArray(int NumCol = 0, int LenCol = 0) : data(0), columns(0) {
        SetDim(NumCol, LenCol);
    }
    DataArray(const DataArray<T> &vec);
    DataArray(DataVector<int, CopyStructFast<int>> &vec) : data(0), columns(0) {
        SetDim(vec);
    }
    inline virtual ~DataArray() {
        Delete();
    }
    ...
    inline DataVector<int, CopyStructFast<int>> SetDim() const {
        return columns;
    }
    inline DataVector<int, CopyStructFast<int>> SetDim(int NumCol, int LenCol = 0) {
        DataVector<int, CopyStructFast<int>> dim(NumCol);
        dim = LenCol;
        return SetDim(dim);
    };
    DataVector<int, CopyStructFast<int>> SetDim(
        DataVector<int, CopyStructFast<int>> &vec);
    DataVector<int, CopyStructFast<int>> Resize(
        DataVector<int, CopyStructFast<int>> &vec);

    void AddColumn(int ColLen, T *ColVar = NULL);
    template <class CpM_>
    inline void AddColumn(const DataVector<T, CpM_> &Col) {
        AddColumn(Col.SetDim(), Col.Ptr());
    }
    template <class CpM_>
    DataVector<T, CpM_> SetColumn(int Col, int ColLen = -1, T *ColVar = NULL);
    template <class CpM_>
    inline void SetColumn(int Col, DataVector<T, CpM_> &Dat) {
        SetColumn<CpM_>(Col, Dat.SetDim(), (T *)Dat);
    }
    template <class CpM_>
...
DataArray<T> &DataArray<T>::operator=(const T &vec) {
    int row, col;
    for(col = 0; col < columns.SetDim(); col++) {
        for(row = 0; row < columns[col]; row++)
            data[col][row] = vec;
    }
    return (*this);
...
template <class T>
DataVector<int, CopyStructFast<int>> DataArray<T>::SetDim(
    DataVector<int, CopyStructFast<int>> &vec) {
    if(vec == columns)
        return columns;
    Delete();
...
template <class T>
DataVector<int, CopyStructFast<int>> DataArray<T>::Resize(
    DataVector<int, CopyStructFast<int>> &vec) {
    if(vec == columns)
        return columns;
...
void DataArray<T>::AddColumn(int ColLen, T *ColVar) {
    ...
    Resize(NewCol);
    if(ColVar != NULL)
        MemoryMove(ColVar, data[N - 1], sizeof(T) * ColLen);
}
...
DataVector<T, CpM_> DataArray<T>::SetColumn(int Col, int ColLen, T *ColVar) {
    ...
    Resize(NewDim);
    if(ColVar != NULL)
        MemoryMove(ColVar, data[Col], sizeof(T) * ColLen);
    ret.SetDim(columns[Col]);
    ret = data[Col];
    return ret;
}
...
DataVector<T, CpM_> DataArray<T>::SetRow(int Row, T *RowVar) {
    ...
    DataVector<T, CpM_> ret(N);
    ret = (T)0;
    CheckProperties();
    if(!same_length) {
        ExEnv::err() << "SetRow, Arr not same_length. Use SetRowMask. col:" << columns;
        return ret;
    }
    if(RowVar == NULL)
        if((N != 0) && (Row >= N)) {
            ExEnv::err() << "SetRow, Arr rows<RowToGet:" << N << " " << Row << "\n";
            return ret;
        } else {
            for(k = 0; k < N; k++)
                ret[k] = data[k][Row];
        ...
        DataVector<int, CopyStructFast<int>> tmp(N);
        Resize(tmp = Row);
        for(k = N; k < Row; k++)
            SetRow<CpM_>(k, RowVar);
        return ret = RowVar;
    } else {
        for(k = 0; k < N; k++)
            data[k][Row] = RowVar[k];
...
DataArray<T> Transpon(DataArray<T> &ar) {
    ...
    if(!ar.same_length) {
        ExEnv::err() << "Error in transpon; Array not same_length. Its dim: \n" << ColVec;
        return ar;
    }
    DataArray<T> ret(0);
    int col, row, Ncol = ColVec.SetDim();
    if(Ncol == 0)
        return ret;
    int Nrow = ColVec[0];
...
DataVector<T, CpM_> DataArray<T>::operator*(const DataVector<T, CpM_> &coef) {
    CheckProperties();
    if(!same_length) {
        ExEnv::err() << "Error in Ar mul Vec; Array not same_len. Its dim: \n" << columns;
        return coef;
    }
    DataVector<T> ret(0);
    int col, row, Ncol = columns.SetDim();
    ...
    ret = (T)0;
    if(coef.SetDim() != Ncol) {
        ExEnv::err() << "Error in Ar mul Vec; Vec_len!=Ncol." << coef.SetDim() << " "
                     << Ncol;
        return ret;
    }
    for(row = 0; row < Nrow; row++) {
...
DataArray<T> DataArray<T>::operator+(const DataVector<T, CpM_> &coef) {
    CheckProperties();
    if(!square) {
        ExEnv::err() << "Error in Ar plus Vec; Array not square. Its dim: \n" << columns;
        return (*this);
    }
    DataArray<T> ret;
    ret = 0;
    ...
    if(coef.SetDim() != Ncol) {
        ExEnv::err() << "Error in Ar plus Vec; Vec_len!=Ncol." << coef.SetDim() << " "
                     << Ncol;
        return ret;
    }
    for(col = 0; col < Ncol; col++)
        data[col][col] = data[col][col] + coef[col];
    return ret;
}
template <class T>
template <class CpM_>
DataArray<T> DataArray<T>::operator-(const DataVector<T, CpM_> &coef) {
    CheckProperties();
    if(!square) {
        ExEnv::err() << "Error in Ar minus Vec; Array not square. Its dim: \n" << columns;
        return (*this);
    }
    DataArray<T> ret = coef;
    int col, Ncol = columns.SetDim();
    if(coef.SetDim() != Ncol) {
        ExEnv::err() << "Error in Ar minus Vec; Vec_len!=Ncol." << coef.SetDim() << " "
                     << Ncol;
        return ret;
    }
    for(col = 0; col < Ncol; col++)
...
DataArray<T> DataArray<T>::operator*(DataArray<T> &coef) {
    CheckProperties();
    if(!same_length) {
        ExEnv::err() << "Error in Ar1 mul Ar2; Ar1 not same_len. Its dim: \n" << columns;
        return coef;
    }
    coef.CheckProperties();
    if(!coef.same_length) {
        ExEnv::err() << "Error in Ar1 mul Ar2; Ar2 not same_len. Its dim: \n"
                     << coef.SetDim();
        return coef;
    }
    DataArray<T> ret(0);
    int col, row, k, N1col = columns.SetDim();
    if(N1col == 0)
        return ret;
    int N1row = columns[0];
    int N2col = coef.SetDim().SetDim();
    if(N2col == 0)
        return ret;
    int N2row = coef.SetDim()[0];
    if(N2row != N1col) {
        ExEnv::err() << "Error in Ar mul Ar; Ar1_col!=Ar2_row." << N1col << " " << N2row;
        return ret;
    }
...
template <class T>
DataArray<T> DataArray<T>::operator+(DataArray<T> &coef) {
    if(columns != coef.SetDim()) {
        ExEnv::err() << "Error in Ar plus Ar; Array with dif dim. Ar dim, coef dim: \n"
                     << columns << "\n"
                     << coef.SetDim() << "\n";
        return coef;
    }
    ...
    for(col = 0; col < Ncol; col++) {
        for(row = 0; row < Ncol; row++)
            ret(row, col) = data[col][row] + coef(row, col);
    }
    return ret;
};
...
DataArray<T> DataArray<T>::operator-(DataArray<T> &coef) {
    if(columns != coef.SetDim()) {
        ExEnv::err() << "Error in Ar minus Ar; Array with dif dim. Ar dim, coef dim: \n"
                     << columns << "\n"
                     << coef.SetDim() << "\n";
        return coef;
    }
