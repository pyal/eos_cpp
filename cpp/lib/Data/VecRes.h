...
public:
    ...
    virtual ~ResizeVector() {
        Delete();
    };

    void Delete() {
        CpM__::DeleteArray(data, ElementNum);
        data = NULL;
        ElementNum = 0;
        ArrayLen = 0;
    };
    ...
    inline int SetNumEl() const {
        return ElementNum;
    }
    inline int SetNumEl(int NewLen) {
        Resize(NewLen);
        return ElementNum;
    }
    ...
    void SetArr(T *arr, int NumToPut, int Len) {
        Resize(NumToPut + Len);
        CpM__::CopyArray(arr, &data[NumToPut], Len);
    }
...
private:                                        //## implementation
    ...
    void FillUnused() {
        for(int k = ElementNum; k < ArrayLen; k++)
            data[k] = FillElement;
    };
...
ResizeVector<T, CpM__>::ResizeVector(int Start, int AddBy, int Release, T *Fill)
    : ElementNum(0), ArrayLen(0), data(NULL) {
    SetPolicy(Start, AddBy, Release, Fill);
    Resize(0);
};

template <class T, class CpM__>
ResizeVector<T, CpM__> &ResizeVector<T, CpM__>::operator=(
    const ResizeVector<T, CpM__> &vec) {
    if(vec.NotFill)
        SetPolicy(vec.StartNum, vec.AddByNum, vec.ReleasePolicy, NULL);
    else {
        T tmp = vec.FillElement;
        SetPolicy(vec.StartNum, vec.AddByNum, vec.ReleasePolicy, &tmp);
    }                         //(vec.FillElement));}
    Resize(vec.ElementNum);   //MemoryMove(vec.data,data,sizeof(T)*ElementNum);
    CpM__::CopyArray(vec.data, data, ElementNum);
    return *this;
...
template <class T, class CpM__>
ResizeVector<T, CpM__> &ResizeVector<T, CpM__>::operator=(const T &vec) {
...
void ResizeVector<T, CpM__>::Resize(int NewElLen, int Force)   // internal use only
{
    if(Force) {
        ...
        if(data != NULL) {
            CpM__::CopyArray(data, tmp, NewElLen);
            Delete();
        }
        data = tmp;
        ArrayLen = NewElLen;
    ...
    if(StartNum >= NewElLen)
        ...
        if(AddByNum > 0)
            ...
            Addition = ArrayLen * (-AddByNum) / 100.;
        if(NewElLen < ArrayLen) {
    ...
    data = tmp;
    if(!NotFill) {
        for(int k = OldArLen; k < NewArLen; k++)
            data[k] = FillElement;
    }
    ArrayLen = NewArLen;
    ElementNum = NewElLen;
...
int ResizeVector<T, CpM__>::save_data_state(FilterOut &so) {
    ...
    if(!NotFill)
        so << KeyDesc("FillElement") << FillElement;
...
int ResizeVector<T, CpM__>::read_data_state(FilterIn &si) {
    ...
    if(!NotF) {
        ...
        SetPolicy(Start, Add, Pol, NULL);
    si >> StdKey >> NewElNum >> StdKey;

    Resize(NewElNum);

    for(int k = 0; k < NewElNum; k++)
        si >> data[k];   //so<<"\n";
    si >> StdKey;
    return 1;
}
