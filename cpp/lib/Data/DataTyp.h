...
struct BoundTemplate : public SavableClass {
    ...
    BoundTemplate(
        const DataVector<T, CopyStructFast<T>> &low,
        const DataVector<T, CopyStructFast<T>> &up) {
        Low = low;
        Up = up;
    };
    ...
    virtual DataVector<T, CopyStructFast<T>> Size() {
        return Up - Low;
    };
    int save_data_state(FilterOut &so) {
        so << KeyDesc("Low") << Low << KeyDesc("Up") << Up;
        return 1;
    };
    int read_data_state(FilterIn &si) {
        si >> StdKey >> Low >> StdKey >> Up;
        return 1;
    };
    BoundTemplate<T> &operator=(const BoundTemplate<T> &bnd) {
        Low = bnd.Low;
        Up = bnd.Up;
        return *this;
    };
    virtual BoundTemplate<T> Set(const DataVector<T> &low, const DataVector<T> &up) {
        Low = low;
        Up = up;
        return *this;
    };
...
struct IndBound : public BoundTemplate<int> {
    ...
    IndBound(const IndexPoint &low, const IndexPoint &up) {
        Low = low;
        Up = up;
    };
    IndBound(const IndBound &bnd) {
        Low = bnd.Low;
        Up = bnd.Up;
    };
    IndBound &operator=(const IndBound &bnd) {
        Low = bnd.Low;
        Up = bnd.Up;
        return *this;
    };
