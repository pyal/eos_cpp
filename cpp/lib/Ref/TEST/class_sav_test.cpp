...
namespace class_sav_test {
    ...
    void MemTest() {
        ...
        cout << ' ' << dt->nreference() << ' ' << ct->nreference() << ' '
             << bt->nreference() << endl;
        ct->reference();
        cout << "The following three numbers should be equal:" << endl;
        cout << ' ' << dt->nreference() << ' ' << ct->nreference() << ' '
        ...
        FilterTextOut fo(
            DataSource::OpenSource("Test", DataSource::Out, DataSource::Memory));
        //  char tmp[256];
        cout << "entering ...";
        string t;
        for(int o = 1; o <= 1; o++) {
        ...
        char s1[256];
        FilterTextIn fi(
            DataSource::OpenSource("Test", DataSource::In, DataSource::Memory));
        fi >> i1 >> s1 >> d1;
        cout << i1 << " " << s1 << " " << d1 << "\n";
        cout << t << "\n";
    ...
    void RefTst() {
        ...
        const Ref<DescribedClass> descl((aaaa == (AAA *)0) ? (DescribedClass *)0 : aaaa);
    ...
    void LibTest(int storage_type) {
        Ref<AAA> tmpref,
            aref = dynamic_cast<AAA *>(ClassDesc::name_to_class_desc("BBB")->create());
        ...
        fo.SetRefMask(storage_type);
        fo << aref;
        FilterTextIn fi("Test", DataSource::Memory);
        fi.SetRefMask(storage_type);
