...
namespace Str {
    ...
    size_t ReverseSkipSpace(const char *str, size_t pos, int CheckSpace ){
        ...
        int IsGlasn(char ch){
            return (strchr("éóåûàîýÿèþ¸",ch)!=NULL);
        }
        int IsSoGlasn(char ch){
            return (strchr("öêíøùõúôâïðëäæ÷ñìòü",ch)!=NULL);
        }
        ...
        bool IsRussianChar(char ch, int CountSpec){
            if (!CountSpec)
                return (strchr("¸éöóêåíãøùçõúôûâàïðîëäæýÿ÷ñìèòüáþ¨ÉÖÓÊÅÍÃØÙÇÕÚÔÛÂÀÏÐÎËÄÆÝß×ÑÌÈÒÜÁÞ", (unsigned char)ch)!=NULL );
            return (strchr("¸éöóêåíãøùçõúôûâàïðîëäæýÿ÷ñìèòüáþ¨ÉÖÓÊÅÍÃØÙÇÕÚÔÛÂÀÏÐÎËÄÆÝß×ÑÌÈÒÜÁÞ.-", (unsigned char)ch)!=NULL );
        }
