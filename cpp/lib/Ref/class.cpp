...
Ref<DescribedClass> ClassDesc::list_category_obj_next(DescribedClass *prev,const char *categ,int AutoDel){
...
ClassDesc* ClassDesc::list_category_start(const char *categ){
    for (std::map<ClassKey,ClassDescP>::iterator ind=ClassDesc::all_->begin();ind!=ClassDesc::all_->end(); ind++){
        ClassDesc* classdesc = ind->second;
        if ((categ==NULL) || (Stricmp(classdesc->category(),categ)==0)) 
		    return classdesc;
        }
