...
static ClassDesc Stroka_ClassDesc(typeid(Stroka),"Stroka",1,"Std_category",create<Stroka>);


char *Stroka::allocate(size_t size, base *old) {
    if (size == 0)
        return null;
    base *ret = old == &nullbase ? // hack for _mt_
...
Stroka &Stroka::replace(size_t pos, size_t del, const char *pc, size_t pos1, size_t ins, size_t len1) {
    size_t len  = length();
...
istream &Stroka::build(istream &is, bool(*f)(istream&,char*,std::streamsize,char), size_t& total, char delim)
{
    char buf[CHUNK_SIZE];
