...
#  define REF_MANAGED_CODE UINT_MAX
  ...
  public:
    ...
    refcount_t nreference() const { return _reference_count_; }
...
#       endif
    ...
    int managed() const { return ManageRef;}
...
inline int lt(const void*i, const void*j) { return i < j; }
/// Greater than for two void pointers.
inline int gt(const void*i, const void*j) { return i > j; }
/// Less than or equal for two void pointers.
inline int le(const void*i, const void*j) { return i <= j; }
/// Greater than or equal for two void pointers.
inline int ge(const void*i, const void*j) { return i >= j; }
/// Equal for two void pointers.
inline int eq(const void*i, const void*j) { return i == j; }
/// Not equal for two void pointers.
inline int ne(const void*i, const void*j) { return i != j; }
...
//cout<<"Creating Ref:"<<(void*)a.pointer()<<" name "<<(tmp!=NULL)?tmp->class_name():""<<"\n";
    ...
    RefCount *parentpointer() const { return dynamic_cast<RefCount*>(p); }
...
//*/
    ...
    void ref_info(ostream& os) const { RefBase::ref_info(p,os); }
