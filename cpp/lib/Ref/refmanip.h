...
//namespace RefManip {
    ...
    FilterIn &operator>>(FilterIn &in, Ref<T> &obj){
        ...
        obj << o;
        if (!(!o) && !obj)
            throw info_except("Object of wrong type<%s> have to be <%s>\n", typeid(*o).name(), typeid(T).name());
        //if (!in)
    ...
    FilterOut &operator<<(FilterOut &out, const std::list<T> &lst){
        out<<" list_{ ";SavableClass::IncIdent();out<<SavableClass::EOLN();
    ...
    FilterOut &operator<<(FilterOut &out, const std::vector<T> &vec){
        out<<" vector_{ ";SavableClass::IncIdent();out<<SavableClass::EOLN();
    ...
    FilterOut &operator<<(FilterOut &out, const std::list<Ref<T> > &lst){
        out<<" list_{ ";SavableClass::IncIdent();out<<SavableClass::EOLN();
        for(typename std::list<Ref<T> >::const_iterator it = lst.begin();it!=lst.end();it++) {
            out.setobject(*it);
            out<<" ";
        }
        SavableClass::DecIdent();
