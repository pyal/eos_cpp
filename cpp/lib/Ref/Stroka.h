...
#define CODEPAGE_ARGUMENT_DEFAULT
    ...
    static inline void y_deallocate(void *p, size_t) { free(p);}
    static inline void* y_reallocate(void* p, size_t, size_t new_sz){ return realloc(p, new_sz);}
    static inline void *y_allocate(size_t n) { return malloc(n);}
    inline bool atomic_increment(volatile long &p, long v) { return (p += v) != 0; }
    inline bool atomic_increment(volatile unsigned long &p) {return ++p != 0; }
    inline bool atomic_decrement(volatile unsigned long &p) {return --p != 0; }
    inline long atomic_add(volatile long &p, long v) {       return p += v;   }
...
// ~~~ Core functions ~~~
    ...
    std::istream &build(std::istream &is, bool(*f)(std::istream&,char*,std::streamsize,char), size_t&total, char delim='\x00');
...
// ~~~ Assignment ~~~ : FAMILY0(Stroka&, assign);
    Stroka &assign(const Stroka &s)
      { if (s.p != null)
            atomic_increment(s.pdata()->refs); // instead of check of assignment to itself
...
// ~~~ main driver: should be protected (in the future)
    Stroka &replace(size_t pos, size_t n, const char *pc, size_t pos1, size_t n1, size_t len1);
...
// ~~~ Full copy ~~~
    Stroka copy() const;
    void copy(char *pc, size_t n) const  { MemoryMove(p, pc, n); }
...
// ~~~ Comparison ~~~ : FAMILY0(int, compare)
    static STROKA_INLINE int compare(const stroka &s1, const stroka &s2 COMMA CODEPAGE_ARGUMENT_DEFAULT);
    static STROKA_INLINE int compare(const char* p, const stroka &s2 COMMA CODEPAGE_ARGUMENT_DEFAULT);
    static STROKA_INLINE int compare(const stroka &s1, const char* p COMMA CODEPAGE_ARGUMENT_DEFAULT);
