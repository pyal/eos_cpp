...
#define MULTIPLY_COEF 1.3
    ...
    void InitAllocation(size_t size = 0, T *ptr = NULL, int f = 0, size_t st = 100) {
        ...
        DemandedSize = size;
        if(ptr)
            MemoryMove(ptr, Ptr(), size * sizeof(T));
    }
...
public:
    ...
    inline size_t AllocSize() {
        return LastAllocationSize;
    }
    inline T *Ptr() {
    ...
    AllocatorIntellect(size_t size = 0, T *ptr = 0, int f = 0, int st = 100) {
        InitAllocation(size, ptr, f, st);
    };
    AllocatorIntellect(const AllocatorIntellect &alloc) {
        InitAllocation(
            alloc.DemandedSize(), alloc.Ptr(), alloc.Frozen, alloc.StartAllocationSize);
    };
    void Empty() {
        if(!Frozen) {
            delete LastAllocationPtr;
    ...
    T &operator[](size_t k) {
        if(LastAllocationSize < k + 2)
            Allocate(k + 1);
        return LastAllocationPtr[k];
    }
