...
namespace DataManip {
    ...
    template <class T>
    std::vector<std::vector<T>> &CopyVector(
    ...
    std::vector<T> &CopyVector(
        ...
        size_t numpnt) {
        assert(
            iDstBeg >= 0 && iSrcBeg > 0 && iDstBeg + numpnt <= dst.size() &&
            iSrcBeg + numpnt <= src.size());
        if(numpnt == 0)
            return dst;
    ...
    std::vector<double> MakeStepVector(
        int Num,
        double low,
        double up,
        int logscale,
    ...
    template <class T>
    void XYTable2Planar(
        std::vector<std::vector<T>> &planar,
    ...
    void XYPlanar2Table(
        ...
        const std::vector<std::vector<T>> &planar) {
    ...
    template <class T>
    void SetLimits(
        const std::vector<std::vector<T>> &dat,
        std::vector<T> &minV,
        std::vector<T> &maxV) {
