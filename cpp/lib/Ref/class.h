...
public:
    ...
    static Ref<DescribedClass> list_category_obj_next(
        DescribedClass *prev,
        const char *categ,
        int AutoDel = 1);
...
public:
    ...
    const char *class_name() const {
        ...
        if(!cur)
            throw info_except(
                " No class description for the class %s\n", typeid(*this).name());
        return cur->name();
    }
...
template <class T>
inline T require_dynamic_cast(DescribedClass *p, const char *errmsg, ...) {
...
// It is an error for the result to be a null pointer.
template <class T>
inline T require_dynamic_cast(const DescribedClass *p, const char *errmsg, ...) {
    T t = dynamic_cast<T>(p);
    if(p && !t) {
        va_list args;
        va_start(args, errmsg);
