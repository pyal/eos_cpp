...
void LeakTest() {
    ...
    if(CheckLeakStatusSet.empty()) {
        cout << " Bad leak test, double after init?\n";
        return;
    }
    ...
    if(_CrtMemDifference(&s3, CheckLeakStatusSet.top(), &s2)) {
        ...
        cout << " UnAlloc normall blocks " << (unsigned int)s3.lCounts[1]
             << " memory there " << (unsigned int)s3.lSizes[1] << "\n";
        cout.flush();
        _CrtMemDumpStatistics(&s3);
    } else
        cout << " No Memory difference registered. OK\n";
    cout.flush();
...
int LeakTest(std::string &Err) {
    ...
    int ret = 0;
    if(CheckLeakStatusSet.empty()) {
        cout << " Bad leak test, double LeakTest after SetLeakTest?";
        return 1;
    }
    //	if ( _CrtMemDifference( &s3, &CheckLeakStatusSet, &s2 ) )
    ...
    if(_CrtMemDifference(&s3, CheckLeakStatusSet.top(), &s2)) {
        Err = "";
        Err.append(" Memory difference registered\n")
            .append(" UnAlloc normall blocks ")
            .append(itoa((int)s3.lCounts[1], &tmp[0], 10))
            .append(" memory there ")
            .append(itoa((int)s3.lSizes[1], tmp1, 10));
        ret = 1;
    } else
        Err = "OK";
    delete CheckLeakStatusSet.top();
...
int seek_str(istream &in, const char *Str) {
    ...
    while(!seek_eof(in)) {
        in >> tmp;
        if(Stricmp(tmp, Str) == 0)
            return 1;
    }
    return 0;
};

void InputArr(istream &f, double *Ar, int &GetPnt, int &GetCol, int MaxCol, int MaxPnt) {
    double d;
    GetCol = 0;
    GetPnt = 1;
    //   char ch=' ';
...
double RandomClass::Rnd(double MaxVal) {
    ...
    if(rndType == RandomInit::StdRandom) {
        ...
        c = (long double)2111111111.0 * Rnd_Buf.Seed[3] +
            ...
            5115.0 * (Rnd_Buf.Seed[1] = Rnd_Buf.Seed[0]) + Rnd_Buf.Seed[4];
...
#else
#ifdef WCPP
    char tmp[_MAX_PATH], drive[_MAX_DRIVE], dir[_MAX_DIR], name[_MAX_FNAME],
        ext[_MAX_EXT];
...
void SetLength(char *name, int length) {
    int k = 0, k1;
    while((name[k]) && (k < length))
        k++;
    if(k != length)
        for(k1 = k; k1 < length; k1++)
            name[k1] = ' ';
    name[length] = 0;
};
...
void WriteStr(ostream &out, double *ar, int NumCol) {
    ...
    for(int k = 1; k <= NumCol; k++) {
        if(ar[k] == NullNumber)
            ...
            out << setw(15) << setprecision(10) << ar[k] << "  ";
    }
    out << "\n";
};
...
#endif
#endif
    if(!control_existence)
        OpenMethod = ios::out;
    out.open(file_name, OpenMethod);
    if(out.fail()) {
        cout << "File: " << file_name << " already exists. Overwrite[y] ?";
        cout.flush();
        int ch = (char)ChRead();
        cout << (char)ch << "\n";
...
int TestLastError(void *res) {
    if(res == NULL) {
        cout << " Error caught; " << GetLastMessage().c_str() << "\n";
        return 0;
    } else
        return 1;
};
...
int _matherr(struct _exception *except) {
    ...
    cout << "MathErr occured. Error type:" << descr[except->type] << " func name "
         << except->name << " arg1 " << except->arg1 << " arg2 " << except->arg2
