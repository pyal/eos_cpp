...
// This function produces a stack backtrace with demangled function & method names.
    static std::string GetModuleName(const char *frame) {
        ...
        if (std::string::npos == p) return name;
        while(line[p] == ' ') p++;
        size_t p1 = line.find(' ', p);
        name = &line[p];
        if (p1 != std::string::npos) name.resize(p1 - p);
    ...
    static std::string Backtrace(int skip = 1)
    {
        ...
        for (int i = skip; i < nFrames; i++) {
            ...
            if (dladdr(callstack[i], &info) && info.dli_sname) {
                ...
                snprintf(buf, sizeof(buf), "%-3d %s %*p %s + %zd\n",
                         i, module.c_str(), int(2 + sizeof(void*) * 2), callstack[i],
                         status == 0 ? demangled :
