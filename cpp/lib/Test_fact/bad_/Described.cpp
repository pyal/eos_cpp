...
void ClassDesc::list_all_classes(const char *categ) {
    ...
    if(categ != NULL)
        ExEnv::out() << "for category <" << categ << ">" << endl;
    for(std::map<ClassKey, ClassDescP>::iterator ind = all_->begin(); ind != all_->end();
        ind++) {
...
DescribedClass *ClassDesc::list_category_next(
    DescribedClass *prev,
    const char *categ,
    int AutoDel) {
