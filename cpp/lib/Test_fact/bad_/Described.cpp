...
ios& indent(ios&o){o.rdbuf()->sputc(' ');return o;};
ios& node0(ios&o){return o;};
...
ClassDesc::ClassDesc(const type_info &ti, const char* name, int version,const char* category,
                     DescribedClass* (*ctor)()
 //                    DescribedClass* (*keyvalctor)(const Ref<KeyVal>&),
//                     DescribedClass* (*stateinctor)(FilterIn&)
                     )
{
  if (!type_info_all_) type_info_all_ = new std::map<type_info_key, ClassDescP>;
...
void ClassDesc::init(const char* name, int version, const char* category,
                DescribedClass* (*ctor)())
//                DescribedClass* (*keyvalctor)(const Ref<KeyVal>&),
//                DescribedClass* (*stateinctor)(FilterIn&))
{
...
//  stateinctor_ = stateinctor;
  ...
  if (!all_) all_ = new std::map<ClassKey,ClassDescP>;
  ...
  else if (me) temp_copy_present = 1;

  classname_ = strcpy(new char[strlen(name)+1],name);
  if (category) category_ =strcpy(new char[strlen(category)+1],category);
  else {category_=new char[1];category_[0]=0;}
  ...
  if (all_->find(key) != all_->end()) {
      ExEnv::err()
          << node0 << indent
          << "ERROR: ClassDesc ctor: already created "
          << name << endl;
      abort();
		}
...
ClassDesc::~ClassDesc()
{
  ...
  if (all_->size() == 0) { delete all_; all_ = 0; }
  // delete local data
  delete[] classname_;
...
{
  if (type_info_all_->find(type_info_key(&ti))== type_info_all_->end()) return 0;
...
DescribedClass *ClassDesc::list_category_start(const char *categ)
{
...
DescribedClass *ClassDesc::list_category_next(DescribedClass *prev,const char *categ,int AutoDel)
{
  ...
  {
      ClassDesc* classdesc = ind->second;
      if ((!found) && (classdesc!=was)) continue;
	  if (!found) { found=1;continue;}
      if ((categ==NULL) || (Stricmp(classdesc->category(),categ)==0)) 
	  {if (AutoDel) delete prev;return classdesc->create();}
    }
  if (AutoDel) delete prev;
  return NULL;
}
...
/*
ostream &operator <<(ostream&o, const void *ref)
{
  DescribedClass *dc = dynamic_cast<DescribedClass*>(ref.parentpointer());
  if (dc)  dc->print(o);
  else o << indent << "reference to null" << endl;
  return o;
}
*/
