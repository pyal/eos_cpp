...
struct TestSuite:TestBase
{
	...
	static void RunTest(const char *name)
	{
		try
		{
			TestCase *test=dynamic_cast<TestCase*>(ClassDesc::name_to_class_desc_check(name)->create());
			if (!test) throw(info_except(string("Cannot create TestCase from name:").append(name).c_str()));
			TestSuite* suite=test->CreateTest();
			if (!suite) throw(info_except(string("TestCase cannot create TestSuite TestCase name:").append(name).c_str()));
			suite->RunTest();
			delete suite;
			delete test;
		}
...
//	TestSuiteComposite(){TestName=strdup("TestSuiteComposite");}
	...
	{
		Ref<TestCase> test; test << ClassDesc::list_category_obj_start(categ);
		while (test)
		{
			AddTestCase(test);
...
beg:
		try
		{
			for (;it!=tests.end();it++)	(*it)->RunTest();
		}
		CATCHEXCEPTION("Error in TestSuiteComposite ");
		if (it!=tests.end()) {it++;goto beg;}
	}
private:
	void AddTestSuite(TestSuite *test){tests.push_back(test);};
...
struct TestCase_Full:TestCase {	virtual void RunTest()=0;};
struct TestSuite_Simple:TestSuite
{
	TestSuite_Simple(TestCase_Full *t):test(t){};
	~TestSuite_Simple(){delete test;}
	void RunTest(){test->RunTest();}
	TestCase_Full *test;
};
...
T* TestCase_RestorePtr(DescribedClass *ptr)
{
	...
	if (!cur) 
        throw info_except(~(Stroka("ptr is not ") + typeid(T).name() + " type. Its type is " + typeid(*ptr).name()));
	return cur;
}
...
struct TestSuite_Std:TestSuite
{
	TestSuite_Std(const char *classname)
	{
		try
		{
			if (!classname) throw(info_except(string("ClassName is zero").c_str()));
			class_name.append(classname);
			test=dynamic_cast<TestCase_Std*>(ClassDesc::name_to_class_desc(classname)->create());
			if (!test) throw(info_except(string(" Class:").append(classname).append(" cannot be converted to TestCase_Std*").c_str()));
		}
		CATCHEXCEPTION("Error in TestSuite_Std\n");
	};
	~TestSuite_Std(){delete test;}
	void RunTest()
	{
...
//ret:
		for (;it!=lst.end();it++) {
			SetLeakTest();
			try	{
				test->Init();it->Ptr(test);test->Finish();
			}
			CATCHEXCEPTION("Error in class "<<class_name.c_str()<<" function "<<it->Name.c_str()<<"\n");
			LeakTest(err);cout<<"Function "<<it->Name.c_str()<<" memory: "<<err.c_str()<<"\n";
		}
		//catch(t_except &e)
		//{
...
//		if (it!=lst.end()) {test->Finish();it++;goto ret;}
	...
	void AddTest(void (*ptr)(TestCase*),const char *name){lst.push_back(ListStruct(ptr,name));}
	TestCase_Std *test;
	list<ListStruct> lst;
	string class_name;
};
...
struct TestCase_TestTest:TestCase_Std
{
	...
	static TestCase_TestTest* Restore(TestCase *ptr)
	{
		...
		if (!cur) throw(info_except("prt is not TestCase_TestTest"));
		return cur;
	}
...
//	StreamManip_Base *manip;
	static TestCase_Buffered* Restore(DescribedClass *ptr)
	{
		...
		if (!cur) throw(info_except("ptr is not TestCase_Buffered"));
		return cur;
	}
	static void Test1(TestCase *ptr)
	{ 
