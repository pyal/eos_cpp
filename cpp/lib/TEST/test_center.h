...
struct TestCase_TestLibs : TestCase_Std   //TestCase_Buffered
{
    ...
    static TestCase_TestLibs *Restore(DescribedClass *ptr) {
        ...
        if(!cur)
            throw(info_except("ptr is not TestCase_TestLibs"));
        return cur;
    }
    static void TestFilterText(TestCase *ptr) {
        ...
        if(i != 234 || d != 34.6 || Stricmp(str, "AAA") != 0 || Stricmp(ch_str, "r") != 0)
            throw info_except(
                "Error int(234)==%i double(34.6)==%f char('r')==%s string(\"AAA\")==%s",
                i,
                d,
                ch_str,
                str);
    ...
    static void TestMemory(TestCase *ptr) {
        ...
        SetLeakTest();
        if(LeakTest(err))
            throw info_except(" Single Leak test error:%s\n", err.c_str());
        SetLeakTest();
        SetLeakTest();
        if(LeakTest(err))
            throw info_except(" Double Leak test1 error:%s\n", err.c_str());
        if(LeakTest(err))
            throw info_except(" Double Leak test2 error:%s\n", err.c_str());
    ...
    static void TestFilterTextMemory(TestCase *ptr) {
        ...
        if(Stricmp(DataSource::GetStr("aa").c_str(), "bad 5 good ") != NULL)
            throw info_except(
                "Got :%s: and have to get :%s:\n",
                DataSource::GetStr("aa").c_str(),
                "bad 5 good ");
    }
    static void TestCopyVector(TestCase *ptr) {
        ...
        double ufr = 0, uto = 1;
        res = DataVector<double, CopyStructFast<double>>::MakeStepVector(
            discr, ufr, uto, centered);
        if(res != test) {
            FilterTextOut out("aa", DataSource::Memory);
    ...
    static void TestFilterOut(TestCase *ptr) {
        ...
        for(int k = 1; k < 1000; k++) {
            ...
            if(i != k)
                throw info_except(
                    " String is bad, have to be a serie for int from 1 to 1000 and is %s\n",
                    str.c_str());
        }
    }
