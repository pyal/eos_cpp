...
struct TObj1 : public SavableClass {
    TObj1() : ptr(NULL) {
        fcout << "TOBbj1 Created\n";
        fcout.flush();
    }
    ~TObj1() {
        fcout << "TOBbj1 Deleted\n";
        fcout.flush();
    }
    SavableClass *ptr;
    int save_data_state(FilterOut &so) {
        so << KeyDesc("TOBbj1_ptr") << ptr;
        return 1;
    };
};
struct TObj2 : public SavableClass {
    TObj2() : ptr(NULL) {
        fcout << "TOBbj2 Created\n";
        fcout.flush();
    }
    ~TObj2() {
        fcout << "TOBbj2 Deleted\n";
        fcout.flush();
    }
    SavableClass *ptr;
    int save_data_state(FilterOut &so) {
        so << KeyDesc("TOBbj2_ptr") << ptr;
        return 1;
    };
};

DescribedClass *create_TObj1_cd() {
    return new TObj1;
}
static ClassDesc TObj1_cd(typeid(TObj1), "TObj1", 1, "Area", &create_TObj1_cd);
DescribedClass *create_TObj2_cd() {
    return new TObj2;
}
...
void CreateGrid() {
    ...
    GridVectorField<double> *doub1_ptr =
        new GridVectorField<double>(2, gr_man, "doub1_ptr");
    fcout << HEAD("Beg");
    gr_man->PrintAllGridVars(fcout);
    fcout << ENDL;
