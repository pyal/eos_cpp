...
struct CenterStorePnt : public SavableClass  
{
  public:
   ...
   CenterStorePnt(const SpacePoint &Spnt,int Npnt=0):IndNum(Npnt),Coord(Spnt),Volume(0),
			                                                             Neibous(0){};
...
//  protected://  private:
    ...
    CenterStorePnt& Set(const SpacePoint &Spnt,int ind,double vol)
     {Coord=Spnt;Volume=vol;IndNum=ind;return *this;}
    CenterStorePnt& Set(DataVector<Ref<CenterStorePnt> > &neib )
     {Neibous=neib;return *this;}
...
void TestStorage()
 {
  Storage stor;
  SpacePoint low(2),up(2);  low=0.;up=1.;
  IndexPoint lowi(2),upi(2);lowi=(int)0;upi=(int)30;
