...
static ClassDesc TestResStruct_cd(
    typeid(TestResStruct),
    "TestResStruct",
    1,
    "Grid");   //,&create_GridField_double_cd);
...
void TestGridCenter() {
    ...
    Ref<GridField<double>> test = new GridField<double>(stor->SetGridManip(), "test");
    //   test=new GridField<double>;
    Ref<GridVarBoundInd> bnd = new GridVarBoundInd(stor->SetGridManip(), "bound_ind");
    GridVectorField<double> *doub =
        new GridVectorField<double>(2, stor->SetGridManip(), "doub");
    *doub = 1.;
