...
int main(int argc, const char *argv[]) {
    ...
    struct MemoryFormat {
        MemoryFormat() : OutT(Used), OutS(Physical) {
            ...
            OutTypeName[1] = "Avail";
            OutTypeName[2] = "Used";
            OutSourceName[0] = "Physical";
            OutSourceName[1] = "PageFile";
            OutSourceName[2] = "Virtual";
            SetDiv('m');
        };
    ...
    try {
        ...
        int Percent = 0;
        if(Cmd.Get("Format").Exist)
            fmt.SetDiv(Cmd.Get("Format").Val.c_str()[0]);
        if(Cmd.Get("Type").Exist)
            OutT = (MemoryFormat::OutType)atoi(Cmd.Get("Format").Val.c_str());
        if(Cmd.Get("Source").Exist)
            OutS = (MemoryFormat::OutSource)atoi(Cmd.Get("Source").Val.c_str());
        if(Cmd.Get("Percent").Exist)
            Percent = 1;
